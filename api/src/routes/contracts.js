...
async function importContracts(request, env, origin) {
  ...
  let updated = 0;
  try {
    const countBeforeRes = await env.DB.prepare('SELECT COUNT(*) as c FROM contracts').first();
    ...
    const countAfterRes = await env.DB.prepare('SELECT COUNT(*) as c FROM contracts').first();
    const countAfter = countAfterRes.c;
    
    return ok({ 
      message: `Berhasil memproses ${inserted + updated} kontrak`,
      metrics: {
        inserted,
        updated,
        countBefore,
