var Ba=Object.defineProperty;var Fe=(a,e)=>()=>(a&&(e=a(a=0)),e);var Re=(a,e)=>{for(var n in e)Ba(a,n,{get:e[n],enumerable:!0})};var de={};Re(de,{API:()=>Ke,apiFetch:()=>f,clearToken:()=>le,getToken:()=>re,getUser:()=>ae,setToken:()=>_e,setUser:()=>oe});function re(){return localStorage.getItem("fm_token")}function _e(a){localStorage.setItem("fm_token",a)}function le(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function ae(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function oe(a){localStorage.setItem("fm_user",JSON.stringify(a))}async function f(a,e={}){let n=re(),i={"Content-Type":"application/json",...n?{Authorization:`Bearer ${n}`}:{},...e.headers||{}};try{let t=`cb=${Date.now()}`,d=a.includes("?")?"&":"?",u=`${Ke}${a}${d}${t}`,m=await fetch(u,{...e,headers:i}),s;try{let r=await m.text();try{s=JSON.parse(r)}catch{s={error:`Server Error (${m.status}): ${r.substring(0,80)}...`}}}catch{s={error:"Gagal membaca respon dari server"}}return m.status===401&&(le(),window.location.hash="#/login"),{ok:m.ok,status:m.status,data:s}}catch(t){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${t.message})`}}}}var Na,Ke,P=Fe(()=>{Na="",Ke=Na});var U={};Re(U,{downloadExcel:()=>T,parseExcel:()=>Ee,renderExcelButtons:()=>$e});function Ee(a){return new Promise((e,n)=>{let i=new FileReader;i.onload=t=>{try{let d=new Uint8Array(t.target.result),u=XLSX.read(d,{type:"array"}),m=u.SheetNames[0],s=u.Sheets[m],r=XLSX.utils.sheet_to_json(s,{defval:""});e(r)}catch(d){n(d)}},i.onerror=t=>n(t),i.readAsArrayBuffer(a)})}function T(a,e){try{let n=XLSX.utils.json_to_sheet(a),i=XLSX.utils.book_new();XLSX.utils.book_append_sheet(i,n,"Data"),XLSX.writeFile(i,`${e}.xlsx`)}catch(n){throw console.error("Error generating Excel file:",n),n}}function $e(a){return`
  ...
  `}var D=Fe(()=>{});P();var we={},be=null;function B(a,e){we[a]=e}function te(a){window.location.hash=a}function qe(){async function a(){let e=window.location.hash.replace("#","")||"/dashboard",[n,...i]=e.split("?"),t=we[n];if(!t){for(let[u,m]of Object.entries(we))if(u.endsWith("/*")&&n.startsWith(u.slice(0,-2))){t=m;break}}be&&(be(),be=null);let d=document.getElementById("main-content");if(d&&(d.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>'),t){let u=new URLSearchParams(i.join("?")),m=n.split("/").filter(Boolean),s=await t({path:n,params:u,segments:m,main:d});s&&(be=s)}else{let u=d||document.getElementById("app");u&&(u.innerHTML='<div class="empty-state"><h2>404 - Halaman tidak ditemukan</h2></div>')}}window.addEventListener("hashchange",a),a()}var ce;function Oa(){return ce||(ce=document.createElement("div"),ce.id="toast-container",document.body.appendChild(ce)),ce}function He(a,e="info",n=3500){let i=Oa(),t=document.createElement("div");t.className=`toast toast-${e}`;let d={success:"\u2713",error:"\u2715",warning:"\u26A0",info:"\u2139"};t.innerHTML=`<span class="toast-icon">${d[e]||"\u2139"}</span><span class="toast-msg">${a}</span>`,i.appendChild(t),requestAnimationFrame(()=>t.classList.add("show")),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>t.remove(),350)},n)}var R=a=>He(a,"success"),H=a=>He(a,"error");function Y({title:a,content:e,onConfirm:n,onCancel:i,confirmText:t="Simpan",cancelText:d="Batal",size:u="md",confirmClass:m="btn-primary"}){let s={sm:"400px",md:"560px",lg:"720px",xl:"900px"},r=document.createElement("div");r.className="modal-overlay",r.innerHTML=`
  ...
  <div class="mini-stat" style="pointer-events:none">
    ...
    </div>`).join("")}async function X(a,e,n=8e3){try{let i=new AbortController,t=setTimeout(()=>i.abort(),n),d=await f(a,{signal:i.signal}).catch(()=>null);if(clearTimeout(t),!d||!d.ok)return e;let u=d.data;return u?u.data!==void 0?u.data??e:u:e}catch{return e}}function Va(){["skel-donut","skel-trend","skel-insp","skel-contract"].forEach(i=>{let t=document.getElementById(i);t&&(t.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract"].forEach(i=>{let t=document.getElementById(i);if(t&&t.style.display==="none"){t.style.display="block";let d=t.parentElement;if(d&&!d.querySelector(".chart-empty")){let u=document.createElement("div");u.className="chart-empty",u.textContent="Belum ada data",t.style.display="none",d.appendChild(u)}}});let a=document.getElementById("kpi-row");a&&a.querySelector(".skeleton")&&We({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&Ye({}),["table-contracts","table-issues"].forEach(i=>{let t=document.getElementById(i);t&&t.querySelector(".skeleton")&&(t.innerHTML='<div class="chart-empty">Belum ada data</div>')});let n=document.getElementById("activity-log");n&&n.querySelector(".skeleton")&&(n.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function ze(a){Ma(),a._dashRefresh&&clearInterval(a._dashRefresh),a._skelTimeout&&clearTimeout(a._skelTimeout),a.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>Ce(a)),a._skelTimeout=setTimeout(()=>Va(),5e3),await Ce(a),a._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?Ce(a):clearInterval(a._dashRefresh)},6e4)}async function Ce(a){a._skelTimeout&&(clearTimeout(a._skelTimeout),a._skelTimeout=null);let[e,n,i,t,d,u,m,s]=await Promise.all([X("/api/dashboard/kpi",{},8e3),X("/api/dashboard/issues-trend",{},8e3),X("/api/dashboard/contracts-chart",{},8e3),X("/api/dashboard/issues-summary",{},8e3),X("/api/dashboard/inspection-bar",{},8e3),X("/api/dashboard/contracts-expiring",[],8e3),X("/api/dashboard/stats",{},8e3),X("/api/dashboard/activity-log",[],8e3)]);try{We(e)}catch(o){console.warn("KPI render:",o)}try{Ye(e)}catch(o){console.warn("MiniStats render:",o)}try{za(Array.isArray(t?.by_category)?t.by_category:[])}catch(o){console.warn("Donut render:",o),Z("skel-donut","chart-donut")}try{Wa(n)}catch(o){console.warn("Trend render:",o),Z("skel-trend","chart-trend")}try{Ya(d)}catch(o){console.warn("InspBar render:",o),Z("skel-insp","chart-insp")}try{Xa(i)}catch(o){console.warn("ContractBar render:",o),Z("skel-contract","chart-contract")}try{let o=Array.isArray(u)?u:[];Za(o)}catch(o){console.warn("ContractsTable render:",o)}try{let o=Array.isArray(m)?m:Array.isArray(m?.recent_issues)?m.recent_issues:[];et(o)}catch(o){console.warn("IssuesTable render:",o)}try{at(Array.isArray(s)?s:[])}catch(o){console.warn("ActivityLog render:",o)}let r=document.getElementById("dash-updated");r&&(r.textContent=`Diperbarui: ${new Date().toLocaleTimeString("id-ID")}`)}function We(a){let e=document.getElementById("kpi-row");if(!e)return;a=a||{};let n=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees",color:"kpi-blue",key:"employees"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts",color:"kpi-green",key:"contracts"},{icon:"\u23F0",label:"Kontrak Habis 30 Hari",sub:"",href:"#/contracts",color:"",key:"expiring30",warn:!0},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues",color:"",key:"issues",warnIfGT0:!0},{icon:"\u{1F91D}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one",color:"kpi-purple",key:"one_on_one"}];e.innerHTML=n.map(i=>{let t=q(a[i.key]?.current,0),d=a[i.key]?.prev,u=d!=null?Fa(t,d):"",m=i.color||"",s=i.sub||"";return i.warn&&(m=t>0?"kpi-amber":"kpi-green",s=t>0?`\u26A0\uFE0F ${t} kontrak segera berakhir`:"\u2705 Semua kontrak aman"),i.warnIfGT0&&(m=t>0?"kpi-red":"kpi-green"),`
    ...
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(i=>Qe(i,parseInt(i.dataset.target)||0,700))}function za(a){Z("skel-donut","chart-donut");let e=document.getElementById("chart-donut");if(!e)return;pe("donut");let n=(a||[]).filter(d=>q(d.count)>0);if(!n.length){ye(e,"Belum ada data permasalahan");return}let i=n.map(d=>j(d.category,"Lainnya")),t=n.map(d=>q(d.count));ee.donut=new Chart(e,{type:"doughnut",data:{labels:i,datasets:[{data:t,backgroundColor:ja.slice(0,n.length),borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{position:window.innerWidth<768?"bottom":"right",labels:{font:G,color:"#475569",usePointStyle:!0,padding:10,boxWidth:8,boxHeight:8}},tooltip:{bodyFont:G,titleFont:{...G,weight:"700"},callbacks:{label:d=>` ${d.label}: ${d.parsed} kasus`}}},cutout:"65%"}})}function Wa(a){Z("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;pe("trend"),a=a||{};let n=(a.labels||[]).map(Ge),i=(a.open||[]).map(d=>q(d)),t=(a.closed||[]).map(d=>q(d));if(!n.length){ye(e,"Belum ada data trend");return}ee.trend=new Chart(e,{type:"line",data:{labels:n,datasets:[{label:"Open",data:i,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:t,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.08)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#10B981",borderWidth:2}]},options:xe({plugins:{legend:{position:"top"}}})})}function Ya(a){Z("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;pe("inspBar"),a=a||{};let n=a.labels||[],i=(a.fc||[]).map(d=>q(d)),t=(a.spv||[]).map(d=>q(d));if(!n.length){ye(e,"Belum ada data inspeksi");return}ee.inspBar=new Chart(e,{type:"bar",data:{labels:n,datasets:[{label:"Skor FC",data:i,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:t,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:xe({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:G,color:ne,maxRotation:45,minRotation:30}},y:{grid:{color:he},ticks:{font:G,color:ne},min:0,max:100}}})})}function Xa(a){Z("skel-contract","chart-contract");let e=document.getElementById("chart-contract");if(!e)return;pe("contractBar"),a=a||{};let n=(a.labels||[]).map(Ge),i=(a.counts||[]).map(d=>q(d));if(!n.length){ye(e,"Belum ada data kontrak");return}let t=i.map(d=>d>5?"rgba(239,68,68,.75)":d>2?"rgba(245,158,11,.75)":"rgba(37,99,235,.65)");ee.contractBar=new Chart(e,{type:"bar",data:{labels:n,datasets:[{label:"Kontrak Berakhir",data:i,backgroundColor:t,borderRadius:6,borderSkipped:!1}]},options:xe({plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:G,color:ne}},y:{grid:{color:he},ticks:{font:G,color:ne,precision:0},beginAtZero:!0}}})})}function Za(a){let e=document.getElementById("table-contracts");if(!e)return;let n=(a||[]).filter(i=>q(i.days_remaining,999)<=30).slice(0,10);if(!n.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada kontrak yang habis dalam 30 hari</div>';return}e.innerHTML=`
    ...
    </table>`}function at(a){let e=document.getElementById("activity-log");if(!e)return;let n=(a||[]).slice(0,15);if(!n.length){e.innerHTML='<div class="chart-empty">Belum ada aktivitas tercatat</div>';return}e.innerHTML=`<div class="activity-list">${n.map(i=>{let t=Ha(i.type),d=j(i.label),u=i.branch?` \u2022 ${j(i.branch)}`:"",m=Aa(i.created_at);return`
      <div class="activity-item">
        ...
        <div class="card-body">
          <div class="import-info-box">
            ...
            <div class="import-sheet-list">
              ${Object.entries(Se).map(([c,{label:p}])=>`<span class="import-sheet-tag">\u{1F4C4} ${c} \u2192 ${p}</span>`).join("")}
            </div>
          </div>
  ...
  `;let e=null,n=null,i=0,t={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function d(c){Object.entries(t).forEach(([p,h])=>{h.style.display=p===c?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let c=document.getElementById("btn-backup-db");c.disabled=!0,c.textContent="\u23F3 Memproses Backup...";try{let p=await f("/api/import/backup");if(p.ok){let h=new Blob([JSON.stringify(p.data,null,2)],{type:"application/json"}),v=URL.createObjectURL(h),S=document.createElement("a");S.href=v,S.download=`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(S),S.click(),document.body.removeChild(S),URL.revokeObjectURL(v),R("Backup berhasil diunduh!")}else H("Gagal memproses backup: "+(p.data?.error||"Unknown error"))}catch(p){H("Gagal memproses backup: "+p.message)}finally{c.disabled=!1,c.textContent="\u{1F4E6} Backup Database"}});let u=document.getElementById("btn-sync-google");u&&u.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let c=u.innerHTML;u.innerHTML='<span class="spinner"></span> Menyinkronkan...',u.disabled=!0;try{let p=await f("/api/sync/google-sheets",{method:"POST"});p.ok?alert("Sinkronisasi Berhasil: "+(p.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(p.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{u.innerHTML=c,u.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{xa(),R("Template Excel berhasil didownload!")});let m=document.getElementById("file-input"),s=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",c=>{c.stopPropagation(),m.click()}),m.addEventListener("change",c=>{c.target.files[0]&&r(c.target.files[0])}),s.addEventListener("dragover",c=>{c.preventDefault(),s.classList.add("drag-over")}),s.addEventListener("dragleave",()=>s.classList.remove("drag-over")),s.addEventListener("drop",c=>{c.preventDefault(),s.classList.remove("drag-over");let p=c.dataTransfer.files[0];p&&p.name.match(/\.xlsx?$/i)?r(p):H("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,m.value="",document.getElementById("file-info").style.display="none",s.style.display="",d("upload")});async function r(c){e=c,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${c.name} (${(c.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",s.style.display="none",await o(c)}async function o(c){d("validating");let p=document.getElementById("validation-status"),h=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");p.textContent="Membaca file Excel...",h.style.width="20%",await me(200);let v=await c.arrayBuffer(),S=window.XLSX.read(v,{type:"array",cellDates:!0});p.textContent=`Memvalidasi ${S.SheetNames.length} sheet...`,h.style.width="50%",await me(100),n=Ca(S),h.style.width="100%",p.textContent="Validasi selesai!",await me(300),l()}catch(v){d("upload"),H("Gagal memproses file: "+v.message),document.getElementById("file-info").style.display="flex",s.style.display="none"}}function l(){d("preview");let c=n.filter(w=>!w.skipped).length,p=n.reduce((w,L)=>w+L.total,0),h=n.reduce((w,L)=>w+L.valid,0),v=n.reduce((w,L)=>w+L.errorCount,0),S=p>0?Math.round(h/p*100):0;document.getElementById("preview-summary-badges").innerHTML=`
