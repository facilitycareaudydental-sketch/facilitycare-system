var ua=Object.defineProperty;var ot=(t,e)=>()=>(t&&(e=t(t=0)),e);var st=(t,e)=>{for(var i in e)ua(t,i,{get:e[i],enumerable:!0})};var Ce={};st(Ce,{API:()=>wt,CLIENT_SIDE_MAX_ROWS:()=>we,IS_DEVELOPMENT:()=>Ne,apiFetch:()=>x,clearToken:()=>Oe,getToken:()=>Me,getUser:()=>ce,setToken:()=>ct,setUser:()=>Re});function Me(){return localStorage.getItem("fm_token")}function ct(t){localStorage.setItem("fm_token",t)}function Oe(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function ce(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Re(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function x(t,e={}){let i=Me(),a={"Content-Type":"application/json",...i?{Authorization:`Bearer ${i}`}:{},...e.headers||{}};try{let r=`cb=${Date.now()}`,s=t.includes("?")?"&":"?",o=`${wt}${t}${s}${r}`,l=await fetch(o,{...e,headers:a}),n;try{let d=await l.text();try{n=JSON.parse(d)}catch{n={error:`Server Error (${l.status}): ${d.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return l.status===401&&(Oe(),window.location.hash="#/login"),{ok:l.ok,status:l.status,data:n}}catch(r){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${r.message})`}}}}var Ne,ga,wt,we,N=ot(()=>{Ne=!1,ga="https://fm-operations-api.facilitycare-audydental.workers.dev",wt=ga,we=1e4});var Ct={};st(Ct,{confirmDialog:()=>je,createModal:()=>de});function de({title:t,content:e,onConfirm:i,onCancel:a,confirmText:r="Simpan",cancelText:s="Batal",size:o="md",confirmClass:l="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},d=document.createElement("div");d.className="modal-overlay",d.innerHTML=`
  ...
  `;function E(){let g=document.getElementById("bulk-toolbar");if(!g)return;let k=document.getElementById("bulk-count"),v=document.getElementById("btn-bulk-delete"),M=document.getElementById("btn-bulk-cancel");k.textContent=`${$.size} item dipilih`,$.size>0?(g.style.display="flex",v.disabled=!1,M.disabled=!1):(g.style.display="none",v.disabled=!0,M.disabled=!0);let A=document.getElementById("select-all-checkbox");if(A){let q=document.querySelectorAll(".row-checkbox");if(q.length>0){let D=[...q].every(ie=>ie.checked),P=[...q].some(ie=>ie.checked);A.checked=D,A.indeterminate=P&&!D}else A.checked=!1,A.indeterminate=!1}}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{$.clear(),document.querySelectorAll(".row-checkbox").forEach(k=>k.checked=!1);let g=document.getElementById("select-all-checkbox");g&&(g.checked=!1),E()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if($.size===0)return;let g=[...$],k=document.createElement("div");k.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",k.innerHTML=`
    ...
    `,document.body.appendChild(k),k.querySelector("#bulk-cancel-btn").addEventListener("click",()=>k.remove()),k.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let v=k.querySelector("#bulk-confirm-btn");v.disabled=!0,v.textContent="Menghapus...";let M=await x(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:g})});k.remove(),M.ok?(Y(`${g.length} ${n} berhasil dihapus.`),$.clear(),E(),ge(a),R()):z(M.data?.error||"Gagal menghapus data.")})});let I=document.getElementById("filter-search"),B;I?.addEventListener("input",g=>{clearTimeout(B),B=setTimeout(()=>{T.search=g.target.value,w=1,$.clear(),E(),R()},400)}),o?.forEach(g=>{(g.type==="select"||g.type==="combobox")&&(document.getElementById(`filter-${g.name}`)?.addEventListener("change",k=>{T[g.name]=k.target.value;let v=document.getElementById(`filter-sheet-${g.name}`);v&&(v.value=k.target.value),w=1,$.clear(),E(),R()}),document.getElementById(`filter-sheet-${g.name}`)?.addEventListener("change",k=>{T[g.name]=k.target.value;let v=document.getElementById(`filter-${g.name}`);v&&(v.value=k.target.value),w=1,$.clear(),E(),R(),document.getElementById("filter-options-wrapper")?.classList.remove("sheet-open")}))}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{T={},I&&(I.value=""),o?.forEach(g=>{let k=document.getElementById(`filter-${g.name}`);k&&(k.value="");let v=document.getElementById(`filter-sheet-${g.name}`);v&&(v.value="")}),w=1,$.clear(),E(),R()}),document.getElementById("btn-reset-filter-sheet")?.addEventListener("click",()=>{T={},I&&(I.value=""),o?.forEach(g=>{let k=document.getElementById(`filter-${g.name}`);k&&(k.value="");let v=document.getElementById(`filter-sheet-${g.name}`);v&&(v.value="")}),w=1,$.clear(),E(),R(),document.getElementById("filter-options-wrapper")?.classList.remove("sheet-open")}),document.getElementById("btn-create")?.addEventListener("click",()=>fe(null)),f&&document.addEventListener("click",function(g){let k=document.getElementById("aksi-menu-main"),v=document.getElementById("btn-aksi-main");k&&v&&!v.contains(g.target)&&!k.contains(g.target)&&k.classList.remove("show-aksi-menu")});let V=document.getElementById("btn-mobile-filter"),ee=document.getElementById("filter-options-wrapper"),re=document.getElementById("btn-close-filter-sheet");if(V&&ee&&(V.addEventListener("click",g=>{g.preventDefault(),ee.classList.add("sheet-open")}),re&&re.addEventListener("click",g=>{g.preventDefault(),ee.classList.remove("sheet-open")})),f){document.getElementById(`btn-export-${f.moduleName}`)?.addEventListener("click",async k=>{let v=k.target,M=v.innerHTML;v.innerHTML="\u23F3 Loading...",v.disabled=!0;try{await f.onExport()}catch{z("Gagal export data")}finally{v.innerHTML=M,v.disabled=!1}}),document.getElementById(`btn-template-${f.moduleName}`)?.addEventListener("click",()=>{f.onTemplate()});let g=document.getElementById(`input-import-${f.moduleName}`);g?.addEventListener("change",async k=>{let v=k.target.files[0];if(!v)return;let M=document.getElementById(`label-import-${f.moduleName}`),A=M?M.querySelector(".import-text"):null,q=A?A.innerText:"";A&&(A.innerText="\u231B Memproses..."),M&&(M.style.pointerEvents="none"),g.disabled=!0;try{let D=await qe(v);if(D.length===0)throw new Error("File kosong atau format salah");await f.onImport(D),Y("Import berhasil!"),ge(a),R()}catch(D){z(D.message||"Gagal import data")}finally{A&&(A.innerText=q),M&&(M.style.pointerEvents="auto"),g.disabled=!1,g.value=""}})}async function R(){E();let g=document.getElementById("table-container");if(!g)return;g.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';let k=_==="client",v=k?1:w,M=k?we:20,A=new URLSearchParams({page:v,limit:M,...Object.fromEntries(Object.entries(T).filter(([,H])=>H))}),q=await x(`${a}?${A}`);if(!q.ok){g.innerHTML=`<div class="empty-state"><p class="text-danger">Gagal memuat data: ${q.data?.error||"Error"}</p></div>`;return}let D=q.data?.data||q.data||[],P=q.data?.pagination,ie=D.length;if(k){D=y(D);let H=D.length,G=20,te=Math.ceil(H/G);w>te&&te>0&&(w=te);let O=(w-1)*G,le=w*G;D=D.slice(O,le),P={page:w,limit:G,total:H,pages:te}}!1,u&&u(D);let ye=Ve({columns:r,data:D,onEdit:h?H=>fe(H):null,actions:b.map(H=>({...H,handler:G=>H.handler(G,R)})),emptyText:`Tidak ada ${String(n||"").toLowerCase()}`,bulkSelect:S?{selectedIds:$,onToggle:E}:null});g.innerHTML="",g.appendChild(ye);let se=document.getElementById("pagination-container");if(se&&(se.innerHTML="",P&&P.pages>1)){let H=We({page:P.page,pages:P.pages,total:P.total,limit:P.limit,onPage:G=>{w=G,R()}});H&&se.appendChild(H)}}function _e(g){let k=typeof s=="function"?s(g):s;return He(k)}function fe(g){let k=!!g,v=document.createElement("form");if(v.noValidate=!0,v.innerHTML=_e(g),k){let A=typeof s=="function"?s(g):s;Xe(v,g)}let{close:M}=de({title:k?`Edit ${n}`:`Tambah ${n}`,content:v,size:"lg",confirmText:k?"Simpan Perubahan":`Tambah ${n}`,onConfirm:async(A,q)=>{if(!v.reportValidity())return;let D=A.querySelector(".modal-confirm");D.disabled=!0,D.textContent="Menyimpan...";let P=Ye(v),ie=typeof s=="function"?s(g):s,ye=async te=>{for(let O of te)if(O.type==="row")await ye(O.fields);else if(O.type==="combobox"&&P[O.name]){let le=P[O.name],ve=(O.options||[]).find(W=>{let ae=String(typeof W=="object"?W.value:W),lt=String(typeof W=="object"?W.label:W);return ae===le||lt===le});if(ve)P[O.name]=typeof ve=="object"?ve.value:ve;else if(O.createApi){let W={};W[O.createApi.field]=le,O.createApi.extra&&Object.assign(W,O.createApi.extra);let ae=await x(O.createApi.path,{method:"POST",body:JSON.stringify(W)});if(ae.ok&&ae.data?.id)P[O.name]=ae.data.id;else if(ae.ok&&!ae.data?.id)P[O.name]=le;else throw new Error(`Gagal membuat master data: ${ae.data?.error||"Unknown error"}`)}}};try{await ye(ie)}catch(te){z(te.message),D.disabled=!1,D.textContent=k?"Simpan Perubahan":`Tambah ${n}`;return}c&&(P=await c(P,g));let se=k?"PUT":"POST",H=k?`${a}/${g.id}`:a,G=await x(H,{method:se,body:JSON.stringify(P)});G.ok?(Y(k?`${n} berhasil diperbarui.`:`${n} berhasil ditambahkan.`),q(),ge(a),R()):(z(G.data?.error||"Gagal menyimpan data."),D.disabled=!1,D.textContent=k?"Simpan Perubahan":`Tambah ${n}`)}})}function ma(g){je(`Hapus ${n} ini? Tindakan tidak dapat dibatalkan.`,async()=>{let k=await x(`${a}/${g.id}`,{method:"DELETE"});k.ok?(Y(`${n} berhasil dihapus.`),ge(a),R()):z(k.data?.error||"Gagal menghapus.")},`Hapus ${n}`)}return R(),R}N();N();var Ee=null,Ze=null;async function Te(t=!1){if(Ee&&!t)return console.log("Employees Raw (Cache Hit)",Ee.slice(0,5)),Ee;let e=await x(`/api/employees?limit=${we}&status=Aktif`);return Ee=(e.data?.data||[]).map(i=>({value:i.id,label:i.full_name})),console.log("Employees Raw",e.data?.data?.slice(0,5)),console.log("Employees Mapped (ID)",Ee.slice(0,5)),Ee}async function X(t=!1){let i=(await Te(t)).map(a=>({value:a.label,label:a.label}));return console.log("Employee Options",i.slice(0,5)),i}async function j(t=!1){return Ze&&!t||(Ze=((await x("/api/branches?all=1")).data?.data||[]).map(i=>({value:i.id,label:i.full_name}))),Ze}function J(t){let e={Done:"badge-success",Aktif:"badge-success",Open:"badge-warning","In Progress":"badge-info",Pending:"badge-warning",Diproses:"badge-info",Selesai:"badge-success","Tidak Aktif":"badge-neutral",Resign:"badge-neutral",Cut:"badge-danger","Tidak Datang":"badge-danger"};return!t||t==="-"||String(t).trim()===""?"":`<span class="badge ${e[t]||"badge-neutral"}">${t}</span>`}function pt(t){return t==null?'<span class="badge badge-neutral">-</span>':t<0?`<span class="badge badge-danger">Expired (${Math.abs(t)}h)</span>`:t<=14?`<span class="badge badge-danger">${t} hari</span>`:t<=30?`<span class="badge badge-warning">${t} hari</span>`:`<span class="badge badge-success">${t} hari</span>`}function De(t){return`<span class="badge ${{"FACILITY CARE":"badge-info",SECURITY:"badge-secondary"}[t]||"badge-neutral"}">${t||"-"}</span>`}function mt(t){return`<span class="badge ${{"Inspeksi Hygiene & Aset Bangunan":"badge-info","General Cleaning":"badge-success","Deep Cleaning":"badge-purple",Fogging:"badge-warning"}[t]||"badge-neutral"}">${t||"-"}</span>`}function pe(t){return`<span class="badge ${{Q1:"badge-info",Q2:"badge-success",Q3:"badge-warning",Q4:"badge-danger"}[t]||"badge-neutral"}">${t||"-"}</span>`}K();function ut(t,e){let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}`;if(!(t.completion_date||t.target_date||t.opening_date||"").startsWith(a))return!1;let s=String(t.status||"").toLowerCase();if(s!=="selesai"&&s!=="completed"&&s!=="done")return!1;let o=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?o.includes("inspeksi"):e==="gcdc"?o.includes("general cleaning")||o.includes("deep cleaning"):!1}N();K();function Et(t,e){let i=String(t.status||"").toLowerCase();return e==="active"?i==="aktif":e==="reliefer"?t.division==="FC - RELIEFER"&&i==="aktif":!1}N();K();function gt(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let r=new Date(a);r.setDate(a.getDate()+30);let s=new Date(t.end_date);return s.setHours(0,0,0,0),s>=a&&s<=r}return!1}N();K();function Tt(t,e){let i=String(t.status||"").toLowerCase();return e==="open"?i==="open":!1}N();function Dt(t,e){let i=String(t.status||"").toLowerCase();return e==="pending"?i==="pending":!1}var Se={};function Pe(t){if(Se[t]){try{Se[t].destroy()}catch{}delete Se[t]}}function ya(){Object.keys(Se).forEach(Pe)}var be=(t,e=0)=>{let i=Number(t);return isNaN(i)||t===null||t===void 0?e:i},Ie=(t,e="\u2014")=>{if(t==null||t==="")return e;let i=String(t).trim();return i===""||i==="[object Object]"?e:i};function Bt(t,e,i=900){if(!t)return;let a=Math.max(0,Math.round(be(e)));if(a===0){t.textContent="0";return}let r=Date.now(),s=()=>{let o=Math.min((Date.now()-r)/i,1),l=1-Math.pow(1-o,3);t.textContent=Math.round(l*a).toLocaleString("id-ID"),o<1?requestAnimationFrame(s):t.textContent=a.toLocaleString("id-ID")};requestAnimationFrame(s)}var va={Done:"pill-success",Aktif:"pill-success",Selesai:"pill-success",Open:"pill-danger",Pending:"pill-warning","In Progress":"pill-info","Tidak Aktif":"pill-neutral",Resign:"pill-neutral",Cut:"pill-neutral"},ka=t=>{let e=Ie(t,"\u2014");return`<span class="status-pill ${va[e]||"pill-neutral"}">${e}</span>`};var me={family:"Inter",size:11},he="#94A3B8",Be="#F1F5F9",bt=["#2563EB","#10B981","#F59E0B","#EF4444","#8B5CF6","#0EA5E9","#F97316","#14B8A6","#6366F1","#EC4899"],Sa=()=>window.innerWidth<768;function et(t={}){return{responsive:!0,maintainAspectRatio:!1,animation:{duration:700,easing:"easeOutQuart"},plugins:{legend:{position:Sa()?"bottom":"top",labels:{font:me,color:"#64748B",usePointStyle:!0,padding:10,boxWidth:8,boxHeight:8}},tooltip:{mode:"index",intersect:!1,bodyFont:me,titleFont:{...me,weight:"700"}}},scales:{x:{grid:{color:Be},ticks:{font:me,color:he,maxRotation:0}},y:{grid:{color:Be},ticks:{font:me,color:he},beginAtZero:!0}},...t}}var wa=()=>Array(5).fill(0).map(()=>`
  ...
  </div>`).join("");function It(t=3){return Array(t).fill(0).map((e,i)=>`<div class="skeleton skeleton-text" style="height:38px;margin-bottom:${i<t-1?"6px":"0"};border-radius:6px"></div>`).join("")}async function ne(t,e,i=8e3){try{let a=new AbortController,r=setTimeout(()=>a.abort(),i),s=await x(t,{signal:a.signal}).catch(()=>null);if(clearTimeout(r),!s||!s.ok)return e;let o=s.data;return o?o.data!==void 0?o.data??e:o:e}catch{return e}}function _a(){["skel-donut","skel-trend","skel-insp","skel-contract","skel-jadwal"].forEach(a=>{let r=document.getElementById(a);r&&(r.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract","chart-jadwal"].forEach(a=>{let r=document.getElementById(a);if(r&&r.style.display==="none"){r.style.display="block";let s=r.parentElement;if(s&&!s.querySelector(".chart-empty")){let o=document.createElement("div");o.className="chart-empty",o.textContent="Belum ada data",r.style.display="none",s.appendChild(o)}}});let t=document.getElementById("kpi-row");t&&t.querySelector(".skeleton")&&Lt({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&Ft({}),["table-contracts","table-issues"].forEach(a=>{let r=document.getElementById(a);r&&r.querySelector(".skeleton")&&(r.innerHTML='<div class="chart-empty">Belum ada data</div>')});let i=document.getElementById("activity-log");i&&i.querySelector(".skeleton")&&(i.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function Pt(t){ya(),t._dashRefresh&&clearInterval(t._dashRefresh),t._skelTimeout&&clearTimeout(t._skelTimeout);let e=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t.innerHTML=`
    <div class="dashboard-wrap" id="dash-root">


      <!-- KPI -->
      <div class="kpi-row" id="kpi-row">${wa()}</div>
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>ht(t)),document.getElementById("filter-jadwal-year")?.addEventListener("change",async i=>{let a=i.target.value,r=document.getElementById("jadwal-year-label");r&&(r.textContent=a);let s=document.getElementById("skel-jadwal"),o=document.getElementById("chart-jadwal");s&&(s.style.display="block",s.style.position="absolute"),o&&(o.style.display="none");let l=await ne(`/api/dashboard/schedule-chart?year=${a}`,{},8e3);try{At(l)}catch(n){console.warn("ScheduleChart render:",n),ue("skel-jadwal","chart-jadwal")}}),document.getElementById("filter-insp-month")?.addEventListener("change",async i=>{let a=i.target.value,r=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",s=document.getElementById("skel-insp"),o=document.getElementById("chart-insp");s&&(s.style.display="block",s.style.position="absolute"),o&&(o.style.display="none");let l=await ne(r,{},8e3);try{Nt(l)}catch(n){console.warn("InspBar render:",n),ue("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>_a(),5e3),await ht(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?ht(t):clearInterval(t._dashRefresh)},6e4)}async function ht(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,i,a,r,s,o,l,n,d,h,p,c,u,y]=await Promise.all([ne("/api/dashboard/kpi",{},8e3),ne("/api/dashboard/issues-trend",{},8e3),ne("/api/dashboard/issues-summary",{},8e3),ne("/api/dashboard/stats",{},8e3),ne("/api/dashboard/calendar",[],8e3),ne("/api/schedule?limit=10000",{data:[]},8e3),ne("/api/employees?limit=10000",{data:[]},8e3),ne("/api/contracts?limit=10000",{data:[]},8e3),ne("/api/issues?limit=10000",{data:[]},8e3),ne("/api/one-on-one?limit=10000",{data:[]},8e3),ne("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3),ne(`/api/dashboard/schedule-chart?year=${document.getElementById("filter-jadwal-year")?.value||new Date().getFullYear()}`,{},8e3),ne("/api/relievers?limit=10000",{data:[]},8e3),ne("/api/reports/fogging?limit=10000",{data:[]},8e3)]),b=document.getElementById("filter-insp-month"),m=b?b.value:"",f=m?`/api/dashboard/inspection-bar?month=${m}`:"/api/dashboard/inspection-bar",S=await ne(f,{},8e3);if(e){let _=Array.isArray(o?.data)?o.data:Array.isArray(o)?o:[];window.dashboardSchedules=_;let C=Array.isArray(l?.data)?l.data:Array.isArray(l)?l:[],w=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],T=Array.isArray(d?.data)?d.data:Array.isArray(d)?d:[],$=Array.isArray(h?.data)?h.data:Array.isArray(h)?h:[],E=Array.isArray(u?.data)?u.data:Array.isArray(u)?u:[];window.dashboardRelievers=E;let I=Array.isArray(y?.data)?y.data:Array.isArray(y)?y:[];if(window.dashboardFogging=I,e.employees&&(e.employees.current=C.filter(B=>Et(B,"active")).length),e.contracts&&(e.contracts.current=w.filter(B=>gt(B,"active")).length),e.expiring30&&(e.expiring30={current:w.filter(B=>gt(B,"expiring30")).length}),e.issues&&(e.issues.current=T.filter(B=>Tt(B,"open")).length),e.one_on_one&&(e.one_on_one.current=$.filter(B=>Dt(B,"pending")).length),e.schedule){let B=`Q${Math.ceil((new Date().getMonth()+1)/3)}`;e.schedule.current=_.filter(V=>{if(V.period===B)return!0;if(V.target_date){let ee=V.target_date.split("-");if(ee.length>=2){let re=parseInt(ee[1],10);return re&&`Q${Math.ceil(re/3)}`===B}}return!1}).length}e.inspection_month&&(e.inspection_month.current=_.filter(B=>ut(B,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=_.filter(B=>ut(B,"gcdc")).length)}try{Lt(e)}catch(_){console.warn("KPI render:",_)}try{Ft(e)}catch(_){console.warn("MiniStats render:",_)}try{At(c)}catch(_){console.warn("ScheduleChart render:",_),ue("skel-jadwal","chart-jadwal")}try{Ca(Array.isArray(a?.by_category)?a.by_category:[])}catch(_){console.warn("Donut render:",_),ue("skel-donut","chart-donut")}try{$a(i)}catch(_){console.warn("Trend render:",_),ue("skel-trend","chart-trend")}try{Nt(S)}catch(_){console.warn("InspBar render:",_),ue("skel-insp","chart-insp")}try{let _=Array.isArray(r)?r:Array.isArray(r?.recent_issues)?r.recent_issues:[];Ta(_)}catch(_){console.warn("IssuesTable render:",_)}try{let _=Array.isArray(r?.expiring_contracts)?r.expiring_contracts:[];Ea(p)}catch(_){console.warn("ContractsTable render:",_)}try{Da(Array.isArray(s)?s:[])}catch(_){console.warn("Agenda render:",_)}try{Ia()}catch(_){console.warn("Quick Actions render:",_)}}function Lt(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let i=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=i.map(a=>{let r=be(t[a.key]?.current,0);return`
  ...
  `,l=[{id:"mini-jadwal",icon:"\u{1F4C5}",label:"Jadwal",dropdown:`
    ...
    <a href="${p.href}" class="mini-stat ${p.color}" style="text-decoration:none" id="${p.id||""}">
      ...
      <div class="mini-stat-body" style="flex:1; min-width:0; overflow:visible;">
        <div style="display:flex; align-items:baseline; gap:3px;">
          <div class="mini-stat-value" data-target="${be(p.val)}">0</div>
          ${p.dropdown?p.dropdown:""}
        </div>
        <div class="mini-stat-text">${p.label}</div>
      </div>
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(p=>Bt(p,parseInt(p.dataset.target)||0,700));let n=document.getElementById("dash-jadwal-period");n&&n.addEventListener("change",p=>{let c=p.target.value,u=(window.dashboardSchedules||[]).filter(m=>{if(m.period===c)return!0;if(m.target_date){let f=m.target_date.split("-");if(f.length>=2){let S=parseInt(f[1],10);return S&&`Q${Math.ceil(S/3)}`===c}}return!1}).length,y=document.querySelector("#mini-jadwal .mini-stat-value");y&&(y.dataset.target=u,y.textContent=u);let b=document.getElementById("mini-jadwal");b&&(b.href=`#/timeline?dash_filter=period_${c.toLowerCase()}`)});let d=(p,c,u,y,b)=>{let m=document.getElementById(p);if(m){let f=S=>{let _=(u||[]).filter(T=>y(T,S)).length,C=document.querySelector(`#${c} .mini-stat-value`);C&&(C.dataset.target=_,C.textContent=_);let w=document.getElementById(c);w&&(w.href=`${b}&month=${S}`)};f(m.value),m.addEventListener("change",S=>f(S.target.value))}},h=p=>{let c=String(p.status||"").toLowerCase();return c==="done"||c==="selesai"||c==="completed"};d("dash-reliefer-month","mini-reliefer",window.dashboardRelievers,(p,c)=>window.parseFlexibleDate(p.backup_date).startsWith(c)&&h(p),"#/relievers?dash_filter=reliever"),d("dash-inspeksi-month","mini-inspeksi",window.dashboardSchedules,(p,c)=>p.activity_type==="Inspeksi Hygiene"&&h(p)&&window.parseFlexibleDate(p.completion_date||p.target_date).startsWith(c),"#/timeline?dash_filter=inspeksi"),d("dash-gcdc-month","mini-gcdc",window.dashboardSchedules,(p,c)=>(p.activity_type==="General Cleaning"||p.activity_type==="Deep Cleaning")&&h(p)&&window.parseFlexibleDate(p.completion_date||p.target_date).startsWith(c),"#/timeline?dash_filter=gcdc"),d("dash-fogging-month","mini-fogging",window.dashboardFogging,(p,c)=>h(p)&&window.parseFlexibleDate(p.activity_date).startsWith(c),"#/reports/fogging?dash_filter=fogging")}function Ca(t){ue("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),i=document.getElementById("donut-legend");if(!e||!i)return;Pe("donut");let a=(t||[]).filter(n=>be(n.count)>0);if(!a.length){Je(e,"Belum ada data permasalahan");return}let r=a.map(n=>`${Ie(n.category,"Lainnya")}`),s=a.map(n=>be(n.count)),o=s.reduce((n,d)=>n+d,0);i.innerHTML=a.map((n,d)=>{let h=bt[d%bt.length],p=o>0?Math.round(n.count/o*100):0;return`
    ...
    `}).join("");let l={id:"centerText",beforeDraw:function(n){let d=n.width,h=n.height,p=n.ctx;p.restore();let c=(h/80).toFixed(2);p.font="bold "+c+"em Inter",p.textBaseline="middle",p.fillStyle="#1E293B";let u=o.toString(),y=Math.round((d-p.measureText(u).width)/2),b=h/2;p.fillText(u,y,b-4),p.font="600 "+(c*.35).toFixed(2)+"em Inter",p.fillStyle="#64748B";let m="Total",f=Math.round((d-p.measureText(m).width)/2);p.fillText(m,f,b+10),p.save()}};Se.donut=new Chart(e,{type:"doughnut",data:{labels:r,datasets:[{data:s,backgroundColor:bt,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:me,titleFont:{...me,weight:"700"},callbacks:{label:n=>` ${n.label}: ${n.parsed} kasus`}}},cutout:"75%"},plugins:[l]})}function $a(t){ue("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;Pe("trend"),t=t||{};let i=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],a=(t.labels||[]).map(o=>{if(!o||typeof o!="string")return"";try{let[l,n]=o.split("-");return(i[Number(n)-1]||n)+" "+String(l).slice(-2)}catch{return o}}),r=(t.open||[]).map(o=>be(o)),s=(t.closed||[]).map(o=>be(o));if(!a.length){Je(e,"Belum ada data trend");return}Se.trend=new Chart(e,{type:"line",data:{labels:a,datasets:[{label:"Open",data:r,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:s,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.1)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#10B981",borderWidth:2}]},options:et({plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{family:"Inter",size:9},color:he,maxRotation:45,autoSkip:!0}},y:{grid:{color:Be},ticks:{font:{family:"Inter",size:9},color:he},beginAtZero:!0}}})})}function At(t){ue("skel-jadwal","chart-jadwal");let e=document.getElementById("chart-jadwal");if(!e)return;Pe("jadwal"),t=t||{};let i=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];if(!Object.values(t).some(n=>Array.isArray(n)&&n.some(d=>d>0))){Je(e,"Belum ada data jadwal");return}let r=t["Inspeksi Hygiene"]||Array(12).fill(0),s=t["General Cleaning"]||Array(12).fill(0),o=t["Deep Cleaning"]||Array(12).fill(0),l=t.Fogging||Array(12).fill(0);Se.jadwal=new Chart(e,{type:"bar",data:{labels:i,datasets:[{label:"Inspeksi",data:r,backgroundColor:"#3B82F6"},{label:"General Cleaning",data:s,backgroundColor:"#10B981"},{label:"Deep Cleaning",data:o,backgroundColor:"#F59E0B"},{label:"Fogging",data:l,backgroundColor:"#EF4444"}]},options:et({plugins:{legend:{display:!1}},datasets:{bar:{barPercentage:.85,categoryPercentage:.9}},scales:{x:{stacked:!0,grid:{display:!1},ticks:{font:{family:"Inter",size:9},color:he,maxRotation:0,autoSkip:!1}},y:{stacked:!0,grid:{color:Be},ticks:{font:{family:"Inter",size:9},color:he},min:0}}})})}function Nt(t){ue("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;Pe("inspBar"),t=t||{};let i=t.labels||[],a=(t.fc||[]).map(s=>be(s)),r=(t.spv||[]).map(s=>be(s));if(!i.length){Je(e,"Belum ada data inspeksi");return}Se.inspBar=new Chart(e,{type:"bar",data:{labels:i,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:r,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:et({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:me,color:he,maxRotation:45,minRotation:30}},y:{grid:{color:Be},ticks:{font:me,color:he},min:0,max:100}}})})}function Ea(t){ue("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;Pe("contractMiniBar"),t=t||{};let i={"01":"Jan","02":"Feb","03":"Mar","04":"Apr","05":"Mei","06":"Jun","07":"Jul","08":"Agu","09":"Sep",10:"Okt",11:"Nov",12:"Des"},a=(t.labels||[]).map(o=>{let l=o.split("-")[1];return i[l]||o}),r=(t.data||[]).map(o=>be(o));if(!a.length){Je(e,"Belum ada data");return}let s=e.getContext("2d");Se.contractMiniBar=new Chart(e,{type:"bar",data:{labels:a,datasets:[{label:"Kontrak Habis",data:r,backgroundColor:"#3B82F6",borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:et({onClick:(o,l)=>{if(l&&l.length>0){let n=l[0].index,d=(t.labels||[])[n];d&&(window.location.hash="#/contracts?month_expiry="+d)}},plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:me,color:he,maxRotation:0,autoSkip:!1}},y:{grid:{color:Be,borderDash:[4,4],drawBorder:!1},ticks:{font:me,color:he,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function Ta(t){let e=document.getElementById("table-issues");if(!e)return;let i=(t||[]).slice(0,8);if(!i.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${i.map(a=>`
        <div class="dash-list-item">
          <div style="flex-shrink:0">${ka(a.status)}</div>
          <div style="flex:1;min-width:0;margin-left:4px">
            <div style="font-size:0.85rem;font-weight:700;color:var(--text-1);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;line-height:1.3">${Ie(a.complaint)}</div>
            <div style="font-size:0.75rem;color:var(--text-3);margin-top:2px">${Ie(a.branch_name)}</div>
          </div>
        </div>
      `).join("")}
    </div>`}function Da(t){let e=document.getElementById("widget-agenda");if(!e)return;let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}-${String(i.getDate()).padStart(2,"0")}`,s=(t||[]).filter(o=>(o.event_date||"").startsWith(a)).slice(0,10);if(!s.length){e.innerHTML="";return}e.innerHTML=`
...
`)}let T=d(_,S),$=null;return T?w||($="Tanggal Mulai kosong atau tidak berformat tanggal"):$="Karyawan tidak ditemukan di Database",{isValid:!!(T&&w),invalidReason:$,rowNum:S,data:{employee_id:T,branch_id:n(String(m.Cabang||"").trim()),division:String(m["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:w,end_date:h(m["Tanggal Selesai"])||"2099-12-31",status:String(m.Status||"").trim(),_rawName:_}}}),c=[],u=[];if(p.forEach(m=>{m.isValid?c.push(m.data):u.push({rowNum:m.rowNum,name:m.data._rawName,reason:m.invalidReason})}),console.log(`Split Validation - Valid: ${c.length}, Invalid: ${u.length}`),c.length===0){let m=`SEMUA BARIS GAGAL IMPORT!
...
`})),alert(b),typeof tt=="function"&&tt()}}})}N();K();var vt=[],Ue=[];function Fa(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let i of e)if(t.some(a=>a.period===i))return i;return"Q3"}async function Kt(t,e){vt=await j();let i=await X();Ue=["Berlin Ariansyah","Ade Surahman"];let a=m=>m&&!Ue.find(f=>String(typeof f=="object"?f.value:f).toLowerCase()===String(m).toLowerCase())?[...Ue,m]:Ue,r=await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),s=m=>{if(!m||m==="-"||String(m).trim()==="")return"";let f=String(m).split("-");return f.length===3&&f[0].length===4?`${f[2]}-${f[1]}-${f[0]}`:m},o=r.data?.data||[],l=Fa(o),n=e?e.get("dash_filter"):null,d=new Date,h=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`,p={},c=e&&e.get("month")?e.get("month"):null;n==="inspeksi"?p={status:"Done",activity_type:"Inspeksi Hygiene",month:c}:n==="gcdc"?p={status:"Done",activity_type:"GCDC",month:c}:n&&n.startsWith("period_")&&(p={period:n.replace("period_","").toUpperCase()});let u=new Date().getFullYear(),b=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"].map((m,f)=>{let S=String(f+1).padStart(2,"0");return{value:`${u}-${S}`,label:`${m} ${u}`}});F({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",enableMobileFilterSheet:!0,defaultFilters:p,onDataLoaded:m=>m.sort((f,S)=>{let _=f.opening_date?new Date(f.opening_date).getTime():0;return(S.opening_date?new Date(S.opening_date).getTime():0)-_}),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:m=>mt(m)},{key:"period",label:"Periode",render:m=>pe(m)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:m=>s(m)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:m=>s(m)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:m=>s(m)},{key:"status",label:"Status",render:m=>J(m)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:vt},{type:"select",name:"activity_type",label:"Kegiatan",options:[{value:"Inspeksi Hygiene",label:"Inspeksi Hygiene"},{value:"General Cleaning",label:"General Cleaning"},{value:"Deep Cleaning",label:"Deep Cleaning"},{value:"Fogging",label:"Fogging"},{value:"GCDC",label:"GCDC (GC & DC)"}]},{type:"select",name:"month",label:"Bulan",options:b},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:Ue}],formFields:m=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:vt,value:m?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:m?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:m?.period},{name:"pic",label:"PIC",type:"combobox",options:a(m?.pic),value:m?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:m?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:m?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:m?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:m?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:m?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let m=await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(m.ok){let f=m.data.data.map(S=>({Cabang:S.branch_name||"",Kegiatan:S.activity_type||"",Periode:S.period||"",PIC:S.pic||"","Tgl Opening":S.opening_date||"","Tgl Target":S.target_date||"","Tgl Selesai":S.completion_date||"",Status:S.status||""}));L(f,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async m=>{let S=(await x("/api/branches?all=1")).data?.data||[],_=$=>{if(!$)return null;let E=String($||"").toLowerCase(),I=S.find(B=>String(B.full_name||"").toLowerCase()===E||String(B.code||"").toLowerCase()===E||String(B.name||"").toLowerCase()===E);return I?I.id:null},C=$=>{if($==null||$==="")return"";if($ instanceof Date&&!isNaN($.getTime()))return $.toISOString().slice(0,10);let E=String($).trim();if(E===""||E==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(E))return E.slice(0,10);if(/^\d{4,5}$/.test(E)){let B=Number(E);if(B>2e4&&B<99999){let V=new Date(Date.UTC(1899,11,30)+B*864e5);return isNaN(V.getTime())?"":V.toISOString().slice(0,10)}}let I=E.split(/[\/\-\.]/);if(I.length===3){let[B,V,ee]=I.map(re=>re.trim());if(B.length===4&&V.length<=2&&ee.length<=2)return`${B}-${V.padStart(2,"0")}-${ee.padStart(2,"0")}`;if(ee.length===4&&V.length<=2&&B.length<=2)return`${ee}-${V.padStart(2,"0")}-${B.padStart(2,"0")}`}return E},w=m.map($=>({branch_id:_(String($.Cabang||"").trim()),activity_type:String($.Kegiatan||"").trim(),period:String($.Periode||"").trim(),pic:String($.PIC||$.Pic||"").trim(),opening_date:C($["Tgl Opening"]||$["Tanggal Opening"]||$["Tgl Openir"]),target_date:C($["Tgl Target"]||$["Tanggal Target"]),completion_date:C($["Tgl Selesai"]||$["Tanggal Selesai"]),status:String($.Status||"").trim(),notes:String($.Catatan||$.Keterangan||"").trim()})).filter($=>$.activity_type&&$.period),T=await x("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:w,onDuplicate:"update"})});if(!T.ok)throw new Error(T.data?.error||"Import gagal");return T.data}}})}N();K();var kt=[],at=[];function Aa(t,e){let i=String(t.status||"").toLowerCase();return e==="open"?i==="open":!1}async function jt(t,e){let i=e?e.get("dash_filter"):null;kt=await j(),at=await X();let a=n=>n&&!at.find(d=>d.value===n)?[...at,{value:n,label:n}]:at,r=new Date().getFullYear(),s=["2025","2026","2027","2028","2029","2030"],l=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"].map((n,d)=>{let h=String(d+1).padStart(2,"0");return{value:`${r}-${h}`,label:`${n} ${r}`}});F({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,enableMobileFilterSheet:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:n=>i?n.filter(d=>Aa(d,i)):n,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:n=>`<span class="badge badge-secondary">${n}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:n=>`<span title="${n}">${n?.length>50?n.slice(0,50)+"\u2026":n}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:n=>`<span title="${n||""}">${n?.length>40?n.slice(0,40)+"\u2026":n||"-"}</span>`},{key:"status",label:"Status",render:n=>J(n)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:n=>window.formatDate(n)},{key:"day_count",label:"Hari",render:n=>n??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:kt},{type:"select",name:"month",label:"Bulan",options:l},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:s}],formFields:n=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:n?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:kt,value:n?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:n?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:n?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:n?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(n?.employee_name),value:n?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(n?.fc_specialist),value:n?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:n?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:n?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:n?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let n=await x(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(n.ok){let d=n.data.data.map(h=>({Tanggal:h.report_date||"",Cabang:h.branch_name||"",Kategori:h.category||"",Sumber:h.source||"",Keluhan:h.complaint||"","Nama FC":h.employee_name||"","FC Spesialis":h.fc_specialist||"",Solusi:h.solution||"","Tgl Selesai":h.completion_date||"",Status:h.status||""}));L(d,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async n=>{let h=(await x("/api/branches?all=1")).data?.data||[],p=y=>{if(!y)return null;let b=String(y||"").toLowerCase(),m=h.find(f=>String(f.full_name||"").toLowerCase()===b||String(f.code||"").toLowerCase()===b||String(f.name||"").toLowerCase()===b);return m?m.id:null},c=n.map(y=>({branch_id:p(String(y.Cabang||"").trim()),report_date:String(y.Tanggal||"").trim(),category:String(y.Kategori||"").trim(),source:String(y.Sumber||"").trim(),complaint:String(y.Keluhan||"").trim(),employee_name:String(y["Nama FC"]||"").trim(),fc_specialist:String(y["FC Spesialis"]||"").trim(),solution:String(y.Solusi||"").trim(),completion_date:String(y["Tgl Selesai"]||"").trim(),status:String(y.Status||"").trim()})).filter(y=>y.report_date&&y.complaint&&y.category),u=await x("/api/import/issues",{method:"POST",body:JSON.stringify({rows:c,onDuplicate:"update"})});if(!u.ok)throw new Error(u.data?.error||"Import gagal");return u.data}}})}N();var Le=[];function Na(t,e){let i=String(t.status||"").toLowerCase();return e==="pending"?i==="pending":!1}async function Ht(t,e){let i=e?e.get("dash_filter"):null;Le=await j();let a=await X(),r=["Ade","Berlin"],s=l=>l&&!a.find(n=>n.value===l)?[...a,{value:l,label:l}]:a,o=l=>l&&!r.find(n=>(typeof n=="object"?n.value:n)===l)?[...r,l]:r;F({container:t,title:"One on One",icon:"\u{1F4AC}",apiPath:"/api/one-on-one",bulkDelete:!0,enableMobileFilterSheet:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:l=>i?l.filter(n=>Na(n,i)):l,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:l=>window.formatDate(l)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:l=>`<span title="${l||""}">${l?.length>50?l.slice(0,50)+"\u2026":l||"-"}</span>`},{key:"solution",label:"Solusi",render:l=>`<span title="${l||""}">${l?.length>40?l.slice(0,40)+"\u2026":l||"-"}</span>`},{key:"status",label:"Status",render:l=>J(l)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:l=>window.formatDate(l)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:l=>l?`<a href="${l}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"select",name:"branch_id",label:"Cabang",options:Le},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async l=>{let n=new URLSearchParams(l||{}).toString(),d=await x(`/api/one-on-one?limit=10000&${n}`);if(d.ok){let h=d.data.data.map(c=>({Tanggal:c.meeting_date||"",Cabang:c.branch_name||"","Nama Karyawan":c.employee_name||"",PIC:c.pic||"",Masalah:c.problem||"",Solusi:c.solution||"",Status:c.status||"","Tgl Selesai":c.completion_date||"",Dokumen:c.document_link||""})),{downloadExcel:p}=await Promise.resolve().then(()=>(K(),oe));p(h,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let l=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(K(),oe));n(l,"Template_Import_OneOnOne")},onImport:async l=>{let n=c=>{if(!c)return null;let u=String(c||"").toLowerCase(),y=Le.find(b=>String(b.label||"").toLowerCase()===u);return y?y.value:null},d=c=>{if(!c)return"";if(c instanceof Date&&!isNaN(c.getTime()))return c.toISOString().slice(0,10);let u=String(c).trim();if(/^\d{4,5}$/.test(u)){let b=Number(u);if(b>2e4&&b<99999){let m=new Date(Date.UTC(1899,11,30)+b*864e5);return isNaN(m.getTime())?"":m.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let y=u.split(/[\/\-\.]/);if(y.length===3){let[b,m,f]=y.map(S=>S.trim());if(b.length===4&&m.length<=2&&f.length<=2)return`${b}-${m.padStart(2,"0")}-${f.padStart(2,"0")}`;if(f.length===4&&m.length<=2&&b.length<=2)return`${f}-${m.padStart(2,"0")}-${b.padStart(2,"0")}`}return u},h=l.map(c=>({meeting_date:d(c.Tanggal),employee_name:String(c["Nama Karyawan"]||"").trim(),branch_id:n(String(c.Cabang||"").trim()),pic:String(c.PIC||"").trim(),problem:String(c.Masalah||"").trim(),solution:String(c.Solusi||"").trim(),status:String(c.Status||"").trim(),completion_date:d(c["Tgl Selesai"]),document_link:String(c.Dokumen||"").trim()})).filter(c=>c.meeting_date&&c.employee_name&&c.branch_id),p=await x("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!p.ok)throw new Error(p.data?.error||"Import gagal");return p.data}},formFields:l=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:l?.meeting_date},{name:"branch_id",label:"Cabang",type:"select",options:l?.branch_id&&!Le.find(n=>n.value==l.branch_id)?[...Le,{value:l.branch_id,label:l.branch_name||l.branch_id}]:Le,createApi:{path:"/api/branches",field:"full_name"},value:l?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"select",required:!0,options:s(l?.employee_name),value:l?.employee_name},{name:"pic",label:"PIC",type:"select",options:o(l?.pic),createApi:{path:"/api/pic",field:"name"},value:l?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:l?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:l?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:l?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:l?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:l?.document_link}]})}N();async function qt(t){let e=await j(),i=await X(),a=["Ade Surahman","Berlin Ariansyah","Mizwar","Fajar","Ade","Berlin"],r=n=>n&&!i.find(d=>d.value===n)?[...i,{value:n,label:n}]:i,s=n=>n&&!a.find(d=>(typeof d=="object"?d.value:d)===n)?[...a,n]:a,o=Array.from({length:5},(n,d)=>String(new Date().getFullYear()-d));F({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:n=>{try{let d=JSON.parse(n);return Array.isArray(d)?d.join(", "):n||"-"}catch{return n||"-"}}},{key:"score",label:"Nilai",render:n=>n!=null?`<strong>${n}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:n=>n?`<a href="${n}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer / peserta..."},{type:"select",name:"batch",label:"Batch",options:["Batch 1","Batch 2","Batch 3","Batch 4","Batch 5"]},{type:"select",name:"branch_id",label:"Cabang",options:e},{type:"select",name:"trainer",label:"Trainer",options:["Berlin Ariansyah"]},{type:"select",name:"year",label:"Tahun",options:o}],exportOptions:{moduleName:"training",onExport:async n=>{let d=new URLSearchParams(n||{}).toString(),h=await x(`/api/training?limit=10000&${d}`);if(h.ok){let p=h.data.data.map(u=>{let y=u.participants||"";try{let b=JSON.parse(y);y=Array.isArray(b)?b.join(", "):y}catch{}return{Tanggal:u.training_date||"",Batch:u.batch||"",Materi:u.subject||"",Cabang:u.branch_name||"",Trainer:u.trainer||"",Peserta:y,Nilai:u.score!==null&&u.score!==void 0?u.score:"",Dokumen:u.document_link||""}}),{downloadExcel:c}=await Promise.resolve().then(()=>(K(),oe));c(p,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let n=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:d}=await Promise.resolve().then(()=>(K(),oe));d(n,"Template_Import_Training")},onImport:async n=>{let d=u=>{if(!u)return null;let y=String(u||"").toLowerCase(),b=e.find(m=>String(m.label||"").toLowerCase()===y);return b?b.value:null},h=u=>{if(!u)return"";if(u instanceof Date&&!isNaN(u.getTime()))return u.toISOString().slice(0,10);let y=String(u).trim();if(/^\d{4,5}$/.test(y)){let m=Number(y);if(m>2e4&&m<99999){let f=new Date(Date.UTC(1899,11,30)+m*864e5);return isNaN(f.getTime())?"":f.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(y))return y.slice(0,10);let b=y.split(/[\/\-\.]/);if(b.length===3){let[m,f,S]=b.map(_=>_.trim());if(m.length===4&&f.length<=2&&S.length<=2)return`${m}-${f.padStart(2,"0")}-${S.padStart(2,"0")}`;if(S.length===4&&f.length<=2&&m.length<=2)return`${S}-${f.padStart(2,"0")}-${m.padStart(2,"0")}`}return y},p=n.map(u=>({training_date:h(u.Tanggal),batch:String(u.Batch||"").trim(),subject:String(u.Materi||"").trim(),branch_id:d(String(u.Cabang||"").trim()),trainer:String(u.Trainer||"").trim(),participants:String(u.Peserta||"").trim(),score:u.Nilai?Number(u.Nilai):null,document_link:String(u.Dokumen||"").trim()})).filter(u=>u.training_date&&u.subject&&u.branch_id),c=await x("/api/training/import",{method:"POST",body:JSON.stringify(p)});if(!c.ok)throw new Error(c.data?.error||"Import gagal")}},formFields:n=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:n?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:n?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:n?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:n?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:s(n?.trainer),value:n?.trainer}]},{name:"participants",label:"Peserta",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let d=JSON.parse(n?.participants);return Array.isArray(d)?d.join(", "):n?.participants||""}catch{return n?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:n?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:n?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:n?.notes}],onBeforeSubmit:async n=>n})}N();xe();K();function Jt({container:t,title:e,icon:i,apiPath:a,columns:r,formFields:s,filterFields:o,defaultFilters:l={},itemLabel:n="Data",canCreate:d=!0,canEdit:h=!0,canDelete:p=!0,onBeforeSubmit:c,onAfterLoad:u,onDataLoaded:y,extraActions:b=[],initialSearch:m="",exportOptions:f=null,bulkDelete:S=!1,paginationMode:_="server"}){let C=ce();C&&C.role==="viewer"&&(d=!1,h=!1,p=!1,S=!1,f=null);let w=1,T={...l};m&&(T.search=m);let $=new Set;t.innerHTML=`
  ...
  `,document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),r()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),r()}),document.getElementById("cal-event-close").addEventListener("click",()=>{document.getElementById("cal-event-list").style.display="none"}),document.querySelectorAll(".cal-filter").forEach(s=>s.addEventListener("change",r));async function a(){try{let s=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`;i=(await x(`/api/dashboard/calendar?month=${s}`)).data?.data||[]}catch(s){console.warn("[Calendar] Failed to load events, rendering empty grid:",s),i=[]}}async function r(){let s=document.getElementById("calendar-grid");if(s){s.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;background:var(--border);">
      ${Array(35).fill('<div style="background:#f8fafc;min-height:70px;"></div>').join("")}
    </div>`,await a();try{let o=e.getFullYear(),l=e.getMonth(),n=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}),d=document.getElementById("cal-month-label");d&&(d.textContent=n);let h=new Set(Array.from(document.querySelectorAll(".cal-filter:checked")).map(C=>C.value)),p=i.filter(C=>h.has(C.type)),c={};p.forEach(C=>{let w=(C.event_date||"").slice(0,10);c[w]||(c[w]=[]),c[w].push(C)});let u=new Date(o,l,1).getDay(),y=new Date(o,l+1,0).getDate(),b=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],m=new Date().toISOString().slice(0,10),f='<div class="calendar-grid">';b.forEach(C=>{f+=`<div class="cal-day-header">${C}</div>`});for(let C=0;C<u;C++)f+='<div class="cal-cell cal-cell-empty"></div>';for(let C=1;C<=y;C++){let w=`${o}-${String(l+1).padStart(2,"0")}-${String(C).padStart(2,"0")}`,T=c[w]||[],$=w===m;f+=`
          <div class="cal-cell ${$?"cal-today":""} ${T.length?"cal-has-events":""}"
               data-date="${w}" tabindex="0" role="button" aria-label="${w}">
            <div class="cal-day-num ${$?"today-num":""}">${C}</div>
            <div class="cal-events-preview">
              ${T.slice(0,3).map(E=>`
                <div class="cal-event-dot cal-color-${E.color||"gray"}" title="${nt(E.title||E.type)}">
                  <span class="cal-event-dot-label">${Ra(E.title||E.branch_name||E.type,18)}</span>
                </div>
              `).join("")}
              ${T.length>3?`<div class="cal-more">+${T.length-3} lagi</div>`:""}
            </div>
          </div>`}let _=(u+y)%7;if(_!==0)for(let C=0;C<7-_;C++)f+='<div class="cal-cell cal-cell-empty"></div>';f+="</div>",s.innerHTML=f,s.querySelectorAll(".cal-cell[data-date]").forEach(C=>{C.addEventListener("click",()=>{let w=C.dataset.date,T=c[w]||[];if(!T.length)return;let $=document.getElementById("cal-event-list"),E=new Date(w+"T00:00:00").toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});document.getElementById("cal-event-date").textContent=E,document.getElementById("cal-event-items").innerHTML=T.map(I=>`
            <div class="cal-event-item cal-color-border-${I.color||"gray"}">
              <div class="cal-event-type">${Ka(I.type)}</div>
              <div class="cal-event-title">${nt(I.title||"-")}</div>
              <div class="cal-event-branch">${nt(I.branch_name||"")}</div>
              ${I.status?`<div class="cal-event-status">${nt(I.status)}</div>`:""}
          ...
          </div>`)}}}r()}function Ra(t,e){return t?t.length>e?t.slice(0,e)+"\u2026":t:""}function nt(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function Ka(t){return{schedule:"\u{1F5D3} Jadwal",issue:"\u26A0\uFE0F Permasalahan",reliever:"\u{1F504} Reliefer",training:"\u{1F393} Training",contract_expiry:"\u{1F4CB} Kontrak Habis"}[t]||t}N();async function ta(t){let e=ce(),i=(e?.full_name||e?.username||"U")[0].toUpperCase(),r={superadmin:"#7C3AED",admin:"#2563EB",manager:"#0891B2",spv:"#059669",viewer:"#64748B"}[e?.role]||"#64748B";t.innerHTML=`
    <div class="page-header">
  ...
  `;let e=null,i=null,a=0,r={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function s(b){Object.entries(r).forEach(([m,f])=>{f.style.display=m===b?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let b=document.getElementById("btn-backup-db");b.disabled=!0,b.textContent="\u23F3 Memproses Backup...";try{let m=await x("/api/import/backup");if(m.ok){if(!window.XLSX){z("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");return}let f=window.XLSX,S=f.utils.book_new();Object.entries(m.data.database).forEach(([_,C])=>{let w=C.length>0?C:[{}],T=f.utils.json_to_sheet(w);f.utils.book_append_sheet(S,T,_.substring(0,31))}),f.writeFile(S,`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.xlsx`),Y("Backup berhasil diunduh!")}else z("Gagal memproses backup: "+(m.data?.error||"Unknown error"))}catch(m){z("Gagal memproses backup: "+m.message)}finally{b.disabled=!1,b.textContent="\u{1F4E6} Backup Database"}});let o=document.getElementById("btn-sync-google");o&&o.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let b=o.innerHTML;o.innerHTML='<span class="spinner"></span> Menyinkronkan...',o.disabled=!0;try{let m=await x("/api/sync/google-sheets",{method:"POST"});m.ok?alert("Sinkronisasi Berhasil: "+(m.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(m.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{o.innerHTML=b,o.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{ia(),Y("Template Excel berhasil didownload!")});let l=document.getElementById("file-input"),n=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",b=>{b.stopPropagation(),l.click()}),l.addEventListener("change",b=>{b.target.files[0]&&d(b.target.files[0])}),n.addEventListener("dragover",b=>{b.preventDefault(),n.classList.add("drag-over")}),n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n.addEventListener("drop",b=>{b.preventDefault(),n.classList.remove("drag-over");let m=b.dataTransfer.files[0];m&&m.name.match(/\.xlsx?$/i)?d(m):z("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,l.value="",document.getElementById("file-info").style.display="none",n.style.display="",s("upload")});async function d(b){e=b,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${b.name} (${(b.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",n.style.display="none",await h(b)}async function h(b){s("validating");let m=document.getElementById("validation-status"),f=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");m.textContent="Membaca file Excel...",f.style.width="20%",await Ge(200);let S=await b.arrayBuffer(),_=window.XLSX.read(S,{type:"array",cellDates:!0});m.textContent=`Memvalidasi ${_.SheetNames.length} sheet...`,f.style.width="50%",await Ge(100),i=na(_),f.style.width="100%",m.textContent="Validasi selesai!",await Ge(300),p()}catch(S){s("upload"),z("Gagal memproses file: "+S.message),document.getElementById("file-info").style.display="flex",n.style.display="none"}}function p(){s("preview");let b=i.filter(E=>!E.skipped).length,m=i.reduce((E,I)=>E+I.total,0),f=i.reduce((E,I)=>E+I.valid,0),S=i.reduce((E,I)=>E+I.errorCount,0),_=m>0?Math.round(f/m*100):0;document.getElementById("preview-summary-badges").innerHTML=`
        ...
        <tbody>
          ${i.map((E,I)=>`
            <tr class="${E.errorCount>0?"row-error":E.skipped?"row-skipped":"row-ok"}">
              ...
              <td style="text-align:center">
                ${E.errorCount>0?`<button class="btn btn-ghost btn-sm btn-detail-error" data-idx="${I}">\u{1F50D} ${E.errorCount} Error</button>`:'<span class="text-muted">\u2013</span>'}
              </td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    `,C.querySelectorAll(".btn-detail-error").forEach(E=>{E.addEventListener("click",()=>{let I=i[Number(E.dataset.idx)];c(I)})});let w=document.getElementById("error-detail-section"),T=document.getElementById("error-detail-container");T.innerHTML="",w.style.display="none";let $=document.getElementById("btn-start-import");f===0?($.disabled=!0,$.innerHTML="\u26A0\uFE0F Tidak Ada Data Valid"):($.disabled=!1,S>0?($.innerHTML=`\u{1F680} Import ${f} Data Valid (${S} dilewati)`,$.title="Baris error akan dilewati, baris valid tetap diimport"):$.innerHTML=`\u{1F680} Mulai Import ${f} Data`)}function c(b){let m=document.getElementById("error-detail-section"),f=document.getElementById("error-detail-container");m.style.display="";let S=b.errors.slice(0,100).map(_=>(Array.isArray(_.errors)?_.errors:[]).map(w=>{let T=typeof w=="object";return`
          <tr>
    ...
    `,m.scrollIntoView({behavior:"smooth",block:"start"})}document.getElementById("btn-back-to-upload").addEventListener("click",()=>{s("upload"),document.getElementById("file-info").style.display="none",n.style.display="",e=null,l.value=""}),document.getElementById("btn-download-log").addEventListener("click",()=>{if(!i)return;ra(i)?Y("Log error berhasil didownload."):Y("Tidak ada error untuk didownload.")}),document.getElementById("btn-start-import").addEventListener("click",()=>{let b=document.querySelector('input[name="dup-strategy"]:checked')?.value||"skip";u(b)});async function u(b){s("importing"),a=Date.now();let m=[];qa.forEach(w=>{let T=i?.find($=>$.module===w&&$.mapped?.length>0);T&&m.push(T)});let f=document.getElementById("import-steps-list");f.innerHTML=m.map(w=>`
    ...
    `).join("");let S=document.getElementById("import-bar"),_=document.getElementById("import-current-status"),C={totalSheets:m.length,totalRows:m.reduce((w,T)=>w+T.mapped.length,0),inserted:0,skipped:0,failed:0,moduleResults:[]};for(let w=0;w<m.length;w++){let T=m[w],$=document.getElementById(`step-icon-${T.module}`),E=document.getElementById(`step-status-${T.module}`);$.textContent="\u{1F504}",E.textContent="Mengimport...",_.textContent=`Mengimport ${T.label}...`,S.style.width=`${Math.round(w/m.length*100)}%`;try{let I=await x(`/api/import/${T.module}`,{method:"POST",body:JSON.stringify({rows:T.mapped,onDuplicate:b})});if(I.ok){let B=I.data;C.inserted+=B.inserted||0,C.skipped+=B.skipped||0,C.moduleResults.push({label:T.label,inserted:B.inserted||0,skipped:B.skipped||0,status:"ok"}),$.textContent="\u2705",E.innerHTML=`<span class="badge badge-success">${B.inserted||0} berhasil</span>${B.skipped>0?` <span class="badge badge-neutral">${B.skipped} skip</span>`:""}`}else C.failed++,C.moduleResults.push({label:T.label,inserted:0,skipped:0,status:"error",error:I.data?.error}),$.textContent="\u274C",E.innerHTML='<span class="badge badge-danger">Gagal</span>'}catch(I){C.failed++,C.moduleResults.push({label:T.label,inserted:0,skipped:0,status:"error",error:I.message}),$.textContent="\u274C",E.innerHTML='<span class="badge badge-danger">Gagal</span>'}await Ge(150)}S.style.width="100%",_.textContent="Selesai!",await Ge(400),y(C)}function y(b){s("summary");let m=((Date.now()-a)/1e3).toFixed(1),f=b.failed===0;document.getElementById("summary-status-icon").innerHTML=`
  ...
  `;de({title:`Notifikasi (${Ae.length})`,content:t,confirmText:"Tutup",onConfirm:(e,i)=>i()})}function pa(){let t=ce(),e=(t?.full_name||"U")[0].toUpperCase();document.getElementById("app").innerHTML=`
  ...
  `;let i=document.getElementById("sidebar"),a=document.getElementById("sidebar-overlay"),r=document.getElementById("topbar-menu-btn"),s=document.getElementById("sidebar-close"),o=()=>{i.classList.add("open"),a.classList.add("show")},l=()=>{i.classList.remove("open"),a.classList.remove("show")};r?.addEventListener("click",o),s?.addEventListener("click",l),a?.addEventListener("click",l),document.querySelectorAll(".nav-item").forEach(d=>d.addEventListener("click",l));function n(){let d=window.location.hash.replace("#","")||"/dashboard";document.querySelectorAll(".nav-item").forEach(c=>{let u=c.dataset.route;c.classList.toggle("active",d===u||u!=="/dashboard"&&d.startsWith(u))});let h=document.getElementById("topbar-title"),p=document.querySelector(".nav-item.active .nav-label");h&&p&&(h.textContent=p.textContent)}window.addEventListener("hashchange",n),n(),Ja(),document.getElementById("btn-fullscreen")?.addEventListener("click",()=>{document.fullscreenElement?document.exitFullscreen?.():document.documentElement.requestFullscreen?.()}),document.getElementById("logout-btn")?.addEventListener("click",async()=>{await x("/api/auth/logout",{method:"POST"}),Oe(),ze&&clearInterval(ze),$e("/login")}),Ua(),Ga(),document.getElementById("btn-notif")?.addEventListener("click",d=>{d.preventDefault(),za()})}async function Qa(){U("/login",({main:e})=>Mt(e)),U("/dashboard",Q(({main:e})=>Pt(e))),U("/calendar",Q(({main:e})=>ea(e))),U("/employees",Q(({main:e,params:i})=>Ot(e,i))),U("/contracts",Q(({main:e,params:i})=>tt(e,i))),U("/sp",Q(({main:e})=>sa(e))),U("/mutasi",Q(({main:e})=>da(e))),U("/timeline",Q(({main:e,params:i})=>Kt(e,i))),U("/issues",Q(({main:e,params:i})=>jt(e,i))),U("/one-on-one",Q(({main:e,params:i})=>Ht(e,i))),U("/training",Q(({main:e})=>qt(e))),U("/relievers",Q(({main:e,params:i})=>Ut(e,i))),U("/reports/inspection",Q(({main:e})=>Gt(e))),U("/reports/cleaning",Q(({main:e})=>zt(e))),U("/reports/fogging",Q(({main:e})=>Qt(e))),U("/reports/basecamp",Q(({main:e})=>Vt(e))),U("/reports/supply",Q(({main:e})=>St(e,"supply"))),U("/sop",Q(({main:e})=>Wt(e))),U("/checklist",Q(({main:e})=>Yt(e))),U("/forms",Q(({main:e})=>St(e))),U("/users",Q(({main:e})=>Xt(e))),U("/branches",Q(({main:e})=>Zt(e))),U("/profile",Q(({main:e})=>ta(e))),U("/settings/import",Q(({main:e})=>la(e)));let t=Me();if(!t&&window.location.hash!=="#/login"&&$e("/login"),t){let e=await x("/api/auth/me");e.ok?(Re(e.data.data),pa()):(Oe(),$e("/login"))}window.addEventListener("fm:login",()=>{pa(),$e("/dashboard")}),xt()}Qa();
