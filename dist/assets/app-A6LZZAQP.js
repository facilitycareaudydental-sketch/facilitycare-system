var da=Object.defineProperty;var nt=(t,e)=>()=>(t&&(e=t(t=0)),e);var it=(t,e)=>{for(var l in e)da(t,l,{get:e[l],enumerable:!0})};var ke={};it(ke,{API:()=>ft,CLIENT_SIDE_MAX_ROWS:()=>ye,IS_DEVELOPMENT:()=>Pe,apiFetch:()=>k,clearToken:()=>Le,getToken:()=>Be,getUser:()=>oe,setToken:()=>rt,setUser:()=>Ae});function Be(){return localStorage.getItem("fm_token")}function rt(t){localStorage.setItem("fm_token",t)}function Le(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function oe(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Ae(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let l=Be(),a={"Content-Type":"application/json",...l?{Authorization:`Bearer ${l}`}:{},...e.headers||{}};try{let s=`cb=${Date.now()}`,o=t.includes("?")?"&":"?",r=`${ft}${t}${o}${s}`,n=await fetch(r,{...e,headers:a}),i;try{let d=await n.text();try{i=JSON.parse(d)}catch{i={error:`Server Error (${n.status}): ${d.substring(0,80)}...`}}}catch{i={error:"Gagal membaca respon dari server"}}return n.status===401&&(Le(),window.location.hash="#/login"),{ok:n.ok,status:n.status,data:i}}catch(s){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${s.message})`}}}}var Pe,ca,ft,ye,O=nt(()=>{Pe=!1,ca="https://fm-operations-api.facilitycare-audydental.workers.dev",ft=ca,ye=1e4});var St={};it(St,{confirmDialog:()=>Fe,createModal:()=>de});function de({title:t,content:e,onConfirm:l,onCancel:a,confirmText:s="Simpan",cancelText:o="Batal",size:r="md",confirmClass:n="btn-primary"}){let i={sm:"400px",md:"560px",lg:"720px",xl:"900px"},d=document.createElement("div");d.className="modal-overlay",d.innerHTML=`
  ...
  `;function C(){if(!document.getElementById("bulk-toolbar"))return;let f=document.getElementById("bulk-count"),D=document.getElementById("btn-bulk-delete"),B=document.getElementById("btn-bulk-cancel");f.textContent=`${$.size} item dipilih`,$.size>0?(D.disabled=!1,B.disabled=!1):(D.disabled=!0,B.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{$.clear(),document.querySelectorAll(".row-checkbox").forEach(f=>f.checked=!1);let v=document.getElementById("select-all-checkbox");v&&(v.checked=!1),C()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if($.size===0)return;let v=[...$],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",f.innerHTML=`
    ...
    `,document.body.appendChild(f),f.querySelector("#bulk-cancel-btn").addEventListener("click",()=>f.remove()),f.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let D=f.querySelector("#bulk-confirm-btn");D.disabled=!0,D.textContent="Menghapus...";let B=await k(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:v})});f.remove(),B.ok?(ee(`${v.length} ${i} berhasil dihapus.`),$.clear(),C(),J()):Q(B.data?.error||"Gagal menghapus data.")})});let L=document.getElementById("filter-search"),R;if(L?.addEventListener("input",v=>{clearTimeout(R),R=setTimeout(()=>{T.search=v.target.value,S=1,$.clear(),J()},400)}),r?.forEach(v=>{v.type==="select"&&document.getElementById(`filter-${v.name}`)?.addEventListener("change",f=>{T[v.name]=f.target.value,S=1,$.clear(),J()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{T={...n},L&&(L.value=""),r?.forEach(v=>{let f=document.getElementById(`filter-${v.name}`);f&&(f.value="")}),S=1,$.clear(),J()}),document.getElementById("btn-create")?.addEventListener("click",()=>me(null)),y){document.getElementById(`btn-export-${y.moduleName}`)?.addEventListener("click",async f=>{let D=f.target,B=D.innerHTML;D.innerHTML="\u23F3 Loading...",D.disabled=!0;try{await y.onExport()}catch{Q("Gagal export data")}finally{D.innerHTML=B,D.disabled=!1}}),document.getElementById(`btn-template-${y.moduleName}`)?.addEventListener("click",()=>{y.onTemplate()});let v=document.getElementById(`input-import-${y.moduleName}`);v?.addEventListener("change",async f=>{let D=f.target.files[0];if(!D)return;v.disabled=!0;let B=document.createElement("div");B.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",B.innerHTML=`
      ...
      `,document.body.appendChild(B);let V=B.querySelector("#import-progress-text"),G=B.querySelector("#import-progress-bar"),I=B.querySelector("#import-summary"),P=B.querySelector("#import-close-btn");P.addEventListener("click",()=>{B.remove(),J()});try{let X=await Oe(D);if(X.length===0)throw new Error("File kosong atau format salah");let Z=500,le=0,ne=0,F=0,M=X.length;V.textContent=`Ditemukan ${M} baris data. Memulai import...`;for(let E=0;E<M;E+=Z){let W=X.slice(E,E+Z);V.textContent=`Mengimport baris ${E+1} - ${Math.min(E+Z,M)} dari ${M}...`,G.style.width=`${Math.round(E/M*100)}%`;try{let j=await y.onImport(W);j?(le+=j.inserted||j.metrics?.inserted||W.length,ne+=j.skipped||j.metrics?.updated||0):le+=W.length}catch(j){console.error("Chunk import failed:",j),F+=W.length}}G.style.width="100%",V.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',I.style.display="block",I.innerHTML=`
        ...
        `,F>0&&(I.innerHTML+='<p style="margin-top:12px;font-size:0.8rem;color:var(--danger)">Sebagian data gagal diimport. Pastikan format kolom sesuai template dan tidak ada data kosong pada kolom wajib.</p>'),P.style.display="block",v.value=""}catch(X){V.innerHTML=`<strong style="color:var(--danger)">\u274C Gagal Memproses File</strong><br>${X.message}`,G.style.background="var(--danger)",G.style.width="100%",P.style.display="block",v.value=""}finally{v.disabled=!1}})}async function J(){C();let v=document.getElementById("table-container");if(!v)return;v.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';let f=x==="client",D=f?1:S,B=f?ye:20,V=new URLSearchParams({page:D,limit:B,...Object.fromEntries(Object.entries(T).filter(([,F])=>F))}),G=await k(`${a}?${V}`);if(!G.ok){v.innerHTML=`<div class="empty-state"><p class="text-danger">Gagal memuat data: ${G.data?.error||"Error"}</p></div>`;return}let I=G.data?.data||G.data||[],P=G.data?.pagination,X=I.length,Z=I;if(f){I=h(I),Z=I;let F=I.length,M=20,E=Math.ceil(F/M);S>E&&E>0&&(S=E);let W=(S-1)*M,j=S*M;I=I.slice(W,j),P={page:S,limit:M,total:F,pages:E}}!1,m&&m(I);let le=Ge({columns:s,data:I,fullData:Z,onEdit:g?F=>me(F):null,actions:u.map(F=>({...F,handler:M=>F.handler(M,J)})),emptyText:`Tidak ada ${String(i||"").toLowerCase()}`,bulkSelect:_?{selectedIds:$,onToggle:C}:null});v.innerHTML="",v.appendChild(le);let ne=document.getElementById("pagination-container");if(ne&&(ne.innerHTML="",P&&P.pages>1)){let F=Qe({page:P.page,pages:P.pages,total:P.total,limit:P.limit,onPage:M=>{S=M,J()}});F&&ne.appendChild(F)}}function ve(v){let f=typeof o=="function"?o(v):o;return Me(f)}function me(v){let f=!!v,D=document.createElement("form");if(D.noValidate=!0,D.innerHTML=ve(v),f){let V=typeof o=="function"?o(v):o;We(D,v)}let{close:B}=de({title:f?`Edit ${i}`:`Tambah ${i}`,content:D,size:"lg",confirmText:f?"Simpan Perubahan":`Tambah ${i}`,onConfirm:async(V,G)=>{if(!D.reportValidity())return;let I=V.querySelector(".modal-confirm");I.disabled=!0,I.textContent="Menyimpan...";let P=ze(D),X=typeof o=="function"?o(v):o,Z=async M=>{for(let E of M)if(E.type==="row")await Z(E.fields);else if(E.type==="combobox"&&P[E.name]){let W=P[E.name],j=(E.options||[]).find(Y=>{let ie=String(typeof Y=="object"?Y.value:Y),at=String(typeof Y=="object"?Y.label:Y);return ie===W||at===W});if(j)P[E.name]=typeof j=="object"?j.value:j;else if(E.createApi){let Y={};Y[E.createApi.field]=W,E.createApi.extra&&Object.assign(Y,E.createApi.extra);let ie=await k(E.createApi.path,{method:"POST",body:JSON.stringify(Y)});if(ie.ok&&ie.data?.id)P[E.name]=ie.data.id;else if(ie.ok&&!ie.data?.id)P[E.name]=W;else throw new Error(`Gagal membuat master data: ${ie.data?.error||"Unknown error"}`)}}};try{await Z(X)}catch(M){Q(M.message),I.disabled=!1,I.textContent=f?"Simpan Perubahan":`Tambah ${i}`;return}p&&(P=await p(P,v));let le=f?"PUT":"POST",ne=f?`${a}/${v.id}`:a,F=await k(ne,{method:le,body:JSON.stringify(P)});F.ok?(ee(f?`${i} berhasil diperbarui.`:`${i} berhasil ditambahkan.`),G(),J()):(Q(F.data?.error||"Gagal menyimpan data."),I.disabled=!1,I.textContent=f?"Simpan Perubahan":`Tambah ${i}`)}})}function Je(v){Fe(`Hapus ${i} ini? Tindakan tidak dapat dibatalkan.`,async()=>{let f=await k(`${a}/${v.id}`,{method:"DELETE"});f.ok?(ee(`${i} berhasil dihapus.`),J()):Q(f.data?.error||"Gagal menghapus.")},`Hapus ${i}`)}return J(),J}O();O();var we=null,Ve=null;async function xe(t=!1){if(we&&!t)return console.log("Employees Raw (Cache Hit)",we.slice(0,5)),we;let e=await k(`/api/employees?limit=${ye}&status=Aktif`);return we=(e.data?.data||[]).map(l=>({value:l.id,label:l.full_name})),console.log("Employees Raw",e.data?.data?.slice(0,5)),console.log("Employees Mapped (ID)",we.slice(0,5)),we}async function te(t=!1){let l=(await xe(t)).map(a=>({value:a.label,label:a.label}));return console.log("Employee Options",l.slice(0,5)),l}async function H(t=!1){return Ve&&!t||(Ve=((await k("/api/branches?all=1")).data?.data||[]).map(l=>({value:l.id,label:l.full_name}))),Ve}function q(t){let e={Done:"badge-success",Aktif:"badge-success",Open:"badge-warning","In Progress":"badge-info",Pending:"badge-warning",Diproses:"badge-info",Selesai:"badge-success","Tidak Aktif":"badge-neutral",Resign:"badge-neutral",Cut:"badge-danger","Tidak Datang":"badge-danger"};return!t||t==="-"||String(t).trim()===""?"":`<span class="badge ${e[t]||"badge-neutral"}">${t}</span>`}function st(t){return t==null?'<span class="badge badge-neutral">-</span>':t<0?`<span class="badge badge-danger">Expired (${Math.abs(t)}h)</span>`:t<=14?`<span class="badge badge-danger">${t} hari</span>`:t<=30?`<span class="badge badge-warning">${t} hari</span>`:`<span class="badge badge-success">${t} hari</span>`}function _e(t){return`<span class="badge ${{"FACILITY CARE":"badge-info",SECURITY:"badge-secondary"}[t]||"badge-neutral"}">${t||"-"}</span>`}function ot(t){return`<span class="badge ${{"Inspeksi Hygiene & Aset Bangunan":"badge-info","General Cleaning":"badge-success","Deep Cleaning":"badge-purple",Fogging:"badge-warning"}[t]||"badge-neutral"}">${t||"-"}</span>`}function ce(t){return`<span class="badge ${{Q1:"badge-info",Q2:"badge-success",Q3:"badge-warning",Q4:"badge-danger"}[t]||"badge-neutral"}">${t||"-"}</span>`}K();function dt(t,e){let l=new Date,a=`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}`;if(!(t.target_date||t.opening_date||"").startsWith(a))return!1;let o=String(t.status||"").toLowerCase();if(o!=="selesai"&&o!=="completed"&&o!=="done")return!1;let r=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?r.includes("inspeksi"):e==="gcdc"?r.includes("general cleaning")||r.includes("deep cleaning"):!1}O();K();function wt(t,e){let l=String(t.status||"").toLowerCase();return e==="active"?l==="aktif":e==="reliefer"?t.division==="FC - RELIEFER"&&l==="aktif":!1}O();K();function ct(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let s=new Date(a);s.setDate(a.getDate()+30);let o=new Date(t.end_date);return o.setHours(0,0,0,0),o>=a&&o<=s}return!1}O();K();function xt(t,e){let l=String(t.status||"").toLowerCase();return e==="open"?l==="open":!1}O();function _t(t,e){let l=String(t.status||"").toLowerCase();return e==="pending"?l==="pending":!1}var he={};function $e(t){if(he[t]){try{he[t].destroy()}catch{}delete he[t]}}function ua(){Object.keys(he).forEach($e)}var ge=(t,e=0)=>{let l=Number(t);return isNaN(l)||t===null||t===void 0?e:l},Ce=(t,e="\u2014")=>{if(t==null||t==="")return e;let l=String(t).trim();return l===""||l==="[object Object]"?e:l};function Tt(t,e,l=900){if(!t)return;let a=Math.max(0,Math.round(ge(e)));if(a===0){t.textContent="0";return}let s=Date.now(),o=()=>{let r=Math.min((Date.now()-s)/l,1),n=1-Math.pow(1-r,3);t.textContent=Math.round(n*a).toLocaleString("id-ID"),r<1?requestAnimationFrame(o):t.textContent=a.toLocaleString("id-ID")};requestAnimationFrame(o)}var ma={Done:"pill-success",Aktif:"pill-success",Selesai:"pill-success",Open:"pill-danger",Pending:"pill-warning","In Progress":"pill-info","Tidak Aktif":"pill-neutral",Resign:"pill-neutral",Cut:"pill-neutral"},ga=t=>{let e=Ce(t,"\u2014");return`<span class="status-pill ${ma[e]||"pill-neutral"}">${e}</span>`};var pe={family:"Inter",size:11},be="#94A3B8",Te="#F1F5F9",pt=["#2563EB","#10B981","#F59E0B","#EF4444","#8B5CF6","#0EA5E9","#F97316","#14B8A6","#6366F1","#EC4899"],ba=()=>window.innerWidth<768;function Ye(t={}){return{responsive:!0,maintainAspectRatio:!1,animation:{duration:700,easing:"easeOutQuart"},plugins:{legend:{position:ba()?"bottom":"top",labels:{font:pe,color:"#64748B",usePointStyle:!0,padding:10,boxWidth:8,boxHeight:8}},tooltip:{mode:"index",intersect:!1,bodyFont:pe,titleFont:{...pe,weight:"700"}}},scales:{x:{grid:{color:Te},ticks:{font:pe,color:be,maxRotation:0}},y:{grid:{color:Te},ticks:{font:pe,color:be},beginAtZero:!0}},...t}}var ha=()=>Array(5).fill(0).map(()=>`
  ...
  </div>`).join("");function Ct(t=3){return Array(t).fill(0).map((e,l)=>`<div class="skeleton skeleton-text" style="height:38px;margin-bottom:${l<t-1?"6px":"0"};border-radius:6px"></div>`).join("")}async function re(t,e,l=8e3){try{let a=new AbortController,s=setTimeout(()=>a.abort(),l),o=await k(t,{signal:a.signal}).catch(()=>null);if(clearTimeout(s),!o||!o.ok)return e;let r=o.data;return r?r.data!==void 0?r.data??e:r:e}catch{return e}}function fa(){["skel-donut","skel-trend","skel-insp","skel-contract","skel-jadwal"].forEach(a=>{let s=document.getElementById(a);s&&(s.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract","chart-jadwal"].forEach(a=>{let s=document.getElementById(a);if(s&&s.style.display==="none"){s.style.display="block";let o=s.parentElement;if(o&&!o.querySelector(".chart-empty")){let r=document.createElement("div");r.className="chart-empty",r.textContent="Belum ada data",s.style.display="none",o.appendChild(r)}}});let t=document.getElementById("kpi-row");t&&t.querySelector(".skeleton")&&Et({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&Dt({}),["table-contracts","table-issues"].forEach(a=>{let s=document.getElementById(a);s&&s.querySelector(".skeleton")&&(s.innerHTML='<div class="chart-empty">Belum ada data</div>')});let l=document.getElementById("activity-log");l&&l.querySelector(".skeleton")&&(l.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function $t(t){ua(),t._dashRefresh&&clearInterval(t._dashRefresh),t._skelTimeout&&clearTimeout(t._skelTimeout);let e=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>ut(t)),document.getElementById("filter-jadwal-year")?.addEventListener("change",async l=>{let a=l.target.value,s=document.getElementById("jadwal-year-label");s&&(s.textContent=a);let o=document.getElementById("skel-jadwal"),r=document.getElementById("chart-jadwal");o&&(o.style.display="block",o.style.position="absolute"),r&&(r.style.display="none");let n=await re(`/api/dashboard/schedule-chart?year=${a}`,{},8e3);try{It(n)}catch(i){console.warn("ScheduleChart render:",i),ue("skel-jadwal","chart-jadwal")}}),document.getElementById("filter-insp-month")?.addEventListener("change",async l=>{let a=l.target.value,s=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",o=document.getElementById("skel-insp"),r=document.getElementById("chart-insp");o&&(o.style.display="block",o.style.position="absolute"),r&&(r.style.display="none");let n=await re(s,{},8e3);try{Pt(n)}catch(i){console.warn("InspBar render:",i),ue("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>fa(),5e3),await ut(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?ut(t):clearInterval(t._dashRefresh)},6e4)}async function ut(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,l,a,s,o,r,n,i,d,g,c,p,m,h]=await Promise.all([re("/api/dashboard/kpi",{},8e3),re("/api/dashboard/issues-trend",{},8e3),re("/api/dashboard/issues-summary",{},8e3),re("/api/dashboard/stats",{},8e3),re("/api/dashboard/calendar",[],8e3),re("/api/schedule?limit=10000",{data:[]},8e3),re("/api/employees?limit=10000",{data:[]},8e3),re("/api/contracts?limit=10000",{data:[]},8e3),re("/api/issues?limit=10000",{data:[]},8e3),re("/api/one-on-one?limit=10000",{data:[]},8e3),re("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3),re(`/api/dashboard/schedule-chart?year=${document.getElementById("filter-jadwal-year")?.value||new Date().getFullYear()}`,{},8e3),re("/api/relievers?limit=10000",{data:[]},8e3),re("/api/reports/fogging?limit=10000",{data:[]},8e3)]),u=document.getElementById("filter-insp-month"),b=u?u.value:"",y=b?`/api/dashboard/inspection-bar?month=${b}`:"/api/dashboard/inspection-bar",_=await re(y,{},8e3);if(e){let x=Array.isArray(r?.data)?r.data:Array.isArray(r)?r:[];window.dashboardSchedules=x;let w=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],S=Array.isArray(i?.data)?i.data:Array.isArray(i)?i:[],T=Array.isArray(d?.data)?d.data:Array.isArray(d)?d:[],$=Array.isArray(g?.data)?g.data:Array.isArray(g)?g:[],C=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[];window.dashboardRelievers=C;let L=Array.isArray(h?.data)?h.data:Array.isArray(h)?h:[];window.dashboardFogging=L,e.employees&&(e.employees.current=w.filter(R=>wt(R,"active")).length),e.contracts&&(e.contracts.current=S.filter(R=>ct(R,"active")).length),e.expiring30&&(e.expiring30={current:S.filter(R=>ct(R,"expiring30")).length}),e.issues&&(e.issues.current=T.filter(R=>xt(R,"open")).length),e.one_on_one&&(e.one_on_one.current=$.filter(R=>_t(R,"pending")).length),e.inspection_month&&(e.inspection_month.current=x.filter(R=>dt(R,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=x.filter(R=>dt(R,"gcdc")).length)}try{Et(e)}catch(x){console.warn("KPI render:",x)}try{Dt(e)}catch(x){console.warn("MiniStats render:",x)}try{It(p)}catch(x){console.warn("ScheduleChart render:",x),ue("skel-jadwal","chart-jadwal")}try{va(Array.isArray(a?.by_category)?a.by_category:[])}catch(x){console.warn("Donut render:",x),ue("skel-donut","chart-donut")}try{ka(l)}catch(x){console.warn("Trend render:",x),ue("skel-trend","chart-trend")}try{Pt(_)}catch(x){console.warn("InspBar render:",x),ue("skel-insp","chart-insp")}try{let x=Array.isArray(s)?s:Array.isArray(s?.recent_issues)?s.recent_issues:[];wa(x)}catch(x){console.warn("IssuesTable render:",x)}try{let x=Array.isArray(s?.expiring_contracts)?s.expiring_contracts:[];Sa(c)}catch(x){console.warn("ContractsTable render:",x)}try{xa(Array.isArray(o)?o:[])}catch(x){console.warn("Agenda render:",x)}try{_a()}catch(x){console.warn("Quick Actions render:",x)}}function Et(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let l=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=l.map(a=>{let s=ge(t[a.key]?.current,0);return`
      ...
      </a>`}).join(""),e.querySelectorAll(".kpi-value").forEach(a=>{Tt(a,parseInt(a.dataset.target)||0)})}function Dt(t){let e=document.getElementById("mini-stats-row");if(!e)return;t=t||{};let l=`Q${Math.ceil((new Date().getMonth()+1)/3)}`,a=new Date().getFullYear(),s=String(new Date().getMonth()+1).padStart(2,"0"),o=`${a}-${s}`,r=c=>`
  ...
  `,n=[{id:"mini-jadwal",icon:"\u{1F4C5}",label:"Jadwal",dropdown:`
    ...
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(c=>Tt(c,parseInt(c.dataset.target)||0,700));let i=document.getElementById("dash-jadwal-period");i&&i.addEventListener("change",c=>{let p=c.target.value,m=(window.dashboardSchedules||[]).filter(b=>b.period===p).length,h=document.querySelector("#mini-jadwal .mini-stat-value");h&&(h.dataset.target=m,h.textContent=m);let u=document.getElementById("mini-jadwal");u&&(u.href=`#/timeline?dash_filter=period_${p.toLowerCase()}`)});let d=(c,p,m,h,u)=>{let b=document.getElementById(c);b&&b.addEventListener("change",y=>{let _=y.target.value,x=(m||[]).filter(T=>h(T,_)).length,w=document.querySelector(`#${p} .mini-stat-value`);w&&(w.dataset.target=x,w.textContent=x);let S=document.getElementById(p);S&&(S.href=`${u}&month=${_}`)})},g=c=>{let p=String(c.status||"").toLowerCase();return p==="done"||p==="selesai"||p==="completed"};d("dash-reliefer-month","mini-reliefer",window.dashboardRelievers,(c,p)=>window.parseFlexibleDate(c.backup_date).startsWith(p)&&g(c),"#/relievers?dash_filter=reliever"),d("dash-inspeksi-month","mini-inspeksi",window.dashboardSchedules,(c,p)=>c.activity_type==="Inspeksi Hygiene"&&g(c)&&window.parseFlexibleDate(c.opening_date||c.target_date).startsWith(p),"#/timeline?dash_filter=inspeksi"),d("dash-gcdc-month","mini-gcdc",window.dashboardSchedules,(c,p)=>(c.activity_type==="General Cleaning"||c.activity_type==="Deep Cleaning")&&g(c)&&window.parseFlexibleDate(c.opening_date||c.target_date).startsWith(p),"#/timeline?dash_filter=gcdc"),d("dash-fogging-month","mini-fogging",window.dashboardFogging,(c,p)=>g(c)&&window.parseFlexibleDate(c.activity_date).startsWith(p),"#/reports/fogging?dash_filter=fogging")}function va(t){ue("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),l=document.getElementById("donut-legend");if(!e||!l)return;$e("donut");let a=(t||[]).filter(i=>ge(i.count)>0);if(!a.length){Ke(e,"Belum ada data permasalahan");return}let s=a.map(i=>`${Ce(i.category,"Lainnya")}`),o=a.map(i=>ge(i.count)),r=o.reduce((i,d)=>i+d,0);l.innerHTML=a.map((i,d)=>{let g=pt[d%pt.length],c=r>0?Math.round(i.count/r*100):0;return`
    ...
    `}).join("");let n={id:"centerText",beforeDraw:function(i){let d=i.width,g=i.height,c=i.ctx;c.restore();let p=(g/80).toFixed(2);c.font="bold "+p+"em Inter",c.textBaseline="middle",c.fillStyle="#1E293B";let m=r.toString(),h=Math.round((d-c.measureText(m).width)/2),u=g/2;c.fillText(m,h,u-4),c.font="600 "+(p*.35).toFixed(2)+"em Inter",c.fillStyle="#64748B";let b="Total",y=Math.round((d-c.measureText(b).width)/2);c.fillText(b,y,u+10),c.save()}};he.donut=new Chart(e,{type:"doughnut",data:{labels:s,datasets:[{data:o,backgroundColor:pt,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:pe,titleFont:{...pe,weight:"700"},callbacks:{label:i=>` ${i.label}: ${i.parsed} kasus`}}},cutout:"75%"},plugins:[n]})}function ka(t){ue("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;$e("trend"),t=t||{};let l=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],a=(t.labels||[]).map(r=>{if(!r||typeof r!="string")return"";try{let[n,i]=r.split("-");return(l[Number(i)-1]||i)+" "+String(n).slice(-2)}catch{return r}}),s=(t.open||[]).map(r=>ge(r)),o=(t.closed||[]).map(r=>ge(r));if(!a.length){Ke(e,"Belum ada data trend");return}he.trend=new Chart(e,{type:"line",data:{labels:a,datasets:[{label:"Open",data:s,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:o,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.1)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#10B981",borderWidth:2}]},options:Ye({plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{family:"Inter",size:9},color:be,maxRotation:0,autoSkip:!1}},y:{grid:{color:Te},ticks:{font:{family:"Inter",size:9},color:be},beginAtZero:!0}}})})}function It(t){ue("skel-jadwal","chart-jadwal");let e=document.getElementById("chart-jadwal");if(!e)return;$e("jadwal"),t=t||{};let l=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];if(!Object.values(t).some(i=>Array.isArray(i)&&i.some(d=>d>0))){Ke(e,"Belum ada data jadwal");return}let s=t["Inspeksi Hygiene"]||Array(12).fill(0),o=t["General Cleaning"]||Array(12).fill(0),r=t["Deep Cleaning"]||Array(12).fill(0),n=t.Fogging||Array(12).fill(0);he.jadwal=new Chart(e,{type:"bar",data:{labels:l,datasets:[{label:"Inspeksi",data:s,backgroundColor:"#3B82F6"},{label:"General Cleaning",data:o,backgroundColor:"#10B981"},{label:"Deep Cleaning",data:r,backgroundColor:"#F59E0B"},{label:"Fogging",data:n,backgroundColor:"#EF4444"}]},options:Ye({plugins:{legend:{display:!1}},datasets:{bar:{barPercentage:.85,categoryPercentage:.9}},scales:{x:{stacked:!0,grid:{display:!1},ticks:{font:{family:"Inter",size:9},color:be,maxRotation:0,autoSkip:!1}},y:{stacked:!0,grid:{color:Te},ticks:{font:{family:"Inter",size:9},color:be},min:0}}})})}function Pt(t){ue("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;$e("inspBar"),t=t||{};let l=t.labels||[],a=(t.fc||[]).map(o=>ge(o)),s=(t.spv||[]).map(o=>ge(o));if(!l.length){Ke(e,"Belum ada data inspeksi");return}he.inspBar=new Chart(e,{type:"bar",data:{labels:l,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:s,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:Ye({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:be,maxRotation:45,minRotation:30}},y:{grid:{color:Te},ticks:{font:pe,color:be},min:0,max:100}}})})}function Sa(t){ue("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;$e("contractMiniBar"),t=t||{};let l={"06":"Jun","07":"Jul","08":"Agu","09":"Sep",10:"Okt",11:"Nov",12:"Des"},a=(t.labels||[]).map(r=>{let n=r.split("-")[1];return l[n]||r}),s=(t.data||[]).map(r=>ge(r));if(!a.length){Ke(e,"Belum ada data");return}let o=e.getContext("2d");he.contractMiniBar=new Chart(e,{type:"bar",data:{labels:a,datasets:[{label:"Kontrak Habis",data:s,backgroundColor:"#3B82F6",borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:Ye({onClick:(r,n)=>{if(n&&n.length>0){let i=n[0].index,d=(t.labels||[])[i];d&&(window.location.hash="#/contracts?month_expiry="+d)}},plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:be,maxRotation:0,autoSkip:!1}},y:{grid:{color:Te,borderDash:[4,4],drawBorder:!1},ticks:{font:pe,color:be,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function wa(t){let e=document.getElementById("table-issues");if(!e)return;let l=(t||[]).slice(0,8);if(!l.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${l.map(a=>`
      ...
      `).join("")}
    </div>`}function xa(t){let e=document.getElementById("widget-agenda");if(!e)return;let l=new Date,a=`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}-${String(l.getDate()).padStart(2,"0")}`,o=(t||[]).filter(r=>(r.event_date||"").startsWith(a)).slice(0,10);if(!o.length){e.innerHTML="";return}e.innerHTML=`
  ...
  `).join("")}function ue(t,e){let l=document.getElementById(t),a=document.getElementById(e);if(l&&(l.style.display="none",l.style.position=""),a){a.style.display="block";let s=a.parentElement;if(s){let o=s.querySelector(".chart-empty");o&&o.remove()}}}function Ke(t,e="Belum ada data"){if(!t)return;t.style.display="none";let l=t.parentElement;if(!l)return;if(!l.querySelector(".chart-empty")){let s=document.createElement("div");s.className="chart-empty",s.textContent=e,l.appendChild(s)}}O();async function Bt(t){document.getElementById("app").innerHTML=`
...
`)}let S=d(_,y),T=null;return S||(T="Karyawan tidak ditemukan di Database"),{isValid:!!S,invalidReason:T,rowNum:y,data:{employee_id:S,branch_id:i(String(u.Cabang||"").trim()),division:String(u["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:w,end_date:g(u["Tanggal Selesai"])||"2099-12-31",status:String(u.Status||"").trim(),_rawName:_}}}),p=[],m=[];if(c.forEach(u=>{u.isValid?p.push(u.data):m.push({rowNum:u.rowNum,name:u.data._rawName,reason:u.invalidReason})}),console.log(`Split Validation - Valid: ${p.length}, Invalid: ${m.length}`),p.length===0)return{inserted:0,skipped:a.length,failed:a.length};let h=await k("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:p,onDuplicate:"update"})});if(!h.ok)throw new Error(h.data?.error||"Import gagal");return h.data}}})}O();K();var bt=[],He=[];function Da(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let l of e)if(t.some(a=>a.period===l))return l;return"Q3"}async function Ft(t,e){bt=await H();let l=await te();He=["Berlin Ariansyah","Ade Surahman"];let a=m=>m&&!He.find(h=>String(typeof h=="object"?h.value:h).toLowerCase()===String(m).toLowerCase())?[...He,m]:He,s=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),o=m=>{if(!m||m==="-"||String(m).trim()==="")return"";let h=String(m).split("-");return h.length===3&&h[0].length===4?`${h[2]}-${h[1]}-${h[0]}`:m},r=s.data?.data||[],n=Da(r),i=e?e.get("dash_filter"):null,d=new Date,g=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`,c={},p=e&&e.get("month")?e.get("month"):null;i==="inspeksi"?c={status:"Done",activity_type:"Inspeksi Hygiene",month:p}:i==="gcdc"?c={status:"Done",activity_type:"GCDC",month:p}:i&&i.startsWith("period_")&&(c={period:i.replace("period_","").toUpperCase()}),N({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:c,onDataLoaded:m=>m.sort((h,u)=>{let b=h.opening_date?new Date(h.opening_date).getTime():0;return(u.opening_date?new Date(u.opening_date).getTime():0)-b}),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:m=>ot(m)},{key:"period",label:"Periode",render:m=>ce(m)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:m=>o(m)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:m=>o(m)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:m=>o(m)},{key:"status",label:"Status",render:m=>q(m)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"activity_type",label:"Kegiatan",options:[{value:"Inspeksi Hygiene",label:"Inspeksi Hygiene"},{value:"General Cleaning",label:"General Cleaning"},{value:"Deep Cleaning",label:"Deep Cleaning"},{value:"Fogging",label:"Fogging"},{value:"GCDC",label:"GCDC (GC & DC)"}]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:He}],formFields:m=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:m?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:m?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:m?.period},{name:"pic",label:"PIC",type:"combobox",options:a(m?.pic),value:m?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:m?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:m?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:m?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:m?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:m?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let m=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(m.ok){let h=m.data.data.map(u=>({Cabang:u.branch_name||"",Kegiatan:u.activity_type||"",Periode:u.period||"",PIC:u.pic||"","Tgl Opening":u.opening_date||"","Tgl Target":u.target_date||"","Tgl Selesai":u.completion_date||"",Status:u.status||""}));A(h,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async m=>{let u=(await k("/api/branches?all=1")).data?.data||[],b=w=>{if(!w)return null;let S=String(w||"").toLowerCase(),T=u.find($=>String($.full_name||"").toLowerCase()===S||String($.code||"").toLowerCase()===S||String($.name||"").toLowerCase()===S);return T?T.id:null},y=w=>{if(w==null||w==="")return"";if(w instanceof Date&&!isNaN(w.getTime()))return w.toISOString().slice(0,10);let S=String(w).trim();if(S===""||S==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(S))return S.slice(0,10);if(/^\d{4,5}$/.test(S)){let $=Number(S);if($>2e4&&$<99999){let C=new Date(Date.UTC(1899,11,30)+$*864e5);return isNaN(C.getTime())?"":C.toISOString().slice(0,10)}}let T=S.split(/[\/\-\.]/);if(T.length===3){let[$,C,L]=T.map(R=>R.trim());if($.length===4&&C.length<=2&&L.length<=2)return`${$}-${C.padStart(2,"0")}-${L.padStart(2,"0")}`;if(L.length===4&&C.length<=2&&$.length<=2)return`${L}-${C.padStart(2,"0")}-${$.padStart(2,"0")}`}return S},_=m.map(w=>({branch_id:b(String(w.Cabang||"").trim()),activity_type:String(w.Kegiatan||"").trim(),period:String(w.Periode||"").trim(),pic:String(w.PIC||w.Pic||"").trim(),opening_date:y(w["Tgl Opening"]||w["Tanggal Opening"]||w["Tgl Openir"]),target_date:y(w["Tgl Target"]||w["Tanggal Target"]),completion_date:y(w["Tgl Selesai"]||w["Tanggal Selesai"]),status:String(w.Status||"").trim(),notes:String(w.Catatan||w.Keterangan||"").trim()})).filter(w=>w.activity_type&&w.period),x=await k("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:_,onDuplicate:"update"})});if(!x.ok)throw new Error(x.data?.error||"Import gagal");return x.data}}})}O();K();var ht=[],Xe=[];function Ia(t,e){let l=String(t.status||"").toLowerCase();return e==="open"?l==="open":!1}async function Mt(t,e){let l=e?e.get("dash_filter"):null;ht=await H(),Xe=await te();let a=r=>r&&!Xe.find(n=>n.value===r)?[...Xe,{value:r,label:r}]:Xe,s=new Date().getFullYear();N({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:r=>l?r.filter(n=>Ia(n,l)):r,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:r=>`<span class="badge badge-secondary">${r}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:r=>`<span title="${r}">${r?.length>50?r.slice(0,50)+"\u2026":r}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:r=>`<span title="${r||""}">${r?.length>40?r.slice(0,40)+"\u2026":r||"-"}</span>`},{key:"status",label:"Status",render:r=>q(r)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:r=>window.formatDate(r)},{key:"day_count",label:"Hari",render:r=>r??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:ht},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:r=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:r?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:ht,value:r?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:r?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:r?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:r?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(r?.employee_name),value:r?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(r?.fc_specialist),value:r?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:r?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:r?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:r?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let r=await k(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(r.ok){let n=r.data.data.map(i=>({Tanggal:i.report_date||"",Cabang:i.branch_name||"",Kategori:i.category||"",Sumber:i.source||"",Keluhan:i.complaint||"","Nama FC":i.employee_name||"","FC Spesialis":i.fc_specialist||"",Solusi:i.solution||"","Tgl Selesai":i.completion_date||"",Status:i.status||""}));A(n,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async r=>{let i=(await k("/api/branches?all=1")).data?.data||[],d=p=>{if(!p)return null;let m=String(p||"").toLowerCase(),h=i.find(u=>String(u.full_name||"").toLowerCase()===m||String(u.code||"").toLowerCase()===m||String(u.name||"").toLowerCase()===m);return h?h.id:null},g=r.map(p=>({branch_id:d(String(p.Cabang||"").trim()),report_date:String(p.Tanggal||"").trim(),category:String(p.Kategori||"").trim(),source:String(p.Sumber||"").trim(),complaint:String(p.Keluhan||"").trim(),employee_name:String(p["Nama FC"]||"").trim(),fc_specialist:String(p["FC Spesialis"]||"").trim(),solution:String(p.Solusi||"").trim(),completion_date:String(p["Tgl Selesai"]||"").trim(),status:String(p.Status||"").trim()})).filter(p=>p.report_date&&p.complaint&&p.category),c=await k("/api/import/issues",{method:"POST",body:JSON.stringify({rows:g,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}}})}O();var Ee=[];function Pa(t,e){let l=String(t.status||"").toLowerCase();return e==="pending"?l==="pending":!1}async function Ot(t,e){let l=e?e.get("dash_filter"):null;Ee=await H();let a=await te(),s=["Ade","Berlin"],o=n=>n&&!a.find(i=>i.value===n)?[...a,{value:n,label:n}]:a,r=n=>n&&!s.find(i=>(typeof i=="object"?i.value:i)===n)?[...s,n]:s;N({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:n=>l?n.filter(i=>Pa(i,l)):n,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:n=>`<span title="${n||""}">${n?.length>50?n.slice(0,50)+"\u2026":n||"-"}</span>`},{key:"solution",label:"Solusi",render:n=>`<span title="${n||""}">${n?.length>40?n.slice(0,40)+"\u2026":n||"-"}</span>`},{key:"status",label:"Status",render:n=>q(n)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:n=>window.formatDate(n)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:n=>n?`<a href="${n}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:Ee},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async n=>{let i=new URLSearchParams(n||{}).toString(),d=await k(`/api/one-on-one?limit=10000&${i}`);if(d.ok){let g=d.data.data.map(p=>({Tanggal:p.meeting_date||"",Cabang:p.branch_name||"","Nama Karyawan":p.employee_name||"",PIC:p.pic||"",Masalah:p.problem||"",Solusi:p.solution||"",Status:p.status||"","Tgl Selesai":p.completion_date||"",Dokumen:p.document_link||""})),{downloadExcel:c}=await Promise.resolve().then(()=>(K(),se));c(g,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let n=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:i}=await Promise.resolve().then(()=>(K(),se));i(n,"Template_Import_OneOnOne")},onImport:async n=>{let i=p=>{if(!p)return null;let m=String(p||"").toLowerCase(),h=Ee.find(u=>String(u.label||"").toLowerCase()===m);return h?h.value:null},d=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let m=String(p).trim();if(/^\d{4,5}$/.test(m)){let u=Number(m);if(u>2e4&&u<99999){let b=new Date(Date.UTC(1899,11,30)+u*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(m))return m.slice(0,10);let h=m.split(/[\/\-\.]/);if(h.length===3){let[u,b,y]=h.map(_=>_.trim());if(u.length===4&&b.length<=2&&y.length<=2)return`${u}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&u.length<=2)return`${y}-${b.padStart(2,"0")}-${u.padStart(2,"0")}`}return m},g=n.map(p=>({meeting_date:d(p.Tanggal),employee_name:String(p["Nama Karyawan"]||"").trim(),branch_id:i(String(p.Cabang||"").trim()),pic:String(p.PIC||"").trim(),problem:String(p.Masalah||"").trim(),solution:String(p.Solusi||"").trim(),status:String(p.Status||"").trim(),completion_date:d(p["Tgl Selesai"]),document_link:String(p.Dokumen||"").trim()})).filter(p=>p.meeting_date&&p.employee_name&&p.branch_id),c=await k("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:g,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:n=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:n?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:n?.branch_id&&!Ee.find(i=>i.value==n.branch_id)?[...Ee,{value:n.branch_id,label:n.branch_name||n.branch_id}]:Ee,createApi:{path:"/api/branches",field:"full_name"},value:n?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:o(n?.employee_name),value:n?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:r(n?.pic),createApi:{path:"/api/pic",field:"name"},value:n?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:n?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:n?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:n?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:n?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:n?.document_link}]})}O();async function Rt(t){let e=await H(),l=await te(),a=["Ade","Berlin"],s=n=>n&&!l.find(i=>i.value===n)?[...l,{value:n,label:n}]:l,o=n=>n&&!a.find(i=>(typeof i=="object"?i.value:i)===n)?[...a,n]:a,r=Array.from({length:5},(n,i)=>String(new Date().getFullYear()-i));N({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:n=>{try{let i=JSON.parse(n);return Array.isArray(i)?i.join(", "):n||"-"}catch{return n||"-"}}},{key:"score",label:"Nilai",render:n=>n!=null?`<strong>${n}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:n=>n?`<a href="${n}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:r}],exportOptions:{moduleName:"training",onExport:async n=>{let i=new URLSearchParams(n||{}).toString(),d=await k(`/api/training?limit=10000&${i}`);if(d.ok){let g=d.data.data.map(p=>{let m=p.participants||"";try{let h=JSON.parse(m);m=Array.isArray(h)?h.join(", "):m}catch{}return{Tanggal:p.training_date||"",Batch:p.batch||"",Materi:p.subject||"",Cabang:p.branch_name||"",Trainer:p.trainer||"",Peserta:m,Nilai:p.score!==null&&p.score!==void 0?p.score:"",Dokumen:p.document_link||""}}),{downloadExcel:c}=await Promise.resolve().then(()=>(K(),se));c(g,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let n=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:i}=await Promise.resolve().then(()=>(K(),se));i(n,"Template_Import_Training")},onImport:async n=>{let i=p=>{if(!p)return null;let m=String(p||"").toLowerCase(),h=e.find(u=>String(u.label||"").toLowerCase()===m);return h?h.value:null},d=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let m=String(p).trim();if(/^\d{4,5}$/.test(m)){let u=Number(m);if(u>2e4&&u<99999){let b=new Date(Date.UTC(1899,11,30)+u*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(m))return m.slice(0,10);let h=m.split(/[\/\-\.]/);if(h.length===3){let[u,b,y]=h.map(_=>_.trim());if(u.length===4&&b.length<=2&&y.length<=2)return`${u}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&u.length<=2)return`${y}-${b.padStart(2,"0")}-${u.padStart(2,"0")}`}return m},g=n.map(p=>({training_date:d(p.Tanggal),batch:String(p.Batch||"").trim(),subject:String(p.Materi||"").trim(),branch_id:i(String(p.Cabang||"").trim()),trainer:String(p.Trainer||"").trim(),participants:String(p.Peserta||"").trim(),score:p.Nilai?Number(p.Nilai):null,document_link:String(p.Dokumen||"").trim()})).filter(p=>p.training_date&&p.subject&&p.branch_id),c=await k("/api/import/training",{method:"POST",body:JSON.stringify({rows:g,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:n=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:n?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:n?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:n?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:n?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:o(n?.trainer),value:n?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let i=JSON.parse(n?.participants);return Array.isArray(i)?i.join(", "):n?.participants||""}catch{return n?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:n?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:n?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:n?.notes}],onBeforeSubmit:async n=>(n.participants&&(n.participants=JSON.stringify(n.participants.split(",").map(i=>i.trim()).filter(Boolean))),n)})}O();fe();K();function Kt({container:t,title:e,icon:l,apiPath:a,columns:s,formFields:o,filterFields:r,defaultFilters:n={},itemLabel:i="Data",canCreate:d=!0,canEdit:g=!0,canDelete:c=!0,onBeforeSubmit:p,onAfterLoad:m,onDataLoaded:h,extraActions:u=[],initialSearch:b="",exportOptions:y=null,bulkDelete:_=!1,paginationMode:x="server"}){let w=oe();w&&typeof w=="object"&&w.role==="viewer"&&(d=!1,g=!1,c=!1,_=!1,y=null);let S=1,T={...n};b&&(T.search=b);let $=new Set;t.innerHTML=`
  ...
  `,document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),s()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),s()}),document.getElementById("cal-event-close").addEventListener("click",()=>{document.getElementById("cal-event-list").style.display="none"}),document.querySelectorAll(".cal-filter").forEach(o=>o.addEventListener("change",s));async function a(){try{let o=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`;l=(await k(`/api/dashboard/calendar?month=${o}`)).data?.data||[]}catch(o){console.warn("[Calendar] Failed to load events, rendering empty grid:",o),l=[]}}async function s(){let o=document.getElementById("calendar-grid");if(o){o.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;background:var(--border);">
      ${Array(35).fill('<div style="background:#f8fafc;min-height:70px;"></div>').join("")}
    </div>`,await a();try{let r=e.getFullYear(),n=e.getMonth(),i=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}),d=document.getElementById("cal-month-label");d&&(d.textContent=i);let g=new Set(Array.from(document.querySelectorAll(".cal-filter:checked")).map(w=>w.value)),c=l.filter(w=>g.has(w.type)),p={};c.forEach(w=>{let S=(w.event_date||"").slice(0,10);p[S]||(p[S]=[]),p[S].push(w)});let m=new Date(r,n,1).getDay(),h=new Date(r,n+1,0).getDate(),u=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],b=new Date().toISOString().slice(0,10),y='<div class="calendar-grid">';u.forEach(w=>{y+=`<div class="cal-day-header">${w}</div>`});for(let w=0;w<m;w++)y+='<div class="cal-cell cal-cell-empty"></div>';for(let w=1;w<=h;w++){let S=`${r}-${String(n+1).padStart(2,"0")}-${String(w).padStart(2,"0")}`,T=p[S]||[],$=S===b;y+=`
          ...
          </div>`}let x=(m+h)%7;if(x!==0)for(let w=0;w<7-x;w++)y+='<div class="cal-cell cal-cell-empty"></div>';y+="</div>",o.innerHTML=y,o.querySelectorAll(".cal-cell[data-date]").forEach(w=>{w.addEventListener("click",()=>{let S=w.dataset.date,T=p[S]||[];if(!T.length)return;let $=document.getElementById("cal-event-list"),C=new Date(S+"T00:00:00").toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});document.getElementById("cal-event-date").textContent=C,document.getElementById("cal-event-items").innerHTML=T.map(L=>`
            <div class="cal-event-item cal-color-border-${L.color||"gray"}">
              <div class="cal-event-type">${Na(L.type)}</div>
              <div class="cal-event-title">${Ze(L.title||"-")}</div>
              <div class="cal-event-branch">${Ze(L.branch_name||"")}</div>
              ${L.status?`<div class="cal-event-status">${Ze(L.status)}</div>`:""}
              ${L.days_remaining!==void 0?`<div class="cal-event-extra">Sisa: ${L.days_remaining} hari</div>`:""}
            </div>
          `).join(""),$.style.display="block"})})}catch(r){console.error("[Calendar] Render error:",r),o&&(o.innerHTML=`
          <div style="padding:40px;text-align:center;color:var(--text-3)">
            <div style="font-size:2rem;margin-bottom:8px">\u{1F4C5}</div>
            <div>Gagal memuat kalender. Silakan refresh.</div>
          </div>`)}}}s()}function Aa(t,e){return t?t.length>e?t.slice(0,e)+"\u2026":t:""}function Ze(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function Na(t){return{schedule:"\u{1F5D3} Jadwal",issue:"\u26A0\uFE0F Permasalahan",reliever:"\u{1F504} Reliefer",training:"\u{1F393} Training",contract_expiry:"\u{1F4CB} Kontrak Habis"}[t]||t}O();async function Yt(t){let e=oe(),l=(e?.full_name||e?.username||"U")[0].toUpperCase(),s={superadmin:"#7C3AED",admin:"#2563EB",manager:"#0891B2",spv:"#059669",viewer:"#64748B"}[e?.role]||"#64748B";t.innerHTML=`
    <div class="page-header">
        ...
        <div class="card-body">
          <div class="import-info-box">
            ...
            <div class="import-sheet-list">
              ${Object.entries(et).map(([u,{label:b}])=>`<span class="import-sheet-tag">\u{1F4C4} ${u} \u2192 ${b}</span>`).join("")}
            </div>
          </div>
  ...
  `;let e=null,l=null,a=0,s={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function o(u){Object.entries(s).forEach(([b,y])=>{y.style.display=b===u?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let u=document.getElementById("btn-backup-db");u.disabled=!0,u.textContent="\u23F3 Memproses Backup...";try{let b=await k("/api/import/backup");if(b.ok){if(!window.XLSX){Q("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");return}let y=window.XLSX,_=y.utils.book_new();Object.entries(b.data.database).forEach(([x,w])=>{let S=w.length>0?w:[{}],T=y.utils.json_to_sheet(S);y.utils.book_append_sheet(_,T,x.substring(0,31))}),y.writeFile(_,`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.xlsx`),ee("Backup berhasil diunduh!")}else Q("Gagal memproses backup: "+(b.data?.error||"Unknown error"))}catch(b){Q("Gagal memproses backup: "+b.message)}finally{u.disabled=!1,u.textContent="\u{1F4E6} Backup Database"}});let r=document.getElementById("btn-sync-google");r&&r.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let u=r.innerHTML;r.innerHTML='<span class="spinner"></span> Menyinkronkan...',r.disabled=!0;try{let b=await k("/api/sync/google-sheets",{method:"POST"});b.ok?alert("Sinkronisasi Berhasil: "+(b.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(b.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{r.innerHTML=u,r.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{ea(),ee("Template Excel berhasil didownload!")});let n=document.getElementById("file-input"),i=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",u=>{u.stopPropagation(),n.click()}),n.addEventListener("change",u=>{u.target.files[0]&&d(u.target.files[0])}),i.addEventListener("dragover",u=>{u.preventDefault(),i.classList.add("drag-over")}),i.addEventListener("dragleave",()=>i.classList.remove("drag-over")),i.addEventListener("drop",u=>{u.preventDefault(),i.classList.remove("drag-over");let b=u.dataTransfer.files[0];b&&b.name.match(/\.xlsx?$/i)?d(b):Q("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,n.value="",document.getElementById("file-info").style.display="none",i.style.display="",o("upload")});async function d(u){e=u,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${u.name} (${(u.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",i.style.display="none",await g(u)}async function g(u){o("validating");let b=document.getElementById("validation-status"),y=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");b.textContent="Membaca file Excel...",y.style.width="20%",await je(200);let _=await u.arrayBuffer(),x=window.XLSX.read(_,{type:"array",cellDates:!0});b.textContent=`Memvalidasi ${x.SheetNames.length} sheet...`,y.style.width="50%",await je(100),l=Zt(x),y.style.width="100%",b.textContent="Validasi selesai!",await je(300),c()}catch(_){o("upload"),Q("Gagal memproses file: "+_.message),document.getElementById("file-info").style.display="flex",i.style.display="none"}}function c(){o("preview");let u=l.filter(C=>!C.skipped).length,b=l.reduce((C,L)=>C+L.total,0),y=l.reduce((C,L)=>C+L.valid,0),_=l.reduce((C,L)=>C+L.errorCount,0),x=b>0?Math.round(y/b*100):0;document.getElementById("preview-summary-badges").innerHTML=`
    ...
    `,w.querySelectorAll(".btn-detail-error").forEach(C=>{C.addEventListener("click",()=>{let L=l[Number(C.dataset.idx)];p(L)})});let S=document.getElementById("error-detail-section"),T=document.getElementById("error-detail-container");T.innerHTML="",S.style.display="none";let $=document.getElementById("btn-start-import");y===0?($.disabled=!0,$.innerHTML="\u26A0\uFE0F Tidak Ada Data Valid"):($.disabled=!1,_>0?($.innerHTML=`\u{1F680} Import ${y} Data Valid (${_} dilewati)`,$.title="Baris error akan dilewati, baris valid tetap diimport"):$.innerHTML=`\u{1F680} Mulai Import ${y} Data`)}function p(u){let b=document.getElementById("error-detail-section"),y=document.getElementById("error-detail-container");b.style.display="";let _=u.errors.slice(0,100).map(x=>(Array.isArray(x.errors)?x.errors:[]).map(S=>{let T=typeof S=="object";return`
          <tr>
    ...
    `,b.scrollIntoView({behavior:"smooth",block:"start"})}document.getElementById("btn-back-to-upload").addEventListener("click",()=>{o("upload"),document.getElementById("file-info").style.display="none",i.style.display="",e=null,n.value=""}),document.getElementById("btn-download-log").addEventListener("click",()=>{if(!l)return;ta(l)?ee("Log error berhasil didownload."):ee("Tidak ada error untuk didownload.")}),document.getElementById("btn-start-import").addEventListener("click",()=>{let u=document.querySelector('input[name="dup-strategy"]:checked')?.value||"skip";m(u)});async function m(u){o("importing"),a=Date.now();let b=[];Oa.forEach(S=>{let T=l?.find($=>$.module===S&&$.mapped?.length>0);T&&b.push(T)});let y=document.getElementById("import-steps-list");y.innerHTML=b.map(S=>`
    ...
    `).join("");let _=document.getElementById("import-bar"),x=document.getElementById("import-current-status"),w={totalSheets:b.length,totalRows:b.reduce((S,T)=>S+T.mapped.length,0),inserted:0,skipped:0,failed:0,moduleResults:[]};for(let S=0;S<b.length;S++){let T=b[S],$=document.getElementById(`step-icon-${T.module}`),C=document.getElementById(`step-status-${T.module}`);$.textContent="\u{1F504}",C.textContent="Mengimport...",x.textContent=`Mengimport ${T.label}...`,_.style.width=`${Math.round(S/b.length*100)}%`;try{let L=await k(`/api/import/${T.module}`,{method:"POST",body:JSON.stringify({rows:T.mapped,onDuplicate:u})});if(L.ok){let R=L.data;w.inserted+=R.inserted||0,w.skipped+=R.skipped||0,w.moduleResults.push({label:T.label,inserted:R.inserted||0,skipped:R.skipped||0,status:"ok"}),$.textContent="\u2705",C.innerHTML=`<span class="badge badge-success">${R.inserted||0} berhasil</span>${R.skipped>0?` <span class="badge badge-neutral">${R.skipped} skip</span>`:""}`}else w.failed++,w.moduleResults.push({label:T.label,inserted:0,skipped:0,status:"error",error:L.data?.error}),$.textContent="\u274C",C.innerHTML='<span class="badge badge-danger">Gagal</span>'}catch(L){w.failed++,w.moduleResults.push({label:T.label,inserted:0,skipped:0,status:"error",error:L.message}),$.textContent="\u274C",C.innerHTML='<span class="badge badge-danger">Gagal</span>'}await je(150)}_.style.width="100%",x.textContent="Selesai!",await je(400),h(w)}function h(u){o("summary");let b=((Date.now()-a)/1e3).toFixed(1),y=u.failed===0;document.getElementById("summary-status-icon").innerHTML=`
  ...
  `;let e=document.getElementById("btnRefreshSync"),l=document.getElementById("queueStatusFilter");e.addEventListener("click",s),l.addEventListener("change",n),document.getElementById("btnRetryAll").addEventListener("click",()=>a("retry",{allFailed:!0})),document.getElementById("btnResetStuck").addEventListener("click",()=>a("reset-stuck",{})),document.getElementById("btnPauseSync").addEventListener("click",()=>a("pause",{})),document.getElementById("btnResumeSync").addEventListener("click",()=>a("resume",{})),document.getElementById("btnReconcile").addEventListener("click",()=>a("reconcile",{module:"ALL",repairMode:!0})),document.getElementById("btnRetrySelected").addEventListener("click",()=>{let d=Array.from(document.querySelectorAll(".chk-queue:checked")).map(g=>g.value);if(d.length===0)return alert("No items selected");a("retry",{ids:d})}),document.getElementById("chkAllQueue").addEventListener("change",d=>{document.querySelectorAll(".chk-queue").forEach(g=>g.checked=d.target.checked)});async function a(d,g){if(confirm(`Are you sure you want to execute action: ${d}?`)){showLoading();try{let c=await k(`/api/sync/actions/${d}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(g)});c.ok?(alert(c.data?.message||"Success"),s()):Q(c.error||"Action failed")}catch(c){Q(c.message)}hideLoading()}}await s();async function s(){showLoading(),await Promise.all([r(),n(),o(),i()]),hideLoading()}async function o(){try{let d=await k("/api/sync/performance");if(!d.ok)return;let{webhook:g,google_api:c,d1:p,queue:m,throughput:h}=d.data;document.getElementById("latencyHistograms").innerHTML=`
      ...
      `).join("")}catch(d){console.error(d)}}async function i(){try{let d=await k("/api/sync/metrics");if(!d.ok)return;let g=document.getElementById("metricsTableBody"),c=d.data||[];if(c.length===0){g.innerHTML='<tr><td colspan="5" class="px-4 py-4 text-center text-gray-500">No metrics found in last 24h</td></tr>';return}g.innerHTML=c.map(p=>`
        <tr class="hover:bg-gray-50">
  ...
  `;de({title:`Notifikasi (${Ie.length})`,content:t,confirmText:"Tutup",onConfirm:(e,l)=>l()})}function oa(){let t=oe(),e=(t?.full_name||"U")[0].toUpperCase();document.getElementById("app").innerHTML=`
  ...
  `;let l=document.getElementById("sidebar"),a=document.getElementById("sidebar-overlay"),s=document.getElementById("topbar-menu-btn"),o=document.getElementById("sidebar-close"),r=()=>{l.classList.add("open"),a.classList.add("show")},n=()=>{l.classList.remove("open"),a.classList.remove("show")};s?.addEventListener("click",r),o?.addEventListener("click",n),a?.addEventListener("click",n),document.querySelectorAll(".nav-item").forEach(d=>d.addEventListener("click",n));function i(){let d=window.location.hash.replace("#","")||"/dashboard";document.querySelectorAll(".nav-item").forEach(p=>{let m=p.dataset.route;p.classList.toggle("active",d===m||m!=="/dashboard"&&d.startsWith(m))});let g=document.getElementById("topbar-title"),c=document.querySelector(".nav-item.active .nav-label");g&&c&&(g.textContent=c.textContent)}window.addEventListener("hashchange",i),i(),Ra(),document.getElementById("btn-fullscreen")?.addEventListener("click",()=>{document.fullscreenElement?document.exitFullscreen?.():document.documentElement.requestFullscreen?.()}),document.getElementById("logout-btn")?.addEventListener("click",async()=>{await k("/api/auth/logout",{method:"POST"}),Le(),qe&&clearInterval(qe),Se("/login")}),Ka(),Ha(),document.getElementById("btn-notif")?.addEventListener("click",d=>{d.preventDefault(),ja()})}async function qa(){U("/login",({main:e})=>Bt(e)),U("/dashboard",z(({main:e})=>$t(e))),U("/calendar",z(({main:e})=>Vt(e))),U("/employees",z(({main:e,params:l})=>Lt(e,l))),U("/contracts",z(({main:e,params:l})=>Nt(e,l))),U("/sp",z(({main:e})=>ia(e))),U("/mutasi",z(({main:e})=>la(e))),U("/sync-dashboard",z(({main:e})=>sa(e))),U("/timeline",z(({main:e,params:l})=>Ft(e,l))),U("/issues",z(({main:e,params:l})=>Mt(e,l))),U("/one-on-one",z(({main:e,params:l})=>Ot(e,l))),U("/training",z(({main:e})=>Rt(e))),U("/relievers",z(({main:e,params:l})=>Ht(e,l))),U("/reports/inspection",z(({main:e})=>jt(e))),U("/reports/cleaning",z(({main:e})=>qt(e))),U("/reports/fogging",z(({main:e})=>Jt(e))),U("/reports/basecamp",z(({main:e})=>Ut(e))),U("/reports/supply",z(({main:e})=>yt(e,"supply"))),U("/sop",z(({main:e})=>Gt(e))),U("/checklist",z(({main:e})=>Qt(e))),U("/forms",z(({main:e})=>yt(e))),U("/users",z(({main:e})=>zt(e))),U("/branches",z(({main:e})=>Wt(e))),U("/profile",z(({main:e})=>Yt(e))),U("/settings/import",z(({main:e})=>aa(e)));let t=Be();if(!t&&window.location.hash!=="#/login"&&Se("/login"),t){let e=await k("/api/auth/me");e.ok?(Ae(e.data.data),oa()):(Le(),Se("/login"))}window.addEventListener("fm:login",()=>{oa(),Se("/dashboard")}),vt()}qa();
