var la=Object.defineProperty;var tt=(t,e)=>()=>(t&&(e=t(t=0)),e);var at=(t,e)=>{for(var s in e)la(t,s,{get:e[s],enumerable:!0})};var ve={};at(ve,{API:()=>yt,CLIENT_SIDE_MAX_ROWS:()=>ge,IS_DEVELOPMENT:()=>De,apiFetch:()=>k,clearToken:()=>Pe,getToken:()=>Ie,getUser:()=>be,setToken:()=>nt,setUser:()=>Le});function Ie(){return localStorage.getItem("fm_token")}function nt(t){localStorage.setItem("fm_token",t)}function Pe(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function be(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Le(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let s=Ie(),a={"Content-Type":"application/json",...s?{Authorization:`Bearer ${s}`}:{},...e.headers||{}};try{let l=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",i=`${yt}${t}${d}${l}`,r=await fetch(i,{...e,headers:a}),n;try{let o=await r.text();try{n=JSON.parse(o)}catch{n={error:`Server Error (${r.status}): ${o.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return r.status===401&&(Pe(),window.location.hash="#/login"),{ok:r.ok,status:r.status,data:n}}catch(l){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${l.message})`}}}}var De,oa,yt,ge,F=tt(()=>{De=!1,oa="https://fm-operations-api.facilitycare-audydental.workers.dev",yt=oa,ge=1e4});var kt={};at(kt,{confirmDialog:()=>Ne,createModal:()=>le});function le({title:t,content:e,onConfirm:s,onCancel:a,confirmText:l="Simpan",cancelText:d="Batal",size:i="md",confirmClass:r="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},o=document.createElement("div");o.className="modal-overlay",o.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>ct(t)),document.getElementById("filter-insp-month")?.addEventListener("change",async s=>{let a=s.target.value,l=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",d=document.getElementById("skel-insp"),i=document.getElementById("chart-insp");d&&(d.style.display="block",d.style.position="absolute"),i&&(i.style.display="none");let r=await se(l,{},8e3);try{Dt(r)}catch(n){console.warn("InspBar render:",n),ue("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>ya(),5e3),await ct(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?ct(t):clearInterval(t._dashRefresh)},6e4)}async function ct(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,s,a,l,d,i,r,n,o,m,c,p]=await Promise.all([se("/api/dashboard/kpi",{},8e3),se("/api/dashboard/issues-trend",{},8e3),se("/api/dashboard/issues-summary",{},8e3),se("/api/dashboard/inspection-bar",{},8e3),se("/api/dashboard/stats",{},8e3),se("/api/dashboard/calendar",[],8e3),se("/api/schedule?limit=10000",{data:[]},8e3),se("/api/employees?limit=10000",{data:[]},8e3),se("/api/contracts?limit=10000",{data:[]},8e3),se("/api/issues?limit=10000",{data:[]},8e3),se("/api/one-on-one?limit=10000",{data:[]},8e3),se("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3)]);if(e){let u=Array.isArray(r?.data)?r.data:Array.isArray(r)?r:[],h=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],g=Array.isArray(o?.data)?o.data:Array.isArray(o)?o:[],b=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[],y=Array.isArray(c?.data)?c.data:Array.isArray(c)?c:[];e.employees&&(e.employees.current=h.filter(x=>St(x,"active")).length),e.contracts&&(e.contracts.current=g.filter(x=>ot(x,"active")).length),e.expiring30&&(e.expiring30={current:g.filter(x=>ot(x,"expiring30")).length}),e.issues&&(e.issues.current=b.filter(x=>xt(x,"open")).length),e.one_on_one&&(e.one_on_one.current=y.filter(x=>wt(x,"pending")).length),e.inspection_month&&(e.inspection_month.current=u.filter(x=>lt(x,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=u.filter(x=>lt(x,"gcdc")).length)}try{Et(e)}catch(u){console.warn("KPI render:",u)}try{$t(e)}catch(u){console.warn("MiniStats render:",u)}try{fa(Array.isArray(a?.by_category)?a.by_category:[])}catch(u){console.warn("Donut render:",u),ue("skel-donut","chart-donut")}try{va(s)}catch(u){console.warn("Trend render:",u),ue("skel-trend","chart-trend")}try{Dt(l)}catch(u){console.warn("InspBar render:",u),ue("skel-insp","chart-insp")}try{let u=Array.isArray(d)?d:Array.isArray(d?.recent_issues)?d.recent_issues:[];Sa(u)}catch(u){console.warn("IssuesTable render:",u)}try{let u=Array.isArray(d?.expiring_contracts)?d.expiring_contracts:[];ka(p)}catch(u){console.warn("ContractsTable render:",u)}try{xa(Array.isArray(i)?i:[])}catch(u){console.warn("Agenda render:",u)}try{wa()}catch(u){console.warn("Quick Actions render:",u)}}function Et(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let s=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=s.map(a=>{let l=ce(t[a.key]?.current,0);return`
      ...
      </a>`}).join(""),e.querySelectorAll(".kpi-value").forEach(a=>{Ct(a,parseInt(a.dataset.target)||0)})}function $t(t){let e=document.getElementById("mini-stats-row");if(!e)return;t=t||{};let s=[{icon:"\u{1F4C5}",label:"Jadwal",val:t.schedule?.current,href:"#/timeline",color:"mini-blue"},{icon:"\u{1F393}",label:"Training",val:t.training_month?.current,href:"#/training",color:"mini-gray"},{icon:"\u{1F504}",label:"Reliefer",val:t.reliever_completed?.current,href:"#/relievers",color:"mini-teal"},{icon:"\u{1F50D}",label:"Inspeksi",val:t.inspection_month?.current,href:"#/timeline?dash_filter=inspeksi",color:"mini-blue"},{icon:"\u{1F9F9}",label:"GCDC",val:t.cleaning_month?.current,href:"#/timeline?dash_filter=gcdc",color:"mini-green"},{icon:"\u{1F4A8}",label:"Fogging",val:t.fogging_month?.current,href:"#/reports/fogging",color:"mini-purple"},{icon:"\u{1F3E2}",label:"Cabang",val:t.branches?.current,href:"#/branches",color:"mini-teal"}];e.innerHTML=s.map(a=>`
