var oa=Object.defineProperty;var at=(t,e)=>()=>(t&&(e=t(t=0)),e);var nt=(t,e)=>{for(var r in e)oa(t,r,{get:e[r],enumerable:!0})};var ke={};nt(ke,{API:()=>yt,CLIENT_SIDE_MAX_ROWS:()=>be,IS_DEVELOPMENT:()=>Pe,apiFetch:()=>k,clearToken:()=>Le,getToken:()=>Be,getUser:()=>he,setToken:()=>it,setUser:()=>Ne});function Be(){return localStorage.getItem("fm_token")}function it(t){localStorage.setItem("fm_token",t)}function Le(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function he(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Ne(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let r=Be(),a={"Content-Type":"application/json",...r?{Authorization:`Bearer ${r}`}:{},...e.headers||{}};try{let l=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",i=`${yt}${t}${d}${l}`,s=await fetch(i,{...e,headers:a}),n;try{let o=await s.text();try{n=JSON.parse(o)}catch{n={error:`Server Error (${s.status}): ${o.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return s.status===401&&(Le(),window.location.hash="#/login"),{ok:s.ok,status:s.status,data:n}}catch(l){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${l.message})`}}}}var Pe,da,yt,be,F=at(()=>{Pe=!1,da="https://fm-operations-api.facilitycare-audydental.workers.dev",yt=da,be=1e4});var kt={};nt(kt,{confirmDialog:()=>Fe,createModal:()=>oe});function oe({title:t,content:e,onConfirm:r,onCancel:a,confirmText:l="Simpan",cancelText:d="Batal",size:i="md",confirmClass:s="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},o=document.createElement("div");o.className="modal-overlay",o.innerHTML=`
  ...
  `;function R(){if(!document.getElementById("bulk-toolbar"))return;let f=document.getElementById("bulk-count"),$=document.getElementById("btn-bulk-delete"),P=document.getElementById("btn-bulk-cancel");f.textContent=`${w.size} item dipilih`,w.size>0?($.disabled=!1,P.disabled=!1):($.disabled=!0,P.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{w.clear(),document.querySelectorAll(".row-checkbox").forEach(f=>f.checked=!1);let v=document.getElementById("select-all-checkbox");v&&(v.checked=!1),R()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(w.size===0)return;let v=[...w],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",f.innerHTML=`
    ...
    `,document.body.appendChild(f),f.querySelector("#bulk-cancel-btn").addEventListener("click",()=>f.remove()),f.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let $=f.querySelector("#bulk-confirm-btn");$.disabled=!0,$.textContent="Menghapus...";let P=await k(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:v})});f.remove(),P.ok?(Z(`${v.length} ${n} berhasil dihapus.`),w.clear(),R(),M()):G(P.data?.error||"Gagal menghapus data.")})});let E=document.getElementById("filter-search"),O;if(E?.addEventListener("input",v=>{clearTimeout(O),O=setTimeout(()=>{x.search=v.target.value,S=1,w.clear(),M()},400)}),i?.forEach(v=>{v.type==="select"&&document.getElementById(`filter-${v.name}`)?.addEventListener("change",f=>{x[v.name]=f.target.value,S=1,w.clear(),M()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{x={...s},E&&(E.value=""),i?.forEach(v=>{let f=document.getElementById(`filter-${v.name}`);f&&(f.value="")}),S=1,w.clear(),M()}),document.getElementById("btn-create")?.addEventListener("click",()=>ge(null)),h){document.getElementById(`btn-export-${h.moduleName}`)?.addEventListener("click",async f=>{let $=f.target,P=$.innerHTML;$.innerHTML="\u23F3 Loading...",$.disabled=!0;try{await h.onExport()}catch{G("Gagal export data")}finally{$.innerHTML=P,$.disabled=!1}}),document.getElementById(`btn-template-${h.moduleName}`)?.addEventListener("click",()=>{h.onTemplate()});let v=document.getElementById(`input-import-${h.moduleName}`);v?.addEventListener("change",async f=>{let $=f.target.files[0];if(!$)return;v.disabled=!0;let P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",P.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>pt(t)),document.getElementById("filter-jadwal-year")?.addEventListener("change",async r=>{let a=r.target.value;document.getElementById("jadwal-year-label").textContent=a;let l=document.getElementById("skel-jadwal"),d=document.getElementById("chart-jadwal");l&&(l.style.display="block",l.style.position="absolute"),d&&(d.style.display="none");let i=await re(`/api/dashboard/schedule-chart?year=${a}`,{},8e3);try{Dt(i)}catch(s){console.warn("ScheduleChart render:",s),ce("skel-jadwal","chart-jadwal")}}),document.getElementById("filter-insp-month")?.addEventListener("change",async r=>{let a=r.target.value,l=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",d=document.getElementById("skel-insp"),i=document.getElementById("chart-insp");d&&(d.style.display="block",d.style.position="absolute"),i&&(i.style.display="none");let s=await re(l,{},8e3);try{It(s)}catch(n){console.warn("InspBar render:",n),ce("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>ya(),5e3),await pt(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?pt(t):clearInterval(t._dashRefresh)},6e4)}async function pt(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,r,a,l,d,i,s,n,o,m,c,p]=await Promise.all([re("/api/dashboard/kpi",{},8e3),re("/api/dashboard/issues-trend",{},8e3),re("/api/dashboard/issues-summary",{},8e3),re("/api/dashboard/stats",{},8e3),re("/api/dashboard/calendar",[],8e3),re("/api/schedule?limit=10000",{data:[]},8e3),re("/api/employees?limit=10000",{data:[]},8e3),re("/api/contracts?limit=10000",{data:[]},8e3),re("/api/issues?limit=10000",{data:[]},8e3),re("/api/one-on-one?limit=10000",{data:[]},8e3),re("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3),re(`/api/dashboard/schedule-chart?year=${document.getElementById("filter-jadwal-year")?.value||new Date().getFullYear()}`,{},8e3)]),g=document.getElementById("filter-insp-month"),y=g?g.value:"",u=y?`/api/dashboard/inspection-bar?month=${y}`:"/api/dashboard/inspection-bar",b=await re(u,{},8e3);if(e){let h=Array.isArray(i?.data)?i.data:Array.isArray(i)?i:[],_=Array.isArray(s?.data)?s.data:Array.isArray(s)?s:[],C=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],S=Array.isArray(o?.data)?o.data:Array.isArray(o)?o:[],x=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[];e.employees&&(e.employees.current=_.filter(w=>St(w,"active")).length),e.contracts&&(e.contracts.current=C.filter(w=>dt(w,"active")).length),e.expiring30&&(e.expiring30={current:C.filter(w=>dt(w,"expiring30")).length}),e.issues&&(e.issues.current=S.filter(w=>xt(w,"open")).length),e.one_on_one&&(e.one_on_one.current=x.filter(w=>wt(w,"pending")).length),e.inspection_month&&(e.inspection_month.current=h.filter(w=>ot(w,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=h.filter(w=>ot(w,"gcdc")).length)}try{Et(e)}catch(h){console.warn("KPI render:",h)}try{$t(e)}catch(h){console.warn("MiniStats render:",h)}try{Dt(p)}catch(h){console.warn("ScheduleChart render:",h),ce("skel-jadwal","chart-jadwal")}try{fa(Array.isArray(a?.by_category)?a.by_category:[])}catch(h){console.warn("Donut render:",h),ce("skel-donut","chart-donut")}try{va(r)}catch(h){console.warn("Trend render:",h),ce("skel-trend","chart-trend")}try{It(b)}catch(h){console.warn("InspBar render:",h),ce("skel-insp","chart-insp")}try{let h=Array.isArray(l)?l:Array.isArray(l?.recent_issues)?l.recent_issues:[];Sa(h)}catch(h){console.warn("IssuesTable render:",h)}try{let h=Array.isArray(l?.expiring_contracts)?l.expiring_contracts:[];ka(c)}catch(h){console.warn("ContractsTable render:",h)}try{xa(Array.isArray(d)?d:[])}catch(h){console.warn("Agenda render:",h)}try{wa()}catch(h){console.warn("Quick Actions render:",h)}}function Et(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let r=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=r.map(a=>{let l=pe(t[a.key]?.current,0);return`
    ...
    `}).join("");let s={id:"centerText",beforeDraw:function(n){let o=n.width,m=n.height,c=n.ctx;c.restore();let p=(m/80).toFixed(2);c.font="bold "+p+"em Inter",c.textBaseline="middle",c.fillStyle="#1E293B";let g=i.toString(),y=Math.round((o-c.measureText(g).width)/2),u=m/2;c.fillText(g,y,u-10),c.font="600 "+(p*.35).toFixed(2)+"em Inter",c.fillStyle="#64748B";let b="Total",h=Math.round((o-c.measureText(b).width)/2);c.fillText(b,h,u+15),c.save()}};ue.donut=new Chart(e,{type:"doughnut",data:{labels:l,datasets:[{data:d,backgroundColor:ct,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:le,titleFont:{...le,weight:"700"},callbacks:{label:n=>` ${n.label}: ${n.parsed} kasus`}}},cutout:"75%"},plugins:[s]})}function va(t){ce("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;Ee("trend"),t=t||{};let r=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],a=(t.labels||[]).map(i=>{if(!i||typeof i!="string")return"";try{let[s,n]=i.split("-");return(r[Number(n)-1]||n)+" "+String(s).slice(-2)}catch{return i}}),l=(t.open||[]).map(i=>pe(i)),d=(t.closed||[]).map(i=>pe(i));if(!a.length){Ke(e,"Belum ada data trend");return}ue.trend=new Chart(e,{type:"line",data:{labels:a,datasets:[{label:"Open",data:l,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:d,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.1)",fill:!0,tension:.4,pointRadius:3,pointHoverRadius:5,pointBackgroundColor:"#10B981",borderWidth:2}]},options:We({plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{family:"Inter",size:9},color:me,maxRotation:0,autoSkip:!1}},y:{grid:{color:Te},ticks:{font:{family:"Inter",size:9},color:me},beginAtZero:!0}}})})}function Dt(t){ce("skel-jadwal","chart-jadwal");let e=document.getElementById("chart-jadwal");if(!e)return;Ee("jadwal"),t=t||{};let r=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];if(!Object.values(t).some(l=>Array.isArray(l)&&l.some(d=>d>0))){Ke(e,"Belum ada data jadwal");return}ue.jadwal=new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:"Inspeksi",data:t["Inspeksi Hygiene"]||Array(12).fill(0),backgroundColor:"#3B82F6",borderRadius:{topLeft:4,topRight:4,bottomLeft:0,bottomRight:0},barPercentage:.85,categoryPercentage:.85},{label:"General Cleaning",data:t["General Cleaning"]||Array(12).fill(0),backgroundColor:"#10B981",borderRadius:{topLeft:4,topRight:4,bottomLeft:0,bottomRight:0},barPercentage:.85,categoryPercentage:.85},{label:"Deep Cleaning",data:t["Deep Cleaning"]||Array(12).fill(0),backgroundColor:"#F59E0B",borderRadius:{topLeft:4,topRight:4,bottomLeft:0,bottomRight:0},barPercentage:.85,categoryPercentage:.85},{label:"Fogging",data:t.Fogging||Array(12).fill(0),backgroundColor:"#EF4444",borderRadius:{topLeft:4,topRight:4,bottomLeft:0,bottomRight:0},barPercentage:.85,categoryPercentage:.85}]},options:We({plugins:{legend:{display:!1}},scales:{x:{stacked:!0,grid:{display:!1},ticks:{font:le,color:me,maxRotation:0,autoSkip:!1}},y:{stacked:!0,grid:{color:Te},ticks:{font:le,color:me},min:0}}})})}function It(t){ce("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;Ee("inspBar"),t=t||{};let r=t.labels||[],a=(t.fc||[]).map(d=>pe(d)),l=(t.spv||[]).map(d=>pe(d));if(!r.length){Ke(e,"Belum ada data inspeksi");return}ue.inspBar=new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:l,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:We({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:le,color:me,maxRotation:45,minRotation:30}},y:{grid:{color:Te},ticks:{font:le,color:me},min:0,max:100}}})})}function ka(t){ce("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;Ee("contractMiniBar"),t=t||{};let r={"06":"Jun","07":"Jul","08":"Agu","09":"Sep",10:"Okt",11:"Nov",12:"Des"},a=(t.labels||[]).map(i=>{let s=i.split("-")[1];return r[s]||i}),l=(t.data||[]).map(i=>pe(i));if(!a.length){Ke(e,"Belum ada data");return}let d=e.getContext("2d");ue.contractMiniBar=new Chart(e,{type:"bar",data:{labels:a,datasets:[{label:"Kontrak Habis",data:l,backgroundColor:"#3B82F6",borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:We({onClick:(i,s)=>{if(s&&s.length>0){let n=s[0].index,o=(t.labels||[])[n];o&&(window.location.hash="#/contracts?month_expiry="+o)}},plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:le,color:me,maxRotation:0,autoSkip:!1}},y:{grid:{color:Te,borderDash:[4,4],drawBorder:!1},ticks:{font:le,color:me,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function Sa(t){let e=document.getElementById("table-issues");if(!e)return;let r=(t||[]).slice(0,8);if(!r.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${r.map(a=>`
...
`)}let x=o(_,h),w=null;return x||(w="Karyawan tidak ditemukan di Database"),{isValid:!!x,invalidReason:w,rowNum:h,data:{employee_id:x,branch_id:n(String(u.Cabang||"").trim()),division:String(u["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:S,end_date:m(u["Tanggal Selesai"])||"2099-12-31",status:String(u.Status||"").trim(),_rawName:_}}}),p=[],g=[];if(c.forEach(u=>{u.isValid?p.push(u.data):g.push({rowNum:u.rowNum,name:u.data._rawName,reason:u.invalidReason})}),console.log(`Split Validation - Valid: ${p.length}, Invalid: ${g.length}`),p.length===0)return{inserted:0,skipped:a.length,failed:a.length};let y=await k("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:p,onDuplicate:"update"})});if(!y.ok)throw new Error(y.data?.error||"Import gagal");return y.data}}})}F();K();var gt=[],He=[];function $a(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let r of e)if(t.some(a=>a.period===r))return r;return"Q3"}function Da(t,e){if(t.period!=="Q3")return!1;let r=String(t.status||"").toLowerCase();if(r!=="selesai"&&r!=="completed"&&r!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}async function At(t,e){gt=await H();let r=await ee();He=["BERLIN ARIANSYAH","ADE SURAHMAN"];let a=o=>o&&!He.find(m=>(typeof m=="object"?m.value:m)===o)?[...He,o]:He,l=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),d=o=>{if(!o||o==="-"||String(o).trim()==="")return"";let m=String(o).split("-");return m.length===3&&m[0].length===4?`${m[2]}-${m[1]}-${m[0]}`:o},i=l.data?.data||[],s=$a(i),n=e?e.get("dash_filter"):null;L({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:n?{period:"Q3"}:{period:s},onDataLoaded:o=>(n&&(o=o.filter(m=>Da(m,n))),o.sort((m,c)=>{let p=m.opening_date?new Date(m.opening_date).getTime():0;return(c.opening_date?new Date(c.opening_date).getTime():0)-p})),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:o=>lt(o)},{key:"period",label:"Periode",render:o=>de(o)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:o=>d(o)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:o=>d(o)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:o=>d(o)},{key:"status",label:"Status",render:o=>q(o)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:gt},{type:"select",name:"activity_type",label:"Kegiatan",options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:He}],formFields:o=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:gt,value:o?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:o?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:o?.period},{name:"pic",label:"PIC",type:"combobox",options:a(o?.pic),value:o?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:o?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:o?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:o?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:o?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:o?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let o=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(o.ok){let m=o.data.data.map(c=>({Cabang:c.branch_name||"",Kegiatan:c.activity_type||"",Periode:c.period||"",PIC:c.pic||"","Tgl Opening":c.opening_date||"","Tgl Target":c.target_date||"","Tgl Selesai":c.completion_date||"",Status:c.status||""}));B(m,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{B([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async o=>{let c=(await k("/api/branches?all=1")).data?.data||[],p=b=>{if(!b)return null;let h=String(b||"").toLowerCase(),_=c.find(C=>String(C.full_name||"").toLowerCase()===h||String(C.code||"").toLowerCase()===h||String(C.name||"").toLowerCase()===h);return _?_.id:null},g=b=>{if(b==null||b==="")return"";if(b instanceof Date&&!isNaN(b.getTime()))return b.toISOString().slice(0,10);let h=String(b).trim();if(h===""||h==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(h))return h.slice(0,10);if(/^\d{4,5}$/.test(h)){let C=Number(h);if(C>2e4&&C<99999){let S=new Date(Date.UTC(1899,11,30)+C*864e5);return isNaN(S.getTime())?"":S.toISOString().slice(0,10)}}let _=h.split(/[\/\-\.]/);if(_.length===3){let[C,S,x]=_.map(w=>w.trim());if(C.length===4&&S.length<=2&&x.length<=2)return`${C}-${S.padStart(2,"0")}-${x.padStart(2,"0")}`;if(x.length===4&&S.length<=2&&C.length<=2)return`${x}-${S.padStart(2,"0")}-${C.padStart(2,"0")}`}return h},y=o.map(b=>({branch_id:p(String(b.Cabang||"").trim()),activity_type:String(b.Kegiatan||"").trim(),period:String(b.Periode||"").trim(),pic:String(b.PIC||b.Pic||"").trim(),opening_date:g(b["Tgl Opening"]||b["Tanggal Opening"]||b["Tgl Openir"]),target_date:g(b["Tgl Target"]||b["Tanggal Target"]),completion_date:g(b["Tgl Selesai"]||b["Tanggal Selesai"]),status:String(b.Status||"").trim(),notes:String(b.Catatan||b.Keterangan||"").trim()})).filter(b=>b.activity_type&&b.period),u=await k("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:y,onDuplicate:"update"})});if(!u.ok)throw new Error(u.data?.error||"Import gagal");return u.data}}})}F();K();var bt=[],Ye=[];function Ia(t,e){let r=String(t.status||"").toLowerCase();return e==="open"?r==="open":!1}async function Ft(t,e){let r=e?e.get("dash_filter"):null;bt=await H(),Ye=await ee();let a=i=>i&&!Ye.find(s=>s.value===i)?[...Ye,{value:i,label:i}]:Ye,l=new Date().getFullYear();L({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:i=>r?i.filter(s=>Ia(s,r)):i,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:i=>`<span class="badge badge-secondary">${i}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:i=>`<span title="${i}">${i?.length>50?i.slice(0,50)+"\u2026":i}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:i=>`<span title="${i||""}">${i?.length>40?i.slice(0,40)+"\u2026":i||"-"}</span>`},{key:"status",label:"Status",render:i=>q(i)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:i=>window.formatDate(i)},{key:"day_count",label:"Hari",render:i=>i??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:i=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:i?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:i?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:i?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:i?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:i?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(i?.employee_name),value:i?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(i?.fc_specialist),value:i?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:i?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:i?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:i?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let i=await k(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(i.ok){let s=i.data.data.map(n=>({Tanggal:n.report_date||"",Cabang:n.branch_name||"",Kategori:n.category||"",Sumber:n.source||"",Keluhan:n.complaint||"","Nama FC":n.employee_name||"","FC Spesialis":n.fc_specialist||"",Solusi:n.solution||"","Tgl Selesai":n.completion_date||"",Status:n.status||""}));B(s,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{B([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async i=>{let n=(await k("/api/branches?all=1")).data?.data||[],o=p=>{if(!p)return null;let g=String(p||"").toLowerCase(),y=n.find(u=>String(u.full_name||"").toLowerCase()===g||String(u.code||"").toLowerCase()===g||String(u.name||"").toLowerCase()===g);return y?y.id:null},m=i.map(p=>({branch_id:o(String(p.Cabang||"").trim()),report_date:String(p.Tanggal||"").trim(),category:String(p.Kategori||"").trim(),source:String(p.Sumber||"").trim(),complaint:String(p.Keluhan||"").trim(),employee_name:String(p["Nama FC"]||"").trim(),fc_specialist:String(p["FC Spesialis"]||"").trim(),solution:String(p.Solusi||"").trim(),completion_date:String(p["Tgl Selesai"]||"").trim(),status:String(p.Status||"").trim()})).filter(p=>p.report_date&&p.complaint&&p.category),c=await k("/api/import/issues",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}}})}F();var $e=[];function Pa(t,e){let r=String(t.status||"").toLowerCase();return e==="pending"?r==="pending":!1}async function Mt(t,e){let r=e?e.get("dash_filter"):null;$e=await H();let a=await ee(),l=["Ade","Berlin"],d=s=>s&&!a.find(n=>n.value===s)?[...a,{value:s,label:s}]:a,i=s=>s&&!l.find(n=>(typeof n=="object"?n.value:n)===s)?[...l,s]:l;L({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:s=>r?s.filter(n=>Pa(n,r)):s,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:s=>window.formatDate(s)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:s=>`<span title="${s||""}">${s?.length>50?s.slice(0,50)+"\u2026":s||"-"}</span>`},{key:"solution",label:"Solusi",render:s=>`<span title="${s||""}">${s?.length>40?s.slice(0,40)+"\u2026":s||"-"}</span>`},{key:"status",label:"Status",render:s=>q(s)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:s=>window.formatDate(s)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:s=>s?`<a href="${s}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:$e},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async s=>{let n=new URLSearchParams(s||{}).toString(),o=await k(`/api/one-on-one?limit=10000&${n}`);if(o.ok){let m=o.data.data.map(p=>({Tanggal:p.meeting_date||"",Cabang:p.branch_name||"","Nama Karyawan":p.employee_name||"",PIC:p.pic||"",Masalah:p.problem||"",Solusi:p.solution||"",Status:p.status||"","Tgl Selesai":p.completion_date||"",Dokumen:p.document_link||""})),{downloadExcel:c}=await Promise.resolve().then(()=>(K(),se));c(m,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let s=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(K(),se));n(s,"Template_Import_OneOnOne")},onImport:async s=>{let n=p=>{if(!p)return null;let g=String(p||"").toLowerCase(),y=$e.find(u=>String(u.label||"").toLowerCase()===g);return y?y.value:null},o=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let g=String(p).trim();if(/^\d{4,5}$/.test(g)){let u=Number(g);if(u>2e4&&u<99999){let b=new Date(Date.UTC(1899,11,30)+u*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(g))return g.slice(0,10);let y=g.split(/[\/\-\.]/);if(y.length===3){let[u,b,h]=y.map(_=>_.trim());if(u.length===4&&b.length<=2&&h.length<=2)return`${u}-${b.padStart(2,"0")}-${h.padStart(2,"0")}`;if(h.length===4&&b.length<=2&&u.length<=2)return`${h}-${b.padStart(2,"0")}-${u.padStart(2,"0")}`}return g},m=s.map(p=>({meeting_date:o(p.Tanggal),employee_name:String(p["Nama Karyawan"]||"").trim(),branch_id:n(String(p.Cabang||"").trim()),pic:String(p.PIC||"").trim(),problem:String(p.Masalah||"").trim(),solution:String(p.Solusi||"").trim(),status:String(p.Status||"").trim(),completion_date:o(p["Tgl Selesai"]),document_link:String(p.Dokumen||"").trim()})).filter(p=>p.meeting_date&&p.employee_name&&p.branch_id),c=await k("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:s=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:s?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:s?.branch_id&&!$e.find(n=>n.value==s.branch_id)?[...$e,{value:s.branch_id,label:s.branch_name||s.branch_id}]:$e,createApi:{path:"/api/branches",field:"full_name"},value:s?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:d(s?.employee_name),value:s?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:i(s?.pic),createApi:{path:"/api/pic",field:"name"},value:s?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:s?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:s?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:s?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:s?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:s?.document_link}]})}F();async function Ot(t){let e=await H(),r=await ee(),a=["Ade","Berlin"],l=s=>s&&!r.find(n=>n.value===s)?[...r,{value:s,label:s}]:r,d=s=>s&&!a.find(n=>(typeof n=="object"?n.value:n)===s)?[...a,s]:a,i=Array.from({length:5},(s,n)=>String(new Date().getFullYear()-n));L({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:s=>window.formatDate(s)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:s=>{try{let n=JSON.parse(s);return Array.isArray(n)?n.join(", "):s||"-"}catch{return s||"-"}}},{key:"score",label:"Nilai",render:s=>s!=null?`<strong>${s}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:s=>s?`<a href="${s}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:i}],exportOptions:{moduleName:"training",onExport:async s=>{let n=new URLSearchParams(s||{}).toString(),o=await k(`/api/training?limit=10000&${n}`);if(o.ok){let m=o.data.data.map(p=>{let g=p.participants||"";try{let y=JSON.parse(g);g=Array.isArray(y)?y.join(", "):g}catch{}return{Tanggal:p.training_date||"",Batch:p.batch||"",Materi:p.subject||"",Cabang:p.branch_name||"",Trainer:p.trainer||"",Peserta:g,Nilai:p.score!==null&&p.score!==void 0?p.score:"",Dokumen:p.document_link||""}}),{downloadExcel:c}=await Promise.resolve().then(()=>(K(),se));c(m,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let s=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(K(),se));n(s,"Template_Import_Training")},onImport:async s=>{let n=p=>{if(!p)return null;let g=String(p||"").toLowerCase(),y=e.find(u=>String(u.label||"").toLowerCase()===g);return y?y.value:null},o=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let g=String(p).trim();if(/^\d{4,5}$/.test(g)){let u=Number(g);if(u>2e4&&u<99999){let b=new Date(Date.UTC(1899,11,30)+u*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(g))return g.slice(0,10);let y=g.split(/[\/\-\.]/);if(y.length===3){let[u,b,h]=y.map(_=>_.trim());if(u.length===4&&b.length<=2&&h.length<=2)return`${u}-${b.padStart(2,"0")}-${h.padStart(2,"0")}`;if(h.length===4&&b.length<=2&&u.length<=2)return`${h}-${b.padStart(2,"0")}-${u.padStart(2,"0")}`}return g},m=s.map(p=>({training_date:o(p.Tanggal),batch:String(p.Batch||"").trim(),subject:String(p.Materi||"").trim(),branch_id:n(String(p.Cabang||"").trim()),trainer:String(p.Trainer||"").trim(),participants:String(p.Peserta||"").trim(),score:p.Nilai?Number(p.Nilai):null,document_link:String(p.Dokumen||"").trim()})).filter(p=>p.training_date&&p.subject&&p.branch_id),c=await k("/api/import/training",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:s=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:s?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:s?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:s?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:s?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:d(s?.trainer),value:s?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let n=JSON.parse(s?.participants);return Array.isArray(n)?n.join(", "):s?.participants||""}catch{return s?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:s?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:s?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:s?.notes}],onBeforeSubmit:async s=>(s.participants&&(s.participants=JSON.stringify(s.participants.split(",").map(n=>n.trim()).filter(Boolean))),s)})}F();ye();K();function Rt({container:t,title:e,icon:r,apiPath:a,columns:l,formFields:d,filterFields:i,defaultFilters:s={},itemLabel:n="Data",canCreate:o=!0,canEdit:m=!0,canDelete:c=!0,onBeforeSubmit:p,onAfterLoad:g,onDataLoaded:y,extraActions:u=[],initialSearch:b="",exportOptions:h=null,bulkDelete:_=!1,paginationMode:C="server"}){let S=1,x={...s};b&&(x.search=b);let w=new Set;t.innerHTML=`
  ...
  `,document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),l()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),l()}),document.getElementById("cal-event-close").addEventListener("click",()=>{document.getElementById("cal-event-list").style.display="none"}),document.querySelectorAll(".cal-filter").forEach(d=>d.addEventListener("change",l));async function a(){try{let d=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`;r=(await k(`/api/dashboard/calendar?month=${d}`)).data?.data||[]}catch(d){console.warn("[Calendar] Failed to load events, rendering empty grid:",d),r=[]}}async function l(){let d=document.getElementById("calendar-grid");if(d){d.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;background:var(--border);">
    ...
    </div>`,await a();try{let i=e.getFullYear(),s=e.getMonth(),n=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}),o=document.getElementById("cal-month-label");o&&(o.textContent=n);let m=new Set(Array.from(document.querySelectorAll(".cal-filter:checked")).map(S=>S.value)),c=r.filter(S=>m.has(S.type)),p={};c.forEach(S=>{let x=(S.event_date||"").slice(0,10);p[x]||(p[x]=[]),p[x].push(S)});let g=new Date(i,s,1).getDay(),y=new Date(i,s+1,0).getDate(),u=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],b=new Date().toISOString().slice(0,10),h='<div class="calendar-grid">';u.forEach(S=>{h+=`<div class="cal-day-header">${S}</div>`});for(let S=0;S<g;S++)h+='<div class="cal-cell cal-cell-empty"></div>';for(let S=1;S<=y;S++){let x=`${i}-${String(s+1).padStart(2,"0")}-${String(S).padStart(2,"0")}`,w=p[x]||[],R=x===b;h+=`
  ...
  `;oe({title:`Notifikasi (${Ie.length})`,content:t,confirmText:"Tutup",onConfirm:(e,r)=>r()})}function la(){let t=he(),e=(t?.full_name||"U")[0].toUpperCase();document.getElementById("app").innerHTML=`
