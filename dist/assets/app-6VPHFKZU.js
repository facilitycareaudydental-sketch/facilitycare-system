var oa=Object.defineProperty;var tt=(t,e)=>()=>(t&&(e=t(t=0)),e);var at=(t,e)=>{for(var i in e)oa(t,i,{get:e[i],enumerable:!0})};var fe={};at(fe,{API:()=>ft,CLIENT_SIDE_MAX_ROWS:()=>ge,IS_DEVELOPMENT:()=>De,apiFetch:()=>k,clearToken:()=>Pe,getToken:()=>Ie,getUser:()=>be,setToken:()=>nt,setUser:()=>Le});function Ie(){return localStorage.getItem("fm_token")}function nt(t){localStorage.setItem("fm_token",t)}function Pe(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function be(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Le(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let i=Ie(),a={"Content-Type":"application/json",...i?{Authorization:`Bearer ${i}`}:{},...e.headers||{}};try{let o=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",r=`${ft}${t}${d}${o}`,s=await fetch(r,{...e,headers:a}),n;try{let l=await s.text();try{n=JSON.parse(l)}catch{n={error:`Server Error (${s.status}): ${l.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return s.status===401&&(Pe(),window.location.hash="#/login"),{ok:s.ok,status:s.status,data:n}}catch(o){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${o.message})`}}}}var De,la,ft,ge,N=tt(()=>{De=!1,la="https://fm-operations-api.facilitycare-audydental.workers.dev",ft=la,ge=1e4});var St={};at(St,{confirmDialog:()=>Ne,createModal:()=>le});function le({title:t,content:e,onConfirm:i,onCancel:a,confirmText:o="Simpan",cancelText:d="Batal",size:r="md",confirmClass:s="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},l=document.createElement("div");l.className="modal-overlay",l.innerHTML=`
  ...
  `,e instanceof HTMLElement&&l.querySelector(".modal-body").appendChild(e);let m=()=>{l.classList.remove("show"),setTimeout(()=>l.remove(),250)};return l.querySelector(".modal-close").addEventListener("click",()=>{a&&a(),m()}),l.querySelector(".modal-cancel").addEventListener("click",()=>{a&&a(),m()}),i&&l.querySelector(".modal-confirm").addEventListener("click",()=>i(l,m)),l.addEventListener("click",c=>{c.target===l&&(a&&a(),m())}),document.body.appendChild(l),requestAnimationFrame(()=>l.classList.add("show")),{overlay:l,close:m}}function Ne(t,e,i="Konfirmasi"){return le({title:i,content:`<p>${t}</p>`,onConfirm:(a,o)=>{e(),o()},confirmText:"Ya, Lanjutkan",confirmClass:"btn-danger"})}var ke=tt(()=>{});var se={};at(se,{downloadExcel:()=>L,parseExcel:()=>Me,renderExcelButtons:()=>Fe});function Me(t){return new Promise((e,i)=>{let a=new FileReader;a.onload=o=>{try{let d=new Uint8Array(o.target.result),r=XLSX.read(d,{type:"array"}),s=r.SheetNames[0],n=r.Sheets[s];console.log("--- START EXCEL PARSING ---"),console.log(`File Name: ${t.name}`),console.log(`File Size: ${(t.size/1024).toFixed(2)} KB`),console.log(`File Type: ${t.type||"unknown"}`),console.log(`Sheets Found: ${r.SheetNames.join(", ")}`),console.log(`Sheet Used: ${s}`);let l=XLSX.utils.decode_range(n["!ref"]||"A1:A1"),m=l.e.r-l.s.r+1,c=l.e.c-l.s.c+1;console.log(`Total Rows (including empty): ${m}`),console.log(`Total Columns: ${c}`);let p=[];for(let h=l.s.c;h<=l.e.c;++h){let g=n[XLSX.utils.encode_cell({c:h,r:l.s.r})];g&&g.v&&p.push(g.v)}console.log(`Headers Found: ${p.join(", ")}`),console.log("---------------------------");let u=XLSX.utils.sheet_to_json(n,{defval:""});Object.defineProperty(u,"__worksheet",{value:n,enumerable:!1}),Object.defineProperty(u,"__headers",{value:p,enumerable:!1}),e(u)}catch(d){i(d)}},a.onerror=o=>i(o),a.readAsArrayBuffer(t)})}function L(t,e){try{let i=XLSX.utils.json_to_sheet(t),a=XLSX.utils.book_new();XLSX.utils.book_append_sheet(a,i,"Data"),XLSX.writeFile(a,`${e}.xlsx`)}catch(i){throw console.error("Error generating Excel file:",i),i}}function Fe(t){return`
  ...
  `}var H=tt(()=>{});N();var it={},qe=null;function U(t,e){it[t]=e}function ve(t){window.location.hash=t}function vt(){async function t(){let e=window.location.hash.replace("#","")||"/dashboard",[i,...a]=e.split("?"),o=it[i];if(!o){for(let[r,s]of Object.entries(it))if(r.endsWith("/*")&&i.startsWith(r.slice(0,-2))){o=s;break}}qe&&(qe(),qe=null);let d=document.getElementById("main-content");if(d&&(d.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>'),o){let r=new URLSearchParams(a.join("?")),s=i.split("/").filter(Boolean),n=await o({path:i,params:r,segments:s,main:d});n&&(qe=n)}else{let r=d||document.getElementById("app");r&&(r.innerHTML='<div class="empty-state"><h2>404 - Halaman tidak ditemukan</h2></div>')}}window.addEventListener("hashchange",t),t()}var Be;function da(){return Be||(Be=document.createElement("div"),Be.id="toast-container",document.body.appendChild(Be)),Be}function kt(t,e="info",i=3500){let a=da(),o=document.createElement("div");o.className=`toast toast-${e}`;let d={success:"\u2713",error:"\u2715",warning:"\u26A0",info:"\u2139"};o.innerHTML=`<span class="toast-icon">${d[e]||"\u2139"}</span><span class="toast-msg">${t}</span>`,a.appendChild(o),requestAnimationFrame(()=>o.classList.add("show")),setTimeout(()=>{o.classList.remove("show"),setTimeout(()=>o.remove(),350)},i)}var te=t=>kt(t,"success"),V=t=>kt(t,"error");ke();N();N();function je({columns:t,data:e,onEdit:i,onDelete:a,onView:o,actions:d=[],emptyText:r="Tidak ada data",bulkSelect:s=null}){let n=document.createElement("div");if(n.className="table-wrapper",!e||e.length===0)return n.innerHTML=`<div class="empty-state"><p>${r}</p></div>`,n;let l=document.createElement("table");l.className="data-table";let m=document.createElement("thead"),c=document.createElement("tr");if(s){let u=document.createElement("th");u.style.width="40px",u.style.textAlign="center";let h=document.createElement("input");h.type="checkbox",h.id="select-all-checkbox",h.title="Pilih semua",h.addEventListener("change",()=>{e.forEach(g=>{h.checked?s.selectedIds.add(g.id):s.selectedIds.delete(g.id)}),n.querySelectorAll(".row-checkbox").forEach(g=>g.checked=h.checked),s.onToggle()}),u.appendChild(h),c.appendChild(u)}if(t.forEach(u=>{let h=document.createElement("th");h.textContent=u.label,u.width&&(h.style.width=u.width),c.appendChild(h)}),i||a||o||d.length>0){let u=document.createElement("th");u.textContent="Aksi",u.style.width="120px",c.appendChild(u)}m.appendChild(c),l.appendChild(m);let p=document.createElement("tbody");return e.forEach(u=>{let h=document.createElement("tr");if(s){let g=document.createElement("td");g.style.textAlign="center",g.style.width="40px";let b=document.createElement("input");b.type="checkbox",b.className="row-checkbox",b.checked=s.selectedIds.has(u.id),b.addEventListener("change",()=>{if(b.checked)s.selectedIds.add(u.id);else{s.selectedIds.delete(u.id);let y=document.getElementById("select-all-checkbox");y&&(y.checked=!1)}s.onToggle()}),g.appendChild(b),h.appendChild(g)}if(t.forEach(g=>{let b=document.createElement("td");if(g.render){let y=g.render(u[g.key],u);y instanceof HTMLElement?b.appendChild(y):b.innerHTML=y||""}else b.textContent=u[g.key]!==null&&u[g.key]!==void 0&&u[g.key]!==""?u[g.key]:"";g.nowrap&&(b.style.whiteSpace="nowrap"),h.appendChild(b)}),i||a||o||d.length>0){let g=document.createElement("td");g.className="actions-cell";let b=document.createElement("div");if(b.className="btn-group",o){let y=document.createElement("button");y.className="btn btn-xs btn-ghost",y.innerHTML="\u{1F441}",y.title="Lihat",y.addEventListener("click",()=>o(u)),b.appendChild(y)}if(i){let y=document.createElement("button");y.className="btn btn-xs btn-secondary",y.innerHTML="\u270F\uFE0F",y.title="Edit",y.addEventListener("click",()=>i(u)),b.appendChild(y)}d.forEach(y=>{let S=document.createElement("button");S.className=`btn btn-xs ${y.class||"btn-ghost"}`,S.innerHTML=y.icon||y.label,S.title=y.label,S.addEventListener("click",()=>y.handler(u)),b.appendChild(S)}),g.appendChild(b),h.appendChild(g)}p.appendChild(h)}),l.appendChild(p),n.appendChild(l),n}function Ue({page:t,pages:e,total:i,limit:a,onPage:o}){if(e<=1)return null;let d=document.createElement("div");d.className="pagination";let r=document.createElement("span");r.className="pagination-info",r.textContent=`Total: ${i} data`,d.appendChild(r);let s=document.createElement("div");s.className="pagination-btns";let n=(c,p,u=!1,h=!1)=>{let g=document.createElement("button");g.className=`btn btn-sm ${h?"btn-primary":"btn-ghost"} pagination-btn`,g.textContent=c,g.disabled=u,g.addEventListener("click",()=>o(p)),s.appendChild(g)};n("\xAB",1,t===1),n("\u2039",t-1,t===1);let l=Math.max(1,t-2),m=Math.min(e,t+2);for(let c=l;c<=m;c++)n(c,c,!1,c===t);return n("\u203A",t+1,t===e),n("\xBB",e,t===e),d.appendChild(s),d}ke();function Ae(t){return t.map(e=>{if(e.type==="hidden")return`<input type="hidden" name="${e.name}" value="${e.value||""}">`;if(e.type==="html")return e.html||"";if(e.type==="row")return`<div class="form-row">${Ae(e.fields)}</div>`;let i=e.required?"required":"",a=e.label?`<label class="form-label">${e.label}${e.required?' <span class="required">*</span>':""}</label>`:"",o="";switch(e.type){case"textarea":o=`<textarea name="${e.name}" class="form-control" placeholder="${e.placeholder||""}" ${i} rows="${e.rows||3}">${e.value||""}</textarea>`;break;case"select":let r=(e.options||[]).map(c=>{let p=typeof c=="object"?c.value:c,u=typeof c=="object"?c.label:c,h=e.value==p?"selected":"";return`<option value="${p}" ${h}>${u}</option>`}).join("");o=`<select name="${e.name}" class="form-control" ${i}><option value="">-- Pilih ${e.label||""} --</option>${r}</select>`;break;case"combobox":let s=`dl-${e.name}-${Math.random().toString(36).substring(7)}`,n=(e.options||[]).map(c=>{let p=typeof c=="object"?c.value:c,u=typeof c=="object"?c.label||c.value||"":c||"";return(u==="undefined"||u==="[object Object]"||u==="null")&&(u=""),u?`<option value="${u}"></option>`:""}).join(""),l=e.value||"";if(e.value){let c=(e.options||[]).find(p=>(typeof p=="object"?p.value:p)==e.value);if(c){let p=typeof c=="object"?c.label||c.value||"":c||"";p&&p!=="undefined"&&p!=="[object Object]"&&p!=="null"&&(l=p)}}o=`
    ...
    <div class="filter-bar card" style="padding: 1rem;">
        ${r.map(v=>{if(v.type==="search")return`<div class="filter-search" style="flex:1; min-width:200px;"><input type="text" class="form-control" autocomplete="off" placeholder="${v.placeholder||"Cari..."}" id="filter-search" value="${_.search||""}"></div>`;if(v.type==="search-combo"){let f="dl-filter-search",E=(v.options||[]).map(I=>`<option value="${typeof I=="object"?I.label:I}"></option>`).join("");return`<div class="filter-search" style="flex:1; min-width:200px;"><input type="text" list="${f}" class="form-control" autocomplete="off" placeholder="${v.placeholder||"Cari..."}" id="filter-search" value="${_.search||""}"><datalist id="${f}">${E}</datalist></div>`}return v.type==="select"?`<select class="form-control filter-select" style="flex:1; min-width:150px;" name="${v.name}" id="filter-${v.name}"><option value="">-- ${v.label} --</option>${(v.options||[]).map(f=>`<option value="${typeof f=="object"?f.value:f}" ${_[v.name]===(typeof f=="object"?f.value:f)?"selected":""}>${typeof f=="object"?f.label:f}</option>`).join("")}</select>`:""}).join("")}
  ...
  `;function O(){if(!document.getElementById("bulk-toolbar"))return;let f=document.getElementById("bulk-count"),E=document.getElementById("btn-bulk-delete"),I=document.getElementById("btn-bulk-cancel");f.textContent=`${C.size} item dipilih`,C.size>0?(E.disabled=!1,I.disabled=!1):(E.disabled=!0,I.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{C.clear(),document.querySelectorAll(".row-checkbox").forEach(f=>f.checked=!1);let v=document.getElementById("select-all-checkbox");v&&(v.checked=!1),O()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(C.size===0)return;let v=[...C],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",f.innerHTML=`
    ...
    `,document.body.appendChild(f),f.querySelector("#bulk-cancel-btn").addEventListener("click",()=>f.remove()),f.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let E=f.querySelector("#bulk-confirm-btn");E.disabled=!0,E.textContent="Menghapus...";let I=await k(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:v})});f.remove(),I.ok?(te(`${v.length} ${n} berhasil dihapus.`),C.clear(),O(),A()):V(I.data?.error||"Gagal menghapus data.")})});let T=document.getElementById("filter-search"),F;if(T?.addEventListener("input",v=>{clearTimeout(F),F=setTimeout(()=>{_.search=v.target.value,x=1,A()},400)}),r?.forEach(v=>{v.type==="select"&&document.getElementById(`filter-${v.name}`)?.addEventListener("change",f=>{_[v.name]=f.target.value,x=1,A()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{_={...s},T&&(T.value=""),r?.forEach(v=>{let f=document.getElementById(`filter-${v.name}`);f&&(f.value="")}),x=1,A()}),document.getElementById("btn-create")?.addEventListener("click",()=>pe(null)),y){document.getElementById(`btn-export-${y.moduleName}`)?.addEventListener("click",async f=>{let E=f.target,I=E.innerHTML;E.innerHTML="\u23F3 Loading...",E.disabled=!0;try{await y.onExport()}catch{V("Gagal export data")}finally{E.innerHTML=I,E.disabled=!1}}),document.getElementById(`btn-template-${y.moduleName}`)?.addEventListener("click",()=>{y.onTemplate()});let v=document.getElementById(`input-import-${y.moduleName}`);v?.addEventListener("change",async f=>{let E=f.target.files[0];if(!E)return;v.disabled=!0;let I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",I.innerHTML=`
      ...
      `,document.body.appendChild(I);let W=I.querySelector("#import-progress-text"),J=I.querySelector("#import-progress-bar"),P=I.querySelector("#import-summary"),$=I.querySelector("#import-close-btn");$.addEventListener("click",()=>{I.remove(),A()});try{let Z=await Me(E);if(Z.length===0)throw new Error("File kosong atau format salah");let ie=500,ee=0,M=0,R=0,K=Z.length;W.textContent=`Ditemukan ${K} baris data. Memulai import...`;for(let D=0;D<K;D+=ie){let z=Z.slice(D,D+ie);W.textContent=`Mengimport baris ${D+1} - ${Math.min(D+ie,K)} dari ${K}...`,J.style.width=`${Math.round(D/K*100)}%`;try{let G=await y.onImport(z);G?(ee+=G.inserted||G.metrics?.inserted||z.length,M+=G.skipped||G.metrics?.updated||0):ee+=z.length}catch(G){console.error("Chunk import failed:",G),R+=z.length}}J.style.width="100%",W.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',P.style.display="block",P.innerHTML=`
          <div style="display:flex;justify-content:space-between;margin-bottom:8px"><span>Total Data:</span> <strong>${K}</strong></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:8px;color:var(--success)"><span>Berhasil Diimport:</span> <strong>${ee}</strong></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:8px;color:var(--warning)"><span>Dilewati (Duplikat):</span> <strong>${M}</strong></div>
          <div style="display:flex;justify-content:space-between;color:var(--danger)"><span>Gagal:</span> <strong>${R}</strong></div>
        `,R>0&&(P.innerHTML+='<p style="margin-top:12px;font-size:0.8rem;color:var(--danger)">Sebagian data gagal diimport. Pastikan format kolom sesuai template dan tidak ada data kosong pada kolom wajib.</p>'),$.style.display="block",v.value=""}catch(Z){W.innerHTML=`<strong style="color:var(--danger)">\u274C Gagal Memproses File</strong><br>${Z.message}`,J.style.background="var(--danger)",J.style.width="100%",$.style.display="block",v.value=""}finally{v.disabled=!1}})}async function A(){C.clear(),O();let v=document.getElementById("table-container");if(!v)return;v.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';let f=w==="client",E=f?1:x,I=f?ge:20,W=new URLSearchParams({page:E,limit:I,...Object.fromEntries(Object.entries(_).filter(([,M])=>M))}),J=await k(`${a}?${W}`);if(!J.ok){v.innerHTML=`<div class="empty-state"><p class="text-danger">Gagal memuat data: ${J.data?.error||"Error"}</p></div>`;return}let P=J.data?.data||J.data||[],$=J.data?.pagination,Z=P.length;if(f){P=h(P);let M=P.length,R=20,K=Math.ceil(M/R);x>K&&K>0&&(x=K);let D=(x-1)*R,z=x*R;P=P.slice(D,z),$={page:x,limit:R,total:M,pages:K}}!1,u&&u(P);let ie=je({columns:o,data:P,onEdit:m?M=>pe(M):null,actions:g.map(M=>({...M,handler:R=>M.handler(R,A)})),emptyText:`Tidak ada ${String(n||"").toLowerCase()}`,bulkSelect:S?{selectedIds:C,onToggle:O}:null});v.innerHTML="",v.appendChild(ie);let ee=document.getElementById("pagination-container");if(ee&&(ee.innerHTML="",$&&$.pages>1)){let M=Ue({page:$.page,pages:$.pages,total:$.total,limit:$.limit,onPage:R=>{x=R,A()}});M&&ee.appendChild(M)}}function he(v){let f=typeof d=="function"?d(v):d;return Ae(f)}function pe(v){let f=!!v,E=document.createElement("form");if(E.noValidate=!0,E.innerHTML=he(v),f){let W=typeof d=="function"?d(v):d;Ge(E,v)}let{close:I}=le({title:f?`Edit ${n}`:`Tambah ${n}`,content:E,size:"lg",confirmText:f?"Simpan Perubahan":`Tambah ${n}`,onConfirm:async(W,J)=>{if(!E.reportValidity())return;let P=W.querySelector(".modal-confirm");P.disabled=!0,P.textContent="Menyimpan...";let $=Je(E),Z=typeof d=="function"?d(v):d,ie=async K=>{for(let D of K)if(D.type==="row")await ie(D.fields);else if(D.type==="combobox"&&$[D.name]){let z=$[D.name],G=(D.options||[]).find(Y=>{let re=String(typeof Y=="object"?Y.value:Y),et=String(typeof Y=="object"?Y.label:Y);return re===z||et===z});if(G)$[D.name]=typeof G=="object"?G.value:G;else if(D.createApi){let Y={};Y[D.createApi.field]=z,D.createApi.extra&&Object.assign(Y,D.createApi.extra);let re=await k(D.createApi.path,{method:"POST",body:JSON.stringify(Y)});if(re.ok&&re.data?.id)$[D.name]=re.data.id;else if(re.ok&&!re.data?.id)$[D.name]=z;else throw new Error(`Gagal membuat master data: ${re.data?.error||"Unknown error"}`)}}};try{await ie(Z)}catch(K){V(K.message),P.disabled=!1,P.textContent=f?"Simpan Perubahan":`Tambah ${n}`;return}p&&($=await p($,v));let ee=f?"PUT":"POST",M=f?`${a}/${v.id}`:a,R=await k(M,{method:ee,body:JSON.stringify($)});R.ok?(te(f?`${n} berhasil diperbarui.`:`${n} berhasil ditambahkan.`),J(),A()):(V(R.data?.error||"Gagal menyimpan data."),P.disabled=!1,P.textContent=f?"Simpan Perubahan":`Tambah ${n}`)}})}function ye(v){Ne(`Hapus ${n} ini? Tindakan tidak dapat dibatalkan.`,async()=>{let f=await k(`${a}/${v.id}`,{method:"DELETE"});f.ok?(te(`${n} berhasil dihapus.`),A()):V(f.data?.error||"Gagal menghapus.")},`Hapus ${n}`)}return A(),A}N();N();var Se=null,Qe=null;async function xe(t=!1){if(Se&&!t)return console.log("Employees Raw (Cache Hit)",Se.slice(0,5)),Se;let e=await k(`/api/employees?limit=${ge}&status=Aktif`);return Se=(e.data?.data||[]).map(i=>({value:i.id,label:i.full_name})),console.log("Employees Raw",e.data?.data?.slice(0,5)),console.log("Employees Mapped (ID)",Se.slice(0,5)),Se}async function ae(t=!1){let i=(await xe(t)).map(a=>({value:a.label,label:a.label}));return console.log("Employee Options",i.slice(0,5)),i}async function q(t=!1){return Qe&&!t||(Qe=((await k("/api/branches?all=1")).data?.data||[]).map(i=>({value:i.id,label:i.full_name}))),Qe}function j(t){let e={Done:"badge-success",Aktif:"badge-success",Open:"badge-warning","In Progress":"badge-info",Pending:"badge-warning",Diproses:"badge-info",Selesai:"badge-success","Tidak Aktif":"badge-neutral",Resign:"badge-neutral",Cut:"badge-danger","Tidak Datang":"badge-danger"};return!t||t==="-"||String(t).trim()===""?"":`<span class="badge ${e[t]||"badge-neutral"}">${t}</span>`}function rt(t){return t==null?'<span class="badge badge-neutral">-</span>':t<0?`<span class="badge badge-danger">Expired (${Math.abs(t)}h)</span>`:t<=14?`<span class="badge badge-danger">${t} hari</span>`:t<=30?`<span class="badge badge-warning">${t} hari</span>`:`<span class="badge badge-success">${t} hari</span>`}function we(t){return`<span class="badge ${{"FACILITY CARE":"badge-info",SECURITY:"badge-secondary"}[t]||"badge-neutral"}">${t||"-"}</span>`}function st(t){return`<span class="badge ${{"Inspeksi Hygiene & Aset Bangunan":"badge-info","General Cleaning":"badge-success","Deep Cleaning":"badge-purple",Fogging:"badge-warning"}[t]||"badge-neutral"}">${t||"-"}</span>`}function de(t){return`<span class="badge ${{Q1:"badge-info",Q2:"badge-success",Q3:"badge-warning",Q4:"badge-danger"}[t]||"badge-neutral"}">${t||"-"}</span>`}H();function ot(t,e){if(t.period!=="Q3")return!1;let i=String(t.status||"").toLowerCase();if(i!=="selesai"&&i!=="completed"&&i!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}N();H();function xt(t,e){let i=String(t.status||"").toLowerCase();return e==="active"?i==="aktif":!1}N();H();function lt(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let o=new Date(a);o.setDate(a.getDate()+30);let d=new Date(t.end_date);return d.setHours(0,0,0,0),d>=a&&d<=o}return!1}N();H();function wt(t,e){let i=String(t.status||"").toLowerCase();return e==="open"?i==="open":!1}N();function _t(t,e){let i=String(t.status||"").toLowerCase();return e==="pending"?i==="pending":!1}var me={};function Oe(t){if(me[t]){try{me[t].destroy()}catch{}delete me[t]}}function ca(){Object.keys(me).forEach(Oe)}var X=(t,e=0)=>{let i=Number(t);return isNaN(i)||t===null||t===void 0?e:i},_e=(t,e="\u2014")=>{if(t==null||t==="")return e;let i=String(t).trim();return i===""||i==="[object Object]"?e:i};var pa=t=>{if(!t||typeof t!="string")return"";try{let[e,i]=t.split("-");return new Date(Number(e),Number(i)-1).toLocaleDateString("id-ID",{month:"short",year:"2-digit"})}catch{return t}};function Ct(t,e,i=900){if(!t)return;let a=Math.max(0,Math.round(X(e)));if(a===0){t.textContent="0";return}let o=Date.now(),d=()=>{let r=Math.min((Date.now()-o)/i,1),s=1-Math.pow(1-r,3);t.textContent=Math.round(s*a).toLocaleString("id-ID"),r<1?requestAnimationFrame(d):t.textContent=a.toLocaleString("id-ID")};requestAnimationFrame(d)}var ma={Done:"pill-success",Aktif:"pill-success",Selesai:"pill-success",Open:"pill-danger",Pending:"pill-warning","In Progress":"pill-info","Tidak Aktif":"pill-neutral",Resign:"pill-neutral",Cut:"pill-neutral"},ua=t=>{let e=_e(t,"\u2014");return`<span class="status-pill ${ma[e]||"pill-neutral"}">${e}</span>`};var ce={family:"Inter",size:11},Ce="#94A3B8",ze="#F1F5F9",dt=["#2563EB","#10B981","#F59E0B","#EF4444","#8B5CF6","#0EA5E9","#F97316","#14B8A6","#6366F1","#EC4899"],ga=()=>window.innerWidth<768;function mt(t={}){return{responsive:!0,maintainAspectRatio:!1,animation:{duration:700,easing:"easeOutQuart"},plugins:{legend:{position:ga()?"bottom":"top",labels:{font:ce,color:"#64748B",usePointStyle:!0,padding:10,boxWidth:8,boxHeight:8}},tooltip:{mode:"index",intersect:!1,bodyFont:ce,titleFont:{...ce,weight:"700"}}},scales:{x:{grid:{color:ze},ticks:{font:ce,color:Ce,maxRotation:0}},y:{grid:{color:ze},ticks:{font:ce,color:Ce},beginAtZero:!0}},...t}}var ba=()=>Array(5).fill(0).map(()=>`
  ...
  </div>`).join("");function ct(t=3){return Array(t).fill(0).map((e,i)=>`<div class="skeleton skeleton-text" style="height:38px;margin-bottom:${i<t-1?"6px":"0"};border-radius:6px"></div>`).join("")}async function oe(t,e,i=8e3){try{let a=new AbortController,o=setTimeout(()=>a.abort(),i),d=await k(t,{signal:a.signal}).catch(()=>null);if(clearTimeout(o),!d||!d.ok)return e;let r=d.data;return r?r.data!==void 0?r.data??e:r:e}catch{return e}}function ya(){["skel-donut","skel-trend","skel-insp","skel-contract"].forEach(a=>{let o=document.getElementById(a);o&&(o.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract"].forEach(a=>{let o=document.getElementById(a);if(o&&o.style.display==="none"){o.style.display="block";let d=o.parentElement;if(d&&!d.querySelector(".chart-empty")){let r=document.createElement("div");r.className="chart-empty",r.textContent="Belum ada data",o.style.display="none",d.appendChild(r)}}});let t=document.getElementById("kpi-row");t&&t.querySelector(".skeleton")&&Et({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&$t({}),["table-contracts","table-issues"].forEach(a=>{let o=document.getElementById(a);o&&o.querySelector(".skeleton")&&(o.innerHTML='<div class="chart-empty">Belum ada data</div>')});let i=document.getElementById("activity-log");i&&i.querySelector(".skeleton")&&(i.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function Tt(t){ca(),t._dashRefresh&&clearInterval(t._dashRefresh),t._skelTimeout&&clearTimeout(t._skelTimeout);let e=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>pt(t)),document.getElementById("filter-insp-month")?.addEventListener("change",async i=>{let a=i.target.value,o=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",d=document.getElementById("skel-insp"),r=document.getElementById("chart-insp");d&&(d.style.display="block",d.style.position="absolute"),r&&(r.style.display="none");let s=await oe(o,{},8e3);try{Dt(s)}catch(n){console.warn("InspBar render:",n),ue("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>ya(),5e3),await pt(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?pt(t):clearInterval(t._dashRefresh)},6e4)}async function pt(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,i,a,o,d,r,s,n,l,m,c,p]=await Promise.all([oe("/api/dashboard/kpi",{},8e3),oe("/api/dashboard/issues-trend",{},8e3),oe("/api/dashboard/issues-summary",{},8e3),oe("/api/dashboard/inspection-bar",{},8e3),oe("/api/dashboard/stats",{},8e3),oe("/api/dashboard/calendar",[],8e3),oe("/api/schedule?limit=10000",{data:[]},8e3),oe("/api/employees?limit=10000",{data:[]},8e3),oe("/api/contracts?limit=10000",{data:[]},8e3),oe("/api/issues?limit=10000",{data:[]},8e3),oe("/api/one-on-one?limit=10000",{data:[]},8e3),oe("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3)]);if(e){let u=Array.isArray(s?.data)?s.data:Array.isArray(s)?s:[],h=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],g=Array.isArray(l?.data)?l.data:Array.isArray(l)?l:[],b=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[],y=Array.isArray(c?.data)?c.data:Array.isArray(c)?c:[];e.employees&&(e.employees.current=h.filter(S=>xt(S,"active")).length),e.contracts&&(e.contracts.current=g.filter(S=>lt(S,"active")).length),e.expiring30&&(e.expiring30={current:g.filter(S=>lt(S,"expiring30")).length}),e.issues&&(e.issues.current=b.filter(S=>wt(S,"open")).length),e.one_on_one&&(e.one_on_one.current=y.filter(S=>_t(S,"pending")).length),e.inspection_month&&(e.inspection_month.current=u.filter(S=>ot(S,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=u.filter(S=>ot(S,"gcdc")).length)}try{Et(e)}catch(u){console.warn("KPI render:",u)}try{$t(e)}catch(u){console.warn("MiniStats render:",u)}try{fa(Array.isArray(a?.by_category)?a.by_category:[])}catch(u){console.warn("Donut render:",u),ue("skel-donut","chart-donut")}try{va(i)}catch(u){console.warn("Trend render:",u),ue("skel-trend","chart-trend")}try{Dt(o)}catch(u){console.warn("InspBar render:",u),ue("skel-insp","chart-insp")}try{let u=Array.isArray(d)?d:Array.isArray(d?.recent_issues)?d.recent_issues:[];Sa(u)}catch(u){console.warn("IssuesTable render:",u)}try{let u=Array.isArray(d?.expiring_contracts)?d.expiring_contracts:[];ka(p)}catch(u){console.warn("ContractsTable render:",u)}try{xa(Array.isArray(r)?r:[])}catch(u){console.warn("Agenda render:",u)}try{wa(e)}catch(u){console.warn("KPI Kebersihan render:",u)}try{_a()}catch(u){console.warn("Quick Actions render:",u)}}function Et(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let i=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=i.map(a=>{let o=X(t[a.key]?.current,0);return`
    ...
    <a href="${a.href}" class="mini-stat ${a.color}" style="text-decoration:none">
      ...
      <div class="mini-stat-body">
        <div class="mini-stat-value" data-target="${X(a.val)}">0</div>
        <div class="mini-stat-text">${a.label}</div>
      </div>
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(a=>Ct(a,parseInt(a.dataset.target)||0,700))}function fa(t){ue("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),i=document.getElementById("donut-legend");if(!e||!i)return;Oe("donut");let a=(t||[]).filter(n=>X(n.count)>0);if(!a.length){Ve(e,"Belum ada data permasalahan");return}let o=a.map(n=>`${_e(n.category,"Lainnya")}`),d=a.map(n=>X(n.count)),r=d.reduce((n,l)=>n+l,0);i.innerHTML=a.map((n,l)=>{let m=dt[l%dt.length],c=r>0?Math.round(n.count/r*100):0;return`
    ...
    `}).join("");let s={id:"centerText",beforeDraw:function(n){let l=n.width,m=n.height,c=n.ctx;c.restore();let p=(m/80).toFixed(2);c.font="bold "+p+"em Inter",c.textBaseline="middle",c.fillStyle="#1E293B";let u=r.toString(),h=Math.round((l-c.measureText(u).width)/2),g=m/2;c.fillText(u,h,g-10),c.font="600 "+(p*.35).toFixed(2)+"em Inter",c.fillStyle="#64748B";let b="Total",y=Math.round((l-c.measureText(b).width)/2);c.fillText(b,y,g+15),c.save()}};me.donut=new Chart(e,{type:"doughnut",data:{labels:o,datasets:[{data:d,backgroundColor:dt,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:ce,titleFont:{...ce,weight:"700"},callbacks:{label:n=>` ${n.label}: ${n.parsed} kasus`}}},cutout:"75%"},plugins:[s]})}function va(t){ue("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;Oe("trend"),t=t||{};let i=(t.labels||[]).map(pa),a=(t.open||[]).map(d=>X(d)),o=(t.closed||[]).map(d=>X(d));if(!i.length){Ve(e,"Belum ada data trend");return}me.trend=new Chart(e,{type:"line",data:{labels:i,datasets:[{label:"Open",data:a,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:o,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#10B981",borderWidth:2}]},options:mt({plugins:{legend:{display:!1}}})})}function Dt(t){ue("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;Oe("inspBar"),t=t||{};let i=t.labels||[],a=(t.fc||[]).map(d=>X(d)),o=(t.spv||[]).map(d=>X(d));if(!i.length){Ve(e,"Belum ada data inspeksi");return}me.inspBar=new Chart(e,{type:"bar",data:{labels:i,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:o,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:mt({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:ce,color:Ce,maxRotation:45,minRotation:30}},y:{grid:{color:ze},ticks:{font:ce,color:Ce},min:0,max:100}}})})}function ka(t){ue("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;Oe("contractMiniBar"),t=t||{};let i=t.labels||[],a=(t.data||[]).map(r=>X(r));if(!i.length){Ve(e,"Belum ada data");return}let d=e.getContext("2d").createLinearGradient(0,0,0,200);d.addColorStop(0,"#60A5FA"),d.addColorStop(1,"#2563EB"),me.contractMiniBar=new Chart(e,{type:"bar",data:{labels:i,datasets:[{label:"Kontrak Habis",data:a,backgroundColor:d,borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:mt({plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:ce,color:Ce,maxRotation:0}},y:{grid:{color:ze,borderDash:[4,4],drawBorder:!1},ticks:{font:ce,color:Ce,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function Sa(t){let e=document.getElementById("table-issues");if(!e)return;let i=(t||[]).slice(0,8);if(!i.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${i.map(a=>`
      ...
      `).join("")}
    </div>`}function xa(t){let e=document.getElementById("widget-agenda");if(!e)return;let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}-${String(i.getDate()).padStart(2,"0")}`,d=(t||[]).filter(r=>(r.event_date||"").startsWith(a)).slice(0,10);if(!d.length){e.innerHTML="";return}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:12px;padding-right:8px">
      ${d.map(r=>{let s="#3B82F6",n="#EFF6FF",l="Agenda",m=(r.title||"").toLowerCase();return m.includes("inspeksi")?(s="#10B981",n="#ECFDF5",l="Inspeksi"):m.includes("cleaning")||m.includes("gcdc")?(s="#3B82F6",n="#EFF6FF",l="Cleaning"):m.includes("reliefer")?(s="#F59E0B",n="#FFFBEB",l="Reliefer"):m.includes("fogging")&&(s="#8B5CF6",n="#F5F3FF",l="Fogging"),`
  ...
  `}function wa(t){let e=document.getElementById("widget-kpi-kebersihan");if(!e)return;let i=t?.kebersihan||{},a=X(i.area?.avg_fc,0),o=X(i.area?.avg_spv,0),d=a>0&&o>0?(a+o)/2:a||o||0,r=X(i.issues?.total,0),s=X(i.issues?.closed,0),n=r>0?Math.round(s/r*100):100,l=X(i.cleaning?.scheduled,0),m=X(i.cleaning?.reported,0),c=l>0?Math.round(m/l*100):100,p=X(i.gcdc?.scheduled,0),u=X(i.gcdc?.reported,0),h=p>0?Math.round(u/p*100):100,g=X(i.issues?.cleanTotal,0),y=X(t?.fogging_month?.current,0)>0?100:0,S=[{label:"Kebersihan Area",val:`${Math.round(d)}%`,target:"Target 95%",icon:"\u{1F9F9}",bg:d>=95?"#ECFDF5":"#FEF2F2",color:d>=95?"#10B981":"#EF4444"},{label:"Penyelesaian Complaint",val:`${n}%`,target:"Target 100%",icon:"\u23F1\uFE0F",bg:n>=100?"#ECFDF5":"#FEF2F2",color:n>=100?"#10B981":"#EF4444"},{label:"Kepatuhan Jadwal Cleaning",val:`${c}%`,target:"Target 100%",icon:"\u23F1\uFE0F",bg:c>=100?"#EFF6FF":"#FEF2F2",color:c>=100?"#3B82F6":"#EF4444"},{label:"Kepatuhan GCDC",val:`${h}%`,target:"Target 100%",icon:"\u{1F9F9}",bg:h>=100?"#EFF6FF":"#FEF2F2",color:h>=100?"#3B82F6":"#EF4444"},{label:"Complaint Cleaning (\u226410)",val:`${g}`,target:"Target \u226410",icon:"\u{1F4DD}",bg:g<=10?"#F5F3FF":"#FEF2F2",color:g<=10?"#8B5CF6":"#EF4444"},{label:"Pelaksanaan Fogging",val:`${y}%`,target:"Target 100%",icon:"\u{1F4A8}",bg:y>=100?"#F5F3FF":"#FEF2F2",color:y>=100?"#8B5CF6":"#EF4444"}];e.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr;gap:12px;padding-bottom:12px;height:160px;overflow-y:auto;padding-right:8px;">
      ${S.map(w=>{let x=w.val.includes("%")?parseInt(w.val):Math.min(100,parseInt(w.val)*10);return`
        <div class="prog-item">
          <div class="prog-header">
            <div class="prog-title">
              <div class="prog-title-icon" style="background:${w.bg};color:${w.color}">${w.icon}</div>
              ${w.label}
            </div>
            <div class="prog-val">${w.val}</div>
          </div>
          <span class="prog-target">${w.target}</span>
          <div class="prog-bar-bg">
            <div class="prog-bar-fill" style="width:${x}%;background:${w.color}"></div>
          </div>
        </div>
      `}).join("")}
    </div>
  ...
  `).join("")}function ue(t,e){let i=document.getElementById(t),a=document.getElementById(e);i&&(i.style.display="none",i.style.position=""),a&&(a.style.display="block")}function Ve(t,e="Belum ada data"){if(!t)return;t.style.display="none";let i=t.parentElement;if(!i)return;if(!i.querySelector(".chart-empty")){let o=document.createElement("div");o.className="chart-empty",o.textContent=e,i.appendChild(o)}}N();async function It(t){document.getElementById("app").innerHTML=`
  ...
  `;let e=document.getElementById("login-form"),i=document.getElementById("login-error"),a=document.getElementById("login-btn"),o=document.getElementById("toggle-password"),d=document.getElementById("login-password");o?.addEventListener("click",()=>{let r=d.type==="text";d.type=r?"password":"text",o.style.color=r?"":"var(--primary)"}),e?.addEventListener("submit",async r=>{r.preventDefault(),i.style.display="none";let s=e.username.value.trim(),n=e.password.value;if(!s||!n){i.textContent="Username dan password wajib diisi.",i.style.display="block";return}a.querySelector(".btn-text").style.display="none",a.querySelector(".btn-spinner").style.display="",a.disabled=!0;try{let l=await k("/api/auth/login",{method:"POST",body:JSON.stringify({username:s,password:n})});l.ok&&l.data.success?(nt(l.data.data.token),Le(l.data.data.user),te("Login berhasil! Selamat datang \u{1F44B}"),window.dispatchEvent(new Event("fm:login"))):(i.textContent=l.data.error||"Username atau password salah.",i.style.display="block",a.classList.add("shake"),setTimeout(()=>a.classList.remove("shake"),600))}catch{i.textContent="Gagal terhubung ke server. Periksa koneksi internet.",i.style.display="block"}finally{a.querySelector(".btn-text").style.display="",a.querySelector(".btn-spinner").style.display="none",a.disabled=!1}})}N();H();async function Ca(){return await q()}function Ta(t,e){let i=String(t.status||"").toLowerCase();return e==="active"?i==="aktif":!1}async function Pt(t,e){let i=await Ca(),a=e?e.get("dash_filter"):null;B({container:t,title:"Karyawan",icon:"\u{1F465}",apiPath:"/api/employees",itemLabel:"Karyawan",bulkDelete:!0,paginationMode:"client",onDataLoaded:o=>a?o.filter(d=>Ta(d,a)):o,columns:[{key:"full_name",label:"Nama Lengkap"},{key:"branch_name",label:"Cabang"},{key:"division",label:"Divisi",render:o=>we(o)},{key:"phone",label:"No. HP",render:o=>o?`<a href="tel:${o}">${o}</a>`:"-"},{key:"join_date",label:"Tgl Masuk",render:o=>window.formatDate(o)},{key:"status",label:"Status",render:o=>j(o)}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:i},{type:"select",name:"division",label:"Divisi",options:["FACILITY CARE","SECURITY"]},{type:"select",name:"status",label:"Status",options:["Aktif","Tidak Aktif","Resign","Cut"]}],formFields:o=>[{type:"row",fields:[{name:"full_name",label:"Nama Lengkap",required:!0,placeholder:"Nama lengkap karyawan",value:o?.full_name},{name:"phone",label:"No. HP",placeholder:"08xx-xxxx-xxxx",value:o?.phone}]},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:i,value:o?.branch_id},{name:"division",label:"Divisi",type:"select",required:!0,options:["FACILITY CARE","SECURITY"],value:o?.division||"FACILITY CARE"}]},{type:"row",fields:[{name:"join_date",label:"Tanggal Masuk",type:"date",value:o?.join_date},{name:"status",label:"Status",type:"select",required:!0,options:["Aktif","Tidak Aktif","Resign","Cut"],value:o?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:o?.notes}],exportOptions:{moduleName:"employees",onExport:async()=>{let o=await k(`/api/employees${window.location.search?window.location.search+"&":"?"}limit=10000`);if(o.ok){let d=o.data.data.map(r=>({"Nama Lengkap":r.full_name,Cabang:r.branch_name||"",Divisi:r.division||"","No. HP":r.phone||"","Tgl Masuk":r.join_date||"",Status:r.status||""}));L(d,"Data_Karyawan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu",Divisi:"FACILITY CARE","No. HP":"08123456789","Tgl Masuk":"2024-01-15",Status:"Aktif"},{"Nama Lengkap":"Andi Saputra",Cabang:"002. Bintaro",Divisi:"SECURITY","No. HP":"08987654321","Tgl Masuk":"2023-11-01",Status:"Aktif"}],"Template_Import_Karyawan")},onImport:async o=>{let d=n=>{if(!n)return null;let l=String(n||"").toLowerCase(),m=i.find(c=>String(c.label||"").toLowerCase()===l);return m?m.value:null},r=o.map(n=>({full_name:String(n["Nama Lengkap"]||"").trim(),branch_id:d(String(n.Cabang||"").trim()),division:String(n.Divisi||"").trim()||"FACILITY CARE",phone:String(n["No. HP"]||"").trim(),join_date:String(n["Tgl Masuk"]||"").trim(),status:String(n.Status||"").trim(),notes:String(n.Catatan||"").trim()})).filter(n=>n.full_name),s=await k("/api/import/employees",{method:"POST",body:JSON.stringify({rows:r,onDuplicate:"update"})});if(!s.ok)throw new Error(s.data?.error||"Import gagal");return s.data}}})}N();H();var gt=[],Lt=[];async function Ea(){gt=await q(),Lt=await xe()}var ut=async t=>{let e=[],i=1;for(;;){let o=await(await Promise.resolve().then(()=>(N(),fe))).apiFetch(`${t}${t.includes("?")?"&":"?"}limit=100&page=${i}`);if(!o.ok)break;let d=o.data?.data||o.data||[],r=Array.isArray(d)?d:[];if(e=e.concat(r),r.length<100||o.data?.pagination&&i>=o.data.pagination.pages)break;i++}return e};function $a(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let o=new Date(a);o.setDate(a.getDate()+30);let d=new Date(t.end_date);return d.setHours(0,0,0,0),d>=a&&d<=o}return!1}async function Bt(t,e){await Ea();let i=e?e.get("dash_filter"):null;B({container:t,title:"Data Kontrak",icon:"\u{1F4CB}",apiPath:"/api/contracts",bulkDelete:!0,itemLabel:"Kontrak",paginationMode:"client",onDataLoaded:a=>i?a.filter(o=>$a(o,i)):a,columns:[{key:"employee_name",label:"Nama Lengkap"},{key:"branch_name",label:"Cabang"},{key:"division",label:"Div / Bagian",render:a=>we(a)},{key:"start_date",label:"Tanggal Mulai",nowrap:!0,render:a=>window.formatDate(a)},{key:"end_date",label:"Tanggal Selesai",nowrap:!0,render:a=>!a||String(a).startsWith("2099")?"Tetap / PKWTT":window.formatDate(a)},{key:"days_remaining",label:"Sisa Kontrak",render:(a,o)=>o.end_date&&String(o.end_date).startsWith("2099")?'<span class="badge badge-success" style="background:#10B981;color:white;padding:4px 8px;border-radius:6px;font-size:0.75rem;font-weight:600">Tetap</span>':rt(a)},{key:"status",label:"Status",render:a=>j(a)}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:gt},{type:"select",name:"status",label:"Status",options:["Aktif","Tidak Aktif","Resign","Cut"]},{type:"select",name:"expiring_days",label:"Akan Habis",options:[{value:"7",label:"7 Hari"},{value:"14",label:"14 Hari"},{value:"30",label:"30 Hari"},{value:"60",label:"60 Hari"}]}],onBeforeSubmit:a=>(a.end_date||(a.end_date="2099-12-31"),a),onAfterLoad:()=>{if(!document.getElementById("btn-find-missing")){let a=document.createElement("button");a.id="btn-find-missing",a.className="btn btn-ghost",a.innerHTML="\u{1F50D} Cek Selisih Karyawan",a.style.marginLeft="8px",a.style.color="#EF4444",a.style.border="1px solid currentColor",a.onclick=async()=>{a.innerHTML="\u231B Mencari...",a.disabled=!0;try{let[d,r]=await Promise.all([ut("/api/employees?status=Aktif"),ut("/api/contracts")]);if(d.length>0){let s=r.filter(c=>c.status==="Aktif"),n=new Set(s.map(c=>c.employee_id)),l=d.filter(c=>!n.has(c.id)),m=`<p style="margin-bottom:12px">Data yang terbaca: <b>${d.length}</b> Karyawan Aktif, dan <b>${s.length}</b> Kontrak Aktif.</p>
              <p style="margin-bottom:12px">Terdapat <b>${l.length}</b> karyawan aktif yang tidak memiliki "Kontrak Aktif". Berikut daftarnya:</p><ul style="padding-left:20px; max-height:400px; overflow-y:auto">`;l.forEach(c=>{let p=r.filter(h=>h.employee_id===c.id),u='<span style="color:#F59E0B">Belum pernah di-input kontrak</span>';if(p.length>0){let h=p[0];u=`Pernah ada kontrak (Status: <b style="color:#EF4444">${h.status}</b>, Selesai: ${window.formatDate(h.end_date)})`}m+=`<li style="margin-bottom:8px"><b>${c.full_name}</b> <br><span style="font-size:0.85em;color:var(--text-2)">Cabang: ${c.branch_name||"-"} | ${u}</span></li>`}),m+="</ul>",Promise.resolve().then(()=>(ke(),St)).then(c=>c.createModal({title:"Karyawan Tanpa Kontrak Aktif",content:m,cancelText:"Tutup"}))}}catch(d){console.error(d)}a.innerHTML="\u{1F50D} Cek Selisih Karyawan",a.disabled=!1};let o=document.querySelector(".page-actions");o&&o.appendChild(a)}},formFields:a=>[{type:"row",fields:[{name:"employee_id",label:"Nama Lengkap",type:"combobox",required:!0,options:Lt,value:a?.employee_id},{name:"branch_id",label:"Cabang",type:"combobox",options:gt,value:a?.branch_id}]},{type:"row",fields:[{name:"division",label:"Div / Bagian",type:"select",required:!0,options:["FACILITY CARE","SECURITY"],value:a?.division||"FACILITY CARE"},{name:"status",label:"Status",type:"select",required:!0,options:["Aktif","Tidak Aktif","Resign","Cut"],value:a?.status||""}]},{type:"row",fields:[{name:"start_date",label:"Tanggal Mulai",type:"date",value:a?.start_date},{name:"end_date",label:"Tanggal Selesai",type:"date",value:a?.end_date&&!String(a.end_date).startsWith("2099")?a.end_date:""}]},{type:"row",fields:[{name:"contract_type",label:"Tipe Kontrak",type:"select",options:["KONTRAK 6 BULAN","KONTRAK 1 TAHUN","KONTRAK 2 TAHUN"],value:a?.contract_type},{name:"pkwt_number",label:"No. PKWT",type:"select",options:["PKWT 1","PKWT 2","PKWT 3","PKWT 4","PKWT 5","PKWT 6"],value:a?.pkwt_number}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:a?.notes}],exportOptions:{moduleName:"contracts",onExport:async()=>{let a=await k(`/api/contracts${window.location.search?window.location.search+"&":"?"}limit=10000`);if(a.ok){let o=a.data.data.map(d=>({"Nama Lengkap":d.employee_name,Cabang:d.branch_name||"","Div / Bagian":d.division||"","Tanggal Mulai":d.start_date||"","Tanggal Selesai":d.end_date&&String(d.end_date).startsWith("2099")?"":d.end_date||"","Sisa Kontrak":d.end_date&&String(d.end_date).startsWith("2099")?"Tetap":d.days_remaining!==null&&d.days_remaining!==void 0?`${d.days_remaining} Hari`:"",Status:d.status||""}));L(o,"Data_Kontrak")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu","Div / Bagian":"FACILITY CARE","Tanggal Mulai":"2024-01-01","Tanggal Selesai":"2024-12-31","Sisa Kontrak":"365 Hari",Status:"Aktif"}],"Template_Import_Kontrak")},onImport:async a=>{let[o,d]=await Promise.all([k("/api/branches?limit=10000"),ut("/api/employees")]),r=o.data?.data||[],s=d||[];console.log(`Total employee yang berhasil dimuat dari database : ${s.length}`),s.length>0&&(console.log("Contoh 5 employee pertama:"),s.slice(0,5).forEach((g,b)=>{console.log(`${b+1}. ID: ${g.id}, Name: ${g.full_name}, Status: ${g.status}`)}));let n=g=>{if(!g)return null;let b=String(g||"").replace(/\s+/g," ").toLowerCase().trim(),y=r.find(S=>String(S.full_name||"").replace(/\s+/g," ").toLowerCase().trim()===b||String(S.code||"").replace(/\s+/g," ").toLowerCase().trim()===b||String(S.name||"").replace(/\s+/g," ").toLowerCase().trim()===b);return y?y.id:null},l=(g,b)=>{if(console.log("------------------------------------------------"),console.log(`Row Excel : ${b}`),console.log(`Nama dari Excel : "${g}"`),!g)return console.log("Alasan gagal mapping : Nama kosong"),null;let y=String(g||"").replace(/\s+/g," ").toLowerCase().trim();console.log(`Nama setelah normalisasi : "${y}"`),console.log(`Jumlah employee di database : ${s.length}`);let S=s.find(w=>String(w.full_name||"").replace(/\s+/g," ").toLowerCase().trim()===y);return S?(console.log("Employee ditemukan atau tidak : Ditemukan"),console.log(`Employee ID jika ditemukan : ${S.id}`),S.id):(console.log("Employee ditemukan atau tidak : TIDAK Ditemukan"),console.log("Alasan gagal mapping : Tidak ada kecocokan full_name setelah normalisasi"),null)},m=g=>{if(!g)return"";if(g instanceof Date&&!isNaN(g.getTime()))return g.toISOString().slice(0,10);let b=String(g).trim();if(/^\d{4,5}(\.\d+)?$/.test(b)){let S=Math.floor(Number(b));if(S>2e4&&S<99999){let w=new Date(Date.UTC(1899,11,30)+S*864e5);return isNaN(w.getTime())?"":w.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(b))return b.slice(0,10);let y=b.split(/[\/\-\.]/);if(y.length===3){let[S,w,x]=y.map(_=>_.trim());if(S.length===4&&w.length<=2&&x.length<=2)return`${S}-${w.padStart(2,"0")}-${x.padStart(2,"0")}`;if(x.length===4&&w.length<=2&&S.length<=2)return`${x}-${w.padStart(2,"0")}-${S.padStart(2,"0")}`}return b},c=a.map((g,b)=>{let y=b+2,S=String(g["Nama Lengkap"]||"").trim(),w=g["Tanggal Mulai"],x=m(w);if(!x){let O=a.__worksheet,T=a.__headers||[],F=T.indexOf("Tanggal Mulai"),A="N/A",he="N/A",pe="N/A";if(F!==-1&&O&&window.XLSX){let v=window.XLSX.utils.encode_cell({c:F,r:y-1});pe=v;let f=O[v];f?(A=f.t||"undefined",he=f.w||"undefined"):A="CELL KOSONG/TIDAK ADA DI WORKSHEET"}let ye="Unknown";w==null||w===""?ye="Kondisi IF: Nilai murni undefined, null, atau string kosong dari parsed JSON.":w instanceof Date&&isNaN(w.getTime())?ye="Kondisi IF: Nilai adalah object Date namun invalid (isNaN).":ye="Kondisi IF: Tidak lolos Regex YYYY-MM-DD maupun konversi serial number Excel.",console.log("=========================="),console.log("[DEBUG] DATE PARSING FAILED"),console.log("=========================="),console.log(`Excel Row Number : ${y}`),console.log(`Employee Name : ${S}`),console.log(`Column Header Used : "Tanggal Mulai" (Index: ${F})`),console.log(`Raw Cell Value : "${w}"`),console.log(`JavaScript Type : ${typeof w}`),console.log(`SheetJS Cell Type : ${A}`),console.log(`SheetJS Formatted Value : "${he}"`),console.log(`Value After Trim : "${String(w||"").trim()}"`),console.log(`Value After Date Parser : "${x}"`),console.log(`Is Empty : ${!w}`),console.log(`Is Invalid Date : ${w instanceof Date?isNaN(w.getTime()):"Bukan JS Date Object"}`),console.log(`Reason : ${ye}`),console.log(`Workbook Sheet : ${O?"Ada":"Tidak Ditemukan"}`),console.log(`Excel Cell Address : ${pe}`),console.log(`
...
`)}let _=l(S,y),C=null;return _||(C="Karyawan tidak ditemukan di Database"),{isValid:!!_,invalidReason:C,rowNum:y,data:{employee_id:_,branch_id:n(String(g.Cabang||"").trim()),division:String(g["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:x,end_date:m(g["Tanggal Selesai"])||"2099-12-31",status:String(g.Status||"").trim(),_rawName:S}}}),p=[],u=[];if(c.forEach(g=>{g.isValid?p.push(g.data):u.push({rowNum:g.rowNum,name:g.data._rawName,reason:g.invalidReason})}),console.log(`Split Validation - Valid: ${p.length}, Invalid: ${u.length}`),p.length===0)return{inserted:0,skipped:a.length,failed:a.length};let h=await k("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:p,onDuplicate:"update"})});if(!h.ok)throw new Error(h.data?.error||"Import gagal");return h.data}}})}N();H();var bt=[],Re=[];function Da(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let i of e)if(t.some(a=>a.period===i))return i;return"Q3"}function Ia(t,e){if(t.period!=="Q3")return!1;let i=String(t.status||"").toLowerCase();if(i!=="selesai"&&i!=="completed"&&i!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}async function Nt(t,e){bt=await q();let i=await ae();Re=["BERLIN ARIANSYAH","ADE SURAHMAN"];let a=l=>l&&!Re.find(m=>(typeof m=="object"?m.value:m)===l)?[...Re,l]:Re,o=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),d=l=>{if(!l||l==="-"||String(l).trim()==="")return"";let m=String(l).split("-");return m.length===3&&m[0].length===4?`${m[2]}-${m[1]}-${m[0]}`:l},r=o.data?.data||[],s=Da(r),n=e?e.get("dash_filter"):null;B({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:n?{period:"Q3"}:{period:s},onDataLoaded:l=>(n&&(l=l.filter(m=>Ia(m,n))),l.sort((m,c)=>{let p=m.opening_date?new Date(m.opening_date).getTime():0;return(c.opening_date?new Date(c.opening_date).getTime():0)-p})),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:l=>st(l)},{key:"period",label:"Periode",render:l=>de(l)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:l=>d(l)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:l=>d(l)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:l=>d(l)},{key:"status",label:"Status",render:l=>j(l)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"activity_type",label:"Kegiatan",options:["Inspeksi Hygiene & Aset Bangunan","General Cleaning","Deep Cleaning","Fogging"]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:Re}],formFields:l=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:l?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene & Aset Bangunan","General Cleaning","Deep Cleaning","Fogging"],value:l?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:l?.period},{name:"pic",label:"PIC",type:"combobox",options:a(l?.pic),value:l?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:l?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:l?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:l?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:l?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:l?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let l=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(l.ok){let m=l.data.data.map(c=>({Cabang:c.branch_name||"",Kegiatan:c.activity_type||"",Periode:c.period||"",PIC:c.pic||"","Tgl Opening":c.opening_date||"","Tgl Target":c.target_date||"","Tgl Selesai":c.completion_date||"",Status:c.status||""}));L(m,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async l=>{let c=(await k("/api/branches?all=1")).data?.data||[],p=b=>{if(!b)return null;let y=String(b||"").toLowerCase(),S=c.find(w=>String(w.full_name||"").toLowerCase()===y||String(w.code||"").toLowerCase()===y||String(w.name||"").toLowerCase()===y);return S?S.id:null},u=b=>{if(b==null||b==="")return"";if(b instanceof Date&&!isNaN(b.getTime()))return b.toISOString().slice(0,10);let y=String(b).trim();if(y===""||y==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(y))return y.slice(0,10);if(/^\d{4,5}$/.test(y)){let w=Number(y);if(w>2e4&&w<99999){let x=new Date(Date.UTC(1899,11,30)+w*864e5);return isNaN(x.getTime())?"":x.toISOString().slice(0,10)}}let S=y.split(/[\/\-\.]/);if(S.length===3){let[w,x,_]=S.map(C=>C.trim());if(w.length===4&&x.length<=2&&_.length<=2)return`${w}-${x.padStart(2,"0")}-${_.padStart(2,"0")}`;if(_.length===4&&x.length<=2&&w.length<=2)return`${_}-${x.padStart(2,"0")}-${w.padStart(2,"0")}`}return y},h=l.map(b=>({branch_id:p(String(b.Cabang||"").trim()),activity_type:String(b.Kegiatan||"").trim(),period:String(b.Periode||"").trim(),pic:String(b.PIC||b.Pic||"").trim(),opening_date:u(b["Tgl Opening"]||b["Tanggal Opening"]||b["Tgl Openir"]),target_date:u(b["Tgl Target"]||b["Tanggal Target"]),completion_date:u(b["Tgl Selesai"]||b["Tanggal Selesai"]),status:String(b.Status||"").trim(),notes:String(b.Catatan||b.Keterangan||"").trim()})).filter(b=>b.activity_type&&b.period),g=await k("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!g.ok)throw new Error(g.data?.error||"Import gagal");return g.data}}})}N();H();var ht=[],We=[];function Pa(t,e){let i=String(t.status||"").toLowerCase();return e==="open"?i==="open":!1}async function At(t,e){let i=e?e.get("dash_filter"):null;ht=await q(),We=await ae();let a=r=>r&&!We.find(s=>s.value===r)?[...We,{value:r,label:r}]:We,o=new Date().getFullYear(),d=Array.from({length:5},(r,s)=>String(o-s));B({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:r=>i?r.filter(s=>Pa(s,i)):r,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:r=>`<span class="badge badge-secondary">${r}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:r=>`<span title="${r}">${r?.length>50?r.slice(0,50)+"\u2026":r}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:r=>`<span title="${r||""}">${r?.length>40?r.slice(0,40)+"\u2026":r||"-"}</span>`},{key:"status",label:"Status",render:r=>j(r)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:r=>window.formatDate(r)},{key:"day_count",label:"Hari",render:r=>r??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"combobox",name:"branch_id",label:"Cabang",options:ht},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:d}],formFields:r=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:r?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:ht,value:r?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:r?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:r?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:r?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(r?.employee_name),value:r?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(r?.fc_specialist),value:r?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:r?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:r?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:r?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let r=await k(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(r.ok){let s=r.data.data.map(n=>({Tanggal:n.report_date||"",Cabang:n.branch_name||"",Kategori:n.category||"",Sumber:n.source||"",Keluhan:n.complaint||"","Nama FC":n.employee_name||"","FC Spesialis":n.fc_specialist||"",Solusi:n.solution||"","Tgl Selesai":n.completion_date||"",Status:n.status||""}));L(s,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async r=>{let n=(await k("/api/branches?all=1")).data?.data||[],l=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=n.find(g=>String(g.full_name||"").toLowerCase()===u||String(g.code||"").toLowerCase()===u||String(g.name||"").toLowerCase()===u);return h?h.id:null},m=r.map(p=>({branch_id:l(String(p.Cabang||"").trim()),report_date:String(p.Tanggal||"").trim(),category:String(p.Kategori||"").trim(),source:String(p.Sumber||"").trim(),complaint:String(p.Keluhan||"").trim(),employee_name:String(p["Nama FC"]||"").trim(),fc_specialist:String(p["FC Spesialis"]||"").trim(),solution:String(p.Solusi||"").trim(),completion_date:String(p["Tgl Selesai"]||"").trim(),status:String(p.Status||"").trim()})).filter(p=>p.report_date&&p.complaint&&p.category),c=await k("/api/import/issues",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}}})}N();var Te=[];function La(t,e){let i=String(t.status||"").toLowerCase();return e==="pending"?i==="pending":!1}async function Mt(t,e){let i=e?e.get("dash_filter"):null;Te=await q();let a=await ae(),o=["Ade","Berlin"],d=s=>s&&!a.find(n=>n.value===s)?[...a,{value:s,label:s}]:a,r=s=>s&&!o.find(n=>(typeof n=="object"?n.value:n)===s)?[...o,s]:o;B({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:s=>i?s.filter(n=>La(n,i)):s,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:s=>window.formatDate(s)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:s=>`<span title="${s||""}">${s?.length>50?s.slice(0,50)+"\u2026":s||"-"}</span>`},{key:"solution",label:"Solusi",render:s=>`<span title="${s||""}">${s?.length>40?s.slice(0,40)+"\u2026":s||"-"}</span>`},{key:"status",label:"Status",render:s=>j(s)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:s=>window.formatDate(s)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:s=>s?`<a href="${s}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:Te},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async s=>{let n=new URLSearchParams(s||{}).toString(),l=await k(`/api/one-on-one?limit=10000&${n}`);if(l.ok){let m=l.data.data.map(p=>({Tanggal:p.meeting_date||"",Cabang:p.branch_name||"","Nama Karyawan":p.employee_name||"",PIC:p.pic||"",Masalah:p.problem||"",Solusi:p.solution||"",Status:p.status||"","Tgl Selesai":p.completion_date||"",Dokumen:p.document_link||""})),{downloadExcel:c}=await Promise.resolve().then(()=>(H(),se));c(m,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let s=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(H(),se));n(s,"Template_Import_OneOnOne")},onImport:async s=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=Te.find(g=>String(g.label||"").toLowerCase()===u);return h?h.value:null},l=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let b=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let h=u.split(/[\/\-\.]/);if(h.length===3){let[g,b,y]=h.map(S=>S.trim());if(g.length===4&&b.length<=2&&y.length<=2)return`${g}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&g.length<=2)return`${y}-${b.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=s.map(p=>({meeting_date:l(p.Tanggal),employee_name:String(p["Nama Karyawan"]||"").trim(),branch_id:n(String(p.Cabang||"").trim()),pic:String(p.PIC||"").trim(),problem:String(p.Masalah||"").trim(),solution:String(p.Solusi||"").trim(),status:String(p.Status||"").trim(),completion_date:l(p["Tgl Selesai"]),document_link:String(p.Dokumen||"").trim()})).filter(p=>p.meeting_date&&p.employee_name&&p.branch_id),c=await k("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:s=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:s?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:s?.branch_id&&!Te.find(n=>n.value==s.branch_id)?[...Te,{value:s.branch_id,label:s.branch_name||s.branch_id}]:Te,createApi:{path:"/api/branches",field:"full_name"},value:s?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:d(s?.employee_name),value:s?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:r(s?.pic),createApi:{path:"/api/pic",field:"name"},value:s?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:s?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:s?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:s?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:s?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:s?.document_link}]})}N();async function Ft(t){let e=await q(),i=await ae(),a=["Ade","Berlin"],o=s=>s&&!i.find(n=>n.value===s)?[...i,{value:s,label:s}]:i,d=s=>s&&!a.find(n=>(typeof n=="object"?n.value:n)===s)?[...a,s]:a,r=Array.from({length:5},(s,n)=>String(new Date().getFullYear()-n));B({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:s=>window.formatDate(s)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:s=>{try{let n=JSON.parse(s);return Array.isArray(n)?n.join(", "):s||"-"}catch{return s||"-"}}},{key:"score",label:"Nilai",render:s=>s!=null?`<strong>${s}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:s=>s?`<a href="${s}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:r}],exportOptions:{moduleName:"training",onExport:async s=>{let n=new URLSearchParams(s||{}).toString(),l=await k(`/api/training?limit=10000&${n}`);if(l.ok){let m=l.data.data.map(p=>{let u=p.participants||"";try{let h=JSON.parse(u);u=Array.isArray(h)?h.join(", "):u}catch{}return{Tanggal:p.training_date||"",Batch:p.batch||"",Materi:p.subject||"",Cabang:p.branch_name||"",Trainer:p.trainer||"",Peserta:u,Nilai:p.score!==null&&p.score!==void 0?p.score:"",Dokumen:p.document_link||""}}),{downloadExcel:c}=await Promise.resolve().then(()=>(H(),se));c(m,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let s=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(H(),se));n(s,"Template_Import_Training")},onImport:async s=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=e.find(g=>String(g.label||"").toLowerCase()===u);return h?h.value:null},l=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let b=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let h=u.split(/[\/\-\.]/);if(h.length===3){let[g,b,y]=h.map(S=>S.trim());if(g.length===4&&b.length<=2&&y.length<=2)return`${g}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&g.length<=2)return`${y}-${b.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=s.map(p=>({training_date:l(p.Tanggal),batch:String(p.Batch||"").trim(),subject:String(p.Materi||"").trim(),branch_id:n(String(p.Cabang||"").trim()),trainer:String(p.Trainer||"").trim(),participants:String(p.Peserta||"").trim(),score:p.Nilai?Number(p.Nilai):null,document_link:String(p.Dokumen||"").trim()})).filter(p=>p.training_date&&p.subject&&p.branch_id),c=await k("/api/import/training",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:s=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:s?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:s?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:s?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:s?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:d(s?.trainer),value:s?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let n=JSON.parse(s?.participants);return Array.isArray(n)?n.join(", "):s?.participants||""}catch{return s?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:s?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:s?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:s?.notes}],onBeforeSubmit:async s=>(s.participants&&(s.participants=JSON.stringify(s.participants.split(",").map(n=>n.trim()).filter(Boolean))),s)})}N();ke();H();function Ot({container:t,title:e,icon:i,apiPath:a,columns:o,formFields:d,filterFields:r,defaultFilters:s={},itemLabel:n="Data",canCreate:l=!0,canEdit:m=!0,canDelete:c=!0,onBeforeSubmit:p,onAfterLoad:u,onDataLoaded:h,extraActions:g=[],initialSearch:b="",exportOptions:y=null,bulkDelete:S=!1,paginationMode:w="server"}){let x=1,_={...s};b&&(_.search=b);let C=new Set;t.innerHTML=`
    <div class="page-header">
      <h1 class="page-title">${i} ${e}</h1>
      <div class="page-actions">
        ${l?`<button class="btn btn-primary" id="btn-create">+ Tambah ${n}</button>`:""}
  ...
  `,document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),o()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),o()}),document.getElementById("cal-event-close").addEventListener("click",()=>{document.getElementById("cal-event-list").style.display="none"}),document.querySelectorAll(".cal-filter").forEach(d=>d.addEventListener("change",o));async function a(){try{let d=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`;i=(await k(`/api/dashboard/calendar?month=${d}`)).data?.data||[]}catch(d){console.warn("[Calendar] Failed to load events, rendering empty grid:",d),i=[]}}async function o(){let d=document.getElementById("calendar-grid");if(d){d.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;background:var(--border);">
    ...
    </div>`,await a();try{let r=e.getFullYear(),s=e.getMonth(),n=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}),l=document.getElementById("cal-month-label");l&&(l.textContent=n);let m=new Set(Array.from(document.querySelectorAll(".cal-filter:checked")).map(x=>x.value)),c=i.filter(x=>m.has(x.type)),p={};c.forEach(x=>{let _=(x.event_date||"").slice(0,10);p[_]||(p[_]=[]),p[_].push(x)});let u=new Date(r,s,1).getDay(),h=new Date(r,s+1,0).getDate(),g=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],b=new Date().toISOString().slice(0,10),y='<div class="calendar-grid">';g.forEach(x=>{y+=`<div class="cal-day-header">${x}</div>`});for(let x=0;x<u;x++)y+='<div class="cal-cell cal-cell-empty"></div>';for(let x=1;x<=h;x++){let _=`${r}-${String(s+1).padStart(2,"0")}-${String(x).padStart(2,"0")}`,C=p[_]||[],O=_===b;y+=`
          <div class="cal-cell ${O?"cal-today":""} ${C.length?"cal-has-events":""}"
               data-date="${_}" tabindex="0" role="button" aria-label="${_}">
            <div class="cal-day-num ${O?"today-num":""}">${x}</div>
            <div class="cal-events-preview">
              ${C.slice(0,3).map(T=>`
                <div class="cal-event-dot cal-color-${T.color||"gray"}" title="${Ye(T.title||T.type)}">
                  <span class="cal-event-dot-label">${Aa(T.title||T.branch_name||T.type,18)}</span>
                </div>
              `).join("")}
              ${C.length>3?`<div class="cal-more">+${C.length-3} lagi</div>`:""}
            </div>
          </div>`}let w=(u+h)%7;if(w!==0)for(let x=0;x<7-w;x++)y+='<div class="cal-cell cal-cell-empty"></div>';y+="</div>",d.innerHTML=y,d.querySelectorAll(".cal-cell[data-date]").forEach(x=>{x.addEventListener("click",()=>{let _=x.dataset.date,C=p[_]||[];if(!C.length)return;let O=document.getElementById("cal-event-list"),T=new Date(_+"T00:00:00").toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});document.getElementById("cal-event-date").textContent=T,document.getElementById("cal-event-items").innerHTML=C.map(F=>`
            <div class="cal-event-item cal-color-border-${F.color||"gray"}">
              <div class="cal-event-type">${Ma(F.type)}</div>
              <div class="cal-event-title">${Ye(F.title||"-")}</div>
              <div class="cal-event-branch">${Ye(F.branch_name||"")}</div>
              ${F.status?`<div class="cal-event-status">${Ye(F.status)}</div>`:""}
              ${F.days_remaining!==void 0?`<div class="cal-event-extra">Sisa: ${F.days_remaining} hari</div>`:""}
            </div>
          `).join(""),O.style.display="block"})})}catch(r){console.error("[Calendar] Render error:",r),d&&(d.innerHTML=`
          <div style="padding:40px;text-align:center;color:var(--text-3)">
            <div style="font-size:2rem;margin-bottom:8px">\u{1F4C5}</div>
            <div>Gagal memuat kalender. Silakan refresh.</div>
          </div>`)}}}o()}function Aa(t,e){return t?t.length>e?t.slice(0,e)+"\u2026":t:""}function Ye(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function Ma(t){return{schedule:"\u{1F5D3} Jadwal",issue:"\u26A0\uFE0F Permasalahan",reliever:"\u{1F504} Reliefer",training:"\u{1F393} Training",contract_expiry:"\u{1F4CB} Kontrak Habis"}[t]||t}N();async function Vt(t){let e=be(),i=(e?.full_name||e?.username||"U")[0].toUpperCase(),o={superadmin:"#7C3AED",admin:"#2563EB",manager:"#0891B2",spv:"#059669",viewer:"#64748B"}[e?.role]||"#64748B";t.innerHTML=`
    <div class="page-header">
  ...
  `;let d=localStorage.getItem("fm_token"),r=document.getElementById("session-info");if(d&&r)try{let s=JSON.parse(atob(d.split(".")[1])),n=new Date(s.exp*1e3);r.textContent=`Berakhir: ${n.toLocaleString("id-ID")}`}catch{r.textContent="Tidak tersedia"}document.getElementById("change-pwd-form")?.addEventListener("submit",async s=>{s.preventDefault();let n=document.getElementById("pwd-error"),l=document.getElementById("pwd-success"),m=document.getElementById("btn-save-pwd");n.style.display="none",l.style.display="none";let c=s.target,p=c.current_password.value,u=c.new_password.value,h=c.confirm_password.value;if(u!==h){n.textContent="\u274C Konfirmasi password tidak cocok.",n.style.display="block";return}if(u.length<6){n.textContent="\u274C Password baru minimal 6 karakter.",n.style.display="block";return}m.disabled=!0,m.textContent="\u23F3 Menyimpan...";let g=await k("/api/auth/change-password",{method:"POST",body:JSON.stringify({current_password:p,new_password:u})});m.disabled=!1,m.innerHTML='<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="margin-right:5px"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Simpan Password',g.ok?(l.textContent="\u2705 Password berhasil diubah.",l.style.display="block",c.reset(),te("Password berhasil diubah.")):(n.textContent=g.data?.error||"Gagal mengubah password.",n.style.display="block")}),document.getElementById("btn-logout")?.addEventListener("click",()=>{confirm("Keluar dari semua sesi? Anda harus login ulang.")&&(localStorage.clear(),window.location.reload())})}N();var Xe={Validasi:{module:"validation",label:"Master Referensi"},SOP:{module:"sop",label:"SOP"},"Master Karyawan":{module:"employees",label:"Karyawan"},"Data Kontrak":{module:"contracts",label:"Kontrak"},Permasalahan:{module:"issues",label:"Permasalahan"},"One on One":{module:"one_on_one",label:"One on One"},"Time Line":{module:"schedule",label:"Jadwal Kegiatan"},"Report Inspeksi Hygiene 2026":{module:"inspection",label:"Laporan Inspeksi"},"Report GC-DC 2026":{module:"cleaning",label:"Laporan GC/DC"},"Report Fogging 2026":{module:"fogging",label:"Laporan Fogging"},"Rekap Laporan Basecamp":{module:"basecamp",label:"Rekap Basecamp"},"Jadwal Reliefer":{module:"relievers",label:"Reliefer"},Training:{module:"training",label:"Training"},"Master Checklist":{module:"checklist",label:"Checklist"},"Master Form":{module:"forms",label:"Master Form"},"Permintaan Chemical":{module:"supply",label:"Inventory Chemical"}};function ne(t){if(t==null||t==="")return null;if(t instanceof Date)return isNaN(t.getTime())?null:t.toISOString().slice(0,10);let e=String(t).trim();if(e===""||e==="0")return null;if(/^\d{4}-\d{2}-\d{2}/.test(e))return e.slice(0,10);if(/^\d{4,5}$/.test(e)){let o=Number(e);if(o>2e4&&o<99999){let d=new Date(Date.UTC(1899,11,30)+o*864e5);return isNaN(d.getTime())?null:d.toISOString().slice(0,10)}}let i=e.split(/[\/\-\.]/);if(i.length===3){let[o,d,r]=i.map(m=>m.trim()),s=Number(o),n=Number(d),l=Number(r);if(o.length===4&&s>1900)return`${o}-${d.padStart(2,"0")}-${r.padStart(2,"0")}`;if(r.length===4&&l>1900)return s>12?`${r}-${d.padStart(2,"0")}-${o.padStart(2,"0")}`:n>12?`${r}-${o.padStart(2,"0")}-${d.padStart(2,"0")}`:`${r}-${d.padStart(2,"0")}-${o.padStart(2,"0")}`;if(r.length===2&&!isNaN(l)){let m=l>=50?`19${r}`:`20${r}`;return s>12?`${m}-${d.padStart(2,"0")}-${o.padStart(2,"0")}`:`${m}-${d.padStart(2,"0")}-${o.padStart(2,"0")}`}}let a=new Date(e);return isNaN(a.getTime())?null:a.toISOString().slice(0,10)}function Wt(t){return Object.values(t).every(e=>e==null||String(e).trim()==="")}var Fa={validation:{required:[],map:t=>({cabang:t.CABANG,pic:t.PIC,kegiatan:t.KEGIATAN,quartal:t.QUARTAL,masa_pkwt:t["MASA PKWT"],pic_pelapor:t["PIC PELAPOR"],kontrak:t.KONTRAK})},sop:{required:[{key:"Nama SOP",label:"Nama SOP"}],map:t=>({name:t["Nama SOP"],category:t.Kategori||"Umum",document_link:t["Link Document"],version:"1.0",effective_date:null,notes:""})},employees:{required:[{key:"Nama Lengkap",label:"Nama Lengkap"}],map:t=>({full_name:t["Nama Lengkap"],branch_name:t.Cabang,division:t["Div / Bagian"]||"FACILITY CARE",phone:t["No. Hp"],join_date:ne(t["Tanggal Masuk"]),status:t.Status||"",notes:""})},contracts:{required:[{key:"Nama Lengkap",label:"Nama Lengkap"}],map:t=>({employee_name:t["Nama Lengkap"],branch_name:t.Cabang,division:t["Div / Bagian"]||"FACILITY CARE",start_date:ne(t["Tanggal Mulai"]),end_date:ne(t["Tanggal Selesai"]),contract_type:t["Tipe Kontrak"]||"",pkwt_number:t.PKWT||"",status:t.Status||"",notes:t.keterangan})},issues:{required:[{key:"Keluhan",label:"Keluhan"}],map:t=>({report_date:ne(t["Tanggal Info"]),branch_name:t.Cabang,category:t.Kategori,source:t["Sumber Laporan"],complaint:t.Keluhan,employee_name:t["Nama FC"],fc_specialist:t["FC Spesialis"],solution:t.Solusi,status:t.Status||"",completion_date:ne(t["Tanggal Selesai"])})},one_on_one:{required:[],map:t=>({meeting_date:ne(t.Tanggal),branch_name:t.Cabang,employee_name:t["Nama Karyawan"],pic:t.Pic,problem:t.Masalah,solution:t.Solusi,status:t.Status||"",completion_date:ne(t["Tanggal Selesai"]),document_link:t["Link Document"]})},schedule:{required:[{key:"Kegiatan",label:"Kegiatan"}],map:t=>({branch_name:t.Cabang,activity_type:t.Kegiatan,period:t.Periode,pic:t.Pic||t.PIC,opening_date:ne(t["Tanggal Opening"]||t["Tgl Opening"]),target_date:ne(t["Tanggal Target"]||t["Tgl Target"]),completion_date:ne(t["Tanggal Selesai"]||t["Tgl Selesai"]),status:t.Status||"",notes:t.Keterangan||t.Catatan})},inspection:{required:[],map:t=>({inspection_date:ne(t.Tanggal),branch_name:t.Cabang,period:t.Periode,status:t.Status||"",fc_score:t["Point FC SP"]!==void 0&&t["Point FC SP"]!==null?parseFloat(String(t["Point FC SP"]).replace(",",".")):null,spv_score:t["Point SPV"]!==void 0&&t["Point SPV"]!==null?parseFloat(String(t["Point SPV"]).replace(",",".")):null,document_link:t.Link,notes:""})},cleaning:{required:[],map:t=>({activity_date:ne(t.Tanggal),branch_name:t.Cabang,activity_type:t["Jenis Kegiatan"]||"General Cleaning",period:t.Periode,status:t.Status||"",document_link:t.Link,notes:""})},fogging:{required:[],map:t=>({activity_date:ne(t.Tanggal),branch_name:t.Cabang,period:t.Periode,status:t.Status||"",document_link:t.Link,notes:""})},basecamp:{required:[{key:"Permasalahan",label:"Permasalahan"}],map:t=>({info_date:ne(t["Tgl Info"]),branch_name:t.Cabang,problem:t.Permasalahan,pic:t.PIC,done_date:ne(t["Tgl Done"]),status:t.Status||"",notes:t.Ket})},relievers:{required:[],map:t=>({branch_name:t.Cabang,original_fc_name:t["Nama Facility care"],period:t.Periode,reliever_name:t.Relifer,backup_date:ne(t["Tanggal Back Up"]),completion_date:ne(t["Tanggal Selesai"]),reason:t.Keterangan,shift:t.Shift,status:t.Status||""})},training:{required:[{key:"Materi",label:"Materi"}],map:t=>({training_date:ne(t.Tanggal),batch:t.Batch,subject:t.Materi,participants:t.Peserta,branch_name:t.Cabang,trainer:t.Trainer,score:t.Nilai!==void 0&&t.Nilai!==null?parseFloat(String(t.Nilai).replace(",",".")):null,notes:""})},checklist:{required:[],map:t=>({name:t["Master Checklist"],category:"Umum",document_link:t["Link Document"],description:""})},forms:{required:[{key:"Master Form",label:"Master Form"}],map:t=>({name:t["Master Form"],category:"Umum",document_link:t["Link Document"],description:""})},supply:{required:[],map:t=>({submitted_at:ne(t.Timestamp),submitter_name:t["Nama Lengkap"],branch_name:t["Kebutuhan Untuk Cabang"],tools_items:t["Alat - Alat / Barang"],tools_quantity:t["Jumlah Permintaan Alat / Barang"],chemical_items:t.Chemical,chemical_quantity:t["Jumlah Permintaan Chemical"],additional_notes:t["Tambahan  Alat / Chemical Jika Ada Permintaan Diluar List."],status:t.Status||""})}};function Oa(t,e){let i=Xe[t];if(!i)return{valid:[],errors:[],mapped:[],skipped:!0};let a=Fa[i.module];if(!a)return{valid:[],errors:[],mapped:[],skipped:!0};let o=[],d=[],r=[];return e.filter(n=>!Wt(n)).forEach((n,l)=>{let m=e.indexOf(n)+2,c=[];a.required.forEach(({key:u,label:h})=>{let g=n[u];if(g==null||String(g).trim()===""){let b=Object.keys(n).filter(y=>y.trim()).join(", ");c.push({column:h,originalValue:g||"",reason:`Kolom "${h}" wajib diisi dan tidak ditemukan`,hint:`Kolom yang tersedia: ${b.slice(0,120)}`})}});let p=a.map(n);c.length>0?d.push({row:m,data:p,raw:n,errors:c}):(o.push(n),r.push(p))}),{valid:o,errors:d,mapped:r}}function Yt(t){let e=[];return t.SheetNames.forEach(i=>{let a=Xe[i];if(!a)return;let o=t.Sheets[i],d=window.XLSX.utils.sheet_to_json(o,{defval:"",raw:!1,dateNF:"yyyy-mm-dd"}),r=Oa(i,d),s=d.filter(n=>!Wt(n));e.push({sheetName:i,module:a.module,label:a.label,total:s.length,valid:r.mapped.length,errorCount:r.errors.length,errors:r.errors,mapped:r.mapped,skipped:!1})}),e}function Xt(){let t=window.XLSX,e=t.utils.book_new();Object.entries({Validasi:[{CABANG:"001. Pondok Bambu","NAMA KARYAWAN":"Budi Santoso",PIC:"Berlin",KEGIATAN:"General Cleaning",QUARTAL:"Q1","PIC PELAPOR":"Berlin",KONTRAK:"PKWT 1","MASA PKWT":"1 Tahun"}],SOP:[{"Nama SOP":"SOP Pembersihan Toilet",Kategori:"Cleaning","Link Document":"https://..."}],"Master Karyawan":[{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu","Div / Bagian":"FACILITY CARE","No. Hp":"081234567890","Tanggal Masuk":"2024-01-15",Status:"Aktif"}],"Data Kontrak":[{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu","Div / Bagian":"FACILITY CARE","Tanggal Mulai":"2024-01-01","Tanggal Selesai":"2024-12-31","Sisa Kontrak":"365",Status:"Aktif",keterangan:""}],Permasalahan:[{"Tanggal Info":"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning","Sumber Laporan":"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi","FC Spesialis":"Fajar",Solusi:"Teguran",Status:"Done","Tanggal Selesai":"2024-03-02",Day:"1"}],"One on One":[{Tanggal:"2024-03-05",Cabang:"001. Pondok Bambu","Nama Karyawan":"Budi Santoso",Pic:"Berlin",Masalah:"Keterlambatan",Solusi:"Coaching",Status:"Done","Tanggal Selesai":"2024-03-06",Day:"1","Link Document":""}],"Jadwal Reliefer":[{Cabang:"001. Pondok Bambu","Nama Facility care":"Budi Santoso",Periode:"Januari",Relifer:"Agung Septiadi","Tanggal Back Up":"2024-03-01","Tanggal Selesai":"2024-03-02",Keterangan:"Cuti",Shift:"Pagi",Status:"Done"}]}).forEach(([a,o])=>{t.utils.book_append_sheet(e,t.utils.json_to_sheet(o),a)}),t.writeFile(e,"Template_Import_Data_Awal_FCMS.xlsx")}function Zt(t){let e=window.XLSX,i=e.utils.book_new(),a=!1;return t.forEach(o=>{if(!o.errors||o.errors.length===0)return;a=!0;let d=o.errors.map(s=>({"No. Baris":s.row,"Kolom Gagal":(s.errors||[]).map(n=>n.column||n).join("; "),"Alasan Error":(s.errors||[]).map(n=>n.reason||n).join("; "),...Object.fromEntries(Object.entries(s.data||{}).map(([n,l])=>[n,l??""]))})),r=e.utils.json_to_sheet(d);e.utils.book_append_sheet(i,r,o.sheetName.replace(/[\\\/\[\]*?:]/g,"_").slice(0,31))}),a?(e.writeFile(i,`Log_Error_Import_${new Date().toISOString().slice(0,10)}.xlsx`),!0):!1}var Ra=["validation","employees","contracts","relievers","schedule","issues","one_on_one","training","checklist","forms","sop","inspection","cleaning","fogging","basecamp","supply"];function ea(t){t.innerHTML=`
    <div class="page-header">
        ...
        <div class="card-body">
          <div class="import-info-box">
            ...
            <div class="import-sheet-list">
              ${Object.entries(Xe).map(([g,{label:b}])=>`<span class="import-sheet-tag">\u{1F4C4} ${g} \u2192 ${b}</span>`).join("")}
            </div>
          </div>
  ...
  `;let e=null,i=null,a=0,o={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function d(g){Object.entries(o).forEach(([b,y])=>{y.style.display=b===g?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let g=document.getElementById("btn-backup-db");g.disabled=!0,g.textContent="\u23F3 Memproses Backup...";try{let b=await k("/api/import/backup");if(b.ok){let y=new Blob([JSON.stringify(b.data,null,2)],{type:"application/json"}),S=URL.createObjectURL(y),w=document.createElement("a");w.href=S,w.download=`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(w),w.click(),document.body.removeChild(w),URL.revokeObjectURL(S),te("Backup berhasil diunduh!")}else V("Gagal memproses backup: "+(b.data?.error||"Unknown error"))}catch(b){V("Gagal memproses backup: "+b.message)}finally{g.disabled=!1,g.textContent="\u{1F4E6} Backup Database"}});let r=document.getElementById("btn-sync-google");r&&r.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let g=r.innerHTML;r.innerHTML='<span class="spinner"></span> Menyinkronkan...',r.disabled=!0;try{let b=await k("/api/sync/google-sheets",{method:"POST"});b.ok?alert("Sinkronisasi Berhasil: "+(b.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(b.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{r.innerHTML=g,r.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{Xt(),te("Template Excel berhasil didownload!")});let s=document.getElementById("file-input"),n=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",g=>{g.stopPropagation(),s.click()}),s.addEventListener("change",g=>{g.target.files[0]&&l(g.target.files[0])}),n.addEventListener("dragover",g=>{g.preventDefault(),n.classList.add("drag-over")}),n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n.addEventListener("drop",g=>{g.preventDefault(),n.classList.remove("drag-over");let b=g.dataTransfer.files[0];b&&b.name.match(/\.xlsx?$/i)?l(b):V("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,s.value="",document.getElementById("file-info").style.display="none",n.style.display="",d("upload")});async function l(g){e=g,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${g.name} (${(g.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",n.style.display="none",await m(g)}async function m(g){d("validating");let b=document.getElementById("validation-status"),y=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");b.textContent="Membaca file Excel...",y.style.width="20%",await Ke(200);let S=await g.arrayBuffer(),w=window.XLSX.read(S,{type:"array",cellDates:!0});b.textContent=`Memvalidasi ${w.SheetNames.length} sheet...`,y.style.width="50%",await Ke(100),i=Yt(w),y.style.width="100%",b.textContent="Validasi selesai!",await Ke(300),c()}catch(S){d("upload"),V("Gagal memproses file: "+S.message),document.getElementById("file-info").style.display="flex",n.style.display="none"}}function c(){d("preview");let g=i.filter(T=>!T.skipped).length,b=i.reduce((T,F)=>T+F.total,0),y=i.reduce((T,F)=>T+F.valid,0),S=i.reduce((T,F)=>T+F.errorCount,0),w=b>0?Math.round(y/b*100):0;document.getElementById("preview-summary-badges").innerHTML=`
      <span class="badge badge-info">${g} sheet</span>
      <span class="badge badge-secondary">${b} baris</span>
      <span class="badge badge-success">${y} valid (${w}%)</span>
      ${S>0?`<span class="badge badge-danger">${S} error</span>`:""}
    `;let x=document.getElementById("preview-table-container");x.innerHTML=`
        ...
        <tbody>
          ${i.map((T,F)=>`
            <tr class="${T.errorCount>0?"row-error":T.skipped?"row-skipped":"row-ok"}">
              ...
              <td style="text-align:center">
                ${T.errorCount>0?`<button class="btn btn-ghost btn-sm btn-detail-error" data-idx="${F}">\u{1F50D} ${T.errorCount} Error</button>`:'<span class="text-muted">\u2013</span>'}
              </td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    `,x.querySelectorAll(".btn-detail-error").forEach(T=>{T.addEventListener("click",()=>{let F=i[Number(T.dataset.idx)];p(F)})});let _=document.getElementById("error-detail-section"),C=document.getElementById("error-detail-container");C.innerHTML="",_.style.display="none";let O=document.getElementById("btn-start-import");y===0?(O.disabled=!0,O.innerHTML="\u26A0\uFE0F Tidak Ada Data Valid"):(O.disabled=!1,S>0?(O.innerHTML=`\u{1F680} Import ${y} Data Valid (${S} dilewati)`,O.title="Baris error akan dilewati, baris valid tetap diimport"):O.innerHTML=`\u{1F680} Mulai Import ${y} Data`)}function p(g){let b=document.getElementById("error-detail-section"),y=document.getElementById("error-detail-container");b.style.display="";let S=g.errors.slice(0,100).map(w=>(Array.isArray(w.errors)?w.errors:[]).map(_=>{let C=typeof _=="object";return`
          <tr>
            <td style="text-align:center"><span class="badge badge-danger">Baris ${w.row}</span></td>
            <td><strong>${C?_.column:"\u2014"}</strong></td>
            <td><code style="font-size:.78rem;color:var(--text-secondary)">${C&&_.originalValue!==void 0?_.originalValue||"(kosong)":"\u2014"}</code></td>
            <td class="error-msg">${C?_.reason:_}</td>
            <td style="font-size:.78rem;color:var(--success)">
              ${C&&_.aliases?`Gunakan salah satu nama kolom:<br><em>${_.aliases}</em>`:C&&_.hint?_.hint:""}
            </td>
          </tr>
    ...
    `,b.scrollIntoView({behavior:"smooth",block:"start"})}document.getElementById("btn-back-to-upload").addEventListener("click",()=>{d("upload"),document.getElementById("file-info").style.display="none",n.style.display="",e=null,s.value=""}),document.getElementById("btn-download-log").addEventListener("click",()=>{if(!i)return;Zt(i)?te("Log error berhasil didownload."):te("Tidak ada error untuk didownload.")}),document.getElementById("btn-start-import").addEventListener("click",()=>{let g=document.querySelector('input[name="dup-strategy"]:checked')?.value||"skip";u(g)});async function u(g){d("importing"),a=Date.now();let b=[];Ra.forEach(_=>{let C=i?.find(O=>O.module===_&&O.mapped?.length>0);C&&b.push(C)});let y=document.getElementById("import-steps-list");y.innerHTML=b.map(_=>`
    ...
    `).join("");let S=document.getElementById("import-bar"),w=document.getElementById("import-current-status"),x={totalSheets:b.length,totalRows:b.reduce((_,C)=>_+C.mapped.length,0),inserted:0,skipped:0,failed:0,moduleResults:[]};for(let _=0;_<b.length;_++){let C=b[_],O=document.getElementById(`step-icon-${C.module}`),T=document.getElementById(`step-status-${C.module}`);O.textContent="\u{1F504}",T.textContent="Mengimport...",w.textContent=`Mengimport ${C.label}...`,S.style.width=`${Math.round(_/b.length*100)}%`;try{let F=await k(`/api/import/${C.module}`,{method:"POST",body:JSON.stringify({rows:C.mapped,onDuplicate:g})});if(F.ok){let A=F.data;x.inserted+=A.inserted||0,x.skipped+=A.skipped||0,x.moduleResults.push({label:C.label,inserted:A.inserted||0,skipped:A.skipped||0,status:"ok"}),O.textContent="\u2705",T.innerHTML=`<span class="badge badge-success">${A.inserted||0} berhasil</span>${A.skipped>0?` <span class="badge badge-neutral">${A.skipped} skip</span>`:""}`}else x.failed++,x.moduleResults.push({label:C.label,inserted:0,skipped:0,status:"error",error:F.data?.error}),O.textContent="\u274C",T.innerHTML='<span class="badge badge-danger">Gagal</span>'}catch(F){x.failed++,x.moduleResults.push({label:C.label,inserted:0,skipped:0,status:"error",error:F.message}),O.textContent="\u274C",T.innerHTML='<span class="badge badge-danger">Gagal</span>'}await Ke(150)}S.style.width="100%",w.textContent="Selesai!",await Ke(400),h(x)}function h(g){d("summary");let b=((Date.now()-a)/1e3).toFixed(1),y=g.failed===0;document.getElementById("summary-status-icon").innerHTML=`
    ...
    `}document.getElementById("btn-import-again").addEventListener("click",()=>{e=null,i=null,s.value="",document.getElementById("file-info").style.display="none",n.style.display="",d("upload")}),document.getElementById("btn-go-to-dashboard").addEventListener("click",()=>{window.location.hash="/dashboard"})}function Ke(t){return new Promise(e=>setTimeout(e,t))}N();var Ze=[],ta=[];async function aa(t){Ze=await q(),ta=await ae(),B({container:t,title:"Data SP (Surat Peringatan)",icon:"\u2709\uFE0F",apiPath:"/api/sp",itemLabel:"SP",bulkDelete:!0,columns:[{key:"employee_name",label:"Nama Karyawan"},{key:"division",label:"Divisi",render:e=>e?`<span class="badge badge-info">${e}</span>`:"-"},{key:"branch_name",label:"Cabang"},{key:"tanggal",label:"Tanggal Sp",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"akhir_sp",label:"Akhir Sp",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"sp_type",label:"Jenis Sp",render:e=>`<span class="badge badge-warning">${e||"-"}</span>`},{key:"document_link",label:"Link Document / Foto",render:e=>e?`<a href="${e}" target="_blank" class="text-primary hover-underline">Lihat</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:Ze}],exportOptions:{moduleName:"sp_data",onExport:async e=>{let i=new URLSearchParams(e||{}).toString(),a=await k(`/api/sp?limit=10000&${i}`);if(a.ok){let o=a.data.data.map(r=>({"Nama Karyawan":r.employee_name||"",Divisi:r.division||"",Cabang:r.branch_name||"","Tanggal Sp":r.tanggal||"","Akhir Sp":r.akhir_sp||"","Jenis Sp":r.sp_type||"","Link Document / Foto":r.document_link||""})),{downloadExcel:d}=await Promise.resolve().then(()=>(H(),se));d(o,`Data_SP_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let e=[{"Nama Karyawan":"Budi Santoso",Divisi:"FACILITY CARE",Cabang:"001. Pondok Bambu","Tanggal Sp":"2026-01-08","Akhir Sp":"2026-07-08","Jenis Sp":"SP 1","Link Document / Foto":"https://link.doc"}],{downloadExcel:i}=await Promise.resolve().then(()=>(H(),se));i(e,"Template_Import_SP")},onImport:async e=>{let i=r=>{if(!r)return null;let s=String(r||"").toLowerCase(),n=Ze.find(l=>String(l.label||"").toLowerCase()===s);return n?n.value:null},a=r=>{if(!r)return"";if(r instanceof Date&&!isNaN(r.getTime()))return r.toISOString().slice(0,10);let s=String(r).trim();if(/^\d{4,5}$/.test(s)){let l=Number(s);if(l>2e4&&l<99999){let m=new Date(Date.UTC(1899,11,30)+l*864e5);return isNaN(m.getTime())?"":m.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(s))return s.slice(0,10);let n=s.split(/[\/\-\.]/);if(n.length===3){let[l,m,c]=n.map(p=>p.trim());if(l.length===4&&m.length<=2&&c.length<=2)return`${l}-${m.padStart(2,"0")}-${c.padStart(2,"0")}`;if(c.length===4&&m.length<=2&&l.length<=2)return`${c}-${m.padStart(2,"0")}-${l.padStart(2,"0")}`}return s},o=e.map(r=>({employee_name:String(r["Nama Karyawan"]||"").trim(),division:String(r.Divisi||"").trim(),branch_id:i(String(r.Cabang||"").trim()),tanggal:a(r["Tanggal Sp"]),akhir_sp:a(r["Akhir Sp"]),sp_type:String(r["Jenis Sp"]||"").trim(),document_link:String(r["Link Document / Foto"]||"").trim()})).filter(r=>r.employee_name&&r.branch_id),d=await k("/api/import/sp",{method:"POST",body:JSON.stringify({rows:o,onDuplicate:"update"})});if(!d.ok)throw new Error(d.data?.error||"Import gagal");return d.data}},formFields:[{type:"combobox",name:"employee_name",label:"Nama Karyawan",required:!0,options:ta},{type:"select",name:"division",label:"Divisi",options:["FACILITY CARE","SECURITY"],required:!0},{type:"combobox",name:"branch_id",label:"Cabang",required:!0,options:Ze,createApi:{path:"/api/branches",field:"full_name"}},{type:"date",name:"tanggal",label:"Tanggal Sp",required:!0},{type:"date",name:"akhir_sp",label:"Akhir Sp",required:!0},{type:"select",name:"sp_type",label:"Jenis Sp",required:!0,options:["SP 1","SP 2","SP 3","Teguran Lisan"]},{type:"url",name:"document_link",label:"Link Document / Foto"}]})}N();var Ee=[],na=[];async function ia(t){Ee=await q(),na=await ae(),B({container:t,title:"Data Mutasi",icon:"\u{1F504}",apiPath:"/api/mutasi",itemLabel:"Mutasi",bulkDelete:!0,columns:[{key:"tanggal",label:"Tanggal",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"employee_name",label:"Nama Karyawan"},{key:"from_branch_name",label:"Cabang Asal"},{key:"to_branch_name",label:"Cabang Tujuan"},{key:"status",label:"Status",render:e=>`<span class="badge ${e==="Selesai"?"badge-success":"badge-warning"}">${e||"-"}</span>`},{key:"document_link",label:"Dokumen",render:e=>e?`<a href="${e}" target="_blank" class="text-primary hover-underline">Lihat</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"from_branch_id",label:"Cabang Asal",options:Ee},{type:"combobox",name:"to_branch_id",label:"Cabang Tujuan",options:Ee}],exportOptions:{moduleName:"mutasi_data",onExport:async e=>{let i=new URLSearchParams(e||{}).toString(),a=await k(`/api/mutasi?limit=10000&${i}`);if(a.ok){let o=a.data.data.map(r=>({Tanggal:r.tanggal||"","Nama Karyawan":r.employee_name||"","Cabang Asal":r.from_branch_name||"","Cabang Tujuan":r.to_branch_name||"",Status:r.status||"",Dokumen:r.document_link||""})),{downloadExcel:d}=await Promise.resolve().then(()=>(H(),se));d(o,`Data_Mutasi_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let e=[{Tanggal:"2026-01-08","Nama Karyawan":"Widya Astuti","Cabang Asal":"001. Pondok Bambu","Cabang Tujuan":"007. Bekasi",Status:"Selesai",Dokumen:"https://link.doc"}],{downloadExcel:i}=await Promise.resolve().then(()=>(H(),se));i(e,"Template_Import_Mutasi")},onImport:async e=>{let i=r=>{if(!r)return null;let s=String(r||"").toLowerCase(),n=Ee.find(l=>String(l.label||"").toLowerCase()===s);return n?n.value:null},a=r=>{if(!r)return"";if(r instanceof Date&&!isNaN(r.getTime()))return r.toISOString().slice(0,10);let s=String(r).trim();if(/^\d{4,5}$/.test(s)){let l=Number(s);if(l>2e4&&l<99999){let m=new Date(Date.UTC(1899,11,30)+l*864e5);return isNaN(m.getTime())?"":m.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(s))return s.slice(0,10);let n=s.split(/[\/\-\.]/);if(n.length===3){let[l,m,c]=n.map(p=>p.trim());if(l.length===4&&m.length<=2&&c.length<=2)return`${l}-${m.padStart(2,"0")}-${c.padStart(2,"0")}`;if(c.length===4&&m.length<=2&&l.length<=2)return`${c}-${m.padStart(2,"0")}-${l.padStart(2,"0")}`}return s},o=e.map(r=>({tanggal:a(r.Tanggal),employee_name:String(r["Nama Karyawan"]||"").trim(),from_branch_id:i(String(r["Cabang Asal"]||"").trim()),to_branch_id:i(String(r["Cabang Tujuan"]||"").trim()),status:String(r.Status||"").trim(),document_link:String(r.Dokumen||"").trim()})).filter(r=>r.tanggal&&r.employee_name&&r.from_branch_id&&r.to_branch_id),d=await k("/api/import/mutasi",{method:"POST",body:JSON.stringify({rows:o,onDuplicate:"update"})});if(!d.ok)throw new Error(d.data?.error||"Import gagal");return d.data}},formFields:[{type:"date",name:"tanggal",label:"Tanggal",required:!0},{type:"combobox",name:"employee_name",label:"Nama Karyawan",required:!0,options:na},{type:"combobox",name:"from_branch_id",label:"Cabang Asal",required:!0,options:Ee,createApi:{path:"/api/branches",field:"full_name"}},{type:"combobox",name:"to_branch_id",label:"Cabang Tujuan",required:!0,options:Ee,createApi:{path:"/api/branches",field:"full_name"}},{type:"select",name:"status",label:"Status",required:!0,options:["Proses","Selesai"]},{type:"url",name:"document_link",label:"Link Dokumen (Opsional)"}]})}N();async function ra(t){t.innerHTML=`
  ...
  `;let e=document.getElementById("btnRefreshSync"),i=document.getElementById("queueStatusFilter");e.addEventListener("click",o),i.addEventListener("change",s),document.getElementById("btnRetryAll").addEventListener("click",()=>a("retry",{allFailed:!0})),document.getElementById("btnResetStuck").addEventListener("click",()=>a("reset-stuck",{})),document.getElementById("btnPauseSync").addEventListener("click",()=>a("pause",{})),document.getElementById("btnResumeSync").addEventListener("click",()=>a("resume",{})),document.getElementById("btnReconcile").addEventListener("click",()=>a("reconcile",{module:"ALL",repairMode:!0})),document.getElementById("btnRetrySelected").addEventListener("click",()=>{let l=Array.from(document.querySelectorAll(".chk-queue:checked")).map(m=>m.value);if(l.length===0)return alert("No items selected");a("retry",{ids:l})}),document.getElementById("chkAllQueue").addEventListener("change",l=>{document.querySelectorAll(".chk-queue").forEach(m=>m.checked=l.target.checked)});async function a(l,m){if(confirm(`Are you sure you want to execute action: ${l}?`)){showLoading();try{let c=await k(`/api/sync/actions/${l}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)});c.ok?(alert(c.data?.message||"Success"),o()):V(c.error||"Action failed")}catch(c){V(c.message)}hideLoading()}}await o();async function o(){showLoading(),await Promise.all([r(),s(),d(),n()]),hideLoading()}async function d(){try{let l=await k("/api/sync/performance");if(!l.ok)return;let{webhook:m,google_api:c,d1:p,queue:u,throughput:h}=l.data;document.getElementById("latencyHistograms").innerHTML=`
      ...
      `,document.getElementById("lblSnapshotStatus").innerHTML='Checked 10 modules. Status: <b>COMPLETED</b><br><span class="text-xs text-gray-500">Run today at 02:00</span>'}catch(l){console.error(l)}}async function r(){try{let l=await k("/api/sync/health");if(!l.ok)return V("Failed to fetch sync health");let{status:m,queue:c,circuit_breaker:p}=l.data,u=`
      ...
      `).join("")}catch(l){console.error(l)}}async function n(){try{let l=await k("/api/sync/metrics");if(!l.ok)return;let m=document.getElementById("metricsTableBody"),c=l.data||[];if(c.length===0){m.innerHTML='<tr><td colspan="5" class="px-4 py-4 text-center text-gray-500">No metrics found in last 24h</td></tr>';return}m.innerHTML=c.map(p=>`
        <tr class="hover:bg-gray-50">
  ...
  `;le({title:`Notifikasi (${$e.length})`,content:t,confirmText:"Tutup",onConfirm:(e,i)=>i()})}function sa(){let t=be(),e=(t?.full_name||"U")[0].toUpperCase();document.getElementById("app").innerHTML=`
  ...
  `;let i=document.getElementById("sidebar"),a=document.getElementById("sidebar-overlay"),o=document.getElementById("topbar-menu-btn"),d=document.getElementById("sidebar-close"),r=()=>{i.classList.add("open"),a.classList.add("show")},s=()=>{i.classList.remove("open"),a.classList.remove("show")};o?.addEventListener("click",r),d?.addEventListener("click",s),a?.addEventListener("click",s),document.querySelectorAll(".nav-item").forEach(l=>l.addEventListener("click",s));function n(){let l=window.location.hash.replace("#","")||"/dashboard";document.querySelectorAll(".nav-item").forEach(p=>{let u=p.dataset.route;p.classList.toggle("active",l===u||u!=="/dashboard"&&l.startsWith(u))});let m=document.getElementById("topbar-title"),c=document.querySelector(".nav-item.active .nav-label");m&&c&&(m.textContent=c.textContent)}window.addEventListener("hashchange",n),n(),Ka(),document.getElementById("btn-fullscreen")?.addEventListener("click",()=>{document.fullscreenElement?document.exitFullscreen?.():document.documentElement.requestFullscreen?.()}),document.getElementById("logout-btn")?.addEventListener("click",async()=>{await k("/api/auth/logout",{method:"POST"}),Pe(),He&&clearInterval(He),ve("/login")}),Ha(),qa(),document.getElementById("btn-notif")?.addEventListener("click",l=>{l.preventDefault(),ja()})}async function Ua(){U("/login",({main:e})=>It(e)),U("/dashboard",Q(({main:e})=>Tt(e))),U("/calendar",Q(({main:e})=>zt(e))),U("/employees",Q(({main:e,params:i})=>Pt(e,i))),U("/contracts",Q(({main:e,params:i})=>Bt(e,i))),U("/sp",Q(({main:e})=>aa(e))),U("/mutasi",Q(({main:e})=>ia(e))),U("/sync-dashboard",Q(({main:e})=>ra(e))),U("/timeline",Q(({main:e,params:i})=>Nt(e,i))),U("/issues",Q(({main:e,params:i})=>At(e,i))),U("/one-on-one",Q(({main:e,params:i})=>Mt(e,i))),U("/training",Q(({main:e})=>Ft(e))),U("/relievers",Q(({main:e,params:i})=>Rt(e,i))),U("/reports/inspection",Q(({main:e})=>Kt(e))),U("/reports/cleaning",Q(({main:e})=>Ht(e))),U("/reports/fogging",Q(({main:e})=>qt(e))),U("/reports/basecamp",Q(({main:e})=>jt(e))),U("/reports/supply",Q(({main:e})=>yt(e,"supply"))),U("/sop",Q(({main:e})=>Ut(e))),U("/checklist",Q(({main:e})=>Jt(e))),U("/forms",Q(({main:e})=>yt(e))),U("/users",Q(({main:e})=>Gt(e))),U("/branches",Q(({main:e})=>Qt(e))),U("/profile",Q(({main:e})=>Vt(e))),U("/settings/import",Q(({main:e})=>ea(e)));let t=Ie();if(!t&&window.location.hash!=="#/login"&&ve("/login"),t){let e=await k("/api/auth/me");e.ok?(Le(e.data.data),sa()):(Pe(),ve("/login"))}window.addEventListener("fm:login",()=>{sa(),ve("/dashboard")}),vt()}Ua();
