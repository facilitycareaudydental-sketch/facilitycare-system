var fa=Object.defineProperty;var dt=(t,e)=>()=>(t&&(e=t(t=0)),e);var ct=(t,e)=>{for(var i in e)fa(t,i,{get:e[i],enumerable:!0})};var Le={};ct(Le,{API:()=>Et,CLIENT_SIDE_MAX_ROWS:()=>Se,IS_DEVELOPMENT:()=>$e,apiFetch:()=>k,clearToken:()=>We,getToken:()=>Qe,getUser:()=>oe,setToken:()=>pt,setUser:()=>Ve});function Qe(){return localStorage.getItem("fm_token")}function pt(t){localStorage.setItem("fm_token",t)}function We(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function oe(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Ve(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let i=Qe(),a={"Content-Type":"application/json",...i?{Authorization:`Bearer ${i}`}:{},...e.headers||{}};try{let o=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",r=`${Et}${t}${d}${o}`,n=await fetch(r,{...e,headers:a}),s;try{let l=await n.text();try{s=JSON.parse(l)}catch{s={error:`Server Error (${n.status}): ${l.substring(0,80)}...`}}}catch{s={error:"Gagal membaca respon dari server"}}return n.status===401&&(We(),window.location.hash="#/login"),{ok:n.ok,status:n.status,data:s}}catch(o){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${o.message})`}}}}var $e,va,Et,Se,j=dt(()=>{$e=!1,va="https://fm-operations-api.facilitycare-audydental.workers.dev",Et=va,Se=1e4});var mt={};ct(mt,{confirmDialog:()=>Ee,createModal:()=>de});function de({title:t,content:e,onConfirm:i,onCancel:a,confirmText:o="Simpan",cancelText:d="Batal",size:r="md",confirmClass:n="btn-primary"}){let s={sm:"400px",md:"560px",lg:"720px",xl:"900px"},l=document.createElement("div");l.className="modal-overlay",l.innerHTML=`
  ...
  `;function F(){let c=document.getElementById("bulk-toolbar");if(!c)return;let b=document.getElementById("bulk-count"),w=document.getElementById("btn-bulk-delete"),x=document.getElementById("btn-bulk-cancel");b.textContent=`${E.size} item dipilih`,E.size>0?(c.style.display="flex",c.classList.add("has-items"),w.disabled=!1,x.disabled=!1,window.innerWidth<=768?(c.style.setProperty("top","auto","important"),c.style.setProperty("bottom","0","important")):(c.style.removeProperty("top"),c.style.removeProperty("bottom"))):(c.style.display="none",c.classList.remove("has-items"),w.disabled=!0,x.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{E.clear(),document.querySelectorAll(".row-checkbox").forEach(b=>b.checked=!1);let c=document.getElementById("select-all-checkbox");c&&(c.checked=!1),F()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(E.size===0)return;let c=[...E],b=document.createElement("div");b.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",b.innerHTML=`
    ...
    `,document.body.appendChild(b),b.querySelector("#bulk-cancel-btn").addEventListener("click",()=>b.remove()),b.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let w=b.querySelector("#bulk-confirm-btn");w.disabled=!0,w.textContent="Menghapus...";let x=await k(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:c})});b.remove(),x.ok?(ne(`${c.length} ${l} berhasil dihapus.`),E.clear(),F(),q()):z(x.data?.error||"Gagal menghapus data.")})});let K=document.getElementById("filter-search"),ue;K?.addEventListener("input",c=>{clearTimeout(ue),ue=setTimeout(()=>{L.search=c.target.value,$=1,E.clear(),q()},400)}),r?.forEach(c=>{c.type==="select"&&document.getElementById(`filter-${c.name}`)?.addEventListener("change",b=>{L[c.name]=b.target.value,$=1,E.clear(),q()}),c.type==="combobox"&&document.getElementById(`filter-${c.name}`)?.addEventListener("change",b=>{let w=b.target.value,x=(c.options||[]).find(T=>{let _=String(typeof T=="object"?T.value:T),P=String(typeof T=="object"?T.label:T);return _===w||P===w});w?L[c.name]=x?typeof x=="object"?x.value:x:w:L[c.name]="",$=1,E.clear(),q()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{L={...n},K&&(K.value=""),r?.forEach(c=>{let b=document.getElementById(`filter-${c.name}`);b&&(b.value="")}),$=1,E.clear(),q()});let me=document.getElementById("btn-mobile-filter"),ie=document.getElementById("filter-options-wrapper"),ge=document.getElementById("btn-close-filter-sheet");if(me&&ie){me.addEventListener("click",b=>{b.preventDefault(),ie.classList.add("sheet-open")}),ge&&ge.addEventListener("click",b=>{b.preventDefault(),ie.classList.remove("sheet-open")});let c=document.getElementById("btn-apply-filter");c&&c.addEventListener("click",b=>{b.preventDefault(),ie.classList.remove("sheet-open")})}let pe=document.getElementById("btn-mobile-aksi"),be=document.getElementById("excel-actions-wrapper"),_e=document.getElementById("btn-close-aksi-sheet");if(pe&&be&&(pe.addEventListener("click",c=>{c.preventDefault(),be.classList.add("sheet-open")}),_e&&_e.addEventListener("click",c=>{c.preventDefault(),be.classList.remove("sheet-open")})),document.getElementById("btn-create")?.addEventListener("click",()=>Ce(null)),S){document.getElementById(`btn-export-${S.moduleName}`)?.addEventListener("click",async b=>{let w=b.target,x=w.innerHTML;w.innerHTML="\u23F3 Loading...",w.disabled=!0;try{await S.onExport()}catch{z("Gagal export data")}finally{w.innerHTML=x,w.disabled=!1}}),document.getElementById(`btn-template-${S.moduleName}`)?.addEventListener("click",()=>{S.onTemplate()});let c=document.getElementById(`input-import-${S.moduleName}`);c?.addEventListener("change",async b=>{let w=b.target.files[0];if(!w)return;c.disabled=!0;let x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",x.innerHTML=`
      ...
      `,document.body.appendChild(x);let T=x.querySelector("#import-progress-text"),_=x.querySelector("#import-progress-bar"),P=x.querySelector("#import-summary"),A=x.querySelector("#import-close-btn");A.addEventListener("click",()=>{x.remove(),q()});try{let W=await De(w);if(W.length===0)throw new Error("File kosong atau format salah");let V=500,ae=0,Z=0,N=0,M=W.length;T.textContent=`Ditemukan ${M} baris data. Memulai import...`;for(let I=0;I<M;I+=V){let J=W.slice(I,I+V);T.textContent=`Mengimport baris ${I+1} - ${Math.min(I+V,M)} dari ${M}...`,_.style.width=`${Math.round(I/M*100)}%`;try{let O=await S.onImport(J);O?(ae+=O.inserted||O.metrics?.inserted||J.length,Z+=O.skipped||O.metrics?.updated||0):ae+=J.length}catch(O){console.error("Chunk import failed:",O),N+=J.length}}_.style.width="100%",T.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',P.style.display="block",P.innerHTML=`
        ...
        `,N>0&&(P.innerHTML+='<p style="margin-top:12px;font-size:0.8rem;color:var(--danger)">Sebagian data gagal diimport. Pastikan format kolom sesuai template dan tidak ada data kosong pada kolom wajib.</p>'),A.style.display="block",c.value=""}catch(W){T.innerHTML=`<strong style="color:var(--danger)">\u274C Gagal Memproses File</strong><br>${W.message}`,_.style.background="var(--danger)",_.style.width="100%",A.style.display="block",c.value=""}finally{c.disabled=!1}})}async function q(){F();let c=document.getElementById("table-container");if(!c)return;c.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';let b=C==="client",w=b?1:$,x=b?Se:20,T=new URLSearchParams({page:w,limit:x,...Object.fromEntries(Object.entries(L).filter(([,N])=>N))}),_=await k(`${a}?${T}`);if(!_.ok){c.innerHTML=`<div class="empty-state"><p class="text-danger">Gagal memuat data: ${_.data?.error||"Error"}</p></div>`;return}let P=_.data?.data||_.data||[],A=_.data?.pagination,W=P.length,V=P;if(b){P=g(P),V=P;let N=P.length,M=20,I=Math.ceil(N/M);$>I&&I>0&&($=I);let J=($-1)*M,O=$*M;P=P.slice(J,O),A={page:$,limit:M,total:N,pages:I}}!1,y&&y(P);let ae=Be({columns:o,data:P,fullData:V,onEdit:p?N=>Ce(N):null,actions:f.map(N=>({...N,handler:M=>N.handler(M,q)})),emptyText:`Tidak ada ${String(l||"").toLowerCase()}`,bulkSelect:D?{selectedIds:E,onToggle:F}:null});c.innerHTML="",c.appendChild(ae);let Z=document.getElementById("pagination-container");if(Z&&(Z.innerHTML="",A&&A.pages>1)){let N=Ae({page:A.page,pages:A.pages,total:A.total,limit:A.limit,onPage:M=>{$=M,q()}});N&&Z.appendChild(N)}}function Ge(c){let b=typeof d=="function"?d(c):d;return Te(b)}function Ce(c){let b=!!c,w=document.createElement("form");if(w.noValidate=!0,w.innerHTML=Ge(c),b){let T=typeof d=="function"?d(c):d;Me(w,c)}let{close:x}=de({title:b?`Edit ${l}`:`Tambah ${l}`,content:w,size:"lg",confirmText:b?"Simpan Perubahan":`Tambah ${l}`,onConfirm:async(T,_)=>{if(!w.reportValidity())return;let P=T.querySelector(".modal-confirm");P.disabled=!0,P.textContent="Menyimpan...";let A=Ne(w),W=typeof d=="function"?d(c):d,V=async M=>{for(let I of M)if(I.type==="row")await V(I.fields);else if(I.type==="combobox"&&A[I.name]){let J=A[I.name],O=(I.options||[]).find(U=>{let ee=String(typeof U=="object"?U.value:U),ze=String(typeof U=="object"?U.label:U);return ee===J||ze===J});if(O)A[I.name]=typeof O=="object"?O.value:O;else if(I.createApi){let U={};U[I.createApi.field]=J,I.createApi.extra&&Object.assign(U,I.createApi.extra);let ee=await k(I.createApi.path,{method:"POST",body:JSON.stringify(U)});if(ee.ok&&ee.data?.id)A[I.name]=ee.data.id;else if(ee.ok&&!ee.data?.id)A[I.name]=J;else throw new Error(`Gagal membuat master data: ${ee.data?.error||"Unknown error"}`)}}};try{await V(W)}catch(M){z(M.message),P.disabled=!1,P.textContent=b?"Simpan Perubahan":`Tambah ${l}`;return}m&&(A=await m(A,c));let ae=b?"PUT":"POST",Z=b?`${a}/${c.id}`:a,N=await k(Z,{method:ae,body:JSON.stringify(A)});N.ok?(ne(b?`${l} berhasil diperbarui.`:`${l} berhasil ditambahkan.`),_(),q()):(z(N.data?.error||"Gagal menyimpan data."),P.disabled=!1,P.textContent=b?"Simpan Perubahan":`Tambah ${l}`)}})}function $t(c){Ee(`Hapus ${l} ini? Tindakan tidak dapat dibatalkan.`,async()=>{let b=await k(`${a}/${c.id}`,{method:"DELETE"});b.ok?(ne(`${l} berhasil dihapus.`),q()):z(b.data?.error||"Gagal menghapus.")},`Hapus ${l}`)}return q(),q}j();j();var Fe=null,nt=null;async function Oe(t=!1){if(Fe&&!t)return console.log("Employees Raw (Cache Hit)",Fe.slice(0,5)),Fe;let e=await k(`/api/employees?limit=${Se}&status=Aktif`);return Fe=(e.data?.data||[]).map(i=>({value:i.id,label:i.full_name})),console.log("Employees Raw",e.data?.data?.slice(0,5)),console.log("Employees Mapped (ID)",Fe.slice(0,5)),Fe}async function re(t=!1){let i=(await Oe(t)).map(a=>({value:a.label,label:a.label}));return console.log("Employee Options",i.slice(0,5)),i}async function Q(t=!1){return nt&&!t||(nt=((await k("/api/branches?all=1")).data?.data||[]).map(i=>({value:i.id,label:i.full_name}))),nt}function X(t){let e={Done:"badge-success",Aktif:"badge-success",Open:"badge-warning","In Progress":"badge-info",Pending:"badge-warning",Diproses:"badge-info",Selesai:"badge-success","Tidak Aktif":"badge-neutral",Resign:"badge-neutral",Cut:"badge-danger","Tidak Datang":"badge-danger"};return!t||t==="-"||String(t).trim()===""?"":`<span class="badge ${e[t]||"badge-neutral"}">${t}</span>`}function bt(t){return t==null?'<span class="badge badge-neutral">-</span>':t<0?`<span class="badge badge-danger">Expired (${Math.abs(t)}h)</span>`:t<=14?`<span class="badge badge-danger">${t} hari</span>`:t<=30?`<span class="badge badge-warning">${t} hari</span>`:`<span class="badge badge-success">${t} hari</span>`}function Re(t){return`<span class="badge ${{"FACILITY CARE":"badge-info",SECURITY:"badge-secondary"}[t]||"badge-neutral"}">${t||"-"}</span>`}function ht(t){return`<span class="badge ${{"Inspeksi Hygiene & Aset Bangunan":"badge-info","General Cleaning":"badge-success","Deep Cleaning":"badge-purple",Fogging:"badge-warning"}[t]||"badge-neutral"}">${t||"-"}</span>`}function he(t){return`<span class="badge ${{Q1:"badge-info",Q2:"badge-success",Q3:"badge-warning",Q4:"badge-danger"}[t]||"badge-neutral"}">${t||"-"}</span>`}G();function yt(t,e){let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}`;if(!(t.target_date||t.opening_date||"").startsWith(a))return!1;let d=String(t.status||"").toLowerCase();if(d!=="selesai"&&d!=="completed"&&d!=="done")return!1;let r=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?r.includes("inspeksi"):e==="gcdc"?r.includes("general cleaning")||r.includes("deep cleaning"):!1}j();we();G();function H({container:t,title:e,icon:i,apiPath:a,columns:o,formFields:d,filterFields:r,defaultFilters:n={},enableMobileFilterSheet:s=!1,itemLabel:l="Data",canCreate:h=!0,canEdit:p=!0,canDelete:u=!0,onBeforeSubmit:m,onAfterLoad:y,onDataLoaded:g,extraActions:f=[],initialSearch:v="",exportOptions:S=null,bulkDelete:D=!1,paginationMode:C="server"}){let B=oe();B&&typeof B=="object"&&B.role==="viewer"&&(h=!1,p=!1,u=!1,D=!1,S=null);let $=1,L={...n};v&&(L.search=v);let E=new Set;t.innerHTML=`
    <div class="crud-layout-wrapper ${s?"mobile-active":""}">
      <div class="page-header">
      <h1 class="page-title">${i} ${e}</h1>
      <div class="page-actions">
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>kt(t)),document.getElementById("filter-jadwal-year")?.addEventListener("change",async i=>{let a=i.target.value,o=document.getElementById("jadwal-year-label");o&&(o.textContent=a);let d=document.getElementById("skel-jadwal"),r=document.getElementById("chart-jadwal");d&&(d.style.display="block",d.style.position="absolute"),r&&(r.style.display="none");let n=await se(`/api/dashboard/schedule-chart?year=${a}`,{},8e3);try{Ot(n)}catch(s){console.warn("ScheduleChart render:",s),fe("skel-jadwal","chart-jadwal")}}),document.getElementById("filter-insp-month")?.addEventListener("change",async i=>{let a=i.target.value,o=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",d=document.getElementById("skel-insp"),r=document.getElementById("chart-insp");d&&(d.style.display="block",d.style.position="absolute"),r&&(r.style.display="none");let n=await se(o,{},8e3);try{Rt(n)}catch(s){console.warn("InspBar render:",s),fe("skel-insp","chart-insp")}}),t._skelTimeout=setTimeout(()=>Ea(),5e3),await kt(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?kt(t):clearInterval(t._dashRefresh)},6e4)}async function kt(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,i,a,o,d,r,n,s,l,h,p,u,m,y]=await Promise.all([se("/api/dashboard/kpi",{},8e3),se("/api/dashboard/issues-trend",{},8e3),se("/api/dashboard/issues-summary",{},8e3),se("/api/dashboard/stats",{},8e3),se("/api/dashboard/calendar",[],8e3),se("/api/schedule?limit=10000",{data:[]},8e3),se("/api/employees?limit=10000",{data:[]},8e3),se("/api/contracts?limit=10000",{data:[]},8e3),se("/api/issues?limit=10000",{data:[]},8e3),se("/api/one-on-one?limit=10000",{data:[]},8e3),se("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3),se(`/api/dashboard/schedule-chart?year=${document.getElementById("filter-jadwal-year")?.value||new Date().getFullYear()}`,{},8e3),se("/api/relievers?limit=10000",{data:[]},8e3),se("/api/reports/fogging?limit=10000",{data:[]},8e3)]),g=document.getElementById("filter-insp-month"),f=g?g.value:"",v=f?`/api/dashboard/inspection-bar?month=${f}`:"/api/dashboard/inspection-bar",S=await se(v,{},8e3);if(e){let D=Array.isArray(r?.data)?r.data:Array.isArray(r)?r:[];window.dashboardSchedules=D;let C=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],B=Array.isArray(s?.data)?s.data:Array.isArray(s)?s:[],$=Array.isArray(l?.data)?l.data:Array.isArray(l)?l:[],L=Array.isArray(h?.data)?h.data:Array.isArray(h)?h:[],E=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[];window.dashboardRelievers=E;let F=Array.isArray(y?.data)?y.data:Array.isArray(y)?y:[];window.dashboardFogging=F,e.employees&&(e.employees.current=C.filter(K=>It(K,"active")).length),e.contracts&&(e.contracts.current=B.filter(K=>ft(K,"active")).length),e.expiring30&&(e.expiring30={current:B.filter(K=>ft(K,"expiring30")).length}),e.issues&&(e.issues.current=$.filter(K=>Lt(K,"open")).length),e.one_on_one&&(e.one_on_one.current=L.filter(K=>Pt(K,"pending")).length),e.inspection_month&&(e.inspection_month.current=D.filter(K=>yt(K,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=D.filter(K=>yt(K,"gcdc")).length)}try{Mt(e)}catch(D){console.warn("KPI render:",D)}try{Ft(e)}catch(D){console.warn("MiniStats render:",D)}try{Ot(u)}catch(D){console.warn("ScheduleChart render:",D),fe("skel-jadwal","chart-jadwal")}try{Ta(Array.isArray(a?.by_category)?a.by_category:[])}catch(D){console.warn("Donut render:",D),fe("skel-donut","chart-donut")}try{Da(i)}catch(D){console.warn("Trend render:",D),fe("skel-trend","chart-trend")}try{Rt(S)}catch(D){console.warn("InspBar render:",D),fe("skel-insp","chart-insp")}try{let D=Array.isArray(o)?o:Array.isArray(o?.recent_issues)?o.recent_issues:[];La(D)}catch(D){console.warn("IssuesTable render:",D)}try{let D=Array.isArray(o?.expiring_contracts)?o.expiring_contracts:[];Ia(p)}catch(D){console.warn("ContractsTable render:",D)}try{Pa(Array.isArray(d)?d:[])}catch(D){console.warn("Agenda render:",D)}try{Ba()}catch(D){console.warn("Quick Actions render:",D)}}function Mt(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let i=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=i.map(a=>{let o=ve(t[a.key]?.current,0);return`
  ...
  `,n=[{id:"mini-jadwal",icon:"\u{1F4C5}",label:"Jadwal",dropdown:`
    ...
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(p=>At(p,parseInt(p.dataset.target)||0,700));let s=document.getElementById("dash-jadwal-period");s&&s.addEventListener("change",p=>{let u=p.target.value,m=(window.dashboardSchedules||[]).filter(f=>f.period===u).length,y=document.querySelector("#mini-jadwal .mini-stat-value");y&&(y.dataset.target=m,y.textContent=m);let g=document.getElementById("mini-jadwal");g&&(g.href=`#/timeline?dash_filter=period_${u.toLowerCase()}`)});let l=(p,u,m,y,g)=>{let f=document.getElementById(p);f&&f.addEventListener("change",v=>{let S=v.target.value,D=(m||[]).filter($=>y($,S)).length,C=document.querySelector(`#${u} .mini-stat-value`);C&&(C.dataset.target=D,C.textContent=D);let B=document.getElementById(u);B&&(B.href=`${g}&month=${S}`)})},h=p=>{let u=String(p.status||"").toLowerCase();return u==="done"||u==="selesai"||u==="completed"};l("dash-reliefer-month","mini-reliefer",window.dashboardRelievers,(p,u)=>window.parseFlexibleDate(p.backup_date).startsWith(u)&&h(p),"#/relievers?dash_filter=reliever"),l("dash-inspeksi-month","mini-inspeksi",window.dashboardSchedules,(p,u)=>p.activity_type==="Inspeksi Hygiene"&&h(p)&&window.parseFlexibleDate(p.opening_date||p.target_date).startsWith(u),"#/timeline?dash_filter=inspeksi"),l("dash-gcdc-month","mini-gcdc",window.dashboardSchedules,(p,u)=>(p.activity_type==="General Cleaning"||p.activity_type==="Deep Cleaning")&&h(p)&&window.parseFlexibleDate(p.opening_date||p.target_date).startsWith(u),"#/timeline?dash_filter=gcdc"),l("dash-fogging-month","mini-fogging",window.dashboardFogging,(p,u)=>h(p)&&window.parseFlexibleDate(p.activity_date).startsWith(u),"#/reports/fogging?dash_filter=fogging")}function Ta(t){fe("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),i=document.getElementById("donut-legend");if(!e||!i)return;He("donut");let a=(t||[]).filter(s=>ve(s.count)>0);if(!a.length){Xe(e,"Belum ada data permasalahan");return}let o=a.map(s=>`${Ke(s.category,"Lainnya")}`),d=a.map(s=>ve(s.count)),r=d.reduce((s,l)=>s+l,0);i.innerHTML=a.map((s,l)=>{let h=vt[l%vt.length],p=r>0?Math.round(s.count/r*100):0;return`
    ...
    <div class="dash-list">
      ...
      `).join("")}
    </div>`}function Pa(t){let e=document.getElementById("widget-agenda");if(!e)return;let i=new Date,a=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}-${String(i.getDate()).padStart(2,"0")}`,d=(t||[]).filter(r=>(r.event_date||"").startsWith(a)).slice(0,10);if(!d.length){e.innerHTML="";return}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:12px;padding-right:8px">
      ${d.map(r=>{let n="#3B82F6",s="#EFF6FF",l="Agenda",h=(r.title||"").toLowerCase();return h.includes("inspeksi")?(n="#10B981",s="#ECFDF5",l="Inspeksi"):h.includes("cleaning")||h.includes("gcdc")?(n="#3B82F6",s="#EFF6FF",l="Cleaning"):h.includes("reliefer")?(n="#F59E0B",s="#FFFBEB",l="Reliefer"):h.includes("fogging")&&(n="#8B5CF6",s="#F5F3FF",l="Fogging"),`
...
`)}let B=l(S,v),$=null;return B||($="Karyawan tidak ditemukan di Database"),{isValid:!!B,invalidReason:$,rowNum:v,data:{employee_id:B,branch_id:s(String(g.Cabang||"").trim()),division:String(g["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:C,end_date:h(g["Tanggal Selesai"])||"2099-12-31",status:String(g.Status||"").trim(),_rawName:S}}}),u=[],m=[];if(p.forEach(g=>{g.isValid?u.push(g.data):m.push({rowNum:g.rowNum,name:g.data._rawName,reason:g.invalidReason})}),console.log(`Split Validation - Valid: ${u.length}, Invalid: ${m.length}`),u.length===0)return{inserted:0,skipped:a.length,failed:a.length};let y=await k("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:u,onDuplicate:"update"})});if(!y.ok)throw new Error(y.data?.error||"Import gagal");return y.data}}})}j();G();var xt=[],Ze=[];function Oa(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let i of e)if(t.some(a=>a.period===i))return i;return"Q3"}async function Jt(t,e){xt=await Q();let i=await re();Ze=["Berlin Ariansyah","Ade Surahman"];let a=m=>m&&!Ze.find(y=>String(typeof y=="object"?y.value:y).toLowerCase()===String(m).toLowerCase())?[...Ze,m]:Ze,o=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),d=m=>{if(!m||m==="-"||String(m).trim()==="")return"";let y=String(m).split("-");return y.length===3&&y[0].length===4?`${y[2]}-${y[1]}-${y[0]}`:m},r=o.data?.data||[],n=Oa(r),s=e?e.get("dash_filter"):null,l=new Date,h=`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}`,p={},u=e&&e.get("month")?e.get("month"):null;s==="inspeksi"?p={status:"Done",activity_type:"Inspeksi Hygiene",month:u}:s==="gcdc"?p={status:"Done",activity_type:"GCDC",month:u}:s&&s.startsWith("period_")&&(p={period:s.replace("period_","").toUpperCase()}),gt({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",enableMobileFilterSheet:!0,defaultFilters:p,onDataLoaded:m=>m.sort((y,g)=>{let f=y.opening_date?new Date(y.opening_date).getTime():0;return(g.opening_date?new Date(g.opening_date).getTime():0)-f}),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:m=>ht(m)},{key:"period",label:"Periode",render:m=>he(m)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:m=>d(m)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:m=>d(m)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:m=>d(m)},{key:"status",label:"Status",render:m=>X(m)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:xt},{type:"select",name:"activity_type",label:"Kegiatan",options:[{value:"Inspeksi Hygiene",label:"Inspeksi Hygiene"},{value:"General Cleaning",label:"General Cleaning"},{value:"Deep Cleaning",label:"Deep Cleaning"},{value:"Fogging",label:"Fogging"},{value:"GCDC",label:"GCDC (GC & DC)"}]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:Ze}],formFields:m=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"select",required:!0,options:xt,value:m?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:m?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:m?.period},{name:"pic",label:"PIC",type:"select",options:a(m?.pic),value:m?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:m?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:m?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:m?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:m?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:m?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let m=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(m.ok){let y=m.data.data.map(g=>({Cabang:g.branch_name||"",Kegiatan:g.activity_type||"",Periode:g.period||"",PIC:g.pic||"","Tgl Opening":g.opening_date||"","Tgl Target":g.target_date||"","Tgl Selesai":g.completion_date||"",Status:g.status||""}));R(y,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{R([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async m=>{let g=(await k("/api/branches?all=1")).data?.data||[],f=C=>{if(!C)return null;let B=String(C||"").toLowerCase(),$=g.find(L=>String(L.full_name||"").toLowerCase()===B||String(L.code||"").toLowerCase()===B||String(L.name||"").toLowerCase()===B);return $?$.id:null},v=C=>{if(C==null||C==="")return"";if(C instanceof Date&&!isNaN(C.getTime()))return C.toISOString().slice(0,10);let B=String(C).trim();if(B===""||B==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(B))return B.slice(0,10);if(/^\d{4,5}$/.test(B)){let L=Number(B);if(L>2e4&&L<99999){let E=new Date(Date.UTC(1899,11,30)+L*864e5);return isNaN(E.getTime())?"":E.toISOString().slice(0,10)}}let $=B.split(/[\/\-\.]/);if($.length===3){let[L,E,F]=$.map(K=>K.trim());if(L.length===4&&E.length<=2&&F.length<=2)return`${L}-${E.padStart(2,"0")}-${F.padStart(2,"0")}`;if(F.length===4&&E.length<=2&&L.length<=2)return`${F}-${E.padStart(2,"0")}-${L.padStart(2,"0")}`}return B},S=m.map(C=>({branch_id:f(String(C.Cabang||"").trim()),activity_type:String(C.Kegiatan||"").trim(),period:String(C.Periode||"").trim(),pic:String(C.PIC||C.Pic||"").trim(),opening_date:v(C["Tgl Opening"]||C["Tanggal Opening"]||C["Tgl Openir"]),target_date:v(C["Tgl Target"]||C["Tanggal Target"]),completion_date:v(C["Tgl Selesai"]||C["Tanggal Selesai"]),status:String(C.Status||"").trim(),notes:String(C.Catatan||C.Keterangan||"").trim()})).filter(C=>C.activity_type&&C.period),D=await k("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:S,onDuplicate:"update"})});if(!D.ok)throw new Error(D.data?.error||"Import gagal");return D.data}}})}j();G();var _t=[],rt=[];function Ra(t,e){let i=String(t.status||"").toLowerCase();return e==="open"?i==="open":!1}async function Ut(t,e){let i=e?e.get("dash_filter"):null;_t=await Q(),rt=await re();let a=r=>r&&!rt.find(n=>n.value===r)?[...rt,{value:r,label:r}]:rt,o=new Date().getFullYear();H({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:r=>i?r.filter(n=>Ra(n,i)):r,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:r=>`<span class="badge badge-secondary">${r}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:r=>`<span title="${r}">${r?.length>50?r.slice(0,50)+"\u2026":r}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:r=>`<span title="${r||""}">${r?.length>40?r.slice(0,40)+"\u2026":r||"-"}</span>`},{key:"status",label:"Status",render:r=>X(r)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:r=>window.formatDate(r)},{key:"day_count",label:"Hari",render:r=>r??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:_t},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:r=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:r?.report_date},{name:"branch_id",label:"Cabang",type:"select",required:!0,options:_t,value:r?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:r?.category},{name:"source",label:"Sumber Laporan",type:"select",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:r?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:r?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"select",options:a(r?.employee_name),value:r?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"select",options:a(r?.fc_specialist),value:r?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:r?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:r?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:r?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let r=await k(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(r.ok){let n=r.data.data.map(s=>({Tanggal:s.report_date||"",Cabang:s.branch_name||"",Kategori:s.category||"",Sumber:s.source||"",Keluhan:s.complaint||"","Nama FC":s.employee_name||"","FC Spesialis":s.fc_specialist||"",Solusi:s.solution||"","Tgl Selesai":s.completion_date||"",Status:s.status||""}));R(n,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{R([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async r=>{let s=(await k("/api/branches?all=1")).data?.data||[],l=u=>{if(!u)return null;let m=String(u||"").toLowerCase(),y=s.find(g=>String(g.full_name||"").toLowerCase()===m||String(g.code||"").toLowerCase()===m||String(g.name||"").toLowerCase()===m);return y?y.id:null},h=r.map(u=>({branch_id:l(String(u.Cabang||"").trim()),report_date:String(u.Tanggal||"").trim(),category:String(u.Kategori||"").trim(),source:String(u.Sumber||"").trim(),complaint:String(u.Keluhan||"").trim(),employee_name:String(u["Nama FC"]||"").trim(),fc_specialist:String(u["FC Spesialis"]||"").trim(),solution:String(u.Solusi||"").trim(),completion_date:String(u["Tgl Selesai"]||"").trim(),status:String(u.Status||"").trim()})).filter(u=>u.report_date&&u.complaint&&u.category),p=await k("/api/import/issues",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!p.ok)throw new Error(p.data?.error||"Import gagal");return p.data}}})}j();var qe=[];function Ka(t,e){let i=String(t.status||"").toLowerCase();return e==="pending"?i==="pending":!1}async function Gt(t,e){let i=e?e.get("dash_filter"):null;qe=await Q();let a=await re(),o=["Ade","Berlin"],d=n=>n&&!a.find(s=>s.value===n)?[...a,{value:n,label:n}]:a,r=n=>n&&!o.find(s=>(typeof s=="object"?s.value:s)===n)?[...o,n]:o;H({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:n=>i?n.filter(s=>Ka(s,i)):n,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:n=>`<span title="${n||""}">${n?.length>50?n.slice(0,50)+"\u2026":n||"-"}</span>`},{key:"solution",label:"Solusi",render:n=>`<span title="${n||""}">${n?.length>40?n.slice(0,40)+"\u2026":n||"-"}</span>`},{key:"status",label:"Status",render:n=>X(n)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:n=>window.formatDate(n)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:n=>n?`<a href="${n}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"select",name:"branch_id",label:"Cabang",options:qe},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async n=>{let s=new URLSearchParams(n||{}).toString(),l=await k(`/api/one-on-one?limit=10000&${s}`);if(l.ok){let h=l.data.data.map(u=>({Tanggal:u.meeting_date||"",Cabang:u.branch_name||"","Nama Karyawan":u.employee_name||"",PIC:u.pic||"",Masalah:u.problem||"",Solusi:u.solution||"",Status:u.status||"","Tgl Selesai":u.completion_date||"",Dokumen:u.document_link||""})),{downloadExcel:p}=await Promise.resolve().then(()=>(G(),ce));p(h,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let n=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:s}=await Promise.resolve().then(()=>(G(),ce));s(n,"Template_Import_OneOnOne")},onImport:async n=>{let s=u=>{if(!u)return null;let m=String(u||"").toLowerCase(),y=qe.find(g=>String(g.label||"").toLowerCase()===m);return y?y.value:null},l=u=>{if(!u)return"";if(u instanceof Date&&!isNaN(u.getTime()))return u.toISOString().slice(0,10);let m=String(u).trim();if(/^\d{4,5}$/.test(m)){let g=Number(m);if(g>2e4&&g<99999){let f=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(f.getTime())?"":f.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(m))return m.slice(0,10);let y=m.split(/[\/\-\.]/);if(y.length===3){let[g,f,v]=y.map(S=>S.trim());if(g.length===4&&f.length<=2&&v.length<=2)return`${g}-${f.padStart(2,"0")}-${v.padStart(2,"0")}`;if(v.length===4&&f.length<=2&&g.length<=2)return`${v}-${f.padStart(2,"0")}-${g.padStart(2,"0")}`}return m},h=n.map(u=>({meeting_date:l(u.Tanggal),employee_name:String(u["Nama Karyawan"]||"").trim(),branch_id:s(String(u.Cabang||"").trim()),pic:String(u.PIC||"").trim(),problem:String(u.Masalah||"").trim(),solution:String(u.Solusi||"").trim(),status:String(u.Status||"").trim(),completion_date:l(u["Tgl Selesai"]),document_link:String(u.Dokumen||"").trim()})).filter(u=>u.meeting_date&&u.employee_name&&u.branch_id),p=await k("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!p.ok)throw new Error(p.data?.error||"Import gagal");return p.data}},formFields:n=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:n?.meeting_date},{name:"branch_id",label:"Cabang",type:"select",options:n?.branch_id&&!qe.find(s=>s.value==n.branch_id)?[...qe,{value:n.branch_id,label:n.branch_name||n.branch_id}]:qe,createApi:{path:"/api/branches",field:"full_name"},value:n?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"select",required:!0,options:d(n?.employee_name),value:n?.employee_name},{name:"pic",label:"PIC",type:"select",options:r(n?.pic),createApi:{path:"/api/pic",field:"name"},value:n?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:n?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:n?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:n?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:n?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:n?.document_link}]})}j();async function zt(t){let e=await Q(),i=await re(),a=["Ade","Berlin"],o=n=>n&&!i.find(s=>s.value===n)?[...i,{value:n,label:n}]:i,d=n=>n&&!a.find(s=>(typeof s=="object"?s.value:s)===n)?[...a,n]:a,r=Array.from({length:5},(n,s)=>String(new Date().getFullYear()-s));H({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:n=>window.formatDate(n)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:n=>{try{let s=JSON.parse(n);return Array.isArray(s)?s.join(", "):n||"-"}catch{return n||"-"}}},{key:"score",label:"Nilai",render:n=>n!=null?`<strong>${n}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:n=>n?`<a href="${n}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:r}],exportOptions:{moduleName:"training",onExport:async n=>{let s=new URLSearchParams(n||{}).toString(),l=await k(`/api/training?limit=10000&${s}`);if(l.ok){let h=l.data.data.map(u=>{let m=u.participants||"";try{let y=JSON.parse(m);m=Array.isArray(y)?y.join(", "):m}catch{}return{Tanggal:u.training_date||"",Batch:u.batch||"",Materi:u.subject||"",Cabang:u.branch_name||"",Trainer:u.trainer||"",Peserta:m,Nilai:u.score!==null&&u.score!==void 0?u.score:"",Dokumen:u.document_link||""}}),{downloadExcel:p}=await Promise.resolve().then(()=>(G(),ce));p(h,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let n=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:s}=await Promise.resolve().then(()=>(G(),ce));s(n,"Template_Import_Training")},onImport:async n=>{let s=u=>{if(!u)return null;let m=String(u||"").toLowerCase(),y=e.find(g=>String(g.label||"").toLowerCase()===m);return y?y.value:null},l=u=>{if(!u)return"";if(u instanceof Date&&!isNaN(u.getTime()))return u.toISOString().slice(0,10);let m=String(u).trim();if(/^\d{4,5}$/.test(m)){let g=Number(m);if(g>2e4&&g<99999){let f=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(f.getTime())?"":f.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(m))return m.slice(0,10);let y=m.split(/[\/\-\.]/);if(y.length===3){let[g,f,v]=y.map(S=>S.trim());if(g.length===4&&f.length<=2&&v.length<=2)return`${g}-${f.padStart(2,"0")}-${v.padStart(2,"0")}`;if(v.length===4&&f.length<=2&&g.length<=2)return`${v}-${f.padStart(2,"0")}-${g.padStart(2,"0")}`}return m},h=n.map(u=>({training_date:l(u.Tanggal),batch:String(u.Batch||"").trim(),subject:String(u.Materi||"").trim(),branch_id:s(String(u.Cabang||"").trim()),trainer:String(u.Trainer||"").trim(),participants:String(u.Peserta||"").trim(),score:u.Nilai?Number(u.Nilai):null,document_link:String(u.Dokumen||"").trim()})).filter(u=>u.training_date&&u.subject&&u.branch_id),p=await k("/api/import/training",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!p.ok)throw new Error(p.data?.error||"Import gagal");return p.data}},formFields:n=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:n?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:n?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:n?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"select",options:e,value:n?.branch_id},{name:"trainer",label:"Trainer",type:"select",options:d(n?.trainer),value:n?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let s=JSON.parse(n?.participants);return Array.isArray(s)?s.join(", "):n?.participants||""}catch{return n?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:n?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:n?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:n?.notes}],onBeforeSubmit:async n=>(n.participants&&(n.participants=JSON.stringify(n.participants.split(",").map(s=>s.trim()).filter(Boolean))),n)})}j();we();G();function Qt({container:t,title:e,icon:i,apiPath:a,columns:o,formFields:d,filterFields:r,defaultFilters:n={},enableMobileFilterSheet:s=!1,itemLabel:l="Data",canCreate:h=!0,canEdit:p=!0,canDelete:u=!0,onBeforeSubmit:m,onAfterLoad:y,onDataLoaded:g,extraActions:f=[],initialSearch:v="",exportOptions:S=null,bulkDelete:D=!1,paginationMode:C="server"}){let B=oe();B&&typeof B=="object"&&B.role==="viewer"&&(h=!1,p=!1,u=!1,D=!1,S=null);let $=1,L={...n};v&&(L.search=v);let E=new Set;t.innerHTML=`
  ...
  `,document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),o()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),o()}),document.getElementById("cal-event-close").addEventListener("click",()=>{document.getElementById("cal-event-list").style.display="none"}),document.querySelectorAll(".cal-filter").forEach(d=>d.addEventListener("change",o));async function a(){try{let d=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`;i=(await k(`/api/dashboard/calendar?month=${d}`)).data?.data||[]}catch(d){console.warn("[Calendar] Failed to load events, rendering empty grid:",d),i=[]}}async function o(){let d=document.getElementById("calendar-grid");if(d){d.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:1px;background:var(--border);">
    ...
    </div>`,await a();try{let r=e.getFullYear(),n=e.getMonth(),s=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}),l=document.getElementById("cal-month-label");l&&(l.textContent=s);let h=new Set(Array.from(document.querySelectorAll(".cal-filter:checked")).map(C=>C.value)),p=i.filter(C=>h.has(C.type)),u={};p.forEach(C=>{let B=(C.event_date||"").slice(0,10);u[B]||(u[B]=[]),u[B].push(C)});let m=new Date(r,n,1).getDay(),y=new Date(r,n+1,0).getDate(),g=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],f=new Date().toISOString().slice(0,10),v='<div class="calendar-grid">';g.forEach(C=>{v+=`<div class="cal-day-header">${C}</div>`});for(let C=0;C<m;C++)v+='<div class="cal-cell cal-cell-empty"></div>';for(let C=1;C<=y;C++){let B=`${r}-${String(n+1).padStart(2,"0")}-${String(C).padStart(2,"0")}`,$=u[B]||[],L=B===f;v+=`
  ...
  `;let e=document.getElementById("btnRefreshSync"),i=document.getElementById("queueStatusFilter");e.addEventListener("click",o),i.addEventListener("change",n),document.getElementById("btnRetryAll").addEventListener("click",()=>a("retry",{allFailed:!0})),document.getElementById("btnResetStuck").addEventListener("click",()=>a("reset-stuck",{})),document.getElementById("btnPauseSync").addEventListener("click",()=>a("pause",{})),document.getElementById("btnResumeSync").addEventListener("click",()=>a("resume",{})),document.getElementById("btnReconcile").addEventListener("click",()=>a("reconcile",{module:"ALL",repairMode:!0})),document.getElementById("btnRetrySelected").addEventListener("click",()=>{let l=Array.from(document.querySelectorAll(".chk-queue:checked")).map(h=>h.value);if(l.length===0)return alert("No items selected");a("retry",{ids:l})}),document.getElementById("chkAllQueue").addEventListener("change",l=>{document.querySelectorAll(".chk-queue").forEach(h=>h.checked=l.target.checked)});async function a(l,h){if(confirm(`Are you sure you want to execute action: ${l}?`)){showLoading();try{let p=await k(`/api/sync/actions/${l}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h)});p.ok?(alert(p.data?.message||"Success"),o()):z(p.error||"Action failed")}catch(p){z(p.message)}hideLoading()}}await o();async function o(){showLoading(),await Promise.all([r(),n(),d(),s()]),hideLoading()}async function d(){try{let l=await k("/api/sync/performance");if(!l.ok)return;let{webhook:h,google_api:p,d1:u,queue:m,throughput:y}=l.data;document.getElementById("latencyHistograms").innerHTML=`
      ...
      `,document.getElementById("lblSnapshotStatus").innerHTML='Checked 10 modules. Status: <b>COMPLETED</b><br><span class="text-xs text-gray-500">Run today at 02:00</span>'}catch(l){console.error(l)}}async function r(){try{let l=await k("/api/sync/health");if(!l.ok)return z("Failed to fetch sync health");let{status:h,queue:p,circuit_breaker:u}=l.data,m=`
      ...
      `;document.getElementById("syncOverviewCards").innerHTML=m;let y=document.getElementById("cbStateBadge"),g=document.getElementById("cbStateDesc"),f=document.getElementById("cbStatusCard");f.className="bg-white rounded-lg shadow p-6 border-l-4",u==="CLOSED"?(y.className="px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800",y.textContent="CLOSED",g.textContent="Traffic is flowing normally to Google Sheets.",f.classList.add("border-green-500")):u==="OPEN"?(y.className="px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800",y.textContent="OPEN",g.textContent="Failing fast. Traffic to Google Sheets is paused due to repeated failures.",f.classList.add("border-red-500")):u==="HALF_OPEN"?(y.className="px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800",y.textContent="HALF-OPEN",g.textContent="Testing recovery. Permitting limited traffic to verify stability.",f.classList.add("border-yellow-500")):y.textContent=u||"UNKNOWN"}catch(l){console.error(l)}}async function n(){try{let l=document.getElementById("queueStatusFilter").value,h=await k("/api/sync/queue?limit=15"+(l?"&status="+l:""));if(!h.ok)return;let p=document.getElementById("queueTableBody"),u=h.data?.data||h.data||[];if(u.length===0){p.innerHTML='<tr><td colspan="6" class="px-4 py-4 text-center text-gray-500">No events found</td></tr>';return}p.innerHTML=u.map(m=>`
      ...
      `).join("")}catch(l){console.error(l)}}async function s(){try{let l=await k("/api/sync/metrics");if(!l.ok)return;let h=document.getElementById("metricsTableBody"),p=l.data||[];if(p.length===0){h.innerHTML='<tr><td colspan="5" class="px-4 py-4 text-center text-gray-500">No metrics found in last 24h</td></tr>';return}h.innerHTML=p.map(u=>`
        <tr class="hover:bg-gray-50">
