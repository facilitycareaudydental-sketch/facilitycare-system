var it=Object.defineProperty;var Ge=(a,e)=>()=>(a&&(e=a(a=0)),e);var Qe=(a,e)=>{for(var r in e)it(a,r,{get:e[r],enumerable:!0})};var de={};Qe(de,{API:()=>ga,CLIENT_SIDE_MAX_ROWS:()=>_e,IS_DEVELOPMENT:()=>Ne,apiFetch:()=>f,clearToken:()=>Ce,getToken:()=>xe,getUser:()=>oe,setToken:()=>Ve,setUser:()=>Te});function xe(){return localStorage.getItem("fm_token")}function Ve(a){localStorage.setItem("fm_token",a)}function Ce(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function oe(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Te(a){localStorage.setItem("fm_user",JSON.stringify(a))}async function f(a,e={}){let r=xe(),t={"Content-Type":"application/json",...r?{Authorization:`Bearer ${r}`}:{},...e.headers||{}};try{let o=`cb=${Date.now()}`,l=a.includes("?")?"&":"?",n=`${ga}${a}${l}${o}`,s=await fetch(n,{...e,headers:t}),i;try{let d=await s.text();try{i=JSON.parse(d)}catch{i={error:`Server Error (${s.status}): ${d.substring(0,80)}...`}}}catch{i={error:"Gagal membaca respon dari server"}}return s.status===401&&(Ce(),window.location.hash="#/login"),{ok:s.ok,status:s.status,data:i}}catch(o){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${o.message})`}}}}var Ne,rt,ga,_e,D=Ge(()=>{Ne=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1",rt=Ne?"http://localhost:8787":"",ga=rt,_e=1e4});var ya={};Qe(ya,{confirmDialog:()=>Ye,createModal:()=>te});function te({title:a,content:e,onConfirm:r,onCancel:t,confirmText:o="Simpan",cancelText:l="Batal",size:n="md",confirmClass:s="btn-primary"}){let i={sm:"400px",md:"560px",lg:"720px",xl:"900px"},d=document.createElement("div");d.className="modal-overlay",d.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>sa(a)),a._skelTimeout=setTimeout(()=>gt(),5e3),await sa(a),a._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?sa(a):clearInterval(a._dashRefresh)},6e4)}async function sa(a){a._skelTimeout&&(clearTimeout(a._skelTimeout),a._skelTimeout=null);let[e,r,t,o,l,n,s,i,d,m,p]=await Promise.all([X("/api/dashboard/kpi",{},8e3),X("/api/dashboard/issues-trend",{},8e3),X("/api/dashboard/issues-summary",{},8e3),X("/api/dashboard/inspection-bar",{},8e3),X("/api/dashboard/stats",{},8e3),X("/api/dashboard/calendar",[],8e3),X("/api/schedule?limit=10000",{data:[]},8e3),X("/api/employees?limit=10000",{data:[]},8e3),X("/api/contracts?limit=10000",{data:[]},8e3),X("/api/issues?limit=10000",{data:[]},8e3),X("/api/one_on_one?limit=10000",{data:[]},8e3)]);if(e){let c=Array.isArray(s?.data)?s.data:Array.isArray(s)?s:[],b=Array.isArray(i?.data)?i.data:Array.isArray(i)?i:[],y=Array.isArray(d?.data)?d.data:Array.isArray(d)?d:[],g=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[],u=Array.isArray(p?.data)?p.data:Array.isArray(p)?p:[];e.employees&&(e.employees.current=b.filter(h=>wa(h,"active")).length),e.contracts&&(e.contracts.current=y.filter(h=>na(h,"active")).length),e.expiring30&&(e.expiring30={current:y.filter(h=>na(h,"expiring30")).length}),e.issues&&(e.issues.current=g.filter(h=>_a(h,"open")).length),e.one_on_one&&(e.one_on_one.current=u.filter(h=>xa(h,"pending")).length),e.inspection_month&&(e.inspection_month.current=c.filter(h=>ta(h,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=c.filter(h=>ta(h,"gcdc")).length)}try{$a(e)}catch(c){console.warn("KPI render:",c)}try{Ea(e)}catch(c){console.warn("MiniStats render:",c)}try{bt(Array.isArray(t?.by_category)?t.by_category:[])}catch(c){console.warn("Donut render:",c),le("skel-donut","chart-donut")}try{ht(r)}catch(c){console.warn("Trend render:",c),le("skel-trend","chart-trend")}try{yt(o)}catch(c){console.warn("InspBar render:",c),le("skel-insp","chart-insp")}try{let c=Array.isArray(l)?l:Array.isArray(l?.recent_issues)?l.recent_issues:[];vt(c)}catch(c){console.warn("IssuesTable render:",c)}try{let c=Array.isArray(l?.expiring_contracts)?l.expiring_contracts:[];ft()}catch(c){console.warn("ContractsTable render:",c)}try{kt(Array.isArray(n)?n:[])}catch(c){console.warn("Agenda render:",c)}try{St(e)}catch(c){console.warn("KPI Kebersihan render:",c)}try{wt()}catch(c){console.warn("Quick Actions render:",c)}}function $a(a){let e=document.getElementById("kpi-row");if(!e)return;a=a||{};let r=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=r.map(t=>{let o=ne(a[t.key]?.current,0);return`
      ...
      </a>`}).join(""),e.querySelectorAll(".kpi-value").forEach(t=>{Ca(t,parseInt(t.dataset.target)||0)})}function Ea(a){let e=document.getElementById("mini-stats-row");if(!e)return;a=a||{};let r=[{icon:"\u{1F4C5}",label:"Jadwal",val:a.schedule?.current,href:"#/timeline",color:"mini-blue"},{icon:"\u{1F393}",label:"Training",val:a.training_month?.current,href:"#/training",color:"mini-gray"},{icon:"\u{1F504}",label:"Total Reliefer",val:a.reliever_total?.current,href:"#/relievers",color:"mini-teal"},{icon:"\u{1F50D}",label:"Inspeksi",val:a.inspection_month?.current,href:"#/timeline?dash_filter=inspeksi",color:"mini-blue"},{icon:"\u{1F9F9}",label:"GCDC",val:a.cleaning_month?.current,href:"#/timeline?dash_filter=gcdc",color:"mini-green"},{icon:"\u{1F4A8}",label:"Fogging",val:a.fogging_month?.current,href:"#/reports/fogging",color:"mini-purple"},{icon:"\u{1F3E2}",label:"Cabang",val:a.branches?.current,href:"#/branches",color:"mini-teal"}];e.innerHTML=r.map(t=>`
    ...
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(t=>Ca(t,parseInt(t.dataset.target)||0,700))}function bt(a){le("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),r=document.getElementById("donut-legend");if(!e||!r)return;De("donut");let t=(a||[]).filter(i=>ne(i.count)>0);if(!t.length){la(e,"Belum ada data permasalahan");return}let o=t.map(i=>`${be(i.category,"Lainnya")}`),l=t.map(i=>ne(i.count)),n=l.reduce((i,d)=>i+d,0);r.innerHTML=t.map((i,d)=>{let m=ia[d%ia.length],p=n>0?Math.round(i.count/n*100):0;return`
