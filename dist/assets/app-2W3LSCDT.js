var da=Object.defineProperty;var nt=(t,e)=>()=>(t&&(e=t(t=0)),e);var it=(t,e)=>{for(var r in e)da(t,r,{get:e[r],enumerable:!0})};var ke={};it(ke,{API:()=>ft,CLIENT_SIDE_MAX_ROWS:()=>ye,IS_DEVELOPMENT:()=>Be,apiFetch:()=>x,clearToken:()=>Ae,getToken:()=>Le,getUser:()=>fe,setToken:()=>rt,setUser:()=>Ne});function Le(){return localStorage.getItem("fm_token")}function rt(t){localStorage.setItem("fm_token",t)}function Ae(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function fe(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Ne(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function x(t,e={}){let r=Le(),a={"Content-Type":"application/json",...r?{Authorization:`Bearer ${r}`}:{},...e.headers||{}};try{let o=`cb=${Date.now()}`,c=t.includes("?")?"&":"?",s=`${ft}${t}${c}${o}`,i=await fetch(s,{...e,headers:a}),n;try{let d=await i.text();try{n=JSON.parse(d)}catch{n={error:`Server Error (${i.status}): ${d.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return i.status===401&&(Ae(),window.location.hash="#/login"),{ok:i.ok,status:i.status,data:n}}catch(o){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${o.message})`}}}}var Be,pa,ft,ye,O=nt(()=>{Be=!1,pa="https://fm-operations-api.facilitycare-audydental.workers.dev",ft=pa,ye=1e4});var xt={};it(xt,{confirmDialog:()=>Me,createModal:()=>ce});function ce({title:t,content:e,onConfirm:r,onCancel:a,confirmText:o="Simpan",cancelText:c="Batal",size:s="md",confirmClass:i="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},d=document.createElement("div");d.className="modal-overlay",d.innerHTML=`
  ...
  `,e instanceof HTMLElement&&d.querySelector(".modal-body").appendChild(e);let m=()=>{d.classList.remove("show"),setTimeout(()=>d.remove(),250)};return d.querySelector(".modal-close").addEventListener("click",()=>{a&&a(),m()}),d.querySelector(".modal-cancel").addEventListener("click",()=>{a&&a(),m()}),r&&d.querySelector(".modal-confirm").addEventListener("click",()=>r(d,m)),d.addEventListener("click",l=>{l.target===d&&(a&&a(),m())}),document.body.appendChild(d),requestAnimationFrame(()=>d.classList.add("show")),{overlay:d,close:m}}function Me(t,e,r="Konfirmasi"){return ce({title:r,content:`<p>${t}</p>`,onConfirm:(a,o)=>{e(),o()},confirmText:"Ya, Lanjutkan",confirmClass:"btn-danger"})}var ve=nt(()=>{});var oe={};it(oe,{downloadExcel:()=>L,parseExcel:()=>Re,renderExcelButtons:()=>Ke});function Re(t){return new Promise((e,r)=>{let a=new FileReader;a.onload=o=>{try{let c=new Uint8Array(o.target.result),s=XLSX.read(c,{type:"array"}),i=s.SheetNames[0],n=s.Sheets[i];console.log("--- START EXCEL PARSING ---"),console.log(`File Name: ${t.name}`),console.log(`File Size: ${(t.size/1024).toFixed(2)} KB`),console.log(`File Type: ${t.type||"unknown"}`),console.log(`Sheets Found: ${s.SheetNames.join(", ")}`),console.log(`Sheet Used: ${i}`);let d=XLSX.utils.decode_range(n["!ref"]||"A1:A1"),m=d.e.r-d.s.r+1,l=d.e.c-d.s.c+1;console.log(`Total Rows (including empty): ${m}`),console.log(`Total Columns: ${l}`);let p=[];for(let b=d.s.c;b<=d.e.c;++b){let g=n[XLSX.utils.encode_cell({c:b,r:d.s.r})];g&&g.v&&p.push(g.v)}console.log(`Headers Found: ${p.join(", ")}`),console.log("---------------------------");let u=XLSX.utils.sheet_to_json(n,{defval:""});Object.defineProperty(u,"__worksheet",{value:n,enumerable:!1}),Object.defineProperty(u,"__headers",{value:p,enumerable:!1}),e(u)}catch(c){r(c)}},a.onerror=o=>r(o),a.readAsArrayBuffer(t)})}function L(t,e){try{let r=XLSX.utils.json_to_sheet(t),a=XLSX.utils.book_new();XLSX.utils.book_append_sheet(a,r,"Data"),XLSX.writeFile(a,`${e}.xlsx`)}catch(r){throw console.error("Error generating Excel file:",r),r}}function Ke(t){return`
  ...
  `;function B(){if(!document.getElementById("bulk-toolbar"))return;let y=document.getElementById("bulk-count"),$=document.getElementById("btn-bulk-delete"),P=document.getElementById("btn-bulk-cancel");y.textContent=`${w.size} item dipilih`,w.size>0?($.disabled=!1,P.disabled=!1):($.disabled=!0,P.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{w.clear(),document.querySelectorAll(".row-checkbox").forEach(y=>y.checked=!1);let k=document.getElementById("select-all-checkbox");k&&(k.checked=!1),B()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(w.size===0)return;let k=[...w],y=document.createElement("div");y.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",y.innerHTML=`
    ...
    `,document.body.appendChild(y),y.querySelector("#bulk-cancel-btn").addEventListener("click",()=>y.remove()),y.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let $=y.querySelector("#bulk-confirm-btn");$.disabled=!0,$.textContent="Menghapus...";let P=await x(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:k})});y.remove(),P.ok?(Z(`${k.length} ${n} berhasil dihapus.`),w.clear(),B(),M()):G(P.data?.error||"Gagal menghapus data.")})});let E=document.getElementById("filter-search"),q;if(E?.addEventListener("input",k=>{clearTimeout(q),q=setTimeout(()=>{S.search=k.target.value,_=1,w.clear(),M()},400)}),s?.forEach(k=>{k.type==="select"&&document.getElementById(`filter-${k.name}`)?.addEventListener("change",y=>{S[k.name]=y.target.value,_=1,w.clear(),M()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{S={...i},E&&(E.value=""),s?.forEach(k=>{let y=document.getElementById(`filter-${k.name}`);y&&(y.value="")}),_=1,w.clear(),M()}),document.getElementById("btn-create")?.addEventListener("click",()=>re(null)),f){document.getElementById(`btn-export-${f.moduleName}`)?.addEventListener("click",async y=>{let $=y.target,P=$.innerHTML;$.innerHTML="\u23F3 Loading...",$.disabled=!0;try{await f.onExport()}catch{G("Gagal export data")}finally{$.innerHTML=P,$.disabled=!1}}),document.getElementById(`btn-template-${f.moduleName}`)?.addEventListener("click",()=>{f.onTemplate()});let k=document.getElementById(`input-import-${f.moduleName}`);k?.addEventListener("change",async y=>{let $=y.target.files[0];if(!$)return;k.disabled=!0;let P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",P.innerHTML=`
      ...
      `,document.body.appendChild(P);let W=P.querySelector("#import-progress-text"),U=P.querySelector("#import-progress-bar"),D=P.querySelector("#import-summary"),I=P.querySelector("#import-close-btn");I.addEventListener("click",()=>{P.remove(),M()});try{let Y=await Re($);if(Y.length===0)throw new Error("File kosong atau format salah");let X=500,ie=0,ae=0,N=0,F=Y.length;W.textContent=`Ditemukan ${F} baris data. Memulai import...`;for(let C=0;C<F;C+=X){let Q=Y.slice(C,C+X);W.textContent=`Mengimport baris ${C+1} - ${Math.min(C+X,F)} dari ${F}...`,U.style.width=`${Math.round(C/F*100)}%`;try{let H=await f.onImport(Q);H?(ie+=H.inserted||H.metrics?.inserted||Q.length,ae+=H.skipped||H.metrics?.updated||0):ie+=Q.length}catch(H){console.error("Chunk import failed:",H),N+=Q.length}}U.style.width="100%",W.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',D.style.display="block",D.innerHTML=`
  ...
  </div>`).join("");function Ct(t=3){return Array(t).fill(0).map((e,r)=>`<div class="skeleton skeleton-text" style="height:38px;margin-bottom:${r<t-1?"6px":"0"};border-radius:6px"></div>`).join("")}async function se(t,e,r=8e3){try{let a=new AbortController,o=setTimeout(()=>a.abort(),r),c=await x(t,{signal:a.signal}).catch(()=>null);if(clearTimeout(o),!c||!c.ok)return e;let s=c.data;return s?s.data!==void 0?s.data??e:s:e}catch{return e}}function ka(){["skel-donut","skel-trend","skel-insp","skel-contract","skel-schbar"].forEach(a=>{let o=document.getElementById(a);o&&(o.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract","chart-schbar"].forEach(a=>{let o=document.getElementById(a);if(o&&o.style.display==="none"){o.style.display="block";let c=o.parentElement;if(c&&!c.querySelector(".chart-empty")){let s=document.createElement("div");s.className="chart-empty",s.textContent="Belum ada data",o.style.display="none",c.appendChild(s)}}});let t=document.getElementById("kpi-row");t&&t.querySelector(".skeleton")&&$t({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&Dt({}),["table-contracts","table-issues"].forEach(a=>{let o=document.getElementById(a);o&&o.querySelector(".skeleton")&&(o.innerHTML='<div class="chart-empty">Belum ada data</div>')});let r=document.getElementById("activity-log");r&&r.querySelector(".skeleton")&&(r.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function Et(t){ua(),t._dashRefresh&&clearInterval(t._dashRefresh),t._skelTimeout&&clearTimeout(t._skelTimeout);let e=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t.innerHTML=`
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>mt(t)),document.getElementById("filter-insp-month")?.addEventListener("change",async r=>{let a=r.target.value,o=a?`/api/dashboard/inspection-bar?month=${a}`:"/api/dashboard/inspection-bar",c=document.getElementById("skel-insp"),s=document.getElementById("chart-insp");c&&(c.style.display="block",c.style.position="absolute"),s&&(s.style.display="none");let i=await se(o,{},8e3);try{Pt(i)}catch(n){console.warn("InspBar render:",n),me("skel-insp","chart-insp")}}),document.getElementById("filter-sch-year")?.addEventListener("change",async r=>{let a=r.target.value;document.getElementById("sch-year-label").textContent=a;let o=`/api/dashboard/schedule-monthly?year=${a}`,c=document.getElementById("skel-schbar"),s=document.getElementById("chart-schbar");c&&(c.style.display="block",c.style.position="absolute"),s&&(s.style.display="none");let i=await se(o,{data:[]},8e3);try{It(i)}catch(n){console.warn("SchBar render:",n),me("skel-schbar","chart-schbar")}}),t._skelTimeout=setTimeout(()=>ka(),5e3),await mt(t),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?mt(t):clearInterval(t._dashRefresh)},6e4)}async function mt(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let[e,r,a,o,c,s,i,n,d,m,l,p]=await Promise.all([se("/api/dashboard/kpi",{},8e3),se("/api/dashboard/issues-trend",{},8e3),se("/api/dashboard/issues-summary",{},8e3),se("/api/dashboard/inspection-bar",{},8e3),se("/api/dashboard/stats",{},8e3),se("/api/dashboard/calendar",[],8e3),se("/api/schedule?limit=10000",{data:[]},8e3),se("/api/employees?limit=10000",{data:[]},8e3),se("/api/contracts?limit=10000",{data:[]},8e3),se("/api/issues?limit=10000",{data:[]},8e3),se("/api/one-on-one?limit=10000",{data:[]},8e3),se("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3)]);if(e){let u=Array.isArray(i?.data)?i.data:Array.isArray(i)?i:[],b=Array.isArray(n?.data)?n.data:Array.isArray(n)?n:[],g=Array.isArray(d?.data)?d.data:Array.isArray(d)?d:[],h=Array.isArray(m?.data)?m.data:Array.isArray(m)?m:[],f=Array.isArray(l?.data)?l.data:Array.isArray(l)?l:[];e.employees&&(e.employees.current=b.filter(v=>St(v,"active")).length),e.contracts&&(e.contracts.current=g.filter(v=>dt(v,"active")).length),e.expiring30&&(e.expiring30={current:g.filter(v=>dt(v,"expiring30")).length}),e.issues&&(e.issues.current=h.filter(v=>wt(v,"open")).length),e.one_on_one&&(e.one_on_one.current=f.filter(v=>_t(v,"pending")).length),e.inspection_month&&(e.inspection_month.current=u.filter(v=>ct(v,"inspeksi")).length),e.cleaning_month&&(e.cleaning_month.current=u.filter(v=>ct(v,"gcdc")).length)}try{$t(e)}catch(u){console.warn("KPI render:",u)}try{Dt(e)}catch(u){console.warn("MiniStats render:",u)}try{xa(Array.isArray(a?.by_category)?a.by_category:[])}catch(u){console.warn("Donut render:",u),me("skel-donut","chart-donut")}try{It(schMonthly)}catch(u){console.warn("SchBar render:",u),me("skel-schbar","chart-schbar")}try{Sa(r)}catch(u){console.warn("Trend render:",u),me("skel-trend","chart-trend")}try{Pt(o)}catch(u){console.warn("InspBar render:",u),me("skel-insp","chart-insp")}try{let u=Array.isArray(c)?c:Array.isArray(c?.recent_issues)?c.recent_issues:[];_a(u)}catch(u){console.warn("IssuesTable render:",u)}try{let u=Array.isArray(c?.expiring_contracts)?c.expiring_contracts:[];wa(p)}catch(u){console.warn("ContractsTable render:",u)}try{Ca(Array.isArray(s)?s:[])}catch(u){console.warn("Agenda render:",u)}try{Ta()}catch(u){console.warn("Quick Actions render:",u)}}function $t(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let r=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=r.map(a=>{let o=be(t[a.key]?.current,0);return`
    ...
    `}).join("");let i={id:"centerText",beforeDraw:function(n){let d=n.width,m=n.height,l=n.ctx;l.restore();let p=(m/80).toFixed(2);l.font="bold "+p+"em Inter",l.textBaseline="middle",l.fillStyle="#1E293B";let u=s.toString(),b=Math.round((d-l.measureText(u).width)/2),g=m/2;l.fillText(u,b,g-10),l.font="600 "+(p*.35).toFixed(2)+"em Inter",l.fillStyle="#64748B";let h="Total",f=Math.round((d-l.measureText(h).width)/2);l.fillText(h,f,g+15),l.save()}};he.donut=new Chart(e,{type:"doughnut",data:{labels:o,datasets:[{data:c,backgroundColor:pt,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:pe,titleFont:{...pe,weight:"700"},callbacks:{label:n=>` ${n.label}: ${n.parsed} kasus`}}},cutout:"75%"},plugins:[i]})}function It(t){me("skel-schbar","chart-schbar");let e=document.getElementById("chart-schbar");if(!e)return;if(Ee("schbar"),!t||!t.data||!t.data.length){He(e,"Belum ada jadwal pada tahun ini.");return}let r=t.data,o=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],c={};r.forEach(d=>{let m=d.activity_type||"Lainnya";m.toLowerCase()==="inspeksi"&&(m="Inspeksi"),(m.toLowerCase()==="gcdc"||m.toLowerCase().includes("general cleaning"))&&(m="General Cleaning"),m.toLowerCase()==="deep cleaning"&&(m="Deep Cleaning"),m.toLowerCase()==="fogging"&&(m="Fogging"),c[m]||(c[m]=Array(12).fill(0));let l=parseInt(d.month,10)-1;l>=0&&l<12&&(c[m][l]+=d.c||0)});let s={Inspeksi:"#4F8FA8","General Cleaning":"#D8A657","Deep Cleaning":"#5E9F77",Fogging:"#B76A5C"},i=d=>{let m=0;for(let l=0;l<d.length;l++)m=d.charCodeAt(l)+((m<<5)-m);return`hsl(${Math.abs(m)%360}, 65%, 60%)`},n=Object.keys(c).map(d=>({label:d,data:c[d],backgroundColor:s[d]||i(d),borderWidth:0,borderRadius:4}));he.schbar=new Chart(e,{type:"bar",data:{labels:o,datasets:n},options:Xe({responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"bottom",labels:{boxWidth:12,usePointStyle:!0,padding:20,font:{family:"Inter",size:12}}},tooltip:{callbacks:{title:d=>{let m=d[0].label;return{Jan:"Januari",Feb:"Februari",Mar:"Maret",Apr:"April",Mei:"Mei",Jun:"Juni",Jul:"Juli",Agu:"Agustus",Sep:"September",Okt:"Oktober",Nov:"November",Des:"Desember"}[m]||m},label:d=>` ${d.dataset.label}: ${d.raw} Jadwal`}}},scales:{x:{stacked:!0,grid:{display:!1}},y:{stacked:!0,border:{display:!1},grid:{color:"#f3f4f6",drawTicks:!1}}}})})}function Sa(t){me("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;Ee("trend"),t=t||{};let r=(t.labels||[]).map(ga),a=(t.open||[]).map(c=>be(c)),o=(t.closed||[]).map(c=>be(c));if(!r.length){He(e,"Belum ada data trend");return}he.trend=new Chart(e,{type:"line",data:{labels:r,datasets:[{label:"Open",data:a,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:o,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#10B981",borderWidth:2}]},options:Xe({plugins:{legend:{display:!1}}})})}function Pt(t){me("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;Ee("inspBar"),t=t||{};let r=t.labels||[],a=(t.fc||[]).map(c=>be(c)),o=(t.spv||[]).map(c=>be(c));if(!r.length){He(e,"Belum ada data inspeksi");return}he.inspBar=new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:o,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:Xe({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:Te,maxRotation:45,minRotation:30}},y:{grid:{color:Ye},ticks:{font:pe,color:Te},min:0,max:100}}})})}function wa(t){me("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;Ee("contractMiniBar"),t=t||{};let r={"06":"Jun","07":"Jul","08":"Agu","09":"Sep",10:"Okt",11:"Nov",12:"Des"},a=(t.labels||[]).map(s=>{let i=s.split("-")[1];return r[i]||s}),o=(t.data||[]).map(s=>be(s));if(!a.length){He(e,"Belum ada data");return}let c=e.getContext("2d");he.contractMiniBar=new Chart(e,{type:"bar",data:{labels:a,datasets:[{label:"Kontrak Habis",data:o,backgroundColor:"#3B82F6",borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:Xe({onClick:(s,i)=>{if(i&&i.length>0){let n=i[0].index,d=(t.labels||[])[n];d&&(window.location.hash="#/contracts?month_expiry="+d)}},plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:Te,maxRotation:0,autoSkip:!1}},y:{grid:{color:Ye,borderDash:[4,4],drawBorder:!1},ticks:{font:pe,color:Te,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function _a(t){let e=document.getElementById("table-issues");if(!e)return;let r=(t||[]).slice(0,8);if(!r.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${r.map(a=>`
    ...
    <div style="display:flex;flex-direction:column;gap:12px;padding-right:8px">
      ${c.map(s=>{let i="#3B82F6",n="#EFF6FF",d="Agenda",m=(s.title||"").toLowerCase();return m.includes("inspeksi")?(i="#10B981",n="#ECFDF5",d="Inspeksi"):m.includes("cleaning")||m.includes("gcdc")?(i="#3B82F6",n="#EFF6FF",d="Cleaning"):m.includes("reliefer")?(i="#F59E0B",n="#FFFBEB",d="Reliefer"):m.includes("fogging")&&(i="#8B5CF6",n="#F5F3FF",d="Fogging"),`
        <div style="display:flex;gap:12px;align-items:flex-start;padding-bottom:12px;border-bottom:1px solid var(--border)">
          ...
          <div style="flex-shrink:0;font-size:0.7rem;font-weight:600;padding:2px 8px;border-radius:6px;background:${n};color:${i}">${d}</div>
        </div>
      `}).join("")}
    </div>
...
`)}let S=d(v,f),w=null;return S||(w="Karyawan tidak ditemukan di Database"),{isValid:!!S,invalidReason:w,rowNum:f,data:{employee_id:S,branch_id:n(String(g.Cabang||"").trim()),division:String(g["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:_,end_date:m(g["Tanggal Selesai"])||"2099-12-31",status:String(g.Status||"").trim(),_rawName:v}}}),p=[],u=[];if(l.forEach(g=>{g.isValid?p.push(g.data):u.push({rowNum:g.rowNum,name:g.data._rawName,reason:g.invalidReason})}),console.log(`Split Validation - Valid: ${p.length}, Invalid: ${u.length}`),p.length===0)return{inserted:0,skipped:a.length,failed:a.length};let b=await x("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:p,onDuplicate:"update"})});if(!b.ok)throw new Error(b.data?.error||"Import gagal");return b.data}}})}O();R();var bt=[],qe=[];function Pa(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let r of e)if(t.some(a=>a.period===r))return r;return"Q3"}function Ba(t,e){if(t.period!=="Q3")return!1;let r=String(t.status||"").toLowerCase();if(r!=="selesai"&&r!=="completed"&&r!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}async function Ft(t,e){bt=await K();let r=await ee(),o=(await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`)).data?.data||[],c=new Set;o.forEach(l=>{l.pic&&l.pic.trim()&&c.add(l.pic.trim())}),qe=Array.from(c).sort();let s=l=>l&&!qe.find(p=>(typeof p=="object"?p.value:p)===l)?[...qe,l]:qe,i=l=>{if(!l||l==="-"||String(l).trim()==="")return"";let p=String(l).split("-");return p.length===3&&p[0].length===4?`${p[2]}-${p[1]}-${p[0]}`:l},d=Pa(o),m=e?e.get("dash_filter"):null;A({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:m?{period:"Q3"}:{period:d},onDataLoaded:l=>(m&&(l=l.filter(p=>Ba(p,m))),l.sort((p,u)=>{let b=p.opening_date?new Date(p.opening_date).getTime():0;return(u.opening_date?new Date(u.opening_date).getTime():0)-b})),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:l=>lt(l)},{key:"period",label:"Periode",render:l=>de(l)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:l=>i(l)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:l=>i(l)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:l=>i(l)},{key:"status",label:"Status",render:l=>j(l)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"activity_type",label:"Kegiatan",options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:qe}],formFields:l=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:l?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:l?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:l?.period},{name:"pic",label:"PIC",type:"combobox",options:s(l?.pic),value:l?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:l?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:l?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:l?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:l?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:l?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let l=await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(l.ok){let p=l.data.data.map(u=>({Cabang:u.branch_name||"",Kegiatan:u.activity_type||"",Periode:u.period||"",PIC:u.pic||"","Tgl Opening":u.opening_date||"","Tgl Target":u.target_date||"","Tgl Selesai":u.completion_date||"",Status:u.status||""}));L(p,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async l=>{let u=(await x("/api/branches?all=1")).data?.data||[],b=v=>{if(!v)return null;let T=String(v||"").toLowerCase(),_=u.find(S=>String(S.full_name||"").toLowerCase()===T||String(S.code||"").toLowerCase()===T||String(S.name||"").toLowerCase()===T);return _?_.id:null},g=v=>{if(v==null||v==="")return"";if(v instanceof Date&&!isNaN(v.getTime()))return v.toISOString().slice(0,10);let T=String(v).trim();if(T===""||T==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(T))return T.slice(0,10);if(/^\d{4,5}$/.test(T)){let S=Number(T);if(S>2e4&&S<99999){let w=new Date(Date.UTC(1899,11,30)+S*864e5);return isNaN(w.getTime())?"":w.toISOString().slice(0,10)}}let _=T.split(/[\/\-\.]/);if(_.length===3){let[S,w,B]=_.map(E=>E.trim());if(S.length===4&&w.length<=2&&B.length<=2)return`${S}-${w.padStart(2,"0")}-${B.padStart(2,"0")}`;if(B.length===4&&w.length<=2&&S.length<=2)return`${B}-${w.padStart(2,"0")}-${S.padStart(2,"0")}`}return T},h=l.map(v=>({branch_id:b(String(v.Cabang||"").trim()),activity_type:String(v.Kegiatan||"").trim(),period:String(v.Periode||"").trim(),pic:String(v.PIC||v.Pic||"").trim(),opening_date:g(v["Tgl Opening"]||v["Tanggal Opening"]||v["Tgl Openir"]),target_date:g(v["Tgl Target"]||v["Tanggal Target"]),completion_date:g(v["Tgl Selesai"]||v["Tanggal Selesai"]),status:String(v.Status||"").trim(),notes:String(v.Catatan||v.Keterangan||"").trim()})).filter(v=>v.activity_type&&v.period),f=await x("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!f.ok)throw new Error(f.data?.error||"Import gagal");return f.data}}})}O();R();var ht=[],Ze=[];function La(t,e){let r=String(t.status||"").toLowerCase();return e==="open"?r==="open":!1}async function Mt(t,e){let r=e?e.get("dash_filter"):null;ht=await K(),Ze=await ee();let a=s=>s&&!Ze.find(i=>i.value===s)?[...Ze,{value:s,label:s}]:Ze,o=new Date().getFullYear();A({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:s=>r?s.filter(i=>La(i,r)):s,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:s=>window.formatDate(s)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:s=>`<span class="badge badge-secondary">${s}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:s=>`<span title="${s}">${s?.length>50?s.slice(0,50)+"\u2026":s}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:s=>`<span title="${s||""}">${s?.length>40?s.slice(0,40)+"\u2026":s||"-"}</span>`},{key:"status",label:"Status",render:s=>j(s)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:s=>window.formatDate(s)},{key:"day_count",label:"Hari",render:s=>s??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:ht},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:s=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:s?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:ht,value:s?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:s?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:s?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:s?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(s?.employee_name),value:s?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(s?.fc_specialist),value:s?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:s?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:s?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:s?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let s=await x(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(s.ok){let i=s.data.data.map(n=>({Tanggal:n.report_date||"",Cabang:n.branch_name||"",Kategori:n.category||"",Sumber:n.source||"",Keluhan:n.complaint||"","Nama FC":n.employee_name||"","FC Spesialis":n.fc_specialist||"",Solusi:n.solution||"","Tgl Selesai":n.completion_date||"",Status:n.status||""}));L(i,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async s=>{let n=(await x("/api/branches?all=1")).data?.data||[],d=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),b=n.find(g=>String(g.full_name||"").toLowerCase()===u||String(g.code||"").toLowerCase()===u||String(g.name||"").toLowerCase()===u);return b?b.id:null},m=s.map(p=>({branch_id:d(String(p.Cabang||"").trim()),report_date:String(p.Tanggal||"").trim(),category:String(p.Kategori||"").trim(),source:String(p.Sumber||"").trim(),complaint:String(p.Keluhan||"").trim(),employee_name:String(p["Nama FC"]||"").trim(),fc_specialist:String(p["FC Spesialis"]||"").trim(),solution:String(p.Solusi||"").trim(),completion_date:String(p["Tgl Selesai"]||"").trim(),status:String(p.Status||"").trim()})).filter(p=>p.report_date&&p.complaint&&p.category),l=await x("/api/import/issues",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}}})}O();var $e=[];function Aa(t,e){let r=String(t.status||"").toLowerCase();return e==="pending"?r==="pending":!1}async function Ot(t,e){let r=e?e.get("dash_filter"):null;$e=await K();let a=await ee(),o=["Ade","Berlin"],c=i=>i&&!a.find(n=>n.value===i)?[...a,{value:i,label:i}]:a,s=i=>i&&!o.find(n=>(typeof n=="object"?n.value:n)===i)?[...o,i]:o;A({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:i=>r?i.filter(n=>Aa(n,r)):i,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:i=>`<span title="${i||""}">${i?.length>50?i.slice(0,50)+"\u2026":i||"-"}</span>`},{key:"solution",label:"Solusi",render:i=>`<span title="${i||""}">${i?.length>40?i.slice(0,40)+"\u2026":i||"-"}</span>`},{key:"status",label:"Status",render:i=>j(i)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:i=>window.formatDate(i)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:i=>i?`<a href="${i}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:$e},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async i=>{let n=new URLSearchParams(i||{}).toString(),d=await x(`/api/one-on-one?limit=10000&${n}`);if(d.ok){let m=d.data.data.map(p=>({Tanggal:p.meeting_date||"",Cabang:p.branch_name||"","Nama Karyawan":p.employee_name||"",PIC:p.pic||"",Masalah:p.problem||"",Solusi:p.solution||"",Status:p.status||"","Tgl Selesai":p.completion_date||"",Dokumen:p.document_link||""})),{downloadExcel:l}=await Promise.resolve().then(()=>(R(),oe));l(m,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let i=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(R(),oe));n(i,"Template_Import_OneOnOne")},onImport:async i=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),b=$e.find(g=>String(g.label||"").toLowerCase()===u);return b?b.value:null},d=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let h=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(h.getTime())?"":h.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let b=u.split(/[\/\-\.]/);if(b.length===3){let[g,h,f]=b.map(v=>v.trim());if(g.length===4&&h.length<=2&&f.length<=2)return`${g}-${h.padStart(2,"0")}-${f.padStart(2,"0")}`;if(f.length===4&&h.length<=2&&g.length<=2)return`${f}-${h.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=i.map(p=>({meeting_date:d(p.Tanggal),employee_name:String(p["Nama Karyawan"]||"").trim(),branch_id:n(String(p.Cabang||"").trim()),pic:String(p.PIC||"").trim(),problem:String(p.Masalah||"").trim(),solution:String(p.Solusi||"").trim(),status:String(p.Status||"").trim(),completion_date:d(p["Tgl Selesai"]),document_link:String(p.Dokumen||"").trim()})).filter(p=>p.meeting_date&&p.employee_name&&p.branch_id),l=await x("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}},formFields:i=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:i?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:i?.branch_id&&!$e.find(n=>n.value==i.branch_id)?[...$e,{value:i.branch_id,label:i.branch_name||i.branch_id}]:$e,createApi:{path:"/api/branches",field:"full_name"},value:i?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:c(i?.employee_name),value:i?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:s(i?.pic),createApi:{path:"/api/pic",field:"name"},value:i?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:i?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:i?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:i?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:i?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:i?.document_link}]})}O();async function Rt(t){let e=await K(),r=await ee(),a=["Ade","Berlin"],o=i=>i&&!r.find(n=>n.value===i)?[...r,{value:i,label:i}]:r,c=i=>i&&!a.find(n=>(typeof n=="object"?n.value:n)===i)?[...a,i]:a,s=Array.from({length:5},(i,n)=>String(new Date().getFullYear()-n));A({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:i=>{try{let n=JSON.parse(i);return Array.isArray(n)?n.join(", "):i||"-"}catch{return i||"-"}}},{key:"score",label:"Nilai",render:i=>i!=null?`<strong>${i}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:i=>i?`<a href="${i}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:s}],exportOptions:{moduleName:"training",onExport:async i=>{let n=new URLSearchParams(i||{}).toString(),d=await x(`/api/training?limit=10000&${n}`);if(d.ok){let m=d.data.data.map(p=>{let u=p.participants||"";try{let b=JSON.parse(u);u=Array.isArray(b)?b.join(", "):u}catch{}return{Tanggal:p.training_date||"",Batch:p.batch||"",Materi:p.subject||"",Cabang:p.branch_name||"",Trainer:p.trainer||"",Peserta:u,Nilai:p.score!==null&&p.score!==void 0?p.score:"",Dokumen:p.document_link||""}}),{downloadExcel:l}=await Promise.resolve().then(()=>(R(),oe));l(m,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let i=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(R(),oe));n(i,"Template_Import_Training")},onImport:async i=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),b=e.find(g=>String(g.label||"").toLowerCase()===u);return b?b.value:null},d=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let h=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(h.getTime())?"":h.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let b=u.split(/[\/\-\.]/);if(b.length===3){let[g,h,f]=b.map(v=>v.trim());if(g.length===4&&h.length<=2&&f.length<=2)return`${g}-${h.padStart(2,"0")}-${f.padStart(2,"0")}`;if(f.length===4&&h.length<=2&&g.length<=2)return`${f}-${h.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=i.map(p=>({training_date:d(p.Tanggal),batch:String(p.Batch||"").trim(),subject:String(p.Materi||"").trim(),branch_id:n(String(p.Cabang||"").trim()),trainer:String(p.Trainer||"").trim(),participants:String(p.Peserta||"").trim(),score:p.Nilai?Number(p.Nilai):null,document_link:String(p.Dokumen||"").trim()})).filter(p=>p.training_date&&p.subject&&p.branch_id),l=await x("/api/import/training",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}},formFields:i=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:i?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:i?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:i?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:i?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:c(i?.trainer),value:i?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let n=JSON.parse(i?.participants);return Array.isArray(n)?n.join(", "):i?.participants||""}catch{return i?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:i?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:i?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:i?.notes}],onBeforeSubmit:async i=>(i.participants&&(i.participants=JSON.stringify(i.participants.split(",").map(n=>n.trim()).filter(Boolean))),i)})}O();ve();R();function Kt({container:t,title:e,icon:r,apiPath:a,columns:o,formFields:c,filterFields:s,defaultFilters:i={},itemLabel:n="Data",canCreate:d=!0,canEdit:m=!0,canDelete:l=!0,onBeforeSubmit:p,onAfterLoad:u,onDataLoaded:b,extraActions:g=[],initialSearch:h="",exportOptions:f=null,bulkDelete:v=!1,paginationMode:T="server"}){let _=1,S={...i};h&&(S.search=h);let w=new Set;t.innerHTML=`
  ...
  `,t.querySelectorAll(".cal-legend-pill").forEach(i=>{let n=i.querySelector('input[type="checkbox"]');i.addEventListener("click",d=>{d.preventDefault(),n.checked=!n.checked,i.classList.toggle("inactive",!n.checked),c()})}),document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),c()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),c()});async function o(){try{let i=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0");r=(await x(`/api/dashboard/calendar?month=${i}-${n}`)).data?.data||[]}catch(i){console.warn("[Calendar] Gagal memuat events:",i),r=[]}}async function c(){let i=document.getElementById("calendar-grid");if(i){i.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:1px;background:#E5E7EB;">
      ${Array(42).fill('<div style="background:#F9FAFB;min-height:90px;"></div>').join("")}
    </div>`,await o();try{let n=e.getFullYear(),d=e.getMonth(),m=document.getElementById("cal-month-label");m&&(m.textContent=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}));let l=new Set(Array.from(t.querySelectorAll(".cal-filter:checked")).map(S=>S.value)),p=r.filter(S=>l.has(S.type)),u={};p.forEach(S=>{let w=(S.event_date||"").slice(0,10);w&&(u[w]||(u[w]=[]),u[w].push(S))});let b=new Date(n,d,1).getDay(),g=new Date(n,d+1,0).getDate(),h=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],f=new Date().toISOString().slice(0,10),v='<div style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:1px;background:#E5E7EB;border-radius:0 0 12px 12px;overflow:hidden;width:100%;">';h.forEach((S,w)=>{v+=`<div style="background:#F1F5F9;padding:10px 4px;text-align:center;font-size:0.68rem;font-weight:800;color:${w===0||w===6?"#EF4444":"#6B7280"};text-transform:uppercase;letter-spacing:0.06em;">${S}</div>`});for(let S=0;S<b;S++)v+='<div style="background:#FAFAFA;min-height:90px;"></div>';for(let S=1;S<=g;S++){let w=`${n}-${String(d+1).padStart(2,"0")}-${String(S).padStart(2,"0")}`,B=u[w]||[],E=w===f,q=new Date(n,d,S).getDay(),M=q===0||q===6,ge=4;v+=`
          <div class="cal-cell" data-date="${w}"
            ...
            <div style="display:flex;flex-direction:column;gap:3px;overflow:hidden;">
              ${B.slice(0,ge).map(re=>{let le=Vt(re.type),k=Ma(re.title||re.branch_name||le.label,18);return`<div class="cal-badge" style="background:${le.bg};" title="${De(re.title||le.label)} \u2014 ${De(re.branch_name||"")}">
                  ${le.icon} ${De(k)}
                </div>`}).join("")}
            </div>
          </div>`}let _=(b+g)%7;if(_!==0)for(let S=0;S<7-_;S++)v+='<div style="background:#FAFAFA;min-height:90px;"></div>';v+="</div>",i.innerHTML=v,i.querySelectorAll(".cal-cell[data-date]").forEach(S=>{S.addEventListener("click",()=>{let w=S.dataset.date,B=u[w]||[];B.length&&s(w,B)})})}catch(n){console.error("[Calendar] Render error:",n),i.innerHTML=`<div style="padding:60px;text-align:center;color:#9CA3AF;">
      ...
      </div>`}}}function s(i,n){document.getElementById("cal-popup-overlay")?.remove();let d=new Date(i+"T00:00:00").toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"}),m=n.map(p=>{let u=Vt(p.type);return`
        <div class="cal-popup-event" style="border-left-color:${u.bg};">
          ...
          <div class="cal-popup-info">
            ...
            <div class="cal-popup-event-title">${De(p.title||"-")}</div>
            ${p.branch_name?`<div class="cal-popup-branch">\u{1F4CD} ${De(p.branch_name)}</div>`:""}
            ${p.status?`<div class="cal-popup-status" style="color:${p.status==="Done"||p.status==="Selesai"?"#10B981":"#F59E0B"};">\u25CF ${De(p.status)}</div>`:""}
            ${p.days_remaining!==void 0?`<div class="cal-popup-sisa">\u23F3 Sisa: ${p.days_remaining} hari</div>`:""}
          </div>
          <!-- Dot warna -->
  ...
  `;let e=null,r=null,a=0,o={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function c(g){Object.entries(o).forEach(([h,f])=>{f.style.display=h===g?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let g=document.getElementById("btn-backup-db");g.disabled=!0,g.textContent="\u23F3 Memproses Backup...";try{let h=await x("/api/import/backup");if(h.ok){if(!window.XLSX){G("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");return}let f=window.XLSX,v=f.utils.book_new();Object.entries(h.data.database).forEach(([T,_])=>{let S=_.length>0?_:[{}],w=f.utils.json_to_sheet(S);f.utils.book_append_sheet(v,w,T.substring(0,31))}),f.writeFile(v,`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.xlsx`),Z("Backup berhasil diunduh!")}else G("Gagal memproses backup: "+(h.data?.error||"Unknown error"))}catch(h){G("Gagal memproses backup: "+h.message)}finally{g.disabled=!1,g.textContent="\u{1F4E6} Backup Database"}});let s=document.getElementById("btn-sync-google");s&&s.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let g=s.innerHTML;s.innerHTML='<span class="spinner"></span> Menyinkronkan...',s.disabled=!0;try{let h=await x("/api/sync/google-sheets",{method:"POST"});h.ok?alert("Sinkronisasi Berhasil: "+(h.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(h.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{s.innerHTML=g,s.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{ta(),Z("Template Excel berhasil didownload!")});let i=document.getElementById("file-input"),n=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",g=>{g.stopPropagation(),i.click()}),i.addEventListener("change",g=>{g.target.files[0]&&d(g.target.files[0])}),n.addEventListener("dragover",g=>{g.preventDefault(),n.classList.add("drag-over")}),n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n.addEventListener("drop",g=>{g.preventDefault(),n.classList.remove("drag-over");let h=g.dataTransfer.files[0];h&&h.name.match(/\.xlsx?$/i)?d(h):G("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,i.value="",document.getElementById("file-info").style.display="none",n.style.display="",c("upload")});async function d(g){e=g,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${g.name} (${(g.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",n.style.display="none",await m(g)}async function m(g){c("validating");let h=document.getElementById("validation-status"),f=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");h.textContent="Membaca file Excel...",f.style.width="20%",await je(200);let v=await g.arrayBuffer(),T=window.XLSX.read(v,{type:"array",cellDates:!0});h.textContent=`Memvalidasi ${T.SheetNames.length} sheet...`,f.style.width="50%",await je(100),r=ea(T),f.style.width="100%",h.textContent="Validasi selesai!",await je(300),l()}catch(v){c("upload"),G("Gagal memproses file: "+v.message),document.getElementById("file-info").style.display="flex",n.style.display="none"}}function l(){c("preview");let g=r.filter(E=>!E.skipped).length,h=r.reduce((E,q)=>E+q.total,0),f=r.reduce((E,q)=>E+q.valid,0),v=r.reduce((E,q)=>E+q.errorCount,0),T=h>0?Math.round(f/h*100):0;document.getElementById("preview-summary-badges").innerHTML=`
    ...
    `,_.querySelectorAll(".btn-detail-error").forEach(E=>{E.addEventListener("click",()=>{let q=r[Number(E.dataset.idx)];p(q)})});let S=document.getElementById("error-detail-section"),w=document.getElementById("error-detail-container");w.innerHTML="",S.style.display="none";let B=document.getElementById("btn-start-import");f===0?(B.disabled=!0,B.innerHTML="\u26A0\uFE0F Tidak Ada Data Valid"):(B.disabled=!1,v>0?(B.innerHTML=`\u{1F680} Import ${f} Data Valid (${v} dilewati)`,B.title="Baris error akan dilewati, baris valid tetap diimport"):B.innerHTML=`\u{1F680} Mulai Import ${f} Data`)}function p(g){let h=document.getElementById("error-detail-section"),f=document.getElementById("error-detail-container");h.style.display="";let v=g.errors.slice(0,100).map(T=>(Array.isArray(T.errors)?T.errors:[]).map(S=>{let w=typeof S=="object";return`
          <tr>
  ...
  `;let e=document.getElementById("btnRefreshSync"),r=document.getElementById("queueStatusFilter");e.addEventListener("click",o),r.addEventListener("change",i),document.getElementById("btnRetryAll").addEventListener("click",()=>a("retry",{allFailed:!0})),document.getElementById("btnResetStuck").addEventListener("click",()=>a("reset-stuck",{})),document.getElementById("btnPauseSync").addEventListener("click",()=>a("pause",{})),document.getElementById("btnResumeSync").addEventListener("click",()=>a("resume",{})),document.getElementById("btnReconcile").addEventListener("click",()=>a("reconcile",{module:"ALL",repairMode:!0})),document.getElementById("btnRetrySelected").addEventListener("click",()=>{let d=Array.from(document.querySelectorAll(".chk-queue:checked")).map(m=>m.value);if(d.length===0)return alert("No items selected");a("retry",{ids:d})}),document.getElementById("chkAllQueue").addEventListener("change",d=>{document.querySelectorAll(".chk-queue").forEach(m=>m.checked=d.target.checked)});async function a(d,m){if(confirm(`Are you sure you want to execute action: ${d}?`)){showLoading();try{let l=await x(`/api/sync/actions/${d}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)});l.ok?(alert(l.data?.message||"Success"),o()):G(l.error||"Action failed")}catch(l){G(l.message)}hideLoading()}}await o();async function o(){showLoading(),await Promise.all([s(),i(),c(),n()]),hideLoading()}async function c(){try{let d=await x("/api/sync/performance");if(!d.ok)return;let{webhook:m,google_api:l,d1:p,queue:u,throughput:b}=d.data;document.getElementById("latencyHistograms").innerHTML=`
      ...
      `).join("")}catch(d){console.error(d)}}async function n(){try{let d=await x("/api/sync/metrics");if(!d.ok)return;let m=document.getElementById("metricsTableBody"),l=d.data||[];if(l.length===0){m.innerHTML='<tr><td colspan="5" class="px-4 py-4 text-center text-gray-500">No metrics found in last 24h</td></tr>';return}m.innerHTML=l.map(p=>`
        <tr class="hover:bg-gray-50">
