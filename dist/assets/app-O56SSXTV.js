var ca=Object.defineProperty;var it=(t,e)=>()=>(t&&(e=t(t=0)),e);var rt=(t,e)=>{for(var s in e)ca(t,s,{get:e[s],enumerable:!0})};var ke={};rt(ke,{API:()=>ft,CLIENT_SIDE_MAX_ROWS:()=>ye,IS_DEVELOPMENT:()=>Le,apiFetch:()=>x,clearToken:()=>Ne,getToken:()=>Ae,getUser:()=>fe,setToken:()=>st,setUser:()=>Fe});function Ae(){return localStorage.getItem("fm_token")}function st(t){localStorage.setItem("fm_token",t)}function Ne(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function fe(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Fe(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function x(t,e={}){let s=Ae(),a={"Content-Type":"application/json",...s?{Authorization:`Bearer ${s}`}:{},...e.headers||{}};try{let o=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",r=`${ft}${t}${d}${o}`,i=await fetch(r,{...e,headers:a}),n;try{let p=await i.text();try{n=JSON.parse(p)}catch{n={error:`Server Error (${i.status}): ${p.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}if(i.status===401&&(Ne(),window.location.hash="#/login"),i.ok){let p=(e.method||"GET").toUpperCase();["POST","PUT","DELETE","PATCH"].includes(p)&&window.dispatchEvent(new CustomEvent("fcms-data-changed"))}return{ok:i.ok,status:i.status,data:n}}catch(o){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${o.message})`}}}}var Le,pa,ft,ye,O=it(()=>{Le=!1,pa="https://fm-operations-api.facilitycare-audydental.workers.dev",ft=pa,ye=1e4});var xt={};rt(xt,{confirmDialog:()=>Oe,createModal:()=>de});function de({title:t,content:e,onConfirm:s,onCancel:a,confirmText:o="Simpan",cancelText:d="Batal",size:r="md",confirmClass:i="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},p=document.createElement("div");p.className="modal-overlay",p.innerHTML=`
    <div class="modal" style="max-width:${n[r]||n.md}">
      ...
      <div class="modal-footer">
        ...
        ${s?`<button class="btn ${i} modal-confirm">${o}</button>`:""}
      </div>
    </div>
  `,e instanceof HTMLElement&&p.querySelector(".modal-body").appendChild(e);let m=()=>{p.classList.remove("show"),setTimeout(()=>p.remove(),250)};return p.querySelector(".modal-close").addEventListener("click",()=>{a&&a(),m()}),p.querySelector(".modal-cancel").addEventListener("click",()=>{a&&a(),m()}),s&&p.querySelector(".modal-confirm").addEventListener("click",()=>s(p,m)),p.addEventListener("click",l=>{l.target===p&&(a&&a(),m())}),document.body.appendChild(p),requestAnimationFrame(()=>p.classList.add("show")),{overlay:p,close:m}}function Oe(t,e,s="Konfirmasi"){return de({title:s,content:`<p>${t}</p>`,onConfirm:(a,o)=>{e(),o()},confirmText:"Ya, Lanjutkan",confirmClass:"btn-danger"})}var ve=it(()=>{});var oe={};rt(oe,{downloadExcel:()=>A,parseExcel:()=>Ke,renderExcelButtons:()=>He});function Ke(t){return new Promise((e,s)=>{let a=new FileReader;a.onload=o=>{try{let d=new Uint8Array(o.target.result),r=XLSX.read(d,{type:"array"}),i=r.SheetNames[0],n=r.Sheets[i];console.log("--- START EXCEL PARSING ---"),console.log(`File Name: ${t.name}`),console.log(`File Size: ${(t.size/1024).toFixed(2)} KB`),console.log(`File Type: ${t.type||"unknown"}`),console.log(`Sheets Found: ${r.SheetNames.join(", ")}`),console.log(`Sheet Used: ${i}`);let p=XLSX.utils.decode_range(n["!ref"]||"A1:A1"),m=p.e.r-p.s.r+1,l=p.e.c-p.s.c+1;console.log(`Total Rows (including empty): ${m}`),console.log(`Total Columns: ${l}`);let c=[];for(let b=p.s.c;b<=p.e.c;++b){let g=n[XLSX.utils.encode_cell({c:b,r:p.s.r})];g&&g.v&&c.push(g.v)}console.log(`Headers Found: ${c.join(", ")}`),console.log("---------------------------");let u=XLSX.utils.sheet_to_json(n,{defval:""});Object.defineProperty(u,"__worksheet",{value:n,enumerable:!1}),Object.defineProperty(u,"__headers",{value:c,enumerable:!1}),e(u)}catch(d){s(d)}},a.onerror=o=>s(o),a.readAsArrayBuffer(t)})}function A(t,e){try{let s=XLSX.utils.json_to_sheet(t),a=XLSX.utils.book_new();XLSX.utils.book_append_sheet(a,s,"Data"),XLSX.writeFile(a,`${e}.xlsx`)}catch(s){throw console.error("Error generating Excel file:",s),s}}function He(t){return`
  ...
  `}var R=it(()=>{});O();var ot={},Ge=null;function J(t,e){ot[t]=e}function xe(t){window.location.hash=t}function vt(){async function t(){let e=window.location.hash.replace("#","")||"/dashboard",[s,...a]=e.split("?"),o=ot[s];if(!o){for(let[r,i]of Object.entries(ot))if(r.endsWith("/*")&&s.startsWith(r.slice(0,-2))){o=i;break}}Ge&&(Ge(),Ge=null);let d=document.getElementById("main-content");if(d&&(d.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>'),o){let r=new URLSearchParams(a.join("?")),i=s.split("/").filter(Boolean),n=await o({path:s,params:r,segments:i,main:d});n&&(Ge=n)}else{let r=d||document.getElementById("app");r&&(r.innerHTML='<div class="empty-state"><h2>404 - Halaman tidak ditemukan</h2></div>')}}window.addEventListener("hashchange",t),t()}var Me;function ua(){return Me||(Me=document.createElement("div"),Me.id="toast-container",document.body.appendChild(Me)),Me}function kt(t,e="info",s=3500){let a=ua(),o=document.createElement("div");o.className=`toast toast-${e}`;let d={success:"\u2713",error:"\u2715",warning:"\u26A0",info:"\u2139"};o.innerHTML=`<span class="toast-icon">${d[e]||"\u2139"}</span><span class="toast-msg">${t}</span>`,a.appendChild(o),requestAnimationFrame(()=>o.classList.add("show")),setTimeout(()=>{o.classList.remove("show"),setTimeout(()=>o.remove(),350)},s)}var Z=t=>kt(t,"success"),G=t=>kt(t,"error");ve();O();O();ve();function ze({columns:t,data:e,onEdit:s,onDelete:a,onView:o,actions:d=[],emptyText:r="Tidak ada data",bulkSelect:i=null}){let n=document.createElement("div");if(n.className="table-wrapper",!e||e.length===0)return n.innerHTML=`<div class="empty-state"><p>${r}</p></div>`,n;let p=document.createElement("table");p.className="data-table";let m=document.createElement("thead"),l=document.createElement("tr");if(i){let u=document.createElement("th");u.style.width="40px",u.style.textAlign="center";let b=document.createElement("input");b.type="checkbox",b.id="select-all-checkbox",b.title="Pilih semua",b.addEventListener("change",()=>{e.forEach(g=>{b.checked?i.selectedIds.add(g.id):i.selectedIds.delete(g.id)}),n.querySelectorAll(".row-checkbox").forEach(g=>g.checked=b.checked),i.onToggle()}),u.appendChild(b),l.appendChild(u)}if(t.forEach(u=>{let b=document.createElement("th");b.textContent=u.label,u.width&&(b.style.width=u.width),l.appendChild(b)}),s||a||o||d.length>0){let u=document.createElement("th");u.textContent="Aksi",u.style.width="120px",l.appendChild(u)}m.appendChild(l),p.appendChild(m);let c=document.createElement("tbody");return e.forEach(u=>{let b=document.createElement("tr");if(i){let g=document.createElement("td");g.style.textAlign="center",g.style.width="40px";let h=document.createElement("input");h.type="checkbox",h.className="row-checkbox",h.checked=i.selectedIds.has(u.id),h.addEventListener("change",()=>{if(h.checked)i.selectedIds.add(u.id);else{i.selectedIds.delete(u.id);let v=document.getElementById("select-all-checkbox");v&&(v.checked=!1)}i.onToggle()}),g.appendChild(h),b.appendChild(g)}if(t.forEach(g=>{let h=document.createElement("td");if(g.render){let v=g.render(u[g.key],u);v instanceof HTMLElement?h.appendChild(v):h.innerHTML=v||""}else h.textContent=u[g.key]!==null&&u[g.key]!==void 0&&u[g.key]!==""?u[g.key]:"";g.nowrap&&(h.style.whiteSpace="nowrap"),b.appendChild(h)}),s||a||o||d.length>0){let g=document.createElement("td");g.className="actions-cell";let h=document.createElement("div");if(h.className="btn-group",o){let v=document.createElement("button");v.className="btn btn-xs btn-ghost",v.innerHTML="\u{1F441}",v.title="Lihat",v.addEventListener("click",()=>o(u)),h.appendChild(v)}if(s){let v=document.createElement("button");v.className="btn btn-xs btn-secondary",v.innerHTML="\u270F\uFE0F",v.title="Edit",v.addEventListener("click",()=>s(u)),h.appendChild(v)}d.forEach(v=>{let S=document.createElement("button");S.className=`btn btn-xs ${v.class||"btn-ghost"}`,S.innerHTML=v.icon||v.label,S.title=v.label,S.addEventListener("click",()=>v.handler(u)),h.appendChild(S)}),g.appendChild(h),b.appendChild(g)}c.appendChild(b)}),p.appendChild(c),n.appendChild(p),n}function Qe({page:t,pages:e,total:s,limit:a,onPage:o}){if(e<=1)return null;let d=document.createElement("div");d.className="pagination";let r=document.createElement("span");r.className="pagination-info",r.textContent=`Total: ${s} data`,d.appendChild(r);let i=document.createElement("div");i.className="pagination-btns";let n=(l,c,u=!1,b=!1)=>{let g=document.createElement("button");g.className=`btn btn-sm ${b?"btn-primary":"btn-ghost"} pagination-btn`,g.textContent=l,g.disabled=u,g.addEventListener("click",()=>o(c)),i.appendChild(g)};n("\xAB",1,t===1),n("\u2039",t-1,t===1);let p=Math.max(1,t-2),m=Math.min(e,t+2);for(let l=p;l<=m;l++)n(l,l,!1,l===t);return n("\u203A",t+1,t===e),n("\xBB",e,t===e),d.appendChild(i),d}ve();function Re(t){return t.map(e=>{if(e.type==="hidden")return`<input type="hidden" name="${e.name}" value="${e.value||""}">`;if(e.type==="html")return e.html||"";if(e.type==="row")return`<div class="form-row">${Re(e.fields)}</div>`;let s=e.required?"required":"",a=e.label?`<label class="form-label">${e.label}${e.required?' <span class="required">*</span>':""}</label>`:"",o="";switch(e.type){case"textarea":o=`<textarea name="${e.name}" class="form-control" placeholder="${e.placeholder||""}" ${s} rows="${e.rows||3}">${e.value||""}</textarea>`;break;case"select":let r=(e.options||[]).map(l=>{let c=typeof l=="object"?l.value:l,u=typeof l=="object"?l.label:l,b=e.value==c?"selected":"";return`<option value="${c}" ${b}>${u}</option>`}).join("");o=`<select name="${e.name}" class="form-control" ${s}><option value="">-- Pilih ${e.label||""} --</option>${r}</select>`;break;case"combobox":let i=`dl-${e.name}-${Math.random().toString(36).substring(7)}`,n=(e.options||[]).map(l=>{let c=typeof l=="object"?l.value:l,u=typeof l=="object"?l.label||l.value||"":l||"";return(u==="undefined"||u==="[object Object]"||u==="null")&&(u=""),u?`<option value="${u}"></option>`:""}).join(""),p=e.value||"";if(e.value){let l=(e.options||[]).find(c=>(typeof c=="object"?c.value:c)==e.value);if(l){let c=typeof l=="object"?l.label||l.value||"":l||"";c&&c!=="undefined"&&c!=="[object Object]"&&c!=="null"&&(p=c)}}o=`
          <input type="text" name="${e.name}" list="${i}" class="form-control" value="${p}" placeholder="Pilih atau ketik baru..." ${s} autocomplete="off">
          <datalist id="${i}">${n}</datalist>
        `;break;case"checkbox":o=`<label class="checkbox-label"><input type="checkbox" name="${e.name}" value="1" ${e.value?"checked":""}> ${e.checkLabel||e.label}</label>`;break;case"date":let m=window.parseFlexibleDate&&e.value?window.parseFlexibleDate(e.value):e.value||"";o=`<input type="date" name="${e.name}" class="form-control" value="${m}" ${s}>`;break;case"number":o=`<input type="number" name="${e.name}" class="form-control" value="${e.value||""}" placeholder="${e.placeholder||""}" min="${e.min||""}" max="${e.max||""}" step="${e.step||"1"}" ${s}>`;break;case"email":o=`<input type="email" name="${e.name}" class="form-control" value="${e.value||""}" placeholder="${e.placeholder||""}" ${s}>`;break;case"url":o=`<input type="url" name="${e.name}" class="form-control" value="${e.value||""}" placeholder="${e.placeholder||"https://..."}" ${s}>`;break;default:o=`<input type="${e.type||"text"}" name="${e.name}" class="form-control" value="${e.value||""}" placeholder="${e.placeholder||""}" ${s} autocomplete="off">`}let d=e.hint?`<div class="form-hint">${e.hint}</div>`:"";return`<div class="form-group ${e.class||""}">${a}${o}${d}</div>`}).join("")}function Ye(t){let e={},s=new FormData(t);for(let[a,o]of s.entries())e[a]=o===""?null:o;return t.querySelectorAll("input[type=checkbox]").forEach(a=>{a.checked||(e[a.name]=null)}),e}function We(t,e){e&&Object.entries(e).forEach(([s,a])=>{let o=t.querySelector(`[name="${s}"]`);o&&(o.hasAttribute("list")||(o.type==="checkbox"?o.checked=!!a:o.type==="date"&&a&&window.parseFlexibleDate?o.value=window.parseFlexibleDate(a):o.value=a??""))})}R();function N({container:t,title:e,icon:s,apiPath:a,columns:o,formFields:d,filterFields:r,defaultFilters:i={},itemLabel:n="Data",canCreate:p=!0,canEdit:m=!0,canDelete:l=!0,onBeforeSubmit:c,onAfterLoad:u,onDataLoaded:b,extraActions:g=[],initialSearch:h="",exportOptions:v=null,bulkDelete:S=!1,paginationMode:C="server"}){let _=1,y={...i};h&&(y.search=h);let w=new Set;t.innerHTML=`
    ...
    <div class="filter-bar card" style="padding: 1rem; display:flex; flex-wrap:wrap; gap:1rem; align-items:center;">
        ${r.map(k=>{if(k.type==="search")return`<div class="filter-search" style="flex:1; min-width:120px;"><input type="text" class="form-control" autocomplete="off" placeholder="${k.placeholder||"Cari..."}" id="filter-search" value="${y.search||""}"></div>`;if(k.type==="search-combo"){let f="dl-filter-search",D=(k.options||[]).map(P=>`<option value="${typeof P=="object"?P.label:P}"></option>`).join("");return`<div class="filter-search" style="flex:1; min-width:120px;"><input type="text" list="${f}" class="form-control" autocomplete="off" placeholder="${k.placeholder||"Cari..."}" id="filter-search" value="${y.search||""}"><datalist id="${f}">${D}</datalist></div>`}return k.type==="select"?`<select class="form-control filter-select" style="flex:1; min-width:100px;" name="${k.name}" id="filter-${k.name}"><option value="">-- ${k.label} --</option>${(k.options||[]).map(f=>`<option value="${typeof f=="object"?f.value:f}" ${y[k.name]===(typeof f=="object"?f.value:f)?"selected":""}>${typeof f=="object"?f.label:f}</option>`).join("")}</select>`:""}).join("")}
  ...
  `;function L(){if(!document.getElementById("bulk-toolbar"))return;let f=document.getElementById("bulk-count"),D=document.getElementById("btn-bulk-delete"),P=document.getElementById("btn-bulk-cancel");f.textContent=`${w.size} item dipilih`,w.size>0?(D.disabled=!1,P.disabled=!1):(D.disabled=!0,P.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{w.clear(),document.querySelectorAll(".row-checkbox").forEach(f=>f.checked=!1);let k=document.getElementById("select-all-checkbox");k&&(k.checked=!1),L()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(w.size===0)return;let k=[...w],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",f.innerHTML=`
    ...
    `,document.body.appendChild(f),f.querySelector("#bulk-cancel-btn").addEventListener("click",()=>f.remove()),f.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let D=f.querySelector("#bulk-confirm-btn");D.disabled=!0,D.textContent="Menghapus...";let P=await x(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:k})});f.remove(),P.ok?(Z(`${k.length} ${n} berhasil dihapus.`),w.clear(),L(),$()):G(P.data?.error||"Gagal menghapus data.")})});let E=document.getElementById("filter-search"),H;if(E?.addEventListener("input",k=>{clearTimeout(H),H=setTimeout(()=>{y.search=k.target.value,_=1,w.clear(),$()},400)}),r?.forEach(k=>{k.type==="select"&&document.getElementById(`filter-${k.name}`)?.addEventListener("change",f=>{y[k.name]=f.target.value,_=1,w.clear(),$()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{y={...i},E&&(E.value=""),r?.forEach(k=>{let f=document.getElementById(`filter-${k.name}`);f&&(f.value="")}),_=1,w.clear(),$()}),document.getElementById("btn-create")?.addEventListener("click",()=>se(null)),v){document.getElementById(`btn-export-${v.moduleName}`)?.addEventListener("click",async f=>{let D=f.target,P=D.innerHTML;D.innerHTML="\u23F3 Loading...",D.disabled=!0;try{await v.onExport()}catch{G("Gagal export data")}finally{D.innerHTML=P,D.disabled=!1}}),document.getElementById(`btn-template-${v.moduleName}`)?.addEventListener("click",()=>{v.onTemplate()});let k=document.getElementById(`input-import-${v.moduleName}`);k?.addEventListener("change",async f=>{let D=f.target.files[0];if(!D)return;k.disabled=!0;let P=document.createElement("div");P.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",P.innerHTML=`
      ...
      `,document.body.appendChild(P);let Y=P.querySelector("#import-progress-text"),U=P.querySelector("#import-progress-bar"),I=P.querySelector("#import-summary"),B=P.querySelector("#import-close-btn");B.addEventListener("click",()=>{P.remove(),$()});try{let V=await Ke(D);if(V.length===0)throw new Error("File kosong atau format salah");let X=500,ie=0,ae=0,F=0,M=V.length;Y.textContent=`Ditemukan ${M} baris data. Memulai import...`;for(let T=0;T<M;T+=X){let Q=V.slice(T,T+X);Y.textContent=`Mengimport baris ${T+1} - ${Math.min(T+X,M)} dari ${M}...`,U.style.width=`${Math.round(T/M*100)}%`;try{let q=await v.onImport(Q);q?(ie+=q.inserted||q.metrics?.inserted||Q.length,ae+=q.skipped||q.metrics?.updated||0):ie+=Q.length}catch(q){console.error("Chunk import failed:",q),F+=Q.length}}U.style.width="100%",Y.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',I.style.display="block",I.innerHTML=`
        ...
        `,F>0&&(I.innerHTML+='<p style="margin-top:12px;font-size:0.8rem;color:var(--danger)">Sebagian data gagal diimport. Pastikan format kolom sesuai template dan tidak ada data kosong pada kolom wajib.</p>'),B.style.display="block",k.value=""}catch(V){Y.innerHTML=`<strong style="color:var(--danger)">\u274C Gagal Memproses File</strong><br>${V.message}`,U.style.background="var(--danger)",U.style.width="100%",B.style.display="block",k.value=""}finally{k.disabled=!1}})}async function $(){L();let k=document.getElementById("table-container");if(!k)return;k.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';let f=C==="client",D=f?1:_,P=f?ye:20,Y=new URLSearchParams({page:D,limit:P,...Object.fromEntries(Object.entries(y).filter(([,F])=>F))}),U=await x(`${a}?${Y}`);if(!U.ok){k.innerHTML=`<div class="empty-state"><p class="text-danger">Gagal memuat data: ${U.data?.error||"Error"}</p></div>`;return}let I=U.data?.data||U.data||[],B=U.data?.pagination,V=I.length,X=I;if(f){I=b(I),X=I;let F=I.length,M=20,T=Math.ceil(F/M);_>T&&T>0&&(_=T);let Q=(_-1)*M,q=_*M;I=I.slice(Q,q),B={page:_,limit:M,total:F,pages:T}}!1,u&&u(I);let ie=ze({columns:o,data:I,fullData:X,onEdit:m?F=>se(F):null,actions:g.map(F=>({...F,handler:M=>F.handler(M,$)})),emptyText:`Tidak ada ${String(n||"").toLowerCase()}`,bulkSelect:S?{selectedIds:w,onToggle:L}:null});k.innerHTML="",k.appendChild(ie);let ae=document.getElementById("pagination-container");if(ae&&(ae.innerHTML="",B&&B.pages>1)){let F=Qe({page:B.page,pages:B.pages,total:B.total,limit:B.limit,onPage:M=>{_=M,$()}});F&&ae.appendChild(F)}}function ge(k){let f=typeof d=="function"?d(k):d;return Re(f)}function se(k){let f=!!k,D=document.createElement("form");if(D.noValidate=!0,D.innerHTML=ge(k),f){let Y=typeof d=="function"?d(k):d;We(D,k)}let{close:P}=de({title:f?`Edit ${n}`:`Tambah ${n}`,content:D,size:"lg",confirmText:f?"Simpan Perubahan":`Tambah ${n}`,onConfirm:async(Y,U)=>{if(!D.reportValidity())return;let I=Y.querySelector(".modal-confirm");I.disabled=!0,I.textContent="Menyimpan...";let B=Ye(D),V=typeof d=="function"?d(k):d,X=async M=>{for(let T of M)if(T.type==="row")await X(T.fields);else if(T.type==="combobox"&&B[T.name]){let Q=B[T.name],q=(T.options||[]).find(W=>{let ne=String(typeof W=="object"?W.value:W),nt=String(typeof W=="object"?W.label:W);return ne===Q||nt===Q});if(q)B[T.name]=typeof q=="object"?q.value:q;else if(T.createApi){let W={};W[T.createApi.field]=Q,T.createApi.extra&&Object.assign(W,T.createApi.extra);let ne=await x(T.createApi.path,{method:"POST",body:JSON.stringify(W)});if(ne.ok&&ne.data?.id)B[T.name]=ne.data.id;else if(ne.ok&&!ne.data?.id)B[T.name]=Q;else throw new Error(`Gagal membuat master data: ${ne.data?.error||"Unknown error"}`)}}};try{await X(V)}catch(M){G(M.message),I.disabled=!1,I.textContent=f?"Simpan Perubahan":`Tambah ${n}`;return}c&&(B=await c(B,k));let ie=f?"PUT":"POST",ae=f?`${a}/${k.id}`:a,F=await x(ae,{method:ie,body:JSON.stringify(B)});F.ok?(Z(f?`${n} berhasil diperbarui.`:`${n} berhasil ditambahkan.`),U(),$()):(G(F.data?.error||"Gagal menyimpan data."),I.disabled=!1,I.textContent=f?"Simpan Perubahan":`Tambah ${n}`)}})}function le(k){Oe(`Hapus ${n} ini? Tindakan tidak dapat dibatalkan.`,async()=>{let f=await x(`${a}/${k.id}`,{method:"DELETE"});f.ok?(Z(`${n} berhasil dihapus.`),$()):G(f.data?.error||"Gagal menghapus.")},`Hapus ${n}`)}return $(),$}O();O();var Se=null,Ve=null;async function we(t=!1){if(Se&&!t)return console.log("Employees Raw (Cache Hit)",Se.slice(0,5)),Se;let e=await x(`/api/employees?limit=${ye}&status=Aktif`);return Se=(e.data?.data||[]).map(s=>({value:s.id,label:s.full_name})),console.log("Employees Raw",e.data?.data?.slice(0,5)),console.log("Employees Mapped (ID)",Se.slice(0,5)),Se}async function ee(t=!1){let s=(await we(t)).map(a=>({value:a.label,label:a.label}));return console.log("Employee Options",s.slice(0,5)),s}async function K(t=!1){return Ve&&!t||(Ve=((await x("/api/branches?all=1")).data?.data||[]).map(s=>({value:s.id,label:s.full_name}))),Ve}function j(t){let e={Done:"badge-success",Aktif:"badge-success",Open:"badge-warning","In Progress":"badge-info",Pending:"badge-warning",Diproses:"badge-info",Selesai:"badge-success","Tidak Aktif":"badge-neutral",Resign:"badge-neutral",Cut:"badge-danger","Tidak Datang":"badge-danger"};return!t||t==="-"||String(t).trim()===""?"":`<span class="badge ${e[t]||"badge-neutral"}">${t}</span>`}function lt(t){return t==null?'<span class="badge badge-neutral">-</span>':t<0?`<span class="badge badge-danger">Expired (${Math.abs(t)}h)</span>`:t<=14?`<span class="badge badge-danger">${t} hari</span>`:t<=30?`<span class="badge badge-warning">${t} hari</span>`:`<span class="badge badge-success">${t} hari</span>`}function _e(t){return`<span class="badge ${{"FACILITY CARE":"badge-info",SECURITY:"badge-secondary"}[t]||"badge-neutral"}">${t||"-"}</span>`}function dt(t){return`<span class="badge ${{"Inspeksi Hygiene & Aset Bangunan":"badge-info","General Cleaning":"badge-success","Deep Cleaning":"badge-purple",Fogging:"badge-warning"}[t]||"badge-neutral"}">${t||"-"}</span>`}function ce(t){return`<span class="badge ${{Q1:"badge-info",Q2:"badge-success",Q3:"badge-warning",Q4:"badge-danger"}[t]||"badge-neutral"}">${t||"-"}</span>`}R();function ct(t,e){if(t.period!=="Q3")return!1;let s=String(t.status||"").toLowerCase();if(s!=="selesai"&&s!=="completed"&&s!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}O();R();function St(t,e){let s=String(t.status||"").toLowerCase();return e==="active"?s==="aktif":e==="reliefer"?t.division==="FC - RELIEFER"&&s==="aktif":!1}O();R();function pt(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let o=new Date(a);o.setDate(a.getDate()+30);let d=new Date(t.end_date);return d.setHours(0,0,0,0),d>=a&&d<=o}return!1}O();R();function wt(t,e){let s=String(t.status||"").toLowerCase();return e==="open"?s==="open":!1}O();function _t(t,e){let s=String(t.status||"").toLowerCase();return e==="pending"?s==="pending":!1}var he={};function $e(t){if(he[t]){try{he[t].destroy()}catch{}delete he[t]}}function ma(){Object.keys(he).forEach($e)}var be=(t,e=0)=>{let s=Number(t);return isNaN(s)||t===null||t===void 0?e:s},Te=(t,e="\u2014")=>{if(t==null||t==="")return e;let s=String(t).trim();return s===""||s==="[object Object]"?e:s};var ga=t=>{if(!t||typeof t!="string")return"";try{let[e,s]=t.split("-");return new Date(Number(e),Number(s)-1).toLocaleDateString("id-ID",{month:"short",year:"2-digit"})}catch{return t}};function Tt(t,e,s=900){if(!t)return;let a=Math.max(0,Math.round(be(e)));if(a===0){t.textContent="0";return}let o=Date.now(),d=()=>{let r=Math.min((Date.now()-o)/s,1),i=1-Math.pow(1-r,3);t.textContent=Math.round(i*a).toLocaleString("id-ID"),r<1?requestAnimationFrame(d):t.textContent=a.toLocaleString("id-ID")};requestAnimationFrame(d)}var ba={Done:"pill-success",Aktif:"pill-success",Selesai:"pill-success",Open:"pill-danger",Pending:"pill-warning","In Progress":"pill-info","Tidak Aktif":"pill-neutral",Resign:"pill-neutral",Cut:"pill-neutral"},ha=t=>{let e=Te(t,"\u2014");return`<span class="status-pill ${ba[e]||"pill-neutral"}">${e}</span>`};var pe={family:"Inter",size:11},Ee="#94A3B8",Xe="#F1F5F9",ut=["#2563EB","#10B981","#F59E0B","#EF4444","#8B5CF6","#0EA5E9","#F97316","#14B8A6","#6366F1","#EC4899"],ya=()=>window.innerWidth<768;function Ze(t={}){return{responsive:!0,maintainAspectRatio:!1,animation:{duration:700,easing:"easeOutQuart"},plugins:{legend:{position:ya()?"bottom":"top",labels:{font:pe,color:"#64748B",usePointStyle:!0,padding:10,boxWidth:8,boxHeight:8}},tooltip:{mode:"index",intersect:!1,bodyFont:pe,titleFont:{...pe,weight:"700"}}},scales:{x:{grid:{color:Xe},ticks:{font:pe,color:Ee,maxRotation:0}},y:{grid:{color:Xe},ticks:{font:pe,color:Ee},beginAtZero:!0}},...t}}var fa=()=>Array(5).fill(0).map(()=>`
  ...
  <div class="mini-stat" style="pointer-events:none">
  ...
  </div>`).join("");function Ct(t=3){return Array(t).fill(0).map((e,s)=>`<div class="skeleton skeleton-text" style="height:38px;margin-bottom:${s<t-1?"6px":"0"};border-radius:6px"></div>`).join("")}async function re(t,e,s=8e3){try{let a=new AbortController,o=setTimeout(()=>a.abort(),s),d=await x(t,{signal:a.signal}).catch(()=>null);if(clearTimeout(o),!d||!d.ok)return e;let r=d.data;return r?r.data!==void 0?r.data??e:r:e}catch{return e}}function ka(){["skel-donut","skel-trend","skel-insp","skel-contract","skel-schbar"].forEach(a=>{let o=document.getElementById(a);o&&(o.style.display="none")}),["chart-donut","chart-trend","chart-insp","chart-contract","chart-schbar"].forEach(a=>{let o=document.getElementById(a);if(o&&o.style.display==="none"){o.style.display="block";let d=o.parentElement;if(d&&!d.querySelector(".chart-empty")){let r=document.createElement("div");r.className="chart-empty",r.textContent="Belum ada data",o.style.display="none",d.appendChild(r)}}});let t=document.getElementById("kpi-row");t&&t.querySelector(".skeleton")&&$t({});let e=document.getElementById("mini-stats-row");e&&e.querySelector(".skeleton")&&Dt({}),["table-contracts","table-issues"].forEach(a=>{let o=document.getElementById(a);o&&o.querySelector(".skeleton")&&(o.innerHTML='<div class="chart-empty">Belum ada data</div>')});let s=document.getElementById("activity-log");s&&s.querySelector(".skeleton")&&(s.innerHTML='<div class="chart-empty">Belum ada aktivitas</div>')}async function Et(t){ma(),t._dashRefresh&&clearInterval(t._dashRefresh),t._skelTimeout&&clearTimeout(t._skelTimeout);let e=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});t.innerHTML=`
    <div class="dashboard-wrap" id="dash-root">
      ...
      <div class="kpi-row" id="kpi-row" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr));">${fa()}</div>

      <!-- Mini Stats -->
      <div class="mini-stats-row" id="mini-stats-row" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr));">${va()}</div>
  ...
  `,document.getElementById("btn-dash-refresh")?.addEventListener("click",()=>Ce(t)),document.getElementById("filter-insp-month")?.addEventListener("change",async a=>{let o=a.target.value,d=o?`/api/dashboard/inspection-bar?month=${o}`:"/api/dashboard/inspection-bar",r=document.getElementById("skel-insp"),i=document.getElementById("chart-insp");r&&(r.style.display="block",r.style.position="absolute"),i&&(i.style.display="none");let n=await re(d,{},8e3);try{Bt(n)}catch(p){console.warn("InspBar render:",p),ue("skel-insp","chart-insp")}}),document.getElementById("filter-sch-year")?.addEventListener("change",async a=>{let o=a.target.value;document.getElementById("sch-year-label").textContent=o;let d=`/api/dashboard/schedule-monthly?year=${o}`,r=document.getElementById("skel-schbar"),i=document.getElementById("chart-schbar");r&&(r.style.display="block",r.style.position="absolute"),i&&(i.style.display="none");let n=await re(d,{data:[]},8e3);try{It(n)}catch(p){console.warn("SchBar render:",p),ue("skel-schbar","chart-schbar")}}),t._skelTimeout=setTimeout(()=>ka(),5e3);async function s(){let a=document.getElementById("global-branch"),o=document.getElementById("global-month");if(!(!a||!o)){if(o.children.length===0){let d=new Date;for(let r=0;r<12;r++){let i=new Date(d.getFullYear(),d.getMonth()-r,1),n=i.getFullYear()+"-"+String(i.getMonth()+1).padStart(2,"0"),p=i.toLocaleDateString("id-ID",{month:"long",year:"numeric"});o.innerHTML+=`<option value="${n}">${p}</option>`}}if(a.children.length===1){let d=await x("/api/branches");d.ok&&d.data&&d.data.data&&d.data.data.forEach(r=>{a.innerHTML+=`<option value="${r.id}">${r.full_name}</option>`})}a.addEventListener("change",()=>Ce(t)),o.addEventListener("change",()=>Ce(t))}}await s(),await Ce(t),window.addEventListener("fcms-data-changed",()=>{document.getElementById("dash-root")&&Ce(t)}),t._dashRefresh=setInterval(()=>{document.getElementById("dash-root")?Ce(t):clearInterval(t._dashRefresh)},3e4)}async function Ce(t){t._skelTimeout&&(clearTimeout(t._skelTimeout),t._skelTimeout=null);let e=document.getElementById("global-branch"),s=document.getElementById("global-month"),a=e?e.value:"",o=s?s.value:"",d=`?branch_id=${a}&month=${o}`,r=document.getElementById("filter-sch-year"),i=r?r.value:new Date().getFullYear(),[n,p,m,l,c,u,b,g,h,v,S,C,_]=await Promise.all([re("/api/dashboard/kpi",{},8e3),re("/api/dashboard/issues-trend",{},8e3),re("/api/dashboard/issues-summary",{},8e3),re("/api/dashboard/inspection-bar",{},8e3),re("/api/dashboard/stats",{},8e3),re("/api/dashboard/calendar",[],8e3),re("/api/schedule?limit=10000",{data:[]},8e3),re("/api/employees?limit=10000",{data:[]},8e3),re("/api/contracts?limit=10000",{data:[]},8e3),re("/api/issues?limit=10000",{data:[]},8e3),re("/api/one-on-one?limit=10000",{data:[]},8e3),re("/api/dashboard/contracts-chart",{labels:[],data:[]},8e3),re(`/api/dashboard/schedule-monthly?year=${document.getElementById("filter-sch-year")?.value||new Date().getFullYear()}`,{data:[]},8e3)]);if(n){let y=Array.isArray(b?.data)?b.data:Array.isArray(b)?b:[],w=Array.isArray(g?.data)?g.data:Array.isArray(g)?g:[],L=Array.isArray(h?.data)?h.data:Array.isArray(h)?h:[],E=Array.isArray(v?.data)?v.data:Array.isArray(v)?v:[],H=Array.isArray(S?.data)?S.data:Array.isArray(S)?S:[];n.employees&&(n.employees.current=w.filter($=>St($,"active")).length),n.contracts&&(n.contracts.current=L.filter($=>pt($,"active")).length),n.expiring30&&(n.expiring30={current:L.filter($=>pt($,"expiring30")).length}),n.issues&&(n.issues.current=E.filter($=>wt($,"open")).length),n.one_on_one&&(n.one_on_one.current=H.filter($=>_t($,"pending")).length),n.inspection_month&&(n.inspection_month.current=y.filter($=>ct($,"inspeksi")).length),n.cleaning_month&&(n.cleaning_month.current=y.filter($=>ct($,"gcdc")).length)}try{$t(n)}catch(y){console.warn("KPI render:",y)}try{Dt(n)}catch(y){console.warn("MiniStats render:",y)}try{xa(Array.isArray(m?.by_category)?m.by_category:[])}catch(y){console.warn("Donut render:",y),ue("skel-donut","chart-donut")}try{It(_)}catch(y){console.warn("SchBar render:",y),ue("skel-schbar","chart-schbar")}try{Sa(p)}catch(y){console.warn("Trend render:",y),ue("skel-trend","chart-trend")}try{Bt(l)}catch(y){console.warn("InspBar render:",y),ue("skel-insp","chart-insp")}try{let y=Array.isArray(c)?c:Array.isArray(c?.recent_issues)?c.recent_issues:[];_a(y)}catch(y){console.warn("IssuesTable render:",y)}try{let y=Array.isArray(c?.expiring_contracts)?c.expiring_contracts:[];wa(C)}catch(y){console.warn("ContractsTable render:",y)}try{Ca(Array.isArray(u)?u:[])}catch(y){console.warn("Agenda render:",y)}try{Ta()}catch(y){console.warn("Quick Actions render:",y)}}function $t(t){let e=document.getElementById("kpi-row");if(!e)return;t=t||{};let s=[{icon:"\u{1F465}",label:"Karyawan Aktif",sub:"Total karyawan aktif",href:"#/employees?dash_filter=active",color:"kpi-blue",key:"employees",trendPct:"+2%",trendColor:"#10B981",points:"0,20 10,18 20,22 30,12 40,15 50,8 60,10 70,5 80,6 90,2 100,0"},{icon:"\u{1F504}",label:"Reliefer Aktif",sub:"Karyawan reliefer",href:"#/employees?dash_filter=reliefer",color:"kpi-purple",key:"reliever_total",trendPct:"0%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u{1F4C4}",label:"Kontrak Aktif",sub:"Kontrak yang masih berjalan",href:"#/contracts?dash_filter=active",color:"kpi-green",key:"contracts",trendPct:"+1%",trendColor:"#10B981",points:"0,15 20,18 40,10 60,12 80,5 100,2"},{icon:"\u23F3",label:"Kontrak Habis 30 Hari",sub:"Akan segera berakhir",href:"#/contracts?dash_filter=expiring30",color:"kpi-warn",key:"expiring30",trendPct:"+25%",trendColor:"#F59E0B",points:"0,25 20,22 40,24 60,15 80,18 100,5"},{icon:"\u26A0\uFE0F",label:"Permasalahan Open",sub:"Belum diselesaikan",href:"#/issues?dash_filter=open",color:"kpi-red",key:"issues",trendPct:"0%",trendColor:"#EF4444",points:"0,20 20,18 40,22 60,19 80,21 100,20"},{icon:"\u{1F4AC}",label:"One on One Pending",sub:"Menunggu tindak lanjut",href:"#/one-on-one?dash_filter=pending",color:"kpi-purple",key:"one_on_one",trendPct:"+8%",trendColor:"#10B981",points:"0,25 20,15 40,18 60,8 80,10 100,2"}];e.innerHTML=s.map(a=>{let o=be(t[a.key]?.current,0);return`
      ...
      </a>`}).join(""),e.querySelectorAll(".kpi-value").forEach(a=>{Tt(a,parseInt(a.dataset.target)||0)})}function Dt(t){let e=document.getElementById("mini-stats-row");if(!e)return;t=t||{};let s=[{icon:"\u{1F4C5}",label:"Jadwal",val:t.schedule?.current,href:"#/timeline",color:"mini-blue"},{icon:"\u{1F393}",label:"Training",val:t.training_month?.current,href:"#/training",color:"mini-gray"},{icon:"\u{1F504}",label:"Reliefer",val:t.reliever_completed?.current,href:"#/relievers?dash_filter=reliever",color:"mini-teal"},{icon:"\u{1F50D}",label:"Inspeksi",val:t.inspection_month?.current,href:"#/timeline?dash_filter=inspeksi",color:"mini-blue"},{icon:"\u{1F9F9}",label:"GCDC",val:t.cleaning_month?.current,href:"#/timeline?dash_filter=gcdc",color:"mini-green"},{icon:"\u{1F4A8}",label:"Fogging",val:t.fogging_month?.current,href:"#/reports/fogging",color:"mini-purple"},{icon:"\u{1F3E2}",label:"Cabang",val:t.branches?.current,href:"#/branches",color:"mini-teal"}];e.innerHTML=s.map(a=>`
    ...
    </a>`).join(""),e.querySelectorAll(".mini-stat-value").forEach(a=>Tt(a,parseInt(a.dataset.target)||0,700))}function xa(t){ue("skel-donut","chart-donut");let e=document.getElementById("chart-donut"),s=document.getElementById("donut-legend");if(!e||!s)return;$e("donut");let a=(t||[]).filter(n=>be(n.count)>0);if(!a.length){qe(e,"Belum ada data permasalahan");return}let o=a.map(n=>`${Te(n.category,"Lainnya")}`),d=a.map(n=>be(n.count)),r=d.reduce((n,p)=>n+p,0);s.innerHTML=a.map((n,p)=>{let m=ut[p%ut.length],l=r>0?Math.round(n.count/r*100):0;return`
    ...
    `}).join("");let i={id:"centerText",beforeDraw:function(n){let p=n.width,m=n.height,l=n.ctx;l.restore();let c=(m/80).toFixed(2);l.font="bold "+c+"em Inter",l.textBaseline="middle",l.fillStyle="#1E293B";let u=r.toString(),b=Math.round((p-l.measureText(u).width)/2),g=m/2;l.fillText(u,b,g-10),l.font="600 "+(c*.35).toFixed(2)+"em Inter",l.fillStyle="#64748B";let h="Total",v=Math.round((p-l.measureText(h).width)/2);l.fillText(h,v,g+15),l.save()}};he.donut=new Chart(e,{type:"doughnut",data:{labels:o,datasets:[{data:d,backgroundColor:ut,borderWidth:2,borderColor:"#fff",hoverBorderColor:"#fff"}]},options:{responsive:!0,maintainAspectRatio:!1,animation:{duration:700},plugins:{legend:{display:!1},tooltip:{bodyFont:pe,titleFont:{...pe,weight:"700"},callbacks:{label:n=>` ${n.label}: ${n.parsed} kasus`}}},cutoutPercentage:78,cutout:"78%"},plugins:[i]})}function It(t){ue("skel-schbar","chart-schbar");let e=document.getElementById("chart-schbar");if(!e)return;if($e("schbar"),!t||!t.data||!t.data.length){qe(e,"Belum ada jadwal pada tahun ini.");return}let s=t.data||[],o=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],d={Inspeksi:Array(12).fill(0),"General Cleaning":Array(12).fill(0),"Deep Cleaning":Array(12).fill(0),Fogging:Array(12).fill(0)};s.forEach(c=>{let u=c.activity_type||"Lainnya";u.toLowerCase().includes("inspeksi")&&(u="Inspeksi"),(u.toLowerCase()==="gcdc"||u.toLowerCase().includes("general cleaning"))&&(u="General Cleaning"),u.toLowerCase().includes("deep cleaning")&&(u="Deep Cleaning"),(u.toLowerCase().includes("fogging")||u.toLowerCase().includes("foging"))&&(u="Fogging"),d[u]||(d[u]=Array(12).fill(0));let b=parseInt(c.month,10)-1;b>=0&&b<12&&(d[u][b]+=c.c||0)});let r={Inspeksi:"#60A5FA","General Cleaning":"#4ADE80","Deep Cleaning":"#FB923C",Fogging:"#EF4444"},i=c=>{let u=0;for(let b=0;b<c.length;b++)u=c.charCodeAt(b)+((u<<5)-u);return`hsl(${Math.abs(u)%360}, 65%, 60%)`},n=["Inspeksi","General Cleaning","Deep Cleaning","Fogging"],p=Object.keys(d).filter(c=>!n.includes(c)),l=[...n,...p].filter(c=>d[c]).map(c=>({label:c,data:d[c],backgroundColor:r[c]||i(c),borderWidth:0,borderRadius:4}));he.schbar=new Chart(e,{type:"bar",data:{labels:o,datasets:l},options:Ze({responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"bottom",align:"center",labels:{boxWidth:10,boxHeight:10,useBorderRadius:!0,borderRadius:3,padding:8,font:{family:"Inter",size:9}},padding:{top:10}},tooltip:{callbacks:{title:c=>{let u=c[0].label;return{Jan:"Januari",Feb:"Februari",Mar:"Maret",Apr:"April",Mei:"Mei",Jun:"Juni",Jul:"Juli",Agu:"Agustus",Sep:"September",Okt:"Oktober",Nov:"November",Des:"Desember"}[u]||u},label:c=>` ${c.dataset.label}: ${c.raw} Jadwal`}}},scales:{x:{stacked:!0,grid:{display:!1}},y:{stacked:!0,border:{display:!1},grid:{color:"#f3f4f6",drawTicks:!1}}}})})}function Sa(t){ue("skel-trend","chart-trend");let e=document.getElementById("chart-trend");if(!e)return;$e("trend"),t=t||{};let s=(t.labels||[]).map(ga),a=(t.open||[]).map(d=>be(d)),o=(t.closed||[]).map(d=>be(d));if(!s.length){qe(e,"Belum ada data trend");return}he.trend=new Chart(e,{type:"line",data:{labels:s,datasets:[{label:"Open",data:a,borderColor:"#EF4444",backgroundColor:"rgba(239,68,68,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#EF4444",borderWidth:2},{label:"Closed",data:o,borderColor:"#10B981",backgroundColor:"rgba(16,185,129,.08)",fill:!0,tension:.4,pointRadius:2,pointHoverRadius:4,pointBackgroundColor:"#10B981",borderWidth:2}]},options:Ze({plugins:{legend:{display:!1}}})})}function Bt(t){ue("skel-insp","chart-insp");let e=document.getElementById("chart-insp");if(!e)return;$e("inspBar"),t=t||{};let s=t.labels||[],a=(t.fc||[]).map(d=>be(d)),o=(t.spv||[]).map(d=>be(d));if(!s.length){qe(e,"Belum ada data inspeksi");return}he.inspBar=new Chart(e,{type:"bar",data:{labels:s,datasets:[{label:"Skor FC",data:a,backgroundColor:"rgba(37,99,235,.75)",borderRadius:4,borderSkipped:!1},{label:"Skor SPV",data:o,backgroundColor:"rgba(16,185,129,.75)",borderRadius:4,borderSkipped:!1}]},options:Ze({plugins:{legend:{position:"top"}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:Ee,maxRotation:45,minRotation:30}},y:{grid:{color:Xe},ticks:{font:pe,color:Ee},min:0,max:100}}})})}function wa(t){ue("skel-contract-mini","chart-contract-mini");let e=document.getElementById("chart-contract-mini");if(!e)return;$e("contractMiniBar"),t=t||{};let s={"06":"Jun","07":"Jul","08":"Agu","09":"Sep",10:"Okt",11:"Nov",12:"Des"},a=(t.labels||[]).map(r=>{let i=r.split("-")[1];return s[i]||r}),o=(t.data||[]).map(r=>be(r));if(!a.length){qe(e,"Belum ada data");return}let d=e.getContext("2d");he.contractMiniBar=new Chart(e,{type:"bar",data:{labels:a,datasets:[{label:"Kontrak Habis",data:o,backgroundColor:"#3B82F6",borderRadius:4,borderSkipped:!1,barPercentage:.6,categoryPercentage:.7}]},options:Ze({onClick:(r,i)=>{if(i&&i.length>0){let n=i[0].index,p=(t.labels||[])[n];p&&(window.location.hash="#/contracts?month_expiry="+p)}},plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:pe,color:Ee,maxRotation:0,autoSkip:!1}},y:{grid:{color:Xe,borderDash:[4,4],drawBorder:!1},ticks:{font:pe,color:Ee,precision:0,maxTicksLimit:5},min:0}},animation:{y:{duration:1e3,easing:"easeOutQuart"}}})})}function _a(t){let e=document.getElementById("table-issues");if(!e)return;let s=(t||[]).slice(0,8);if(!s.length){e.innerHTML='<div class="chart-empty">\u2705 Tidak ada permasalahan terbuka</div>';return}e.innerHTML=`
    <div class="dash-list">
      ${s.map(a=>`
      ...
      `).join("")}
    </div>`}function Ca(t){let e=document.getElementById("widget-agenda");if(!e)return;let s=new Date,a=`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}-${String(s.getDate()).padStart(2,"0")}`,d=(t||[]).filter(r=>(r.event_date||"").startsWith(a)).slice(0,10);if(!d.length){e.innerHTML="";return}e.innerHTML=`
    <div style="display:flex;flex-direction:column;gap:12px;padding-right:8px">
      ${d.map(r=>{let i="#3B82F6",n="#EFF6FF",p="Agenda",m=(r.title||"").toLowerCase();return m.includes("inspeksi")?(i="#10B981",n="#ECFDF5",p="Inspeksi"):m.includes("cleaning")||m.includes("gcdc")?(i="#3B82F6",n="#EFF6FF",p="Cleaning"):m.includes("reliefer")?(i="#F59E0B",n="#FFFBEB",p="Reliefer"):m.includes("fogging")&&(i="#8B5CF6",n="#F5F3FF",p="Fogging"),`
  ...
  `).join("")}function ue(t,e){let s=document.getElementById(t),a=document.getElementById(e);if(s&&(s.style.display="none",s.style.position=""),a){a.style.display="block";let o=a.parentElement;if(o){let d=o.querySelector(".chart-empty");d&&d.remove()}}}function qe(t,e="Belum ada data"){if(!t)return;t.style.display="none";let s=t.parentElement;if(!s)return;if(!s.querySelector(".chart-empty")){let o=document.createElement("div");o.className="chart-empty",o.textContent=e,s.appendChild(o)}}O();async function Pt(t){document.getElementById("app").innerHTML=`
  ...
  `;let e=document.getElementById("login-form"),s=document.getElementById("login-error"),a=document.getElementById("login-btn"),o=document.getElementById("toggle-password"),d=document.getElementById("login-password");o?.addEventListener("click",()=>{let r=d.type==="text";d.type=r?"password":"text",o.style.color=r?"":"var(--primary)"}),e?.addEventListener("submit",async r=>{r.preventDefault(),s.style.display="none";let i=e.username.value.trim(),n=e.password.value;if(!i||!n){s.textContent="Username dan password wajib diisi.",s.style.display="block";return}a.querySelector(".btn-text").style.display="none",a.querySelector(".btn-spinner").style.display="",a.disabled=!0;try{let p=await x("/api/auth/login",{method:"POST",body:JSON.stringify({username:i,password:n})});p.ok&&p.data.success?(st(p.data.data.token),Fe(p.data.data.user),Z("Login berhasil! Selamat datang \u{1F44B}"),window.dispatchEvent(new Event("fm:login"))):(s.textContent=p.data.error||"Username atau password salah.",s.style.display="block",a.classList.add("shake"),setTimeout(()=>a.classList.remove("shake"),600))}catch{s.textContent="Gagal terhubung ke server. Periksa koneksi internet.",s.style.display="block"}finally{a.querySelector(".btn-text").style.display="",a.querySelector(".btn-spinner").style.display="none",a.disabled=!1}})}O();R();async function Ea(){return await K()}function $a(t,e){let s=String(t.status||"").toLowerCase();return e==="active"?s==="aktif":e==="reliefer"?t.division==="FC - RELIEFER"&&s==="aktif":!1}async function Lt(t,e){let s=await Ea(),a=e?e.get("dash_filter"):null;N({container:t,title:"Karyawan",icon:"\u{1F465}",apiPath:"/api/employees",itemLabel:"Karyawan",bulkDelete:!0,paginationMode:"client",onDataLoaded:o=>a?o.filter(d=>$a(d,a)):o,columns:[{key:"full_name",label:"Nama Lengkap"},{key:"branch_name",label:"Cabang"},{key:"division",label:"Divisi",render:o=>_e(o)},{key:"phone",label:"No. HP",render:o=>o?`<a href="tel:${o}">${o}</a>`:"-"},{key:"join_date",label:"Tgl Masuk",render:o=>window.formatDate(o)},{key:"status",label:"Status",render:o=>j(o)}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:s},{type:"select",name:"division",label:"Divisi",options:["FACILITY CARE","SECURITY","FC - RELIEFER"]},{type:"select",name:"status",label:"Status",options:["Aktif","Tidak Aktif","Resign","Cut"]}],formFields:o=>[{type:"row",fields:[{name:"full_name",label:"Nama Lengkap",required:!0,placeholder:"Nama lengkap karyawan",value:o?.full_name},{name:"phone",label:"No. HP",placeholder:"08xx-xxxx-xxxx",value:o?.phone}]},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:s,value:o?.branch_id},{name:"division",label:"Divisi",type:"select",required:!0,options:["FACILITY CARE","SECURITY","FC - RELIEFER"],value:o?.division||"FACILITY CARE"}]},{type:"row",fields:[{name:"join_date",label:"Tanggal Masuk",type:"date",value:o?.join_date},{name:"status",label:"Status",type:"select",required:!0,options:["Aktif","Tidak Aktif","Resign","Cut"],value:o?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:o?.notes}],exportOptions:{moduleName:"employees",onExport:async()=>{let o=await x(`/api/employees${window.location.search?window.location.search+"&":"?"}limit=10000`);if(o.ok){let d=o.data.data.map(r=>({"Nama Lengkap":r.full_name,Cabang:r.branch_name||"",Divisi:r.division||"","No. HP":r.phone||"","Tgl Masuk":r.join_date||"",Status:r.status||""}));A(d,"Data_Karyawan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu",Divisi:"FACILITY CARE","No. HP":"08123456789","Tgl Masuk":"2024-01-15",Status:"Aktif"},{"Nama Lengkap":"Andi Saputra",Cabang:"002. Bintaro",Divisi:"SECURITY","No. HP":"08987654321","Tgl Masuk":"2023-11-01",Status:"Aktif"}],"Template_Import_Karyawan")},onImport:async o=>{let d=n=>{if(!n)return null;let p=String(n||"").toLowerCase(),m=s.find(l=>String(l.label||"").toLowerCase()===p);return m?m.value:null},r=o.map(n=>({full_name:String(n["Nama Lengkap"]||"").trim(),branch_id:d(String(n.Cabang||"").trim()),division:String(n.Divisi||"").trim()||"FACILITY CARE",phone:String(n["No. HP"]||"").trim(),join_date:String(n["Tgl Masuk"]||"").trim(),status:String(n.Status||"").trim(),notes:String(n.Catatan||"").trim()})).filter(n=>n.full_name),i=await x("/api/import/employees",{method:"POST",body:JSON.stringify({rows:r,onDuplicate:"update"})});if(!i.ok)throw new Error(i.data?.error||"Import gagal");return i.data}}})}O();R();var gt=[],At=[];async function Da(){gt=await K(),At=await we()}var mt=async t=>{let e=[],s=1;for(;;){let o=await(await Promise.resolve().then(()=>(O(),ke))).apiFetch(`${t}${t.includes("?")?"&":"?"}limit=100&page=${s}`);if(!o.ok)break;let d=o.data?.data||o.data||[],r=Array.isArray(d)?d:[];if(e=e.concat(r),r.length<100||o.data?.pagination&&s>=o.data.pagination.pages)break;s++}return e};function Ia(t,e){if(String(t.status||"").toLowerCase()!=="aktif")return!1;if(e==="active")return!0;if(e==="expiring30"){if(!t.end_date)return!1;let a=new Date;a.setHours(0,0,0,0);let o=new Date(a);o.setDate(a.getDate()+30);let d=new Date(t.end_date);return d.setHours(0,0,0,0),d>=a&&d<=o}return!1}async function Nt(t,e){await Da();let s=e?e.get("dash_filter"):null;N({container:t,title:"Data Kontrak",icon:"\u{1F4CB}",apiPath:"/api/contracts",bulkDelete:!0,itemLabel:"Kontrak",paginationMode:"client",onDataLoaded:a=>s?a.filter(o=>Ia(o,s)):a,columns:[{key:"employee_name",label:"Nama Lengkap"},{key:"branch_name",label:"Cabang"},{key:"division",label:"Div / Bagian",render:a=>_e(a)},{key:"start_date",label:"Tanggal Mulai",nowrap:!0,render:a=>window.formatDate(a)},{key:"end_date",label:"Tanggal Selesai",nowrap:!0,render:a=>!a||String(a).startsWith("2099")?"Tetap / PKWTT":window.formatDate(a)},{key:"days_remaining",label:"Sisa Kontrak",render:(a,o)=>o.end_date&&String(o.end_date).startsWith("2099")?'<span class="badge badge-success" style="background:#10B981;color:white;padding:4px 8px;border-radius:6px;font-size:0.75rem;font-weight:600">Tetap</span>':lt(a)},{key:"status",label:"Status",render:a=>j(a)}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:gt},{type:"select",name:"status",label:"Status",options:["Aktif","Tidak Aktif","Resign","Cut"]},{type:"select",name:"month_expiry",label:"Bulan Habis",options:[{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"},{value:"2027-01",label:"Jan 2027"},{value:"2027-02",label:"Feb 2027"},{value:"2027-03",label:"Mar 2027"},{value:"2027-04",label:"Apr 2027"},{value:"2027-05",label:"Mei 2027"},{value:"2027-06",label:"Jun 2027"},{value:"2027-07",label:"Jul 2027"}]},{type:"select",name:"expiring_days",label:"Akan Habis",options:[{value:"7",label:"7 Hari"},{value:"14",label:"14 Hari"},{value:"30",label:"30 Hari"},{value:"60",label:"60 Hari"}]}],onBeforeSubmit:a=>(a.end_date||(a.end_date="2099-12-31"),a),onAfterLoad:()=>{if(!document.getElementById("btn-find-missing")){let a=document.createElement("button");a.id="btn-find-missing",a.className="btn btn-ghost",a.innerHTML="\u{1F50D} Cek Selisih Karyawan",a.style.marginLeft="8px",a.style.color="#EF4444",a.style.border="1px solid currentColor",a.onclick=async()=>{a.innerHTML="\u231B Mencari...",a.disabled=!0;try{let[d,r]=await Promise.all([mt("/api/employees?status=Aktif"),mt("/api/contracts")]);if(d.length>0){let i=r.filter(l=>l.status==="Aktif"),n=new Set(i.map(l=>l.employee_id)),p=d.filter(l=>!n.has(l.id)),m=`<p style="margin-bottom:12px">Data yang terbaca: <b>${d.length}</b> Karyawan Aktif, dan <b>${i.length}</b> Kontrak Aktif.</p>
              <p style="margin-bottom:12px">Terdapat <b>${p.length}</b> karyawan aktif yang tidak memiliki "Kontrak Aktif". Berikut daftarnya:</p><ul style="padding-left:20px; max-height:400px; overflow-y:auto">`;p.forEach(l=>{let c=r.filter(b=>b.employee_id===l.id),u='<span style="color:#F59E0B">Belum pernah di-input kontrak</span>';if(c.length>0){let b=c[0];u=`Pernah ada kontrak (Status: <b style="color:#EF4444">${b.status}</b>, Selesai: ${window.formatDate(b.end_date)})`}m+=`<li style="margin-bottom:8px"><b>${l.full_name}</b> <br><span style="font-size:0.85em;color:var(--text-2)">Cabang: ${l.branch_name||"-"} | ${u}</span></li>`}),m+="</ul>",Promise.resolve().then(()=>(ve(),xt)).then(l=>l.createModal({title:"Karyawan Tanpa Kontrak Aktif",content:m,cancelText:"Tutup"}))}}catch(d){console.error(d)}a.innerHTML="\u{1F50D} Cek Selisih Karyawan",a.disabled=!1};let o=document.querySelector(".page-actions");o&&o.appendChild(a)}},formFields:a=>[{type:"row",fields:[{name:"employee_id",label:"Nama Lengkap",type:"combobox",required:!0,options:At,value:a?.employee_id},{name:"branch_id",label:"Cabang",type:"combobox",options:gt,value:a?.branch_id}]},{type:"row",fields:[{name:"division",label:"Div / Bagian",type:"select",required:!0,options:["FACILITY CARE","SECURITY"],value:a?.division||"FACILITY CARE"},{name:"status",label:"Status",type:"select",required:!0,options:["Aktif","Tidak Aktif","Resign","Cut"],value:a?.status||""}]},{type:"row",fields:[{name:"start_date",label:"Tanggal Mulai",type:"date",value:a?.start_date},{name:"end_date",label:"Tanggal Selesai",type:"date",value:a?.end_date&&!String(a.end_date).startsWith("2099")?a.end_date:""}]},{type:"row",fields:[{name:"contract_type",label:"Tipe Kontrak",type:"select",options:["KONTRAK 6 BULAN","KONTRAK 1 TAHUN","KONTRAK 2 TAHUN"],value:a?.contract_type},{name:"pkwt_number",label:"No. PKWT",type:"select",options:["PKWT 1","PKWT 2","PKWT 3","PKWT 4","PKWT 5","PKWT 6"],value:a?.pkwt_number}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:a?.notes}],exportOptions:{moduleName:"contracts",onExport:async()=>{let a=await x(`/api/contracts${window.location.search?window.location.search+"&":"?"}limit=10000`);if(a.ok){let o=a.data.data.map(d=>({"Nama Lengkap":d.employee_name,Cabang:d.branch_name||"","Div / Bagian":d.division||"","Tanggal Mulai":d.start_date||"","Tanggal Selesai":d.end_date&&String(d.end_date).startsWith("2099")?"":d.end_date||"","Sisa Kontrak":d.end_date&&String(d.end_date).startsWith("2099")?"Tetap":d.days_remaining!==null&&d.days_remaining!==void 0?`${d.days_remaining} Hari`:"",Status:d.status||""}));A(o,"Data_Kontrak")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{"Nama Lengkap":"Budi Santoso",Cabang:"001. Pondok Bambu","Div / Bagian":"FACILITY CARE","Tanggal Mulai":"2024-01-01","Tanggal Selesai":"2024-12-31","Sisa Kontrak":"365 Hari",Status:"Aktif"}],"Template_Import_Kontrak")},onImport:async a=>{let[o,d]=await Promise.all([x("/api/branches?limit=10000"),mt("/api/employees")]),r=o.data?.data||[],i=d||[];console.log(`Total employee yang berhasil dimuat dari database : ${i.length}`),i.length>0&&(console.log("Contoh 5 employee pertama:"),i.slice(0,5).forEach((g,h)=>{console.log(`${h+1}. ID: ${g.id}, Name: ${g.full_name}, Status: ${g.status}`)}));let n=g=>{if(!g)return null;let h=String(g||"").replace(/\s+/g," ").toLowerCase().trim(),v=r.find(S=>String(S.full_name||"").replace(/\s+/g," ").toLowerCase().trim()===h||String(S.code||"").replace(/\s+/g," ").toLowerCase().trim()===h||String(S.name||"").replace(/\s+/g," ").toLowerCase().trim()===h);return v?v.id:null},p=(g,h)=>{if(console.log("------------------------------------------------"),console.log(`Row Excel : ${h}`),console.log(`Nama dari Excel : "${g}"`),!g)return console.log("Alasan gagal mapping : Nama kosong"),null;let v=String(g||"").replace(/\s+/g," ").toLowerCase().trim();console.log(`Nama setelah normalisasi : "${v}"`),console.log(`Jumlah employee di database : ${i.length}`);let S=i.find(C=>String(C.full_name||"").replace(/\s+/g," ").toLowerCase().trim()===v);return S?(console.log("Employee ditemukan atau tidak : Ditemukan"),console.log(`Employee ID jika ditemukan : ${S.id}`),S.id):(console.log("Employee ditemukan atau tidak : TIDAK Ditemukan"),console.log("Alasan gagal mapping : Tidak ada kecocokan full_name setelah normalisasi"),null)},m=g=>{if(!g)return"";if(g instanceof Date&&!isNaN(g.getTime()))return g.toISOString().slice(0,10);let h=String(g).trim();if(/^\d{4,5}(\.\d+)?$/.test(h)){let S=Math.floor(Number(h));if(S>2e4&&S<99999){let C=new Date(Date.UTC(1899,11,30)+S*864e5);return isNaN(C.getTime())?"":C.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(h))return h.slice(0,10);let v=h.split(/[\/\-\.]/);if(v.length===3){let[S,C,_]=v.map(y=>y.trim());if(S.length===4&&C.length<=2&&_.length<=2)return`${S}-${C.padStart(2,"0")}-${_.padStart(2,"0")}`;if(_.length===4&&C.length<=2&&S.length<=2)return`${_}-${C.padStart(2,"0")}-${S.padStart(2,"0")}`}return h},l=a.map((g,h)=>{let v=h+2,S=String(g["Nama Lengkap"]||"").trim(),C=g["Tanggal Mulai"],_=m(C);if(!_){let L=a.__worksheet,E=a.__headers||[],H=E.indexOf("Tanggal Mulai"),$="N/A",ge="N/A",se="N/A";if(H!==-1&&L&&window.XLSX){let k=window.XLSX.utils.encode_cell({c:H,r:v-1});se=k;let f=L[k];f?($=f.t||"undefined",ge=f.w||"undefined"):$="CELL KOSONG/TIDAK ADA DI WORKSHEET"}let le="Unknown";C==null||C===""?le="Kondisi IF: Nilai murni undefined, null, atau string kosong dari parsed JSON.":C instanceof Date&&isNaN(C.getTime())?le="Kondisi IF: Nilai adalah object Date namun invalid (isNaN).":le="Kondisi IF: Tidak lolos Regex YYYY-MM-DD maupun konversi serial number Excel.",console.log("=========================="),console.log("[DEBUG] DATE PARSING FAILED"),console.log("=========================="),console.log(`Excel Row Number : ${v}`),console.log(`Employee Name : ${S}`),console.log(`Column Header Used : "Tanggal Mulai" (Index: ${H})`),console.log(`Raw Cell Value : "${C}"`),console.log(`JavaScript Type : ${typeof C}`),console.log(`SheetJS Cell Type : ${$}`),console.log(`SheetJS Formatted Value : "${ge}"`),console.log(`Value After Trim : "${String(C||"").trim()}"`),console.log(`Value After Date Parser : "${_}"`),console.log(`Is Empty : ${!C}`),console.log(`Is Invalid Date : ${C instanceof Date?isNaN(C.getTime()):"Bukan JS Date Object"}`),console.log(`Reason : ${le}`),console.log(`Workbook Sheet : ${L?"Ada":"Tidak Ditemukan"}`),console.log(`Excel Cell Address : ${se}`),console.log(`
...
`)}let y=p(S,v),w=null;return y||(w="Karyawan tidak ditemukan di Database"),{isValid:!!y,invalidReason:w,rowNum:v,data:{employee_id:y,branch_id:n(String(g.Cabang||"").trim()),division:String(g["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:_,end_date:m(g["Tanggal Selesai"])||"2099-12-31",status:String(g.Status||"").trim(),_rawName:S}}}),c=[],u=[];if(l.forEach(g=>{g.isValid?c.push(g.data):u.push({rowNum:g.rowNum,name:g.data._rawName,reason:g.invalidReason})}),console.log(`Split Validation - Valid: ${c.length}, Invalid: ${u.length}`),c.length===0)return{inserted:0,skipped:a.length,failed:a.length};let b=await x("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:c,onDuplicate:"update"})});if(!b.ok)throw new Error(b.data?.error||"Import gagal");return b.data}}})}O();R();var bt=[],je=[];function Ba(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let s of e)if(t.some(a=>a.period===s))return s;return"Q3"}function Pa(t,e){if(t.period!=="Q3")return!1;let s=String(t.status||"").toLowerCase();if(s!=="selesai"&&s!=="completed"&&s!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}async function Ft(t,e){bt=await K();let s=await ee(),o=(await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`)).data?.data||[],d=new Set;o.forEach(l=>{l.pic&&l.pic.trim()&&d.add(l.pic.trim())}),je=Array.from(d).sort();let r=l=>l&&!je.find(c=>(typeof c=="object"?c.value:c)===l)?[...je,l]:je,i=l=>{if(!l||l==="-"||String(l).trim()==="")return"";let c=String(l).split("-");return c.length===3&&c[0].length===4?`${c[2]}-${c[1]}-${c[0]}`:l},p=Ba(o),m=e?e.get("dash_filter"):null;N({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:m?{period:"Q3"}:{period:p},onDataLoaded:l=>(m&&(l=l.filter(c=>Pa(c,m))),l.sort((c,u)=>{let b=c.opening_date?new Date(c.opening_date).getTime():0;return(u.opening_date?new Date(u.opening_date).getTime():0)-b})),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:l=>dt(l)},{key:"period",label:"Periode",render:l=>ce(l)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:l=>i(l)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:l=>i(l)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:l=>i(l)},{key:"status",label:"Status",render:l=>j(l)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"activity_type",label:"Kegiatan",options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:je}],formFields:l=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:l?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:l?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:l?.period},{name:"pic",label:"PIC",type:"combobox",options:r(l?.pic),value:l?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:l?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:l?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:l?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:l?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:l?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let l=await x(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(l.ok){let c=l.data.data.map(u=>({Cabang:u.branch_name||"",Kegiatan:u.activity_type||"",Periode:u.period||"",PIC:u.pic||"","Tgl Opening":u.opening_date||"","Tgl Target":u.target_date||"","Tgl Selesai":u.completion_date||"",Status:u.status||""}));A(c,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async l=>{let u=(await x("/api/branches?all=1")).data?.data||[],b=S=>{if(!S)return null;let C=String(S||"").toLowerCase(),_=u.find(y=>String(y.full_name||"").toLowerCase()===C||String(y.code||"").toLowerCase()===C||String(y.name||"").toLowerCase()===C);return _?_.id:null},g=S=>{if(S==null||S==="")return"";if(S instanceof Date&&!isNaN(S.getTime()))return S.toISOString().slice(0,10);let C=String(S).trim();if(C===""||C==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(C))return C.slice(0,10);if(/^\d{4,5}$/.test(C)){let y=Number(C);if(y>2e4&&y<99999){let w=new Date(Date.UTC(1899,11,30)+y*864e5);return isNaN(w.getTime())?"":w.toISOString().slice(0,10)}}let _=C.split(/[\/\-\.]/);if(_.length===3){let[y,w,L]=_.map(E=>E.trim());if(y.length===4&&w.length<=2&&L.length<=2)return`${y}-${w.padStart(2,"0")}-${L.padStart(2,"0")}`;if(L.length===4&&w.length<=2&&y.length<=2)return`${L}-${w.padStart(2,"0")}-${y.padStart(2,"0")}`}return C},h=l.map(S=>({branch_id:b(String(S.Cabang||"").trim()),activity_type:String(S.Kegiatan||"").trim(),period:String(S.Periode||"").trim(),pic:String(S.PIC||S.Pic||"").trim(),opening_date:g(S["Tgl Opening"]||S["Tanggal Opening"]||S["Tgl Openir"]),target_date:g(S["Tgl Target"]||S["Tanggal Target"]),completion_date:g(S["Tgl Selesai"]||S["Tanggal Selesai"]),status:String(S.Status||"").trim(),notes:String(S.Catatan||S.Keterangan||"").trim()})).filter(S=>S.activity_type&&S.period),v=await x("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!v.ok)throw new Error(v.data?.error||"Import gagal");return v.data}}})}O();R();var ht=[],et=[];function La(t,e){let s=String(t.status||"").toLowerCase();return e==="open"?s==="open":!1}async function Mt(t,e){let s=e?e.get("dash_filter"):null;ht=await K(),et=await ee();let a=r=>r&&!et.find(i=>i.value===r)?[...et,{value:r,label:r}]:et,o=new Date().getFullYear();N({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:r=>s?r.filter(i=>La(i,s)):r,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:r=>`<span class="badge badge-secondary">${r}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:r=>`<span title="${r}">${r?.length>50?r.slice(0,50)+"\u2026":r}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:r=>`<span title="${r||""}">${r?.length>40?r.slice(0,40)+"\u2026":r||"-"}</span>`},{key:"status",label:"Status",render:r=>j(r)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:r=>window.formatDate(r)},{key:"day_count",label:"Hari",render:r=>r??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:ht},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:r=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:r?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:ht,value:r?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:r?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:r?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:r?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(r?.employee_name),value:r?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(r?.fc_specialist),value:r?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:r?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:r?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:r?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let r=await x(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(r.ok){let i=r.data.data.map(n=>({Tanggal:n.report_date||"",Cabang:n.branch_name||"",Kategori:n.category||"",Sumber:n.source||"",Keluhan:n.complaint||"","Nama FC":n.employee_name||"","FC Spesialis":n.fc_specialist||"",Solusi:n.solution||"","Tgl Selesai":n.completion_date||"",Status:n.status||""}));A(i,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{A([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async r=>{let n=(await x("/api/branches?all=1")).data?.data||[],p=c=>{if(!c)return null;let u=String(c||"").toLowerCase(),b=n.find(g=>String(g.full_name||"").toLowerCase()===u||String(g.code||"").toLowerCase()===u||String(g.name||"").toLowerCase()===u);return b?b.id:null},m=r.map(c=>({branch_id:p(String(c.Cabang||"").trim()),report_date:String(c.Tanggal||"").trim(),category:String(c.Kategori||"").trim(),source:String(c.Sumber||"").trim(),complaint:String(c.Keluhan||"").trim(),employee_name:String(c["Nama FC"]||"").trim(),fc_specialist:String(c["FC Spesialis"]||"").trim(),solution:String(c.Solusi||"").trim(),completion_date:String(c["Tgl Selesai"]||"").trim(),status:String(c.Status||"").trim()})).filter(c=>c.report_date&&c.complaint&&c.category),l=await x("/api/import/issues",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}}})}O();var De=[];function Aa(t,e){let s=String(t.status||"").toLowerCase();return e==="pending"?s==="pending":!1}async function Ot(t,e){let s=e?e.get("dash_filter"):null;De=await K();let a=await ee(),o=["Ade","Berlin"],d=i=>i&&!a.find(n=>n.value===i)?[...a,{value:i,label:i}]:a,r=i=>i&&!o.find(n=>(typeof n=="object"?n.value:n)===i)?[...o,i]:o;N({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:i=>s?i.filter(n=>Aa(n,s)):i,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:i=>`<span title="${i||""}">${i?.length>50?i.slice(0,50)+"\u2026":i||"-"}</span>`},{key:"solution",label:"Solusi",render:i=>`<span title="${i||""}">${i?.length>40?i.slice(0,40)+"\u2026":i||"-"}</span>`},{key:"status",label:"Status",render:i=>j(i)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:i=>window.formatDate(i)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:i=>i?`<a href="${i}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:De},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async i=>{let n=new URLSearchParams(i||{}).toString(),p=await x(`/api/one-on-one?limit=10000&${n}`);if(p.ok){let m=p.data.data.map(c=>({Tanggal:c.meeting_date||"",Cabang:c.branch_name||"","Nama Karyawan":c.employee_name||"",PIC:c.pic||"",Masalah:c.problem||"",Solusi:c.solution||"",Status:c.status||"","Tgl Selesai":c.completion_date||"",Dokumen:c.document_link||""})),{downloadExcel:l}=await Promise.resolve().then(()=>(R(),oe));l(m,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let i=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(R(),oe));n(i,"Template_Import_OneOnOne")},onImport:async i=>{let n=c=>{if(!c)return null;let u=String(c||"").toLowerCase(),b=De.find(g=>String(g.label||"").toLowerCase()===u);return b?b.value:null},p=c=>{if(!c)return"";if(c instanceof Date&&!isNaN(c.getTime()))return c.toISOString().slice(0,10);let u=String(c).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let h=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(h.getTime())?"":h.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let b=u.split(/[\/\-\.]/);if(b.length===3){let[g,h,v]=b.map(S=>S.trim());if(g.length===4&&h.length<=2&&v.length<=2)return`${g}-${h.padStart(2,"0")}-${v.padStart(2,"0")}`;if(v.length===4&&h.length<=2&&g.length<=2)return`${v}-${h.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=i.map(c=>({meeting_date:p(c.Tanggal),employee_name:String(c["Nama Karyawan"]||"").trim(),branch_id:n(String(c.Cabang||"").trim()),pic:String(c.PIC||"").trim(),problem:String(c.Masalah||"").trim(),solution:String(c.Solusi||"").trim(),status:String(c.Status||"").trim(),completion_date:p(c["Tgl Selesai"]),document_link:String(c.Dokumen||"").trim()})).filter(c=>c.meeting_date&&c.employee_name&&c.branch_id),l=await x("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}},formFields:i=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:i?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:i?.branch_id&&!De.find(n=>n.value==i.branch_id)?[...De,{value:i.branch_id,label:i.branch_name||i.branch_id}]:De,createApi:{path:"/api/branches",field:"full_name"},value:i?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:d(i?.employee_name),value:i?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:r(i?.pic),createApi:{path:"/api/pic",field:"name"},value:i?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:i?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:i?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:i?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:i?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:i?.document_link}]})}O();async function Rt(t){let e=await K(),s=await ee(),a=["Ade","Berlin"],o=i=>i&&!s.find(n=>n.value===i)?[...s,{value:i,label:i}]:s,d=i=>i&&!a.find(n=>(typeof n=="object"?n.value:n)===i)?[...a,i]:a,r=Array.from({length:5},(i,n)=>String(new Date().getFullYear()-n));N({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:i=>{try{let n=JSON.parse(i);return Array.isArray(n)?n.join(", "):i||"-"}catch{return i||"-"}}},{key:"score",label:"Nilai",render:i=>i!=null?`<strong>${i}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:i=>i?`<a href="${i}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:r}],exportOptions:{moduleName:"training",onExport:async i=>{let n=new URLSearchParams(i||{}).toString(),p=await x(`/api/training?limit=10000&${n}`);if(p.ok){let m=p.data.data.map(c=>{let u=c.participants||"";try{let b=JSON.parse(u);u=Array.isArray(b)?b.join(", "):u}catch{}return{Tanggal:c.training_date||"",Batch:c.batch||"",Materi:c.subject||"",Cabang:c.branch_name||"",Trainer:c.trainer||"",Peserta:u,Nilai:c.score!==null&&c.score!==void 0?c.score:"",Dokumen:c.document_link||""}}),{downloadExcel:l}=await Promise.resolve().then(()=>(R(),oe));l(m,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let i=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(R(),oe));n(i,"Template_Import_Training")},onImport:async i=>{let n=c=>{if(!c)return null;let u=String(c||"").toLowerCase(),b=e.find(g=>String(g.label||"").toLowerCase()===u);return b?b.value:null},p=c=>{if(!c)return"";if(c instanceof Date&&!isNaN(c.getTime()))return c.toISOString().slice(0,10);let u=String(c).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let h=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(h.getTime())?"":h.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let b=u.split(/[\/\-\.]/);if(b.length===3){let[g,h,v]=b.map(S=>S.trim());if(g.length===4&&h.length<=2&&v.length<=2)return`${g}-${h.padStart(2,"0")}-${v.padStart(2,"0")}`;if(v.length===4&&h.length<=2&&g.length<=2)return`${v}-${h.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=i.map(c=>({training_date:p(c.Tanggal),batch:String(c.Batch||"").trim(),subject:String(c.Materi||"").trim(),branch_id:n(String(c.Cabang||"").trim()),trainer:String(c.Trainer||"").trim(),participants:String(c.Peserta||"").trim(),score:c.Nilai?Number(c.Nilai):null,document_link:String(c.Dokumen||"").trim()})).filter(c=>c.training_date&&c.subject&&c.branch_id),l=await x("/api/import/training",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!l.ok)throw new Error(l.data?.error||"Import gagal");return l.data}},formFields:i=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:i?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:i?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:i?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:i?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:d(i?.trainer),value:i?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let n=JSON.parse(i?.participants);return Array.isArray(n)?n.join(", "):i?.participants||""}catch{return i?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:i?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:i?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:i?.notes}],onBeforeSubmit:async i=>(i.participants&&(i.participants=JSON.stringify(i.participants.split(",").map(n=>n.trim()).filter(Boolean))),i)})}O();ve();R();function Kt({container:t,title:e,icon:s,apiPath:a,columns:o,formFields:d,filterFields:r,defaultFilters:i={},itemLabel:n="Data",canCreate:p=!0,canEdit:m=!0,canDelete:l=!0,onBeforeSubmit:c,onAfterLoad:u,onDataLoaded:b,extraActions:g=[],initialSearch:h="",exportOptions:v=null,bulkDelete:S=!1,paginationMode:C="server"}){let _=1,y={...i};h&&(y.search=h);let w=new Set;t.innerHTML=`
  ...
  `,t.querySelectorAll(".cal-legend-pill").forEach(i=>{let n=i.querySelector('input[type="checkbox"]');i.addEventListener("click",p=>{p.preventDefault(),n.checked=!n.checked,i.classList.toggle("inactive",!n.checked),d()})}),document.getElementById("cal-prev").addEventListener("click",()=>{e.setMonth(e.getMonth()-1),d()}),document.getElementById("cal-next").addEventListener("click",()=>{e.setMonth(e.getMonth()+1),d()});async function o(){try{let i=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0");s=(await x(`/api/dashboard/calendar?month=${i}-${n}`)).data?.data||[]}catch(i){console.warn("[Calendar] Gagal memuat events:",i),s=[]}}async function d(){let i=document.getElementById("calendar-grid");if(i){i.innerHTML=`<div style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:1px;background:#E5E7EB;">
      ${Array(42).fill('<div style="background:#F9FAFB;min-height:90px;"></div>').join("")}
    </div>`,await o();try{let n=e.getFullYear(),p=e.getMonth(),m=document.getElementById("cal-month-label");m&&(m.textContent=e.toLocaleDateString("id-ID",{month:"long",year:"numeric"}));let l=new Set(Array.from(t.querySelectorAll(".cal-filter:checked")).map(y=>y.value)),c=s.filter(y=>l.has(y.type)),u={};c.forEach(y=>{let w=(y.event_date||"").slice(0,10);w&&(u[w]||(u[w]=[]),u[w].push(y))});let b=new Date(n,p,1).getDay(),g=new Date(n,p+1,0).getDate(),h=["Min","Sen","Sel","Rab","Kam","Jum","Sab"],v=new Date().toISOString().slice(0,10),S='<div style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:1px;background:#E5E7EB;border-radius:0 0 12px 12px;overflow:hidden;width:100%;">';h.forEach((y,w)=>{S+=`<div style="background:#F1F5F9;padding:10px 4px;text-align:center;font-size:0.68rem;font-weight:800;color:${w===0||w===6?"#EF4444":"#6B7280"};text-transform:uppercase;letter-spacing:0.06em;">${y}</div>`});for(let y=0;y<b;y++)S+='<div style="background:#FAFAFA;min-height:90px;"></div>';for(let y=1;y<=g;y++){let w=`${n}-${String(p+1).padStart(2,"0")}-${String(y).padStart(2,"0")}`,L=u[w]||[],E=w===v,H=new Date(n,p,y).getDay(),$=H===0||H===6,ge=4;S+=`
          <div class="cal-cell" data-date="${w}"
               style="background:${E?"#EFF6FF":"#fff"};min-height:90px;padding:6px 5px 5px;overflow:hidden;
                      ...
                      cursor:${L.length?"pointer":"default"};">
            <!-- Nomor tanggal -->
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
              <span style="font-size:${E?"0.9rem":"0.78rem"};font-weight:${E?"900":"600"};
                    color:${E?"#fff":$?"#EF4444":"#374151"};
                    ${E?"background:#2563EB;border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;":""}">
                ${y}
              </span>
              ${L.length>ge?`<span class="cal-more-btn">+${L.length-ge}</span>`:""}
            </div>

            <!-- Badge events -->
            <div style="display:flex;flex-direction:column;gap:3px;overflow:hidden;">
              ${L.slice(0,ge).map(se=>{let le=Wt(se.type),k=Ma(se.title||se.branch_name||le.label,18);return`<div class="cal-badge" style="background:${le.bg};" title="${Ie(se.title||le.label)} \u2014 ${Ie(se.branch_name||"")}">
                  ${le.icon} ${Ie(k)}
                </div>`}).join("")}
            </div>
          </div>`}let _=(b+g)%7;if(_!==0)for(let y=0;y<7-_;y++)S+='<div style="background:#FAFAFA;min-height:90px;"></div>';S+="</div>",i.innerHTML=S,i.querySelectorAll(".cal-cell[data-date]").forEach(y=>{y.addEventListener("click",()=>{let w=y.dataset.date,L=u[w]||[];L.length&&r(w,L)})})}catch(n){console.error("[Calendar] Render error:",n),i.innerHTML=`<div style="padding:60px;text-align:center;color:#9CA3AF;">
      ...
      </div>`}}}function r(i,n){document.getElementById("cal-popup-overlay")?.remove();let p=new Date(i+"T00:00:00").toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"}),m=n.map(c=>{let u=Wt(c.type);return`
      ...
      </div>`,document.body.appendChild(l),document.getElementById("cal-popup-close-btn").addEventListener("click",()=>l.remove()),l.addEventListener("click",c=>{c.target===l&&l.remove()})}d()}function Ma(t,e){return t?t.length>e?t.slice(0,e)+"\u2026":t:""}function Ie(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}O();async function Xt(t){let e=fe(),s=(e?.full_name||e?.username||"U")[0].toUpperCase(),o={superadmin:"#7C3AED",admin:"#2563EB",manager:"#0891B2",spv:"#059669",viewer:"#64748B"}[e?.role]||"#64748B";t.innerHTML=`
    <div class="page-header">
  ...
  `;let e=null,s=null,a=0,o={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function d(g){Object.entries(o).forEach(([h,v])=>{v.style.display=h===g?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let g=document.getElementById("btn-backup-db");g.disabled=!0,g.textContent="\u23F3 Memproses Backup...";try{let h=await x("/api/import/backup");if(h.ok){if(!window.XLSX){G("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");return}let v=window.XLSX,S=v.utils.book_new();Object.entries(h.data.database).forEach(([C,_])=>{let y=_.length>0?_:[{}],w=v.utils.json_to_sheet(y);v.utils.book_append_sheet(S,w,C.substring(0,31))}),v.writeFile(S,`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.xlsx`),Z("Backup berhasil diunduh!")}else G("Gagal memproses backup: "+(h.data?.error||"Unknown error"))}catch(h){G("Gagal memproses backup: "+h.message)}finally{g.disabled=!1,g.textContent="\u{1F4E6} Backup Database"}});let r=document.getElementById("btn-sync-google");r&&r.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let g=r.innerHTML;r.innerHTML='<span class="spinner"></span> Menyinkronkan...',r.disabled=!0;try{let h=await x("/api/sync/google-sheets",{method:"POST"});h.ok?alert("Sinkronisasi Berhasil: "+(h.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(h.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{r.innerHTML=g,r.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{ta(),Z("Template Excel berhasil didownload!")});let i=document.getElementById("file-input"),n=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",g=>{g.stopPropagation(),i.click()}),i.addEventListener("change",g=>{g.target.files[0]&&p(g.target.files[0])}),n.addEventListener("dragover",g=>{g.preventDefault(),n.classList.add("drag-over")}),n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n.addEventListener("drop",g=>{g.preventDefault(),n.classList.remove("drag-over");let h=g.dataTransfer.files[0];h&&h.name.match(/\.xlsx?$/i)?p(h):G("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,i.value="",document.getElementById("file-info").style.display="none",n.style.display="",d("upload")});async function p(g){e=g,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${g.name} (${(g.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",n.style.display="none",await m(g)}async function m(g){d("validating");let h=document.getElementById("validation-status"),v=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");h.textContent="Membaca file Excel...",v.style.width="20%",await Je(200);let S=await g.arrayBuffer(),C=window.XLSX.read(S,{type:"array",cellDates:!0});h.textContent=`Memvalidasi ${C.SheetNames.length} sheet...`,v.style.width="50%",await Je(100),s=ea(C),v.style.width="100%",h.textContent="Validasi selesai!",await Je(300),l()}catch(S){d("upload"),G("Gagal memproses file: "+S.message),document.getElementById("file-info").style.display="flex",n.style.display="none"}}function l(){d("preview");let g=s.filter(E=>!E.skipped).length,h=s.reduce((E,H)=>E+H.total,0),v=s.reduce((E,H)=>E+H.valid,0),S=s.reduce((E,H)=>E+H.errorCount,0),C=h>0?Math.round(v/h*100):0;document.getElementById("preview-summary-badges").innerHTML=`
        ...
        <tbody>
          ${s.map((E,H)=>`
            <tr class="${E.errorCount>0?"row-error":E.skipped?"row-skipped":"row-ok"}">
              ...
              <td style="text-align:center">
                ${E.errorCount>0?`<button class="btn btn-ghost btn-sm btn-detail-error" data-idx="${H}">\u{1F50D} ${E.errorCount} Error</button>`:'<span class="text-muted">\u2013</span>'}
              </td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    `,_.querySelectorAll(".btn-detail-error").forEach(E=>{E.addEventListener("click",()=>{let H=s[Number(E.dataset.idx)];c(H)})});let y=document.getElementById("error-detail-section"),w=document.getElementById("error-detail-container");w.innerHTML="",y.style.display="none";let L=document.getElementById("btn-start-import");v===0?(L.disabled=!0,L.innerHTML="\u26A0\uFE0F Tidak Ada Data Valid"):(L.disabled=!1,S>0?(L.innerHTML=`\u{1F680} Import ${v} Data Valid (${S} dilewati)`,L.title="Baris error akan dilewati, baris valid tetap diimport"):L.innerHTML=`\u{1F680} Mulai Import ${v} Data`)}function c(g){let h=document.getElementById("error-detail-section"),v=document.getElementById("error-detail-container");h.style.display="";let S=g.errors.slice(0,100).map(C=>(Array.isArray(C.errors)?C.errors:[]).map(y=>{let w=typeof y=="object";return`
          <tr>
            <td style="text-align:center"><span class="badge badge-danger">Baris ${C.row}</span></td>
            <td><strong>${w?y.column:"\u2014"}</strong></td>
            <td><code style="font-size:.78rem;color:var(--text-secondary)">${w&&y.originalValue!==void 0?y.originalValue||"(kosong)":"\u2014"}</code></td>
            <td class="error-msg">${w?y.reason:y}</td>
            <td style="font-size:.78rem;color:var(--success)">
              ${w&&y.aliases?`Gunakan salah satu nama kolom:<br><em>${y.aliases}</em>`:w&&y.hint?y.hint:""}
            </td>
          </tr>
    ...
    `,h.scrollIntoView({behavior:"smooth",block:"start"})}document.getElementById("btn-back-to-upload").addEventListener("click",()=>{d("upload"),document.getElementById("file-info").style.display="none",n.style.display="",e=null,i.value=""}),document.getElementById("btn-download-log").addEventListener("click",()=>{if(!s)return;aa(s)?Z("Log error berhasil didownload."):Z("Tidak ada error untuk didownload.")}),document.getElementById("btn-start-import").addEventListener("click",()=>{let g=document.querySelector('input[name="dup-strategy"]:checked')?.value||"skip";u(g)});async function u(g){d("importing"),a=Date.now();let h=[];Ka.forEach(y=>{let w=s?.find(L=>L.module===y&&L.mapped?.length>0);w&&h.push(w)});let v=document.getElementById("import-steps-list");v.innerHTML=h.map(y=>`
      <div class="import-step-item" id="step-item-${y.module}">
        <span class="step-item-icon" id="step-icon-${y.module}">\u23F8\uFE0F</span>
        <span class="step-item-label">${y.label} <span class="step-item-count">(${y.mapped.length} data)</span></span>
        <span class="step-item-status" id="step-status-${y.module}"></span>
      </div>
    `).join("");let S=document.getElementById("import-bar"),C=document.getElementById("import-current-status"),_={totalSheets:h.length,totalRows:h.reduce((y,w)=>y+w.mapped.length,0),inserted:0,skipped:0,failed:0,moduleResults:[]};for(let y=0;y<h.length;y++){let w=h[y],L=document.getElementById(`step-icon-${w.module}`),E=document.getElementById(`step-status-${w.module}`);L.textContent="\u{1F504}",E.textContent="Mengimport...",C.textContent=`Mengimport ${w.label}...`,S.style.width=`${Math.round(y/h.length*100)}%`;try{let H=await x(`/api/import/${w.module}`,{method:"POST",body:JSON.stringify({rows:w.mapped,onDuplicate:g})});if(H.ok){let $=H.data;_.inserted+=$.inserted||0,_.skipped+=$.skipped||0,_.moduleResults.push({label:w.label,inserted:$.inserted||0,skipped:$.skipped||0,status:"ok"}),L.textContent="\u2705",E.innerHTML=`<span class="badge badge-success">${$.inserted||0} berhasil</span>${$.skipped>0?` <span class="badge badge-neutral">${$.skipped} skip</span>`:""}`}else _.failed++,_.moduleResults.push({label:w.label,inserted:0,skipped:0,status:"error",error:H.data?.error}),L.textContent="\u274C",E.innerHTML='<span class="badge badge-danger">Gagal</span>'}catch(H){_.failed++,_.moduleResults.push({label:w.label,inserted:0,skipped:0,status:"error",error:H.message}),L.textContent="\u274C",E.innerHTML='<span class="badge badge-danger">Gagal</span>'}await Je(150)}S.style.width="100%",C.textContent="Selesai!",await Je(400),b(_)}function b(g){d("summary");let h=((Date.now()-a)/1e3).toFixed(1),v=g.failed===0;document.getElementById("summary-status-icon").innerHTML=`
    ...
    `}document.getElementById("btn-import-again").addEventListener("click",()=>{e=null,s=null,i.value="",document.getElementById("file-info").style.display="none",n.style.display="",d("upload")}),document.getElementById("btn-go-to-dashboard").addEventListener("click",()=>{window.location.hash="/dashboard"})}function Je(t){return new Promise(e=>setTimeout(e,t))}O();var at=[],ia=[];async function ra(t){at=await K(),ia=await ee(),N({container:t,title:"Data SP (Surat Peringatan)",icon:"\u2709\uFE0F",apiPath:"/api/sp",itemLabel:"SP",bulkDelete:!0,columns:[{key:"employee_name",label:"Nama Karyawan"},{key:"division",label:"Divisi",render:e=>e?`<span class="badge badge-info">${e}</span>`:"-"},{key:"branch_name",label:"Cabang"},{key:"tanggal",label:"Tanggal Sp",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"akhir_sp",label:"Akhir Sp",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"sp_type",label:"Jenis Sp",render:e=>`<span class="badge badge-warning">${e||"-"}</span>`},{key:"document_link",label:"Link Document / Foto",render:e=>e?`<a href="${e}" target="_blank" class="text-primary hover-underline">Lihat</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"branch_id",label:"Cabang",options:at}],exportOptions:{moduleName:"sp_data",onExport:async e=>{let s=new URLSearchParams(e||{}).toString(),a=await x(`/api/sp?limit=10000&${s}`);if(a.ok){let o=a.data.data.map(r=>({"Nama Karyawan":r.employee_name||"",Divisi:r.division||"",Cabang:r.branch_name||"","Tanggal Sp":r.tanggal||"","Akhir Sp":r.akhir_sp||"","Jenis Sp":r.sp_type||"","Link Document / Foto":r.document_link||""})),{downloadExcel:d}=await Promise.resolve().then(()=>(R(),oe));d(o,`Data_SP_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let e=[{"Nama Karyawan":"Budi Santoso",Divisi:"FACILITY CARE",Cabang:"001. Pondok Bambu","Tanggal Sp":"2026-01-08","Akhir Sp":"2026-07-08","Jenis Sp":"SP 1","Link Document / Foto":"https://link.doc"}],{downloadExcel:s}=await Promise.resolve().then(()=>(R(),oe));s(e,"Template_Import_SP")},onImport:async e=>{let s=r=>{if(!r)return null;let i=String(r||"").toLowerCase(),n=at.find(p=>String(p.label||"").toLowerCase()===i);return n?n.value:null},a=r=>{if(!r)return"";if(r instanceof Date&&!isNaN(r.getTime()))return r.toISOString().slice(0,10);let i=String(r).trim();if(/^\d{4,5}$/.test(i)){let p=Number(i);if(p>2e4&&p<99999){let m=new Date(Date.UTC(1899,11,30)+p*864e5);return isNaN(m.getTime())?"":m.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(i))return i.slice(0,10);let n=i.split(/[\/\-\.]/);if(n.length===3){let[p,m,l]=n.map(c=>c.trim());if(p.length===4&&m.length<=2&&l.length<=2)return`${p}-${m.padStart(2,"0")}-${l.padStart(2,"0")}`;if(l.length===4&&m.length<=2&&p.length<=2)return`${l}-${m.padStart(2,"0")}-${p.padStart(2,"0")}`}return i},o=e.map(r=>({employee_name:String(r["Nama Karyawan"]||"").trim(),division:String(r.Divisi||"").trim(),branch_id:s(String(r.Cabang||"").trim()),tanggal:a(r["Tanggal Sp"]),akhir_sp:a(r["Akhir Sp"]),sp_type:String(r["Jenis Sp"]||"").trim(),document_link:String(r["Link Document / Foto"]||"").trim()})).filter(r=>r.employee_name&&r.branch_id),d=await x("/api/import/sp",{method:"POST",body:JSON.stringify({rows:o,onDuplicate:"update"})});if(!d.ok)throw new Error(d.data?.error||"Import gagal");return d.data}},formFields:[{type:"combobox",name:"employee_name",label:"Nama Karyawan",required:!0,options:ia},{type:"select",name:"division",label:"Divisi",options:["FACILITY CARE","SECURITY"],required:!0},{type:"combobox",name:"branch_id",label:"Cabang",required:!0,options:at,createApi:{path:"/api/branches",field:"full_name"}},{type:"date",name:"tanggal",label:"Tanggal Sp",required:!0},{type:"date",name:"akhir_sp",label:"Akhir Sp",required:!0},{type:"select",name:"sp_type",label:"Jenis Sp",required:!0,options:["SP 1","SP 2","SP 3","Teguran Lisan"]},{type:"url",name:"document_link",label:"Link Document / Foto"}]})}O();var Be=[],sa=[];async function oa(t){Be=await K(),sa=await ee(),N({container:t,title:"Data Mutasi",icon:"\u{1F504}",apiPath:"/api/mutasi",itemLabel:"Mutasi",bulkDelete:!0,columns:[{key:"tanggal",label:"Tanggal",render:e=>e?new Date(e).toLocaleDateString("id-ID",{year:"numeric",month:"short",day:"numeric"}):"-"},{key:"employee_name",label:"Nama Karyawan"},{key:"from_branch_name",label:"Cabang Asal"},{key:"to_branch_name",label:"Cabang Tujuan"},{key:"status",label:"Status",render:e=>`<span class="badge ${e==="Selesai"?"badge-success":"badge-warning"}">${e||"-"}</span>`},{key:"document_link",label:"Dokumen",render:e=>e?`<a href="${e}" target="_blank" class="text-primary hover-underline">Lihat</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama karyawan..."},{type:"combobox",name:"from_branch_id",label:"Cabang Asal",options:Be},{type:"combobox",name:"to_branch_id",label:"Cabang Tujuan",options:Be}],exportOptions:{moduleName:"mutasi_data",onExport:async e=>{let s=new URLSearchParams(e||{}).toString(),a=await x(`/api/mutasi?limit=10000&${s}`);if(a.ok){let o=a.data.data.map(r=>({Tanggal:r.tanggal||"","Nama Karyawan":r.employee_name||"","Cabang Asal":r.from_branch_name||"","Cabang Tujuan":r.to_branch_name||"",Status:r.status||"",Dokumen:r.document_link||""})),{downloadExcel:d}=await Promise.resolve().then(()=>(R(),oe));d(o,`Data_Mutasi_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let e=[{Tanggal:"2026-01-08","Nama Karyawan":"Widya Astuti","Cabang Asal":"001. Pondok Bambu","Cabang Tujuan":"007. Bekasi",Status:"Selesai",Dokumen:"https://link.doc"}],{downloadExcel:s}=await Promise.resolve().then(()=>(R(),oe));s(e,"Template_Import_Mutasi")},onImport:async e=>{let s=r=>{if(!r)return null;let i=String(r||"").toLowerCase(),n=Be.find(p=>String(p.label||"").toLowerCase()===i);return n?n.value:null},a=r=>{if(!r)return"";if(r instanceof Date&&!isNaN(r.getTime()))return r.toISOString().slice(0,10);let i=String(r).trim();if(/^\d{4,5}$/.test(i)){let p=Number(i);if(p>2e4&&p<99999){let m=new Date(Date.UTC(1899,11,30)+p*864e5);return isNaN(m.getTime())?"":m.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(i))return i.slice(0,10);let n=i.split(/[\/\-\.]/);if(n.length===3){let[p,m,l]=n.map(c=>c.trim());if(p.length===4&&m.length<=2&&l.length<=2)return`${p}-${m.padStart(2,"0")}-${l.padStart(2,"0")}`;if(l.length===4&&m.length<=2&&p.length<=2)return`${l}-${m.padStart(2,"0")}-${p.padStart(2,"0")}`}return i},o=e.map(r=>({tanggal:a(r.Tanggal),employee_name:String(r["Nama Karyawan"]||"").trim(),from_branch_id:s(String(r["Cabang Asal"]||"").trim()),to_branch_id:s(String(r["Cabang Tujuan"]||"").trim()),status:String(r.Status||"").trim(),document_link:String(r.Dokumen||"").trim()})).filter(r=>r.tanggal&&r.employee_name&&r.from_branch_id&&r.to_branch_id),d=await x("/api/import/mutasi",{method:"POST",body:JSON.stringify({rows:o,onDuplicate:"update"})});if(!d.ok)throw new Error(d.data?.error||"Import gagal");return d.data}},formFields:[{type:"date",name:"tanggal",label:"Tanggal",required:!0},{type:"combobox",name:"employee_name",label:"Nama Karyawan",required:!0,options:sa},{type:"combobox",name:"from_branch_id",label:"Cabang Asal",required:!0,options:Be,createApi:{path:"/api/branches",field:"full_name"}},{type:"combobox",name:"to_branch_id",label:"Cabang Tujuan",required:!0,options:Be,createApi:{path:"/api/branches",field:"full_name"}},{type:"select",name:"status",label:"Status",required:!0,options:["Proses","Selesai"]},{type:"url",name:"document_link",label:"Link Dokumen (Opsional)"}]})}O();async function la(t){t.innerHTML=`
  ...
  `;let e=document.getElementById("btnRefreshSync"),s=document.getElementById("queueStatusFilter");e.addEventListener("click",o),s.addEventListener("change",i),document.getElementById("btnRetryAll").addEventListener("click",()=>a("retry",{allFailed:!0})),document.getElementById("btnResetStuck").addEventListener("click",()=>a("reset-stuck",{})),document.getElementById("btnPauseSync").addEventListener("click",()=>a("pause",{})),document.getElementById("btnResumeSync").addEventListener("click",()=>a("resume",{})),document.getElementById("btnReconcile").addEventListener("click",()=>a("reconcile",{module:"ALL",repairMode:!0})),document.getElementById("btnRetrySelected").addEventListener("click",()=>{let p=Array.from(document.querySelectorAll(".chk-queue:checked")).map(m=>m.value);if(p.length===0)return alert("No items selected");a("retry",{ids:p})}),document.getElementById("chkAllQueue").addEventListener("change",p=>{document.querySelectorAll(".chk-queue").forEach(m=>m.checked=p.target.checked)});async function a(p,m){if(confirm(`Are you sure you want to execute action: ${p}?`)){showLoading();try{let l=await x(`/api/sync/actions/${p}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)});l.ok?(alert(l.data?.message||"Success"),o()):G(l.error||"Action failed")}catch(l){G(l.message)}hideLoading()}}await o();async function o(){showLoading(),await Promise.all([r(),i(),d(),n()]),hideLoading()}async function d(){try{let p=await x("/api/sync/performance");if(!p.ok)return;let{webhook:m,google_api:l,d1:c,queue:u,throughput:b}=p.data;document.getElementById("latencyHistograms").innerHTML=`
      ...
      `,document.getElementById("lblSnapshotStatus").innerHTML='Checked 10 modules. Status: <b>COMPLETED</b><br><span class="text-xs text-gray-500">Run today at 02:00</span>'}catch(p){console.error(p)}}async function r(){try{let p=await x("/api/sync/health");if(!p.ok)return G("Failed to fetch sync health");let{status:m,queue:l,circuit_breaker:c}=p.data,u=`
      ...
      `;document.getElementById("syncOverviewCards").innerHTML=u;let b=document.getElementById("cbStateBadge"),g=document.getElementById("cbStateDesc"),h=document.getElementById("cbStatusCard");h.className="bg-white rounded-lg shadow p-6 border-l-4",c==="CLOSED"?(b.className="px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800",b.textContent="CLOSED",g.textContent="Traffic is flowing normally to Google Sheets.",h.classList.add("border-green-500")):c==="OPEN"?(b.className="px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800",b.textContent="OPEN",g.textContent="Failing fast. Traffic to Google Sheets is paused due to repeated failures.",h.classList.add("border-red-500")):c==="HALF_OPEN"?(b.className="px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800",b.textContent="HALF-OPEN",g.textContent="Testing recovery. Permitting limited traffic to verify stability.",h.classList.add("border-yellow-500")):b.textContent=c||"UNKNOWN"}catch(p){console.error(p)}}async function i(){try{let p=document.getElementById("queueStatusFilter").value,m=await x("/api/sync/queue?limit=15"+(p?"&status="+p:""));if(!m.ok)return;let l=document.getElementById("queueTableBody"),c=m.data?.data||m.data||[];if(c.length===0){l.innerHTML='<tr><td colspan="6" class="px-4 py-4 text-center text-gray-500">No events found</td></tr>';return}l.innerHTML=c.map(u=>`
      ...
      `).join("")}catch(p){console.error(p)}}async function n(){try{let p=await x("/api/sync/metrics");if(!p.ok)return;let m=document.getElementById("metricsTableBody"),l=p.data||[];if(l.length===0){m.innerHTML='<tr><td colspan="5" class="px-4 py-4 text-center text-gray-500">No metrics found in last 24h</td></tr>';return}m.innerHTML=l.map(c=>`
        <tr class="hover:bg-gray-50">
      ...
      `).join("")}catch(p){console.error(p)}}}window.parseFlexibleDate=t=>{if(!t||t==="-")return"";if(t=String(t).trim(),/^\d{5}$/.test(t)){let e=Math.floor(Number(t)-25569);return new Date(e*86400*1e3).toISOString().split("T")[0]}if(t.match(/^\d{2}[\/\-]\d{2}[\/\-]\d{4}$/)){let e=t.split(/[\/\-]/);return`${e[2]}-${e[1]}-${e[0]}`}return t.split("T")[0]};window.formatDate=t=>{let e=window.parseFlexibleDate(t);if(!e)return"";let s=e.split("-");if(s.length===3&&s[0].length===4){let a=["Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"],o=parseInt(s[2],10),d=a[parseInt(s[1],10)-1];return`${o} ${d} ${s[0]}`}return e};function z(t){return async e=>{if(!Ae()){xe("/login");return}return t(e)}}var Ue=null;function Ha(){Ue&&clearInterval(Ue);let t=()=>{let e=new Date,s=e.toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit",second:"2-digit"}),a=e.toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"}),o=document.getElementById("header-clock-time"),d=document.getElementById("header-clock-date");o&&(o.textContent=s),d&&(d.textContent=a)};t(),Ue=setInterval(t,1e3)}async function qa(){try{let t=await x("/api/dashboard/kpi");if(!t.ok)return;let e=t.data?.data||t.data||{},s=(a,o)=>{let d=document.getElementById(a);d&&(d.textContent=o>0?o:"",d.style.display=o>0?"inline-flex":"none")};s("badge-issues",e.issues?.current||0),s("badge-contracts",e.expiring30?.current||0),s("badge-oo1",e.one_on_one?.current||0),s("badge-schedule",e.schedule?.current||0),s("badge-supply",e.supply?.current||0)}catch{}}var Pe=[];async function ja(){try{let t=await x("/api/dashboard/notifications");if(!t.ok)return;Pe=t.data?.data||t.data||[];let e=document.getElementById("notif-dot");e&&(e.style.display=Pe.length>0?"block":"none",e.textContent=Pe.length)}catch{}}function Ja(){if(!Pe.length){de({title:"Notifikasi",content:'<div class="empty-state"><p>Tidak ada notifikasi baru.</p></div>',confirmText:"Tutup",onConfirm:(e,s)=>s()});return}let t=`
  ...
  `;de({title:`Notifikasi (${Pe.length})`,content:t,confirmText:"Tutup",onConfirm:(e,s)=>s()})}function da(){let t=fe(),e=(t?.full_name||"U")[0].toUpperCase();document.getElementById("app").innerHTML=`
  ...
  `;let s=document.getElementById("sidebar"),a=document.getElementById("sidebar-overlay"),o=document.getElementById("topbar-menu-btn"),d=document.getElementById("sidebar-close"),r=()=>{s.classList.add("open"),a.classList.add("show")},i=()=>{s.classList.remove("open"),a.classList.remove("show")};o?.addEventListener("click",r),d?.addEventListener("click",i),a?.addEventListener("click",i),document.querySelectorAll(".nav-item").forEach(p=>p.addEventListener("click",i));function n(){let p=window.location.hash.replace("#","")||"/dashboard";document.querySelectorAll(".nav-item").forEach(c=>{let u=c.dataset.route;c.classList.toggle("active",p===u||u!=="/dashboard"&&p.startsWith(u))});let m=document.getElementById("topbar-title"),l=document.querySelector(".nav-item.active .nav-label");m&&l&&(m.textContent=l.textContent)}window.addEventListener("hashchange",n),n(),Ha(),document.getElementById("btn-fullscreen")?.addEventListener("click",()=>{document.fullscreenElement?document.exitFullscreen?.():document.documentElement.requestFullscreen?.()}),document.getElementById("logout-btn")?.addEventListener("click",async()=>{await x("/api/auth/logout",{method:"POST"}),Ne(),Ue&&clearInterval(Ue),xe("/login")}),qa(),ja(),document.getElementById("btn-notif")?.addEventListener("click",p=>{p.preventDefault(),Ja()})}async function Ua(){J("/login",({main:e})=>Pt(e)),J("/dashboard",z(({main:e})=>Et(e))),J("/calendar",z(({main:e})=>Vt(e))),J("/employees",z(({main:e,params:s})=>Lt(e,s))),J("/contracts",z(({main:e,params:s})=>Nt(e,s))),J("/sp",z(({main:e})=>ra(e))),J("/mutasi",z(({main:e})=>oa(e))),J("/sync-dashboard",z(({main:e})=>la(e))),J("/timeline",z(({main:e,params:s})=>Ft(e,s))),J("/issues",z(({main:e,params:s})=>Mt(e,s))),J("/one-on-one",z(({main:e,params:s})=>Ot(e,s))),J("/training",z(({main:e})=>Rt(e))),J("/relievers",z(({main:e,params:s})=>Ht(e,s))),J("/reports/inspection",z(({main:e})=>qt(e))),J("/reports/cleaning",z(({main:e})=>jt(e))),J("/reports/fogging",z(({main:e})=>Jt(e))),J("/reports/basecamp",z(({main:e})=>Ut(e))),J("/reports/supply",z(({main:e})=>yt(e,"supply"))),J("/sop",z(({main:e})=>Gt(e))),J("/checklist",z(({main:e})=>zt(e))),J("/forms",z(({main:e})=>yt(e))),J("/users",z(({main:e})=>Qt(e))),J("/branches",z(({main:e})=>Yt(e))),J("/profile",z(({main:e})=>Xt(e))),J("/settings/import",z(({main:e})=>na(e)));let t=Ae();if(!t&&window.location.hash!=="#/login"&&xe("/login"),t){let e=await x("/api/auth/me");e.ok?(Fe(e.data.data),da()):(Ne(),xe("/login"))}window.addEventListener("fm:login",()=>{da(),xe("/dashboard")}),vt()}Ua();
