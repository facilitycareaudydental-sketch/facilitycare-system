var ra=Object.defineProperty;var Je=(t,e)=>()=>(t&&(e=t(t=0)),e);var Ge=(t,e)=>{for(var r in e)ra(t,r,{get:e[r],enumerable:!0})};var pe={};Ge(pe,{API:()=>gt,CLIENT_SIDE_MAX_ROWS:()=>xe,IS_DEVELOPMENT:()=>Qe,apiFetch:()=>f,clearToken:()=>Te,getToken:()=>Ce,getUser:()=>ce,setToken:()=>Ve,setUser:()=>$e});function Ce(){return localStorage.getItem("fm_token")}function Ve(t){localStorage.setItem("fm_token",t)}function Te(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function ce(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function $e(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function f(t,e={}){let r=Ce(),a={"Content-Type":"application/json",...r?{Authorization:`Bearer ${r}`}:{},...e.headers||{}};try{let l=`cb=${Date.now()}`,o=t.includes("?")?"&":"?",i=`${gt}${t}${o}${l}`,s=await fetch(i,{...e,headers:a}),n;try{let c=await s.text();try{n=JSON.parse(c)}catch{n={error:`Server Error (${s.status}): ${c.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return s.status===401&&(Te(),window.location.hash="#/login"),{ok:s.ok,status:s.status,data:n}}catch(l){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${l.message})`}}}}var Qe,sa,gt,xe,D=Je(()=>{Qe=!1,sa="https://fm-operations-api.facilitycare-audydental.workers.dev",gt=sa,xe=1e4});var yt={};Ge(yt,{confirmDialog:()=>ze,createModal:()=>ae});function ae({title:t,content:e,onConfirm:r,onCancel:a,confirmText:l="Simpan",cancelText:o="Batal",size:i="md",confirmClass:s="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},c=document.createElement("div");c.className="modal-overlay",c.innerHTML=`
  ...
  `;function I(){if(!document.getElementById("bulk-toolbar"))return;let x=document.getElementById("bulk-count"),A=document.getElementById("btn-bulk-delete"),j=document.getElementById("btn-bulk-cancel");x.textContent=`${C.size} item dipilih`,C.size>0?(A.disabled=!1,j.disabled=!1):(A.disabled=!0,j.disabled=!0);let U=document.getElementById("select-all-checkbox");if(U){let Y=document.querySelectorAll(".row-checkbox");if(Y.length>0){let O=[...Y].every(le=>C.has(le.value)),R=[...Y].some(le=>C.has(le.value));U.checked=O,U.indeterminate=R&&!O}}}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{C.clear(),document.querySelectorAll(".row-checkbox").forEach(x=>x.checked=!1);let _=document.getElementById("select-all-checkbox");_&&(_.checked=!1),I()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(C.size===0)return;let _=[...C],x=document.createElement("div");x.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",x.innerHTML=`
