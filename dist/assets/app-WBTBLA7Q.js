var la=Object.defineProperty;var tt=(t,e)=>()=>(t&&(e=t(t=0)),e);var at=(t,e)=>{for(var s in e)la(t,s,{get:e[s],enumerable:!0})};var fe={};at(fe,{API:()=>yt,CLIENT_SIDE_MAX_ROWS:()=>ge,IS_DEVELOPMENT:()=>De,apiFetch:()=>k,clearToken:()=>Pe,getToken:()=>Ie,getUser:()=>be,setToken:()=>nt,setUser:()=>Le});function Ie(){return localStorage.getItem("fm_token")}function nt(t){localStorage.setItem("fm_token",t)}function Pe(){localStorage.removeItem("fm_token"),localStorage.removeItem("fm_user")}function be(){try{return JSON.parse(localStorage.getItem("fm_user")||"null")}catch{return null}}function Le(t){localStorage.setItem("fm_user",JSON.stringify(t))}async function k(t,e={}){let s=Ie(),a={"Content-Type":"application/json",...s?{Authorization:`Bearer ${s}`}:{},...e.headers||{}};try{let l=`cb=${Date.now()}`,d=t.includes("?")?"&":"?",i=`${yt}${t}${d}${l}`,r=await fetch(i,{...e,headers:a}),n;try{let o=await r.text();try{n=JSON.parse(o)}catch{n={error:`Server Error (${r.status}): ${o.substring(0,80)}...`}}}catch{n={error:"Gagal membaca respon dari server"}}return r.status===401&&(Pe(),window.location.hash="#/login"),{ok:r.ok,status:r.status,data:n}}catch(l){return{ok:!1,status:0,data:{error:`Koneksi terputus. Periksa jaringan Anda. (${l.message})`}}}}var De,oa,yt,ge,N=tt(()=>{De=!1,oa="https://fm-operations-api.facilitycare-audydental.workers.dev",yt=oa,ge=1e4});var kt={};at(kt,{confirmDialog:()=>Ne,createModal:()=>le});function le({title:t,content:e,onConfirm:s,onCancel:a,confirmText:l="Simpan",cancelText:d="Batal",size:i="md",confirmClass:r="btn-primary"}){let n={sm:"400px",md:"560px",lg:"720px",xl:"900px"},o=document.createElement("div");o.className="modal-overlay",o.innerHTML=`
  ...
  `;function O(){if(!document.getElementById("bulk-toolbar"))return;let f=document.getElementById("bulk-count"),E=document.getElementById("btn-bulk-delete"),I=document.getElementById("btn-bulk-cancel");f.textContent=`${C.size} item dipilih`,C.size>0?(E.disabled=!1,I.disabled=!1):(E.disabled=!0,I.disabled=!0)}document.getElementById("btn-bulk-cancel")?.addEventListener("click",()=>{C.clear(),document.querySelectorAll(".row-checkbox").forEach(f=>f.checked=!1);let v=document.getElementById("select-all-checkbox");v&&(v.checked=!1),O()}),document.getElementById("btn-bulk-delete")?.addEventListener("click",()=>{if(C.size===0)return;let v=[...C],f=document.createElement("div");f.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center",f.innerHTML=`
    ...
    `,document.body.appendChild(f),f.querySelector("#bulk-cancel-btn").addEventListener("click",()=>f.remove()),f.querySelector("#bulk-confirm-btn").addEventListener("click",async()=>{let E=f.querySelector("#bulk-confirm-btn");E.disabled=!0,E.textContent="Menghapus...";let I=await k(`${a}/bulk`,{method:"DELETE",body:JSON.stringify({ids:v})});f.remove(),I.ok?(ee(`${v.length} ${n} berhasil dihapus.`),C.clear(),O(),A()):V(I.data?.error||"Gagal menghapus data.")})});let T=document.getElementById("filter-search"),M;if(T?.addEventListener("input",v=>{clearTimeout(M),M=setTimeout(()=>{w.search=v.target.value,x=1,A()},400)}),i?.forEach(v=>{v.type==="select"&&document.getElementById(`filter-${v.name}`)?.addEventListener("change",f=>{w[v.name]=f.target.value,x=1,A()})}),document.getElementById("btn-reset-filter")?.addEventListener("click",()=>{w={...r},T&&(T.value=""),i?.forEach(v=>{let f=document.getElementById(`filter-${v.name}`);f&&(f.value="")}),x=1,A()}),document.getElementById("btn-create")?.addEventListener("click",()=>pe(null)),y){document.getElementById(`btn-export-${y.moduleName}`)?.addEventListener("click",async f=>{let E=f.target,I=E.innerHTML;E.innerHTML="\u23F3 Loading...",E.disabled=!0;try{await y.onExport()}catch{V("Gagal export data")}finally{E.innerHTML=I,E.disabled=!1}}),document.getElementById(`btn-template-${y.moduleName}`)?.addEventListener("click",()=>{y.onTemplate()});let v=document.getElementById(`input-import-${y.moduleName}`);v?.addEventListener("change",async f=>{let E=f.target.files[0];if(!E)return;v.disabled=!0;let I=document.createElement("div");I.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center",I.innerHTML=`
      ...
      `,document.body.appendChild(I);let W=I.querySelector("#import-progress-text"),U=I.querySelector("#import-progress-bar"),P=I.querySelector("#import-summary"),$=I.querySelector("#import-close-btn");$.addEventListener("click",()=>{I.remove(),A()});try{let X=await Fe(E);if(X.length===0)throw new Error("File kosong atau format salah");let ne=500,Z=0,F=0,R=0,K=X.length;W.textContent=`Ditemukan ${K} baris data. Memulai import...`;for(let D=0;D<K;D+=ne){let z=X.slice(D,D+ne);W.textContent=`Mengimport baris ${D+1} - ${Math.min(D+ne,K)} dari ${K}...`,U.style.width=`${Math.round(D/K*100)}%`;try{let G=await y.onImport(z);G?(Z+=G.inserted||G.metrics?.inserted||z.length,F+=G.skipped||G.metrics?.updated||0):Z+=z.length}catch(G){console.error("Chunk import failed:",G),R+=z.length}}U.style.width="100%",W.innerHTML='<strong style="color:var(--success)">\u2705 Import Selesai!</strong>',P.style.display="block",P.innerHTML=`
...
`)}let w=o(S,y),C=null;return w||(C="Karyawan tidak ditemukan di Database"),{isValid:!!w,invalidReason:C,rowNum:y,data:{employee_id:w,branch_id:n(String(g.Cabang||"").trim()),division:String(g["Div / Bagian"]||"").trim()||"FACILITY CARE",start_date:x,end_date:m(g["Tanggal Selesai"])||"2099-12-31",status:String(g.Status||"").trim(),_rawName:S}}}),p=[],u=[];if(c.forEach(g=>{g.isValid?p.push(g.data):u.push({rowNum:g.rowNum,name:g.data._rawName,reason:g.invalidReason})}),console.log(`Split Validation - Valid: ${p.length}, Invalid: ${u.length}`),p.length===0)return{inserted:0,skipped:a.length,failed:a.length};let h=await k("/api/import/contracts",{method:"POST",body:JSON.stringify({rows:p,onDuplicate:"update"})});if(!h.ok)throw new Error(h.data?.error||"Import gagal");return h.data}}})}N();H();var gt=[],Re=[];function $a(t){if(!Array.isArray(t))return"Q3";let e=["Q4","Q3","Q2","Q1"];for(let s of e)if(t.some(a=>a.period===s))return s;return"Q3"}function Da(t,e){if(t.period!=="Q3")return!1;let s=String(t.status||"").toLowerCase();if(s!=="selesai"&&s!=="completed"&&s!=="done")return!1;let a=String(t.activity_type||"").toLowerCase();return e==="inspeksi"?a.includes("inspeksi"):e==="gcdc"?a.includes("general cleaning")||a.includes("deep cleaning"):!1}async function Nt(t,e){gt=await q();let s=await te();Re=["BERLIN ARIANSYAH","ADE SURAHMAN"];let a=o=>o&&!Re.find(m=>(typeof m=="object"?m.value:m)===o)?[...Re,o]:Re,l=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`),d=o=>{if(!o||o==="-"||String(o).trim()==="")return"";let m=String(o).split("-");return m.length===3&&m[0].length===4?`${m[2]}-${m[1]}-${m[0]}`:o},i=l.data?.data||[],r=$a(i),n=e?e.get("dash_filter"):null;B({container:t,title:"Jadwal Kegiatan",icon:"\u{1F4C5}",apiPath:"/api/schedule",bulkDelete:!0,itemLabel:"Jadwal",paginationMode:"client",defaultFilters:n?{period:"Q3"}:{period:r},onDataLoaded:o=>(n&&(o=o.filter(m=>Da(m,n))),o.sort((m,c)=>{let p=m.opening_date?new Date(m.opening_date).getTime():0;return(c.opening_date?new Date(c.opening_date).getTime():0)-p})),columns:[{key:"branch_name",label:"Cabang"},{key:"activity_type",label:"Kegiatan",render:o=>st(o)},{key:"period",label:"Periode",render:o=>oe(o)},{key:"pic",label:"PIC"},{key:"opening_date",label:"Tgl Opening",nowrap:!0,render:o=>d(o)},{key:"target_date",label:"Tgl Target",nowrap:!0,render:o=>d(o)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:o=>d(o)},{key:"status",label:"Status",render:o=>j(o)}],filterFields:[{type:"select",name:"branch_id",label:"Cabang",options:gt},{type:"select",name:"activity_type",label:"Kegiatan",options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"]},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"period",label:"Periode",options:["Q1","Q2","Q3","Q4"]},{type:"select",name:"status",label:"Status",options:["Pending","In Progress","Done"]},{type:"select",name:"pic",label:"PIC",options:Re}],formFields:o=>[{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:gt,value:o?.branch_id},{name:"activity_type",label:"Jenis Kegiatan",type:"select",required:!0,options:["Inspeksi Hygiene","General Cleaning","Deep Cleaning","Fogging"],value:o?.activity_type}]},{type:"row",fields:[{name:"period",label:"Periode",type:"select",required:!0,options:["Q1","Q2","Q3","Q4"],value:o?.period},{name:"pic",label:"PIC",type:"combobox",options:a(o?.pic),value:o?.pic}]},{type:"row",fields:[{name:"opening_date",label:"Tanggal Opening",type:"date",value:o?.opening_date},{name:"target_date",label:"Tanggal Target",type:"date",value:o?.target_date}]},{type:"row",fields:[{name:"completion_date",label:"Tanggal Selesai",type:"date",value:o?.completion_date},{name:"status",label:"Status",type:"select",required:!0,options:["Pending","In Progress","Done"],value:o?.status||""}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:o?.notes}],exportOptions:{moduleName:"schedule",onExport:async()=>{let o=await k(`/api/schedule${window.location.search?window.location.search+"&":"?"}limit=10000`);if(o.ok){let m=o.data.data.map(c=>({Cabang:c.branch_name||"",Kegiatan:c.activity_type||"",Periode:c.period||"",PIC:c.pic||"","Tgl Opening":c.opening_date||"","Tgl Target":c.target_date||"","Tgl Selesai":c.completion_date||"",Status:c.status||""}));L(m,"Data_Jadwal_Kegiatan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Cabang:"001. Pondok Bambu",Kegiatan:"General Cleaning",Periode:"Q1",PIC:"Fajar","Tgl Opening":"2024-02-01","Tgl Target":"2024-02-15","Tgl Selesai":"2024-02-14",Status:"Done"}],"Template_Import_Jadwal")},onImport:async o=>{let c=(await k("/api/branches?all=1")).data?.data||[],p=b=>{if(!b)return null;let y=String(b||"").toLowerCase(),S=c.find(_=>String(_.full_name||"").toLowerCase()===y||String(_.code||"").toLowerCase()===y||String(_.name||"").toLowerCase()===y);return S?S.id:null},u=b=>{if(b==null||b==="")return"";if(b instanceof Date&&!isNaN(b.getTime()))return b.toISOString().slice(0,10);let y=String(b).trim();if(y===""||y==="0")return"";if(/^\d{4}-\d{2}-\d{2}/.test(y))return y.slice(0,10);if(/^\d{4,5}$/.test(y)){let _=Number(y);if(_>2e4&&_<99999){let x=new Date(Date.UTC(1899,11,30)+_*864e5);return isNaN(x.getTime())?"":x.toISOString().slice(0,10)}}let S=y.split(/[\/\-\.]/);if(S.length===3){let[_,x,w]=S.map(C=>C.trim());if(_.length===4&&x.length<=2&&w.length<=2)return`${_}-${x.padStart(2,"0")}-${w.padStart(2,"0")}`;if(w.length===4&&x.length<=2&&_.length<=2)return`${w}-${x.padStart(2,"0")}-${_.padStart(2,"0")}`}return y},h=o.map(b=>({branch_id:p(String(b.Cabang||"").trim()),activity_type:String(b.Kegiatan||"").trim(),period:String(b.Periode||"").trim(),pic:String(b.PIC||b.Pic||"").trim(),opening_date:u(b["Tgl Opening"]||b["Tanggal Opening"]||b["Tgl Openir"]),target_date:u(b["Tgl Target"]||b["Tanggal Target"]),completion_date:u(b["Tgl Selesai"]||b["Tanggal Selesai"]),status:String(b.Status||"").trim(),notes:String(b.Catatan||b.Keterangan||"").trim()})).filter(b=>b.activity_type&&b.period),g=await k("/api/import/schedule",{method:"POST",body:JSON.stringify({rows:h,onDuplicate:"update"})});if(!g.ok)throw new Error(g.data?.error||"Import gagal");return g.data}}})}N();H();var bt=[],We=[];function Ia(t,e){let s=String(t.status||"").toLowerCase();return e==="open"?s==="open":!1}async function At(t,e){let s=e?e.get("dash_filter"):null;bt=await q(),We=await te();let a=i=>i&&!We.find(r=>r.value===i)?[...We,{value:i,label:i}]:We,l=new Date().getFullYear();B({container:t,title:"Permasalahan",icon:"\u26A0\uFE0F",apiPath:"/api/issues",bulkDelete:!0,itemLabel:"Permasalahan",paginationMode:"client",onDataLoaded:i=>s?i.filter(r=>Ia(r,s)):i,columns:[{key:"report_date",label:"Tanggal",nowrap:!0,render:i=>window.formatDate(i)},{key:"branch_name",label:"Cabang"},{key:"category",label:"Kategori",render:i=>`<span class="badge badge-secondary">${i}</span>`},{key:"source",label:"Sumber"},{key:"complaint",label:"Keluhan",render:i=>`<span title="${i}">${i?.length>50?i.slice(0,50)+"\u2026":i}</span>`},{key:"employee_name",label:"Nama FC"},{key:"fc_specialist",label:"FC Spesialis"},{key:"solution",label:"Solusi",render:i=>`<span title="${i||""}">${i?.length>40?i.slice(0,40)+"\u2026":i||"-"}</span>`},{key:"status",label:"Status",render:i=>j(i)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:i=>window.formatDate(i)},{key:"day_count",label:"Hari",render:i=>i??"-"}],filterFields:[{type:"search",placeholder:"Cari keluhan / nama FC..."},{type:"select",name:"branch_id",label:"Cabang",options:bt},{type:"select",name:"month",label:"Bulan",options:[{value:"2026-01",label:"Jan 2026"},{value:"2026-02",label:"Feb 2026"},{value:"2026-03",label:"Mar 2026"},{value:"2026-04",label:"Apr 2026"},{value:"2026-05",label:"Mei 2026"},{value:"2026-06",label:"Jun 2026"},{value:"2026-07",label:"Jul 2026"},{value:"2026-08",label:"Agu 2026"},{value:"2026-09",label:"Sep 2026"},{value:"2026-10",label:"Okt 2026"},{value:"2026-11",label:"Nov 2026"},{value:"2026-12",label:"Des 2026"}]},{type:"select",name:"category",label:"Kategori",options:["SDM","Cleaning","Aset","K3","Lainnya"]},{type:"select",name:"status",label:"Status",options:["Open","In Progress","Done"]},{type:"select",name:"year",label:"Tahun",options:["2025","2026","2027","2028","2029","2030"]}],formFields:i=>[{type:"row",fields:[{name:"report_date",label:"Tanggal Info",type:"date",required:!0,value:i?.report_date},{name:"branch_id",label:"Cabang",type:"combobox",required:!0,options:bt,value:i?.branch_id}]},{type:"row",fields:[{name:"category",label:"Kategori",type:"select",required:!0,options:["SDM","Cleaning","Aset","K3","Lainnya"],value:i?.category},{name:"source",label:"Sumber Laporan",type:"combobox",options:["SPV","AM","RCP","Perawat","FC","Berlin","Ade","Pattrel","Dentrel"],value:i?.source}]},{name:"complaint",label:"Keluhan",type:"textarea",required:!0,rows:3,value:i?.complaint},{type:"row",fields:[{name:"employee_name",label:"Nama FC / Security",type:"combobox",options:a(i?.employee_name),value:i?.employee_name},{name:"fc_specialist",label:"FC Spesialis",type:"combobox",options:a(i?.fc_specialist),value:i?.fc_specialist}]},{name:"solution",label:"Solusi / Tindakan",type:"textarea",rows:3,value:i?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","In Progress","Done"],value:i?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:i?.completion_date}]}],exportOptions:{moduleName:"issues",onExport:async()=>{let i=await k(`/api/issues${window.location.search?window.location.search+"&":"?"}limit=10000`);if(i.ok){let r=i.data.data.map(n=>({Tanggal:n.report_date||"",Cabang:n.branch_name||"",Kategori:n.category||"",Sumber:n.source||"",Keluhan:n.complaint||"","Nama FC":n.employee_name||"","FC Spesialis":n.fc_specialist||"",Solusi:n.solution||"","Tgl Selesai":n.completion_date||"",Status:n.status||""}));L(r,"Data_Permasalahan")}else throw new Error("Gagal mengambil data")},onTemplate:()=>{L([{Tanggal:"2024-03-01",Cabang:"001. Pondok Bambu",Kategori:"Cleaning",Sumber:"SPV",Keluhan:"Lantai kotor","Nama FC":"Budi Santoso","FC Spesialis":"Fajar",Solusi:"Teguran lisan","Tgl Selesai":"2024-03-02",Status:"Done"}],"Template_Import_Permasalahan")},onImport:async i=>{let n=(await k("/api/branches?all=1")).data?.data||[],o=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=n.find(g=>String(g.full_name||"").toLowerCase()===u||String(g.code||"").toLowerCase()===u||String(g.name||"").toLowerCase()===u);return h?h.id:null},m=i.map(p=>({branch_id:o(String(p.Cabang||"").trim()),report_date:String(p.Tanggal||"").trim(),category:String(p.Kategori||"").trim(),source:String(p.Sumber||"").trim(),complaint:String(p.Keluhan||"").trim(),employee_name:String(p["Nama FC"]||"").trim(),fc_specialist:String(p["FC Spesialis"]||"").trim(),solution:String(p.Solusi||"").trim(),completion_date:String(p["Tgl Selesai"]||"").trim(),status:String(p.Status||"").trim()})).filter(p=>p.report_date&&p.complaint&&p.category),c=await k("/api/import/issues",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}}})}N();var Te=[];function Pa(t,e){let s=String(t.status||"").toLowerCase();return e==="pending"?s==="pending":!1}async function Ft(t,e){let s=e?e.get("dash_filter"):null;Te=await q();let a=await te(),l=["Ade","Berlin"],d=r=>r&&!a.find(n=>n.value===r)?[...a,{value:r,label:r}]:a,i=r=>r&&!l.find(n=>(typeof n=="object"?n.value:n)===r)?[...l,r]:l;B({container:t,title:"One on One",icon:"\u{1F91D}",apiPath:"/api/one-on-one",bulkDelete:!0,itemLabel:"One on One",paginationMode:"client",onDataLoaded:r=>s?r.filter(n=>Pa(n,s)):r,columns:[{key:"meeting_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"branch_name",label:"Cabang"},{key:"employee_name",label:"Nama Karyawan"},{key:"pic",label:"PIC"},{key:"problem",label:"Masalah",render:r=>`<span title="${r||""}">${r?.length>50?r.slice(0,50)+"\u2026":r||"-"}</span>`},{key:"solution",label:"Solusi",render:r=>`<span title="${r||""}">${r?.length>40?r.slice(0,40)+"\u2026":r||"-"}</span>`},{key:"status",label:"Status",render:r=>j(r)},{key:"completion_date",label:"Tgl Selesai",nowrap:!0,render:r=>window.formatDate(r)},{key:"day_count",label:"Hari"},{key:"document_link",label:"Dokumen",render:r=>r?`<a href="${r}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari nama / masalah..."},{type:"combobox",name:"branch_id",label:"Cabang",options:Te},{type:"select",name:"status",label:"Status",options:["Open","Done"]}],exportOptions:{moduleName:"one_on_one",onExport:async r=>{let n=new URLSearchParams(r||{}).toString(),o=await k(`/api/one-on-one?limit=10000&${n}`);if(o.ok){let m=o.data.data.map(p=>({Tanggal:p.meeting_date||"",Cabang:p.branch_name||"","Nama Karyawan":p.employee_name||"",PIC:p.pic||"",Masalah:p.problem||"",Solusi:p.solution||"",Status:p.status||"","Tgl Selesai":p.completion_date||"",Dokumen:p.document_link||""})),{downloadExcel:c}=await Promise.resolve().then(()=>(H(),re));c(m,`Data_One_on_One_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let r=[{Tanggal:"2026-01-08",Cabang:"001. Pondok Bambu","Nama Karyawan":"Widya Astuti",PIC:"Rina",Masalah:"Terlambat terus",Solusi:"Teguran",Status:"Open","Tgl Selesai":"",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(H(),re));n(r,"Template_Import_OneOnOne")},onImport:async r=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=Te.find(g=>String(g.label||"").toLowerCase()===u);return h?h.value:null},o=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let b=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let h=u.split(/[\/\-\.]/);if(h.length===3){let[g,b,y]=h.map(S=>S.trim());if(g.length===4&&b.length<=2&&y.length<=2)return`${g}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&g.length<=2)return`${y}-${b.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=r.map(p=>({meeting_date:o(p.Tanggal),employee_name:String(p["Nama Karyawan"]||"").trim(),branch_id:n(String(p.Cabang||"").trim()),pic:String(p.PIC||"").trim(),problem:String(p.Masalah||"").trim(),solution:String(p.Solusi||"").trim(),status:String(p.Status||"").trim(),completion_date:o(p["Tgl Selesai"]),document_link:String(p.Dokumen||"").trim()})).filter(p=>p.meeting_date&&p.employee_name&&p.branch_id),c=await k("/api/import/one_on_one",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:r=>[{type:"row",fields:[{name:"meeting_date",label:"Tanggal",type:"date",required:!0,value:r?.meeting_date},{name:"branch_id",label:"Cabang",type:"combobox",options:r?.branch_id&&!Te.find(n=>n.value==r.branch_id)?[...Te,{value:r.branch_id,label:r.branch_name||r.branch_id}]:Te,createApi:{path:"/api/branches",field:"full_name"},value:r?.branch_id}]},{type:"row",fields:[{name:"employee_name",label:"Nama Karyawan",type:"combobox",required:!0,options:d(r?.employee_name),value:r?.employee_name},{name:"pic",label:"PIC",type:"combobox",options:i(r?.pic),createApi:{path:"/api/pic",field:"name"},value:r?.pic}]},{name:"problem",label:"Masalah",type:"textarea",required:!0,rows:3,value:r?.problem},{name:"solution",label:"Solusi",type:"textarea",rows:3,value:r?.solution},{type:"row",fields:[{name:"status",label:"Status",type:"select",required:!0,options:["Open","Done"],value:r?.status||""},{name:"completion_date",label:"Tanggal Selesai",type:"date",value:r?.completion_date}]},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://drive.google.com/...",value:r?.document_link}]})}N();async function Mt(t){let e=await q(),s=await te(),a=["Ade","Berlin"],l=r=>r&&!s.find(n=>n.value===r)?[...s,{value:r,label:r}]:s,d=r=>r&&!a.find(n=>(typeof n=="object"?n.value:n)===r)?[...a,r]:a,i=Array.from({length:5},(r,n)=>String(new Date().getFullYear()-n));B({container:t,title:"Training",icon:"\u{1F393}",apiPath:"/api/training",bulkDelete:!0,itemLabel:"Training",columns:[{key:"training_date",label:"Tanggal",nowrap:!0,render:r=>window.formatDate(r)},{key:"batch",label:"Batch"},{key:"subject",label:"Materi"},{key:"branch_name",label:"Cabang"},{key:"trainer",label:"Trainer"},{key:"participants",label:"Peserta",render:r=>{try{let n=JSON.parse(r);return Array.isArray(n)?n.join(", "):r||"-"}catch{return r||"-"}}},{key:"score",label:"Nilai",render:r=>r!=null?`<strong>${r}</strong>`:"-"},{key:"document_link",label:"Dokumen",render:r=>r?`<a href="${r}" target="_blank" rel="noopener" class="btn btn-xs btn-ghost">\u{1F4C4} Buka</a>`:"-"}],filterFields:[{type:"search",placeholder:"Cari materi / trainer..."},{type:"select",name:"year",label:"Tahun",options:i}],exportOptions:{moduleName:"training",onExport:async r=>{let n=new URLSearchParams(r||{}).toString(),o=await k(`/api/training?limit=10000&${n}`);if(o.ok){let m=o.data.data.map(p=>{let u=p.participants||"";try{let h=JSON.parse(u);u=Array.isArray(h)?h.join(", "):u}catch{}return{Tanggal:p.training_date||"",Batch:p.batch||"",Materi:p.subject||"",Cabang:p.branch_name||"",Trainer:p.trainer||"",Peserta:u,Nilai:p.score!==null&&p.score!==void 0?p.score:"",Dokumen:p.document_link||""}}),{downloadExcel:c}=await Promise.resolve().then(()=>(H(),re));c(m,`Data_Training_${new Date().toISOString().slice(0,10)}`)}else throw new Error("Gagal mengambil data")},onTemplate:async()=>{let r=[{Tanggal:"2026-01-08",Batch:"Batch 1",Materi:"Standar Kebersihan",Cabang:"001. Pondok Bambu",Trainer:"Budi",Peserta:"Rina, Agus",Nilai:"85",Dokumen:"https://link.doc"}],{downloadExcel:n}=await Promise.resolve().then(()=>(H(),re));n(r,"Template_Import_Training")},onImport:async r=>{let n=p=>{if(!p)return null;let u=String(p||"").toLowerCase(),h=e.find(g=>String(g.label||"").toLowerCase()===u);return h?h.value:null},o=p=>{if(!p)return"";if(p instanceof Date&&!isNaN(p.getTime()))return p.toISOString().slice(0,10);let u=String(p).trim();if(/^\d{4,5}$/.test(u)){let g=Number(u);if(g>2e4&&g<99999){let b=new Date(Date.UTC(1899,11,30)+g*864e5);return isNaN(b.getTime())?"":b.toISOString().slice(0,10)}}if(/^\d{4}-\d{2}-\d{2}/.test(u))return u.slice(0,10);let h=u.split(/[\/\-\.]/);if(h.length===3){let[g,b,y]=h.map(S=>S.trim());if(g.length===4&&b.length<=2&&y.length<=2)return`${g}-${b.padStart(2,"0")}-${y.padStart(2,"0")}`;if(y.length===4&&b.length<=2&&g.length<=2)return`${y}-${b.padStart(2,"0")}-${g.padStart(2,"0")}`}return u},m=r.map(p=>({training_date:o(p.Tanggal),batch:String(p.Batch||"").trim(),subject:String(p.Materi||"").trim(),branch_id:n(String(p.Cabang||"").trim()),trainer:String(p.Trainer||"").trim(),participants:String(p.Peserta||"").trim(),score:p.Nilai?Number(p.Nilai):null,document_link:String(p.Dokumen||"").trim()})).filter(p=>p.training_date&&p.subject&&p.branch_id),c=await k("/api/import/training",{method:"POST",body:JSON.stringify({rows:m,onDuplicate:"update"})});if(!c.ok)throw new Error(c.data?.error||"Import gagal");return c.data}},formFields:r=>[{type:"row",fields:[{name:"training_date",label:"Tanggal Training",type:"date",required:!0,value:r?.training_date},{name:"batch",label:"Batch",placeholder:"Batch 1, Batch 2, ...",value:r?.batch}]},{name:"subject",label:"Materi / Topik Training",required:!0,placeholder:"Judul materi training",value:r?.subject},{type:"row",fields:[{name:"branch_id",label:"Cabang",type:"combobox",options:e,value:r?.branch_id},{name:"trainer",label:"Trainer",type:"combobox",options:d(r?.trainer),value:r?.trainer}]},{name:"participants",label:"Peserta (pisahkan dengan koma)",type:"textarea",rows:3,placeholder:"Nama Peserta 1, Nama Peserta 2, ...",value:(()=>{try{let n=JSON.parse(r?.participants);return Array.isArray(n)?n.join(", "):r?.participants||""}catch{return r?.participants||""}})()},{type:"row",fields:[{name:"score",label:"Nilai / Score",type:"number",step:"0.1",min:"0",max:"100",value:r?.score},{name:"document_link",label:"Link Dokumen",type:"url",placeholder:"https://...",value:r?.document_link}]},{name:"notes",label:"Catatan",type:"textarea",rows:2,value:r?.notes}],onBeforeSubmit:async r=>(r.participants&&(r.participants=JSON.stringify(r.participants.split(",").map(n=>n.trim()).filter(Boolean))),r)})}N();ke();H();function Ot({container:t,title:e,icon:s,apiPath:a,columns:l,formFields:d,filterFields:i,defaultFilters:r={},itemLabel:n="Data",canCreate:o=!0,canEdit:m=!0,canDelete:c=!0,onBeforeSubmit:p,onAfterLoad:u,onDataLoaded:h,extraActions:g=[],initialSearch:b="",exportOptions:y=null,bulkDelete:S=!1,paginationMode:_="server"}){let x=1,w={...r};b&&(w.search=b);let C=new Set;t.innerHTML=`
      ...
      ${Array(35).fill('<div style="background:#f8fafc;min-height:70px;"></div>').join("")}
          ...
          <div class="cal-cell ${O?"cal-today":""} ${C.length?"cal-has-events":""}"
            ...
            <div class="cal-event-item cal-color-border-${M.color||"gray"}">
              <div class="cal-event-type">${Aa(M.type)}</div>
  ...
  `;let e=null,s=null,a=0,l={upload:document.getElementById("step-upload"),validating:document.getElementById("step-validating"),preview:document.getElementById("step-preview"),importing:document.getElementById("step-importing"),summary:document.getElementById("step-summary")};function d(g){Object.entries(l).forEach(([b,y])=>{y.style.display=b===g?"":"none"})}document.getElementById("btn-backup-db")?.addEventListener("click",async()=>{let g=document.getElementById("btn-backup-db");g.disabled=!0,g.textContent="\u23F3 Memproses Backup...";try{let b=await k("/api/import/backup");if(b.ok){let y=new Blob([JSON.stringify(b.data,null,2)],{type:"application/json"}),S=URL.createObjectURL(y),_=document.createElement("a");_.href=S,_.download=`FCMS_Database_Backup_${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(_),_.click(),document.body.removeChild(_),URL.revokeObjectURL(S),ee("Backup berhasil diunduh!")}else V("Gagal memproses backup: "+(b.data?.error||"Unknown error"))}catch(b){V("Gagal memproses backup: "+b.message)}finally{g.disabled=!1,g.textContent="\u{1F4E6} Backup Database"}});let i=document.getElementById("btn-sync-google");i&&i.addEventListener("click",async()=>{if(!confirm("Peringatan: Mensinkronkan data dengan Google Sheets akan memperbarui dan menambahkan data baru dari Google Sheets ke dalam FCMS. Data yang sudah Anda buat di FCMS TIDAK akan terhapus. Lanjutkan?"))return;let g=i.innerHTML;i.innerHTML='<span class="spinner"></span> Menyinkronkan...',i.disabled=!0;try{let b=await k("/api/sync/google-sheets",{method:"POST"});b.ok?alert("Sinkronisasi Berhasil: "+(b.data?.message||"Data Karyawan & PIC telah diperbarui.")):alert("Gagal Sinkronisasi: "+(b.data?.error||"Unknown error"))}catch{alert("Terjadi kesalahan koneksi.")}finally{i.innerHTML=g,i.disabled=!1}}),document.getElementById("btn-download-template").addEventListener("click",()=>{Xt(),ee("Template Excel berhasil didownload!")});let r=document.getElementById("file-input"),n=document.getElementById("upload-zone");document.getElementById("btn-browse").addEventListener("click",g=>{g.stopPropagation(),r.click()}),r.addEventListener("change",g=>{g.target.files[0]&&o(g.target.files[0])}),n.addEventListener("dragover",g=>{g.preventDefault(),n.classList.add("drag-over")}),n.addEventListener("dragleave",()=>n.classList.remove("drag-over")),n.addEventListener("drop",g=>{g.preventDefault(),n.classList.remove("drag-over");let b=g.dataTransfer.files[0];b&&b.name.match(/\.xlsx?$/i)?o(b):V("Hanya file .xlsx atau .xls yang didukung.")}),document.getElementById("btn-clear-file").addEventListener("click",()=>{e=null,r.value="",document.getElementById("file-info").style.display="none",n.style.display="",d("upload")});async function o(g){e=g,document.getElementById("file-name-display").textContent=`\u{1F4C4} ${g.name} (${(g.size/1024).toFixed(1)} KB)`,document.getElementById("file-info").style.display="flex",n.style.display="none",await m(g)}async function m(g){d("validating");let b=document.getElementById("validation-status"),y=document.getElementById("validation-bar");try{if(!window.XLSX)throw new Error("Library SheetJS belum termuat. Refresh halaman dan coba lagi.");b.textContent="Membaca file Excel...",y.style.width="20%",await Ke(200);let S=await g.arrayBuffer(),_=window.XLSX.read(S,{type:"array",cellDates:!0});b.textContent=`Memvalidasi ${_.SheetNames.length} sheet...`,y.style.width="50%",await Ke(100),s=Yt(_),y.style.width="100%",b.textContent="Validasi selesai!",await Ke(300),c()}catch(S){d("upload"),V("Gagal memproses file: "+S.message),document.getElementById("file-info").style.display="flex",n.style.display="none"}}function c(){d("preview");let g=s.filter(T=>!T.skipped).length,b=s.reduce((T,M)=>T+M.total,0),y=s.reduce((T,M)=>T+M.valid,0),S=s.reduce((T,M)=>T+M.errorCount,0),_=b>0?Math.round(y/b*100):0;document.getElementById("preview-summary-badges").innerHTML=`
    ...
    `).join("");let S=document.getElementById("import-bar"),_=document.getElementById("import-current-status"),x={totalSheets:b.length,totalRows:b.reduce((w,C)=>w+C.mapped.length,0),inserted:0,skipped:0,failed:0,moduleResults:[]};for(let w=0;w<b.length;w++){let C=b[w],O=document.getElementById(`step-icon-${C.module}`),T=document.getElementById(`step-status-${C.module}`);O.textContent="\u{1F504}",T.textContent="Mengimport...",_.textContent=`Mengimport ${C.label}...`,S.style.width=`${Math.round(w/b.length*100)}%`;try{let M=await k(`/api/import/${C.module}`,{method:"POST",body:JSON.stringify({rows:C.mapped,onDuplicate:g})});if(M.ok){let A=M.data;x.inserted+=A.inserted||0,x.skipped+=A.skipped||0,x.moduleResults.push({label:C.label,inserted:A.inserted||0,skipped:A.skipped||0,status:"ok"}),O.textContent="\u2705",T.innerHTML=`<span class="badge badge-success">${A.inserted||0} berhasil</span>${A.skipped>0?` <span class="badge badge-neutral">${A.skipped} skip</span>`:""}`}else x.failed++,x.moduleResults.push({label:C.label,inserted:0,skipped:0,status:"error",error:M.data?.error}),O.textContent="\u274C",T.innerHTML='<span class="badge badge-danger">Gagal</span>'}catch(M){x.failed++,x.moduleResults.push({label:C.label,inserted:0,skipped:0,status:"error",error:M.message}),O.textContent="\u274C",T.innerHTML='<span class="badge badge-danger">Gagal</span>'}await Ke(150)}S.style.width="100%",_.textContent="Selesai!",await Ke(400),h(x)}function h(g){d("summary");let b=((Date.now()-a)/1e3).toFixed(1),y=g.failed===0;document.getElementById("summary-status-icon").innerHTML=`
